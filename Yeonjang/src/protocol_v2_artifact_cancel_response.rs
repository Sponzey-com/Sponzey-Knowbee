//! Signed v2 acknowledgement for one admitted artifact cancellation.
//!
//! This is a control result, not the producing command's terminal response.
//! It binds the exact artifact owner and transfer while exposing only the
...
use crate::artifact_transfer_use_case::{ArtifactCancelResult, ArtifactTransferReject};
...
const SCHEMA_ID: &str = "yeonjang.artifact-cancel-ack.v2";

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
enum V2ArtifactCancelOutcome {
    Cancelled,
    AlreadyCancelled,
    Rejected,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
enum V2ArtifactCancelRejectReason {
    Missing,
    WrongOwner,
    WrongTransfer,
    RevisionConflict,
...
struct V2ArtifactCancelAckPayload {
    artifact_ref: String,
    owner_request_id: String,
    owner_operation_id: String,
    transfer_id: String,
    observed_revision: u64,
    outcome: V2ArtifactCancelOutcome,
    #[serde(skip_serializing_if = "Option::is_none")]
    lifecycle_revision: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    reason: Option<V2ArtifactCancelRejectReason>,
}

#[derive(Clone, PartialEq, Eq, Serialize)]
struct V2ArtifactCancelResponseAuthorization {
    schema_version: u16,
    issuer: String,
    key_id: String,
    audience: String,
...
#[derive(Clone, PartialEq, Eq, Serialize)]
pub struct V2ArtifactCancelResponseEnvelope {
    ...
    expires_at: i64,
    sequence: u64,
    payload: V2ArtifactCancelAckPayload,
    authorization: V2ArtifactCancelResponseAuthorization,
    #[serde(skip)]
    payload_digest: [u8; 32],
}

impl fmt::Debug for V2ArtifactCancelResponseEnvelope {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("V2ArtifactCancelResponseEnvelope")
...
impl V2ArtifactCancelResponseEnvelope {
    pub fn sign(
        request: &V2ArtifactEnvelope,
        result: ArtifactCancelResult,
        context: V2ResponseSigningContext,
        signer: &dyn V2ResponseSigner,
    ) -> Result<Self, V2ArtifactCancelResponseError> {
        validate_context(request, &context)?;
        let (outcome, lifecycle_revision, reason) = match result {
            ArtifactCancelResult::Cancelled { lifecycle_revision } => (
                V2ArtifactCancelOutcome::Cancelled,
                Some(lifecycle_revision),
                None,
            ),
            ArtifactCancelResult::AlreadyCancelled { lifecycle_revision } => (
                V2ArtifactCancelOutcome::AlreadyCancelled,
                Some(lifecycle_revision),
                None,
            ),
            ArtifactCancelResult::Rejected { reason } => (
                V2ArtifactCancelOutcome::Rejected,
                None,
                Some(map_rejection(reason)),
            ),
        };
        let payload = V2ArtifactCancelAckPayload {
        ...
        let payload_digest = Sha256::digest(
            serde_json::to_vec(&payload)
                .map_err(|_| V2ArtifactCancelResponseError::Serialization)?,
        )
        .into();
        ...
        if !is_hex_digest(&signature) {
            return Err(V2ArtifactCancelResponseError::InvalidSignature);
        }
    ...
    pub fn authorization_signing_bytes(&self) -> Vec<u8> {
        ...
        append_text(
            &mut bytes,
            "domain",
            "yeonjang.artifact-cancel-ack.authorization.v2",
        );
        append_u64(
            &mut bytes,
            "protocol_version",
...
pub enum V2ArtifactCancelResponseError {
    InvalidIdentity,
    IdentityMismatch,
    InvalidTiming,
    Serialization,
...
fn validate_context(
    request: &V2ArtifactEnvelope,
    context: &V2ResponseSigningContext,
) -> Result<(), V2ArtifactCancelResponseError> {
    for value in [
        ...
        validate_identifier(value).map_err(|_| V2ArtifactCancelResponseError::InvalidIdentity)?;
    }
    if context.audience != request.requester_id() {
        return Err(V2ArtifactCancelResponseError::IdentityMismatch);
    }
    if context.issued_at < 0 || context.expires_at <= context.issued_at {
        return Err(V2ArtifactCancelResponseError::InvalidTiming);
    }
    Ok(())
}

fn map_rejection(reason: ArtifactTransferReject) -> V2ArtifactCancelRejectReason {
    match reason {
        ArtifactTransferReject::Missing => V2ArtifactCancelRejectReason::Missing,
        ArtifactTransferReject::WrongOwner => V2ArtifactCancelRejectReason::WrongOwner,
        ArtifactTransferReject::WrongTransfer => V2ArtifactCancelRejectReason::WrongTransfer,
        ArtifactTransferReject::DigestMismatch => V2ArtifactCancelRejectReason::DigestMismatch,
        ArtifactTransferReject::RevisionConflict => V2ArtifactCancelRejectReason::RevisionConflict,
        ArtifactTransferReject::InvalidState => V2ArtifactCancelRejectReason::InvalidState,
        ArtifactTransferReject::Expired => V2ArtifactCancelRejectReason::Expired,
        ArtifactTransferReject::SourceUnavailable => {
            V2ArtifactCancelRejectReason::SourceUnavailable
        }
        ArtifactTransferReject::VerificationFailed => {
            V2ArtifactCancelRejectReason::VerificationFailed
        }
        ArtifactTransferReject::StorageConflict => V2ArtifactCancelRejectReason::StorageConflict,
        ArtifactTransferReject::Unavailable => V2ArtifactCancelRejectReason::Unavailable,
    }
}

fn map_signer_error(_: V2ResponseSignerError) -> V2ArtifactCancelResponseError {
    V2ArtifactCancelResponseError::SignerUnavailable
}
