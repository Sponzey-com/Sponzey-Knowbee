...
Knowbee와 모든 서브 에이전트가 각자 독립된 메모리 상태를 갖고, 이름 변경이나 채널 차이와 무관하게 내부 ID 기준으로 안전하게 격리되게 만든다.  
이 태스크의 목표는 다음 3가지다.

1. Knowbee와 서브 에이전트용 `agent_memory_state`와 owner scope mapping을 도입한다.
...
- [x] Knowbee 본체와 sub-agent의 owner scope 규칙을 분리한다.
...
- [x] 같은 session 안에 Knowbee와 child agent가 동시에 있어도 owner scope가 분리되는지 확인한다.
...
- [x] Knowbee와 모든 sub-agent가 owner-scoped memory state를 가진다.
