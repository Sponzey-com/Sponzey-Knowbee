...
- `packages/core/src/orchestration/mode*`: orchestration mode snapshot을 만들 때 legacy `single_knowbee` 값을 읽거나 표시할 수 있다.
...
- `packages/core/src/ui/sub-agent-settings*`: 설정 화면 view model에서 단일 노우비 상태를 사용자-facing 상태로 표현할 수 있다.
