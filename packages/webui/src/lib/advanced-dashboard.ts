...
import { toRunSourceText, toRunStatusText } from "./run-labels"

export type AdvancedDashboardCardId =
  | "connections"
  | "recent_runs"
  | "pending_approvals"
  | "warnings"
...
function operationsHealthStatusLabel(
  status: OperationsSummary["repeatedIssues"][number]["status"],
  language: UiLanguage,
): string {
  switch (status) {
    case "ok":
      return pickUiText(language, "정상", "OK")
    case "degraded":
      return pickUiText(language, "주의", "Needs attention")
    case "down":
      return pickUiText(language, "차단", "Blocked")
  }
}

function doctorSummaryStatusLabel(
  status: NonNullable<AdvancedDashboardSources["doctor"]>["overallStatus"],
  language: UiLanguage,
): string {
  switch (status) {
    case "ok":
    ...
    case "blocked":
      return pickUiText(language, "차단", "Blocked")
    case "unknown":
      return pickUiText(language, "확인 필요", "Needs check")
  }
}

function dashboardRunTitle(run: RootRun, language: UiLanguage): string {
  return run.title?.trim() || pickUiText(language, "제목 없는 실행", "Untitled run")
}

export async function loadAdvancedDashboardSources(
  loaders: AdvancedDashboardLoaders,
  ...
  const entries = Object.entries(loaders) as Array<
    [AdvancedDashboardSourceKey, () => Promise<unknown>]
  >
...
export function buildAdvancedDashboardCards(input: {
  ...
  const pendingRuns = runs.filter(
    (run) => run.status === "awaiting_approval" || run.status === "awaiting_user",
  )
  const failedRuns = runs.filter(
    (run) => run.status === "failed" || run.status === "cancelled" || run.status === "interrupted",
  )
  const recentRuns = runs.slice(0, 5)
  const aiConfigured = Boolean(
    input.status?.primaryAiTarget ||
      input.draft.aiBackends.some((backend) => backend.enabled && backend.defaultModel.trim()),
  )
  const channelConfigured = Boolean(
    input.draft.channels.telegramEnabled ||
      input.draft.channels.slackEnabled ||
      input.checks?.telegramConfigured,
  )
  ...
  const connectionCount = [aiConfigured, channelConfigured, yeonjangConfigured, setupReady].filter(
    Boolean,
  ).length
  ...
  return [
    {
      ...
      status: sourceStatus({
        ...
        error: errors.status,
        hasData: Boolean(input.status || input.checks),
        emptyIsIdle: false,
      }),
      ...
      summary:
        errors.status ??
        t(
          "AI, 채널, 연장, 저장 상태를 한 번에 확인합니다.",
          "Review AI, channels, extension, and setup storage together.",
        ),
      items: [
    ...
    },
    {
      ...
      summary:
        errors.runs ??
        (recentRuns[0]
          ? dashboardRunTitle(recentRuns[0], input.language)
          : t("최근 실행이 없습니다.", "No recent runs.")),
      items: recentRuns
        .map(
          (run) =>
            `${dashboardRunTitle(run, input.language)} · ${toRunStatusText(run.status, input.language)} · ${toRunSourceText(run.source, input.language)}`,
        )
        .slice(0, 4),
      href: "/work/runs",
    },
    {
      ...
      status: sourceStatus({
        loading: input.loading === true,
        error: errors.runs || errors.operations,
        hasData: Boolean(input.runs || operations),
        emptyIsIdle: true,
      }),
      ...
      summary:
        ...
        (pendingRuns.length > 0
          ? t("사용자 확인이 필요한 실행이 있습니다.", "Some runs need user approval.")
          : t("대기 중인 승인이 없습니다.", "No pending approvals.")),
      items: pendingRuns
        .map(
          (run) =>
            `${dashboardRunTitle(run, input.language)} · ${toRunStatusText(run.status, input.language)}`,
        )
        .slice(0, 4),
      href: "/work/runs",
    },
    {
      ...
      summary:
        ...
        (warningCount > 0
          ? t(
              "확인할 실패 또는 반복 문제가 있습니다.",
              "There are failures or repeated issues to inspect.",
            )
          : t("눈에 띄는 경고가 없습니다.", "No visible warnings.")),
      items: [
        ...repeatedIssues.map(
          (issue) =>
            `${issue.label} · ${operationsHealthStatusLabel(issue.status, input.language)} · ${t("반복", "Repeated")} ${issue.count}`,
        ),
        ...failedRuns.map(
          (run) =>
            `${dashboardRunTitle(run, input.language)} · ${toRunStatusText(run.status, input.language)}`,
    ...
    {
      id: "doctor",
      title: t("운영 진단 요약", "Doctor summary"),
      status: sourceStatus({
        loading: input.loading === true,
        error: errors.doctor,
        hasData: Boolean(input.doctor),
        emptyIsIdle: false,
      }),
      value: input.doctor
        ? doctorSummaryStatusLabel(input.doctor.overallStatus, input.language)
        : "-",
      summary:
        errors.doctor ??
        (input.doctor
          ? t("운영 진단 결과를 요약했습니다.", "Operational diagnostics are summarized.")
          : t("진단 결과를 불러오는 중입니다.", "Loading diagnostics.")),
      items: input.doctor
        ? [
            `${t("정상", "OK")} ${input.doctor.summary.ok}`,
            `${t("주의", "Warning")} ${doctorWarning}`,
            `${t("차단", "Blocked")} ${doctorBlocked}`,
            `${t("확인 필요", "Needs check")} ${input.doctor.summary.unknown}`,
          ]
        : [],
