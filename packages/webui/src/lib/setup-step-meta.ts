...
export function createSetupSteps(
  ...
  const steps: SetupStepMeta[] = [
    ...
    withCapability(
      ...
      t("메인 에이전트가 사용할 외부 도구와 기능을 연결합니다.", "Connect external tools and capabilities for the main agent."),
      capabilities.find((item) => item.key === "mcp.client"),
      false,
      [
        t("연결할 외부 기능을 추가합니다.", "Add the external features to connect."),
    ...
    {
      id: "skills",
      label: t("작업 능력 확장", "Work abilities"),
      ...
      reason: hasConfiguredSkills && !hasSkillsReady ? t("등록한 작업 능력의 상태를 다시 확인해야 합니다.", "Check the registered work ability status again.") : undefined,
      required: false,
      highlights: [
        t("로컬 또는 기본 작업 능력을 등록합니다.", "Add local or built-in work abilities."),
        t("필요한 작업 능력만 켜고 설명을 정리합니다.", "Enable only the needed work abilities and keep descriptions clear."),
        t("로컬 작업 능력은 경로 확인을 통해 준비 상태를 확인합니다.", "Verify local work ability paths before using them."),
      ],
    ...
    withCapability(
      ...
      false,
      [
        t("파일 실행이나 도구 사용 전에 얼마나 자주 확인받을지 정합니다.", "Set how often the main agent should ask before using tools or files."),
    ...
    withSetupChannelCapability(
      ...
      t("메신저에서 메인 에이전트와 대화할 채널을 연결합니다.", "Connect the messaging channels used to talk with the main agent."),
      ...
      [
        ...
        t("메신저에서 메인 에이전트와 대화할 준비를 마칩니다.", "Finish preparing the chat channel for the main agent."),
      ],
      hasTelegramChannel,
      language,
    ),
    withCapability(
      ...
      [
        ...
        t("로컬에서만 사용할 경우 선택하지 않고 넘어갑니다.", "Leave this off when you only use the local device."),
      ],
      hasRemoteAccess,
    ),
    {
