...
- `packages/webui/src/components/topology`는 사용자가 서브 에이전트를 그리고, 연결하고, 선택한 서브 에이전트를 정의하는 UI 컴포넌트 영역이다.

## 기본 topology UX 기준

- 기본 서브 에이전트 설정 화면은 서브 에이전트와 위임 연결선이 중심이다.
...
- 서브 에이전트 선택, 후보 판단, 위험 경계 판단은 WebUI에서 구현하지 않는다.
