...
import type {
  AgentTool,
  ArtifactDeliveryResultDetails,
  ToolContext,
  ToolResult,
  ToolSideEffectObservation,
  ToolSideEffectPreparation,
  ToolSideEffectRecoveryEvidence,
} from "../types.js"
...
function canonicalYeonjangCameraOperation(
...
function prepareYeonjangCameraOperation(
  params: YeonjangCameraCaptureParams,
  _ctx: ToolContext,
): ToolSideEffectPreparation<YeonjangCameraCaptureParams> {
  ...
  const executionParams: YeonjangCameraCaptureParams = {
...
function cameraObservedState(input: {
  expectedState: ReturnType<typeof cameraExpectedState>
  resolvedDeviceId: string
}): ReturnType<typeof cameraExpectedState> | { reason: string } {
  if (input.expectedState.requestedDevice.kind === "exact") {
    if (!input.resolvedDeviceId) return { reason: "camera_resolved_device_missing" }
...
const CAMERA_DEVICE_CONSTRAINT_SATISFIED_REF =
  "side-effect-fact:camera-device-constraint-satisfied:v1"

function cameraRecoveryEvidence(input: {
  result: ToolResult
  observedState: ReturnType<typeof cameraExpectedState> | { reason: string }
  resolvedDevicePresent: boolean
}): ToolSideEffectRecoveryEvidence | undefined {
  if (!("reason" in input.observedState)) return undefined
  if (
    input.observedState.reason !== "camera_resolved_device_missing" &&
...
function cameraEffectEvidenceRefs(
  params: YeonjangCameraCaptureParams,
  result: ToolResult,
): readonly string[] {
  const details = result.details && typeof result.details === "object" && !Array.isArray(result.details)
...
async function observeCurrentYeonjangCameraCapture(
  params: YeonjangCameraCaptureParams,
  ctx: ToolContext,
  effectEvidenceRefs: readonly string[],
): Promise<ToolSideEffectObservation> {
  const expectedState = cameraExpectedState(params)
  const artifactRefs = effectEvidenceRefs.filter((ref) =>
    /^artifact:[0-9a-f-]{36}$/iu.test(ref)
  )
  const deviceConstraintSatisfied = effectEvidenceRefs.includes(
    CAMERA_DEVICE_CONSTRAINT_SATISFIED_REF,
  )
  ...
  const recoveryEvidence: ToolSideEffectRecoveryEvidence | undefined =
    requiresDeviceConstraint && artifactResolution?.ok && artifactAvailable && !deviceConstraintSatisfied
      ? {
          kind: "artifact_candidate",
...
interface YeonjangCameraCaptureDetails {
  ...
  constraint?: "camera_facing_selection_unsupported" | "camera_facing_capability_unknown"
  fileName?: string
...
function cameraDeviceFacing(device: YeonjangCameraDevice): RequestedCameraFacing | null {
  const position = device.position?.trim().toLowerCase()
  if (position === "front") return "front"
  if (position === "rear" || position === "back") return "rear"
  return null
}
...
function resolveCameraCaptureOperationBudgetMs(timeoutSec?: number): number {
  return resolveTimeoutMs(timeoutSec) ?? DEFAULT_CAMERA_CAPTURE_OPERATION_BUDGET_MS
}

function cameraCaptureTransportTimeoutMs(operationBudgetMs: number): number {
...
type CameraCaptureRuntimeFailureReason =
  keyof typeof CAMERA_CAPTURE_RUNTIME_FAILURES

function cameraCaptureFailureProjection(error: unknown): {
  errorCode: string
  reasonCode?: string
  ...
  const attempt =
    error instanceof Error
    && (error as Error & { attempt?: unknown }).attempt
    && typeof (error as Error & { attempt: unknown }).attempt === "object"
    && !Array.isArray((error as Error & { attempt: unknown }).attempt)
      ? (error as Error & { attempt: Record<string, unknown> }).attempt
      : null
  ...
  if (Object.hasOwn(CAMERA_CAPTURE_RUNTIME_FAILURES, code) || (code && boundAttempt)) {
    const reasonCode = code as CameraCaptureRuntimeFailureReason
    return {
