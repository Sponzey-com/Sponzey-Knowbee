...
function displayName(extension: ReturnType<typeof getMqttExtensionSnapshots>[number]): string {
  return extension.displayName?.trim()
    || extension.instanceAlias?.trim()
    || extension.extensionId
}

export const yeonjangStatusTool: AgentTool<Record<string, never>> = {
  ...
  async execute(): Promise<ToolResult> {
    ...
    if (snapshots.length === 0) {
      ...
      if (broker.reason) lines.push(`상태 이유: ${broker.reason}`)
    } else {
