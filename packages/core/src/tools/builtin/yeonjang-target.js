import { buildYeonjangTargetSelectorSchemaProperty, normalizeYeonjangTargetSelector, serializeYeonjangTargetSelector, validateYeonjangTargetSelector, } from "../../contracts/yeonjang-target.js";
...
function defaultValidationResults() {
    return {
...
function buildProof(params) {
    ...
    const validationResults = {
        ...
        ...(params.input.explicitTarget ? { selector: "pass" } : {}),
...
function selectionFromDefault(selection, input) {
    return {
        ok: selection.ok,
...
function buildCandidate(instance) {
    return {
...
function selectStructuredCandidates(selector, instances) {
...
function selectLegacyCandidates(requested, instances) {
...
function validateMatchedTarget(instance, context, matchedField, matchedValue, reasonCode) {
...
export function resolveYeonjangTargetSelection(params) {
    ...
    return validateMatchedTarget(selected.candidates[0], context, selected.matchedField, selected.matchedValue, selected.reasonCode);
}
...
export function buildYeonjangTargetResolutionDetails(selection) {
    return {
        selectionStatus: selection.status,
...
export function recordYeonjangRemoteExecutionApproval(params) {
    if (!params.selection.ok || !params.selection.instanceId)
        return;
    const candidate = params.selection.proof.candidateList[0];
    if (!candidate || candidate.location !== "remote")
        return;
...
export function buildYeonjangTargetParameterProperties(defaultExtensionId) {
    return {
        extensionId: {
            type: "string",
...
export function buildYeonjangTargetSelectionFailure(selection) {
    const message = (() => {
