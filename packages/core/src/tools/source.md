...
- `screen_capture`도 local artifact가 비어 있거나 Yeonjang 응답이 불완전하면 `screen_capture_artifact_empty` 또는 `yeonjang_screen_capture_remote_failure`를 typed side-effect result와 recovery input에 보존합니다. v2 terminal이 `permission_denied + not_started`를 서명해 보내면 Gateway는 이를 `screen_permission_denied`의 pre-effect `EFFECT_REJECTED`로 보존하고, 안전하게 고정된 운영체제 화면 권한 안내만 사용자에게 전달합니다. 이 상태는 캡처 성공·연결 장애·사용자 승인 거부로 바꾸지 않으며, 동일 target/method/params 호출은 `recovery_strategy_unchanged`로 거절해 새 승인이나 가짜 성공을 만들지 않습니다.
- `screen_capture`의 capability preflight는 MQTT v2 signed capability lease를 사용합니다. 유효한
  v2 projection이 있을 때 legacy `node.capabilities` RPC로 내려가지 않으며, capability matrix가
  실제로 없다는 pre-dispatch 계약 거절은 `yeonjang_capability_matrix_required`와
  `stopAfterFailure`를 보존해 같은 캡처를 LLM recovery가 반복하지 못하게 합니다.
