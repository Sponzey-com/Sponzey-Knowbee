...
import { buildPreparedSideEffectOperation, buildSideEffectOperationAuthorization, buildSideEffectOperationIdentity, buildSideEffectOperationReceipt, } from "../contracts/side-effect-operation.js";
...
import { prepareSideEffectOperation, } from "../runs/side-effect-operation-use-case.js";
...
export function resolveToolSideEffectOperation(input) {
    ...
    if (adapterPreparation?.status === "rejected")
        return adapterPreparation;
    const executionParams = adapterPreparation?.status === "prepared"
        ? adapterPreparation.executionParams
        : input.params;
    const targetRef = adapterPreparation?.status === "prepared"
        ? adapterPreparation.targetRef
        : contract.targetRef(input.params, input.ctx);
    const effectParams = adapterPreparation?.status === "prepared"
    ...
    const expectedState = adapterPreparation?.status === "prepared"
    ...
    return {
        ...
        operation: {
            ...
            targetRef,
            expectedState,
            authorizationParams: adapterPreparation?.status === "prepared"
                ? {
                    operationId: identity.operationId,
                    operationBindingHash,
                }
...
function existingPreparationResult(admission) {
        ...
        case "active_existing":
            return {
                ...
                details: {
                    ...
                    reasonCode: admission.status === "compensated_existing"
...
export function admitResolvedToolSideEffectOperation(operation) {
    ...
    if (admission.status === "reserved_new"
...
function boundedEffectEvidenceRefs(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return [];
    const refs = value.effectEvidenceRefs;
...
function boundedRecoveryEvidence(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return undefined;
...
function boundedEffectFailure(value) {
    if (!value?.details
        || typeof value.details !== "object"
        || Array.isArray(value.details)) {
        return undefined;
    }
    const failure = value.details.failure;
    ...
    const retrySafety = failure.retrySafety;
    return {
        reasonCode,
...
export async function executeToolWithSideEffectLedger(input) {
    ...
    const preparation = input.preparedOperation
        ? { status: "resolved", operation: input.preparedOperation }
    ...
    const { prepared, executionParams, targetRef, expectedState, authorizationParams, } = preparation.operation;
    const authorizationBinding = policy
        ? buildToolAuthorizationBinding(authorizationParams, policy.executionTargetFingerprint
            ? {
    ...
    const result = await executeSideEffectOperation({
        ...
        executeEffect: async () => {
            ...
            const preEffectRejection = !value.success
                ...
                : undefined;
            const effectEvidenceRefs = contract.effectEvidenceRefs?.(executionParams, input.ctx, value) ?? [];
            return {
                value,
        ...
        observePostState: async (value) => {
            ...
            return {
                ...
                targetFingerprint: observation.targetRef === targetRef
        ...
        ...(contract.observeCurrent
            ? {
                observeCurrentPostState: async ({ effectEvidenceRefs }) => {
                    const observation = await contract.observeCurrent?.(executionParams, input.ctx, effectEvidenceRefs);
                    return {
                        available: observation?.available ?? false,
                        targetFingerprint: (observation?.targetRef ?? targetRef) === targetRef
                            ? identity.targetFingerprint
                            : fingerprint({
                                targetRef: observation?.targetRef ?? "",
                                executionTargetFingerprint: null,
        ...
        createReceipt: ({ identity: receiptIdentity, event, operationRevision, evidence }) => {
            ...
            const effectEvidenceRefs = event === "RECORD_EFFECT" ? boundedEffectEvidenceRefs(evidence) : [];
    ...
    switch (result.status) {
        ...
        case "effect_rejected": {
            ...
            return {
                ...
                error: result.reasonCode === "side_effect_existing_effect_rejected"
