...
function canonicalObservedLink(rawHref: string, baseUrl: string): string | null {
  try {
...
function htmlToMarkdown(
  ...
  title: string
  markdown: string
  linkObservations: readonly { ordinal: number; url: string }[]
} {
  ...
  return {
    title,
    markdown,
    linkObservations: Object.freeze(
      observedUrls.map((observedUrl, index) =>
        Object.freeze({ ordinal: index + 1, url: observedUrl }),
      ),
    ),
  }
}

export function createPublicWebDocumentAdapter(
  dependencies: PublicWebDocumentDependencies = {},
): WebDocumentFetchPort {
  ...
  return async (input) => {
    ...
    try {
      ...
      let title: string
      let markdown: string
      let linkObservations: readonly { ordinal: number; url: string }[] = Object.freeze([])
      ...
      if (!markdown) return { ok: false, reasonCode: "web_document_empty", retryable: false }
      const truncated = markdown.length > input.maxMarkdownCharacters
      if (truncated) markdown = markdown.slice(0, input.maxMarkdownCharacters)
