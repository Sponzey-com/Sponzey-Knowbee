...
import { projectWebSearchMarkdown, validateWebSearchQuery, validateWebSearchResults, } from "../contracts/web-retrieval.js";
import { evaluatePublicNetworkTarget } from "../security/network-target-policy.js";
export const DEFAULT_DUCKDUCKGO_PROVIDER = Object.freeze({
...
function publicCandidateUrl(rawHref) {
    let parsed;
    try {
...
function failure(reasonCode, retryable) {
    return { ok: false, reasonCode, retryable };
}
function providerFailure(status) {
    if (status === 429)
        return failure("web_search_rate_limited", true);
    if (status >= 500)
        return failure("web_search_provider_unavailable", true);
    return failure("web_search_provider_rejected", false);
}
function buildRequestUrl(descriptor, input) {
...
export function createDuckDuckGoHtmlSearchAdapter(dependencies = {}) {
    const fetcher = dependencies.fetcher ?? globalThis.fetch;
    const now = dependencies.now ?? (() => new Date());
    const descriptor = Object.freeze(dependencies.descriptor ?? DEFAULT_DUCKDUCKGO_PROVIDER);
    return async (input) => {
        const validated = validateWebSearchQuery(input);
        if (!validated.ok)
            return failure("web_search_evidence_invalid", false);
        if (input.signal.aborted)
            return failure("web_search_cancelled", false);
        ...
        try {
            ...
            if (timeoutController.signal.aborted)
                return failure("web_search_timeout", true);
...
//# sourceMappingURL=duckduckgo-html-search.js.map