...
import {
  ...
  type LiveAcceptanceRuntimeIdentityAdmission,
} from "../release/live-acceptance-runtime-identity.js"
import {
  buildRuntimeBuildStatus,
  getGatewayProcessStartTimeMs,
  type RuntimeBuildStatus,
} from "./build-status.js"
import { getWorkspaceRootPath } from "../version.js"

interface GatewayBundleManifest {
  ...
  readonly bundledInputsSha256: string
}
...
function readManifest(value: string): GatewayBundleManifest | undefined {
  try {
    ...
    const bundledInputs = [...new Set(parsed.bundledInputs as string[])].sort()
    if (
      bundledInputs.length !== parsed.bundledInputs.length
      || bundledInputs.some(
        (path, index) => path !== (parsed.bundledInputs as string[])[index],
      )
    ) {
      return undefined
    }
...
export function createLiveAcceptanceRuntimeIdentityInspector(
  input: LiveAcceptanceRuntimeIdentityAdapterInput = {},
  ...
  const readBytes = input.readBytes ?? ((path: string) => readFileSync(path))
  const readMtimeMs = input.readMtimeMs ?? ((path: string) => statSync(path).mtimeMs)
  ...
  const readCurrent = () => {
    ...
    const bundledInputPaths = manifest.bundledInputs.map((path) =>
      resolve(workspaceRoot, path),
    )
    if (
      !inside(workspaceRoot, artifactPath)
    ...
    const bundledInputsSha256 = digestFileSet(
      manifest.bundledInputs,
      (path) => readBytes(resolve(workspaceRoot, path)),
    )
    if (
  ...
  let activeIdentity: ReturnType<typeof readCurrent>
  try {
    ...
    try {
      ...
      if (!current || !activeIdentity) {
        return Object.freeze({
          ...
          reasonCode: "live_acceptance_runtime_identity_invalid" as const,
        })
      }
      const activeBundleMatchesArtifact =
        activeIdentity.artifactSha256 === current.artifactSha256
      ...
      return admitLiveAcceptanceRuntimeIdentity({
        ...
        restartRequired:
          status.restartRequired && !activeBundleMatchesArtifact,
        manifestMatchesArtifact:
          current.manifest.bundleSha256 === current.artifactSha256,
