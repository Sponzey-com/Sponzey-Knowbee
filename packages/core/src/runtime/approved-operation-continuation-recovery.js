...
import { createArtifactStorageContext, resolveArtifactReference, } from "../artifacts/lifecycle.js";
...
import { getDb, getMessagesForRun, getSession, hasArtifactReceipt, } from "../db/index.js";
...
import { consumeApprovedOperationContinuation, } from "../runs/approved-operation-continuation-consumer.js";
import { getRootRun } from "../runs/store.js";
import { evaluateAndRecordToolPolicy } from "../security/tool-policy.js";
import { executeToolWithSideEffectLedger, resolveToolSideEffectOperation, } from "../tools/side-effect-runtime.js";
import { resolveApprovedArtifactDeliveryOperation, } from "../tools/approved-artifact-delivery-operation.js";
import { telegramSendFileTool } from "../tools/builtin/telegram-send.js";
import { createTelegramSendContinuationAdapter, } from "../tools/builtin/telegram-send-continuation.js";
import { yeonjangCameraCaptureTool } from "../tools/builtin/yeonjang.js";
import { createYeonjangCameraContinuationAdapter, } from "../tools/builtin/yeonjang-camera-continuation.js";
...
const CAMERA_TOOL = yeonjangCameraCaptureTool;
const TELEGRAM_SEND_TOOL = telegramSendFileTool;
function approvalExecutionTargetFingerprint(approvalId) {
...
function parseCameraCandidate(value) {
    ...
    const requestedFacing = input.requestedFacing === "front" || input.requestedFacing === "rear"
...
function findExactCameraToolUseId(input) {
    const messages = getMessagesForRun(input.sessionId, input.continuation.runId);
    for (let index = messages.length - 1; index >= 0; index -= 1) {
        ...
        for (const block of blocks) {
            ...
            if (projected?.operationId === input.continuation.operationId
...
function verifiedCameraResult(result, input) {
    ...
    if (result.success
        ...
        const resolved = resolveArtifactReference({
            ...
            requestGroupId: input.continuation.requestGroupId ?? input.continuation.runId,
    ...
    const aggregate = repository.listByRun(input.continuation.runId).find((candidate) => candidate.identity.operationId === input.continuation.operationId
        && candidate.state === "VERIFIED");
    const effectTransition = aggregate?.transitions.find((transition) => transition.event === "RECORD_EFFECT");
    const receipt = effectTransition
        ? repository.loadReceipt(effectTransition.receiptRef)
        : undefined;
    const artifactRef = receipt?.evidenceRefs.find((ref) => ref.startsWith("artifact:"));
    if (!artifactRef)
        return undefined;
    const resolved = resolveArtifactReference({
        artifactRef,
    ...
    if (!resolved.ok || resolved.sizeBytes < 1)
        return undefined;
    return {
        success: true,
...
function createCameraAdapter(input) {
    ...
    const executionTargetFingerprint = approvalExecutionTargetFingerprint(input.continuation.approvalId);
    const project = (params) => {
        ...
        return resolution.status === "resolved"
            ? {
                ...
                operationBindingHash: resolution.operation.prepared.operationBindingHash,
    ...
    return createYeonjangCameraContinuationAdapter({
        ...
        execute: async (params, continuation, signal) => {
            ...
            const result = await executeToolWithSideEffectLedger({
                ...
                ctx: {
                    ...
                    authorizationReceipt: Object.freeze({
                        ...
                        requestGroupId: continuation.requestGroupId ?? continuation.runId,
                        ...(executionTargetFingerprint
                            ? { executionTargetFingerprint }
                            : {}),
                        approvalDecision: continuation.decision,
...
function createTelegramDeliveryAdapter(input) {
    ...
    return createTelegramSendContinuationAdapter({
        ...
        projectOperation: (candidate) => {
            ...
            return operation
                ? {
                    ...
                    operationBindingHash: operation.binding.operationBindingHash,
                }
                : null;
        },
        execute: async (candidate, continuation, signal) => {
            ...
            const target = session?.source === "telegram" && session.source_id
            ...
            const exactTarget = `${target.chatId}${target.threadId !== undefined
                ? `:${target.threadId}`
                : ""}`;
            const directDelivery = delivery?.artifactDeliveries?.some((entry) => entry.channel === "telegram"
                && entry.filePath === resolved.filePath
                && !entry.url
                && !entry.downloadUrl
                && entry.deliveryReceipts?.some((receipt) => receipt.status === "sent"
            ...
            if (!directDelivery && !durableDelivery) {
                ...
                return {
                    ...
                    result: {
                        success: false,
                        output: "The approved Telegram artifact delivery did not produce a verified provider receipt.",
                        error: "TELEGRAM_DELIVERY_PROVIDER_UNVERIFIED",
                        details: {
                            ...result.details,
                            deliveryVerification: {
                                status: "failed",
                                reasonCode: "telegram_delivery_continuation_provider_unverified",
...
export async function recoverApprovedOperationContinuations(input) {
    const repository = new SqliteApprovedOperationContinuationRepository(getDb());
    const ownerId = input.ownerId?.trim() || `startup-continuation:${randomUUID()}`;
    let claimed = 0;
    let completed = 0;
    let blocked = 0;
            ...
            adapters: [
                ...
                createTelegramDeliveryAdapter({
                    ...
                    ...(input.resolveDeliveryHandler
                        ? {
                            resolveDeliveryHandler: input.resolveDeliveryHandler,
                        }
                        : {}),
                }),
            ],
            handoffCompletedResult: async ({ continuation, toolUseId, result, }) => handoffApprovedOperationResult({
                continuation,
                toolUseId,
                result,
            }),
...
//# sourceMappingURL=approved-operation-continuation-recovery.js.map