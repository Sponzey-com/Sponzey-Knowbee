...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    version: 53,
    up(db) {
      const occupiedNames = new Map<string, { entityType: "agent" | "team"; entityId: string }>()
      const agentNames = db
        .prepare<[], { agent_id: string; agent_name: string }>(
          "SELECT agent_id, agent_name FROM agent_configs ORDER BY agent_id ASC",
        )
        .all()
      for (const row of agentNames) {
        const normalized = normalizeAgentName(row.agent_name)
        if (!normalized) throw new Error(`Agent ${row.agent_id} has no canonical name.`)
        occupiedNames.set(normalized, { entityType: "agent", entityId: row.agent_id })
      }

      const teamNames = db
        .prepare<[], { team_id: string; display_name: string }>(
          "SELECT team_id, display_name FROM team_configs ORDER BY team_id ASC",
        )
        .all()
      for (const row of teamNames) {
        const normalized = normalizeAgentName(row.display_name)
        if (!normalized) throw new Error(`Team ${row.team_id} has no canonical display name.`)
        const existing = occupiedNames.get(normalized)
        if (existing) {
          throw new Error(
            `Team ${row.team_id} display name conflicts with ${existing.entityType} ${existing.entityId}.`,
          )
        }
      ...
      const teamRows = db
        ...
        >(
          `SELECT team_id, display_name, status, source, config_json, created_at, updated_at
           FROM team_configs ORDER BY team_id ASC`,
        )
        .all()
      const updateConfig = db.prepare<[string, string]>(
        "UPDATE team_configs SET config_json = ? WHERE team_id = ?",
      )
      ...
      for (const row of teamRows) {
        const config = parseJsonRecord(row.config_json) ?? {}
        const canonicalConfig: Record<string, unknown> = {
          ...config,
        ...
        insertNamespace.run(
          normalizeAgentName(row.display_name),
          row.team_id,
          row.display_name,
          row.status,
          row.source,
      ...
      const planRows = db
        .prepare<[], { team_execution_plan_id: string; contract_json: string }>(
          "SELECT team_execution_plan_id, contract_json FROM team_execution_plans",
        )
        .all()
      const updatePlan = db.prepare<[string, string]>(
        "UPDATE team_execution_plans SET contract_json = ? WHERE team_execution_plan_id = ?",
      )
      for (const row of planRows) {
        ...
        if (!asString(contract["teamNameSnapshot"])) {
          ...
          if (legacySnapshot) contract["teamNameSnapshot"] = legacySnapshot
        }
