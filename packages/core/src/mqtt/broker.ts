...
import {
  fencePersistedYeonjangMqttV2LivenessAtStartup,
...
} from "../yeonjang/mqtt-v2-contract.js"
import {
  projectYeonjangMqttV2CapabilitiesToRegistryObservation,
  projectYeonjangMqttV2StatusToRegistryObservation,
} from "../yeonjang/mqtt-v2-registry-adapter.js"
import { createMqttClientErrorLogThrottle } from "./client-error-log-throttle.js"

function mqttBrokerErrorMessage(error: unknown): string {
...
export type MqttV2RequesterRouteAdmission =
  | { readonly ok: true; readonly requesterId: string | null }
  | { readonly ok: false; readonly reasonCode: "mqtt_v2_requester_config_required" | "mqtt_v2_requester_mismatch" }
 ...
 */
export function admitMqttV2RequesterRoute(topic: unknown, configuredRequesterId: string): MqttV2RequesterRouteAdmission {
  if (typeof topic !== "string") return { ok: true, requesterId: null }
  ...
  if (!isRequesterRoute) return { ok: true, requesterId: null }
  const requesterId = segments[7] ?? ""
  const expected = configuredRequesterId.trim()
  if (!expected) return { ok: false, reasonCode: "mqtt_v2_requester_config_required" }
  if (requesterId !== expected) return { ok: false, reasonCode: "mqtt_v2_requester_mismatch" }
...
const authenticatedClientGenerationById = new Map<string, number>()
const activeClientGenerationById = new Map<string, number>()
const admittedMqttV2RequesterGenerationById = new Map<string, number>()
...
const mqttV2StatusGenerationByExtensionId = new Map<string, number>()
const mqttV2CapabilitiesGenerationByExtensionId = new Map<string, number>()
const exchangeLogs: MqttExchangeLogEntry[] = []
const MAX_EXCHANGE_LOGS = 120
let exchangeLogSequence = 0
let mqttV2ObservationHmacKey: Buffer | null = null
let mqttV2LivenessSweepTimer: ReturnType<typeof setInterval> | null = null
...
function recordAuthenticatedMqttClient(client: Client | undefined): void {
...
export type MqttV2RequesterRouteFailureReason =
  | "mqtt_v2_requester_config_required"
  | "mqtt_v2_requester_mismatch"

/** Keeps broker credential failure distinct from exact requester admission. */
export function createMqttV2RequesterRouteError(
  reasonCode: MqttV2RequesterRouteFailureReason,
): Error & { readonly code: MqttV2RequesterRouteFailureReason; readonly returnCode: 5 } {
  return Object.assign(
    new Error(`MQTT v2 requester route rejected: ${reasonCode}`),
    { code: reasonCode, returnCode: 5 as const },
  )
}
...
function releaseExtensionClaimsForClient(
  clientId: string | null | undefined,
  client?: Client,
): void {
  const normalizedClientId = clientId?.trim()
  if (!normalizedClientId) return
  if (client && activeClientsById.get(normalizedClientId) !== client) return
...
function handleBrokerPublish(
  packet: { topic?: unknown; payload?: unknown; retain?: unknown },
  client: Client | undefined,
): void {
  const topic = typeof packet?.topic === "string" ? packet.topic : null
  if (!topic) return
  if (handleMqttV2Observation(packet, client, topic)) return
...
function handleMqttV2Observation(
  packet: { payload?: unknown; retain?: unknown },
  client: Client | undefined,
  topic: string,
): boolean {
  ...
  const existing = listYeonjangRegistryInstances().find(
    (instance) => instance.instanceId === admitted.observation.instanceId,
  ) ?? null
  const previous = extensionSnapshots.get(admitted.observation.instanceId)
  if (admitted.observation.kind === "capabilities") {
    if (
      mqttV2CapabilitiesGenerationByExtensionId.get(admitted.observation.instanceId)
        === activeGeneration
      &&
    ...
    mqttV2CapabilitiesGenerationByExtensionId.set(
  ...
  if (
    mqttV2StatusGenerationByExtensionId.get(admitted.observation.instanceId)
      === activeGeneration
    &&
    previous?.sessionId === admitted.observation.sessionId
    && previous.v2StatusSequence != null
  ...
  if (
    existing?.session?.sessionId === admitted.observation.sessionId
  ...
  mqttV2StatusGenerationByExtensionId.set(
    admitted.observation.instanceId,
    activeGeneration,
  )
...
export function expireMqttV2Observations(nowMs: number): { readonly expiredCount: number } {
...
function startMqttV2LivenessSweep(): void {
  if (mqttV2LivenessSweepTimer) return
...
function stopMqttV2LivenessSweep(): void {
  if (!mqttV2LivenessSweepTimer) return
...
export async function startMqttBroker(config: MqttConfig): Promise<void> {
  ...
  if (startupFence.fencedInstanceCount > 0) {
    log.info(
      `MQTT v2 persisted liveness fenced at broker startup (count=${startupFence.fencedInstanceCount})`,
    )
  }
  ...
  brokerInstance.on("clientReady", (client) => {
    ...
    if (clientId && client) {
      ...
      if (generation !== undefined) activeClientGenerationById.set(clientId, generation)
    }
    syncClientCount()
  })
  brokerInstance.on("clientDisconnect", (client) => {
    releaseExtensionClaimsForClient(
      (client as Client | undefined)?.id,
  ...
  brokerInstance.on("publish", (packet, client) => {
    handleBrokerPublish(
      packet as { topic?: unknown; payload?: unknown; retain?: unknown },
      client as Client | undefined,
    )
  })
...
function clearMqttV2ObservationKey(): void {
...
export async function restartMqttBrokerFromConfig(config: Pick<KnowbeeConfig, "mqtt">): Promise<void> {
  if (server || broker) {
    try {
