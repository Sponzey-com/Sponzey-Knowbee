...
function exact(existing, expected) {
...
function recordSequence(input, dependencies) {
    ...
    for (const [index, item] of input.receipts.entries()) {
        ...
        if (!event)
            return { ok: false, reasonCode: "topology_event_sequence_invalid" };
...
export function buildCanonicalTopologyAdmissionDescriptor(input) {
    if (input.signalAborted)
        return { ok: false, reasonCode: "topology_execution_cancelled" };
    ...
    const selectedExecutorId = input.route.selectedExecutorId?.trim() || input.route.entryNodeId.trim();
...
export function recordCanonicalTopologyAdmission(descriptor, dependencies) {
...
export function buildCanonicalTopologyResultDescriptor(input) {
    const runId = input.runId.trim();
    if (!runId)
        return { ok: false, reasonCode: "topology_result_invalid" };
    ...
    if (!input.result.ok) {
        ...
        if (REQUIRED_SOLUTION_PATHS.some((path) => !terminal.evidenceRefs.some((ref) => ref.startsWith(`solution-path-review:${path}:`)))) {
            return { ok: false, reasonCode: "topology_terminal_paths_incomplete" };
        }
        if (terminal.reasonCode === "solution_paths_exhausted" &&
        ...
        const terminalEvent = policyBlocked ? "POLICY_BLOCKED" : "PATHS_EXHAUSTED";
        const finalOutcome = policyBlocked ? "blocked" : "exhausted";
        return {
            ok: true,
            descriptor: {
                runId,
    ...
    const partialOutputIds = report.outputs
        ...
        .map((output) => output.outputId)
        .sort();
    const verificationEvent = report.status === "completed" &&
...
export function recordCanonicalTopologyResult(descriptor, dependencies) {
    ...
    const events = descriptor.verificationEvent
...
//# sourceMappingURL=canonical-topology-lifecycle.js.map