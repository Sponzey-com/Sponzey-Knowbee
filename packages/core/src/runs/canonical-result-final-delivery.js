...
const INTERNAL_DETAIL = /(?:diagnosis|evidence|receipt|request|run|session|agent|goal|work)[_-]?(?:id|ref)?\s*[:=]/iu;
...
function invalidLanguage(value, expected) {
...
export async function renderCanonicalResultReport(input) {
    ...
    const llmInput = {
        instruction: "Return the result, one or two verified reason sentences, one concise next action when supplied, and final user-facing text. Use only supplied facts and the requested language.",
    ...
    for (let attempt = 0; attempt <= policy.maxRepairAttempts; attempt += 1) {
        const output = await input.render(attempt === 0 ? llmInput : { ...llmInput, reviewFeedback: lastReasonCode });
...
function reviewCanonicalResultOutput(output, facts, policy) {
    if (output.result !== facts.outcome)
        return "result_mismatch";
    const reason = output.reason.trim();
    if (sentenceCount(reason) < 1 || sentenceCount(reason) > 2) {
        return "reason_sentence_count_invalid";
    }
    if (reason.length > policy.maxReasonCharacters)
        return "reason_too_long";
    if (SPECULATION.test(reason))
    ...
    if (facts.outcome !== "completed" && !nextAction)
        return "next_action_missing";
    if (nextAction.length > policy.maxNextActionCharacters)
        return "next_action_too_long";
    const text = output.text.trim();
    if (!text)
        return "report_text_missing";
    if (text.length > policy.maxReportCharacters)
        return "report_text_too_long";
    if (invalidLanguage([reason, nextAction, text].filter(Boolean).join(" "), facts.primaryLanguage)) {
...
//# sourceMappingURL=canonical-result-final-delivery.js.map