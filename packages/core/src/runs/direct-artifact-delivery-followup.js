...
export function enforceDirectArtifactDeliveryFollowup(input) {
    if (!input.deliveryOutcome.directArtifactDeliveryRequested
...
function latestVerifiedArtifact(tools) {
    for (const tool of [...tools].reverse()) {
        const details = record(tool.details);
        if (!details || details.kind !== "camera_artifact")
            continue;
...
//# sourceMappingURL=direct-artifact-delivery-followup.js.map