...
export function buildCanonicalApprovalTransitionDescriptor(input) {
    ...
    if (!/^sha256:[a-f0-9]{64}$/u.test(input.operationBinding.operationBindingHash)
    ...
    const digest = createHash("sha256")
        ...
        continuationSchemaVersion: input.operationBinding.continuationSchemaVersion,
    }))
        .digest("hex");
...
export function recordCanonicalApprovalTransition(descriptor, dependencies) {
    const receipt = {
    ...
    if (!issued.issued) {
        ...
        const exact = existing
            ...
            && existing.evidenceRefs.every((ref, index) => ref === receipt.evidenceRefs[index])
    ...
    return transition.status === "applied"
        ...
        : {
            ok: false,
            reasonCode: transition.reasonCode ?? "canonical_approval_transition_rejected",
        };
}
//# sourceMappingURL=canonical-approval-transition.js.map