...
export function evaluateSuccessfulToolEvidenceTrust(evidence) {
...
function trustEligibleSuccessfulTools(evidence) {
...
function recoveryPromptSectionText(key, variables = {}) {
    ...
    if (!value)
        throw new Error(`recovery prompt section text missing: ${key}`);
    return value;
}
...
export function buildYeonjangFailureEvidenceRecoveryPayload(evidence) {
    if (!evidence.toolName.startsWith("yeonjang_"))
        return null;
    ...
    const boundedPostCheckReason = postCheckReason && /^[a-z0-9_:-]{1,96}$/u.test(postCheckReason)
        ? postCheckReason
        : postCheckReason
            ? "post_check_failed"
            : null;
    const methodIds = Array.isArray(normalizedEvidence?.methodIds)
        ? normalizedEvidence.methodIds.filter((method) => typeof method === "string" && method.trim().length > 0)
        : [];
