...
function hasFinalResponseContext(
  context: LoopDirectiveResponseContext | undefined,
): context is LoopDirectiveResponseContext {
  return Boolean(
...
type TerminalLoopDirective = Extract<LoopDirective, { kind: "awaiting_user" | "stop" }>

function shouldRewriteTerminalDirective(directive: TerminalLoopDirective): boolean {
  const source = resolveTerminalDirectiveTextSource(directive)
  return userFacingTextSourceRequiresFinalResponseReview(source)
}

function hasAppendedTerminalDirectiveText(directive: TerminalLoopDirective): boolean {
  return Boolean(
    directive.preview.trim() ||
      directive.reason?.trim() ||
      directive.remainingItems?.some((item) => item.trim()),
  )
}

function resolveTerminalDirectiveTextSource(
  directive: TerminalLoopDirective,
): UserFacingTextSource {
  const source = directive.userMessageSource ?? "runtime_deterministic"
  if (
    (source === "llm_generated" || source === "llm_reviewed") &&
...
async function resolveTerminalDirectiveMessage(params: {
  ...
  renderFinalResponseText: typeof renderFinalResponseText
}): Promise<{ userMessage?: string | undefined; userMessageSource: UserFacingTextSource }> {
  ...
  if (!hasFinalResponseContext(responseContext)) {
    params.appendRunEvent(
      params.runId,
      `user_facing_${params.directive.kind}_rewrite_blocked:missing_context`,
    )
    return {
      userMessageSource: source,
    }
  ...
  if (!rawText.trim()) {
    params.appendRunEvent(
      params.runId,
      `user_facing_${params.directive.kind}_rewrite_blocked:empty_input`,
    )
    return {
      userMessageSource: source,
    }
  ...
  try {
    ...
    params.appendRunEvent(
      params.runId,
      `user_facing_${params.directive.kind}_rewrite_blocked:empty_output`,
    )
  } catch (error) {
    const detail = safeRenderErrorDetail(error)
    params.appendRunEvent(
      params.runId,
      `user_facing_${params.directive.kind}_rewrite_blocked:error:${detail}`,
    )
  }
  return {
    userMessageSource: source,
...
export async function applyLoopDirective(
  ...
  if (params.directive.kind === "complete") {
    if (params.directive.notice) {
      params.finalizationDependencies.appendRunEvent(
        params.runId,
        `user_facing_loop_directive_notice:${params.directive.notice.textSource}:non_final`,
      )
    }
    params.finalizationDependencies.appendRunEvent(
      params.runId,
      `user_facing_text_source:${params.directive.textSource}`,
    )
    params.finalizationDependencies.appendRunEvent(
      params.runId,
      hasFinalResponseContext(params.responseContext)
  ...
  if (params.directive.kind === "retry_intake" || params.directive.kind === "execute") {
    throw new Error(
      `${params.directive.kind} directive must be handled inside the main loop before applyLoopDirective`,
    )
  }
  ...
  params.finalizationDependencies.appendRunEvent(
    params.runId,
    `user_facing_${params.directive.kind}_message_source:${resolvedTerminalMessage.userMessageSource}`,
  )
  return "break"
}
