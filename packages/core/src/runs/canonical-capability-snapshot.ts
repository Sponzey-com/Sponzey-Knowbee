...
function riskForTool(tool: AnyTool): CanonicalCapabilityRisk {
  return tool.requiresApproval || tool.riskLevel !== "safe" ? "approval_required" : "safe"
}

function runtimeHealthKey(observation: CapabilityRuntimeHealthObservation): string {
  return `${observation.capabilityId}\u0000${observation.targetId}`
}

function normalizedRuntimeHealthObservations(
  observations: CapabilityRuntimeHealthObservation[] | undefined,
): CapabilityRuntimeHealthObservation[] | undefined {
  if (!observations) return undefined
  const unique = new Map<string, CapabilityRuntimeHealthObservation>()
  for (const observation of observations) {
    const normalized: CapabilityRuntimeHealthObservation = {
  ...
  return [...unique.values()].sort((left, right) =>
    runtimeHealthKey(left).localeCompare(runtimeHealthKey(right)),
  )
}

function fallbackAgentEligibility(agent: OrchestrationRegistrySnapshot["agents"][number]): {
  eligible: boolean
  reasonCodes: string[]
} {
  const reasonCodes: string[] = []
  if (agent.status !== "enabled") reasonCodes.push(`agent_${agent.status}`)
  if (!agent.delegationEnabled) reasonCodes.push("delegation_disabled")
  ...
  if (agent.modelSummary?.availability === "unavailable") reasonCodes.push("model_unavailable")
...
function agentEligibility(
  ...
  agent: OrchestrationRegistrySnapshot["agents"][number],
): { eligible: boolean; reasonCodes: string[] } {
  const index = registry.capabilityIndex
  if (!index) return fallbackAgentEligibility(agent)
  const candidates = (index.candidatesByAgentId[agent.agentId] ?? []).filter(
    (candidate) => candidate.parentAgentId === index.rootAgentId,
  )
...
function projectAgentCapabilities(
  ...
  const projected = new Map<string, AgentCapabilityProjection>()
  const add = (candidate: AgentCapabilityProjection): void => {
    ...
    projected.set(capabilityId, {
      ...
      risk:
        existing.risk === "approval_required" || normalized.risk === "approval_required"
          ? "approval_required"
          : "safe",
    })
  }
  ...
  for (const binding of bindings) {
    ...
    for (const toolName of binding.enabledToolNames) {
      const toolDisabled =
        disabledTools.has(toolName) || (binding.disabledToolNames ?? []).includes(toolName)
      add({
...
export function projectCanonicalCapabilitySnapshot(input: {
  ...
  const bindings: CanonicalCapabilityBindingSnapshot[] = []
  const exclusions: CanonicalCapabilityExclusionSnapshot[] = []
  ...
  for (const tool of input.tools) {
    ...
    if (
      ...
      const matching = healthObservations.filter(
        (observation) => observation.capabilityId === capabilityId,
      )
      if (tool.runtimeHealthMode === "additional") {
        bindings.push({ capabilityId, targetId: rootAgentId, risk: riskForTool(tool) })
      }
      if (matching.length === 0) {
        if (tool.runtimeHealthMode === "additional") continue
        exclusions.push({
          capabilityId,
          targetId: rootAgentId,
  ...
  const uniqueBindings = new Map<string, CanonicalCapabilityBindingSnapshot>()
  for (const binding of bindings) {
    uniqueBindings.set(`${binding.capabilityId}\u0000${binding.targetId}`, binding)
  }
  const uniqueExclusions = new Map<string, CanonicalCapabilityExclusionSnapshot>()
  for (const exclusion of exclusions) {
    uniqueExclusions.set(`${exclusion.capabilityId}\u0000${exclusion.targetId}`, exclusion)
  }
  const byIdentity = <T extends { capabilityId: string; targetId: string }>(left: T, right: T) =>
    `${left.capabilityId}\u0000${left.targetId}`.localeCompare(
      `${right.capabilityId}\u0000${right.targetId}`,
    )
  return {
