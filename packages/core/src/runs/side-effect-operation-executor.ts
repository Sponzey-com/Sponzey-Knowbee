import {
  type SideEffectOperationAuthorization,
...
} from "./side-effect-operation-use-case.js"
import {
  type SideEffectObservationEvidence,
  decideResumedSideEffectVerification,
  decideSideEffectRemediation,
  decideSideEffectVerification,
} from "./side-effect-verification.js"

type Fingerprint = `sha256:${string}`

export type SideEffectOperationExecutionResult<T> =
  | { status: "verified"; value: T; aggregate: SideEffectOperationAggregate }
  | { status: "duplicate_verified"; aggregate: SideEffectOperationAggregate }
...
export async function executeSideEffectOperation<T>(
  input: {
    ...
    verifyCompensation?:
      | (() => Promise<{ verified: boolean; receiptEvidence: unknown }>)
      | undefined
  },
  dependencies: {
  ...
  const transition = (
    ...
    if (result.status === "applied") aggregate = result.aggregate
    return result
  }

  if (
    aggregate.state === "EFFECT_RECORDED" ||
  ...
  if (
    remediation.action === "manual_intervention" ||
    !input.compensate ||
  ...
  const compensationEvent =
    compensation.success && compensationVerification.verified
