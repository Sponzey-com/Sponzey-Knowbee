...
type CanonicalFinalizationReceiptKind =
  | "verification"
  | "exhaustion"
  | "blocker"
  | "input_requirement"
...
export function buildCanonicalPolicyBlockedDescriptor(input: {
  ...
  | { ok: true; descriptor: CanonicalFinalizationTransitionDescriptor }
...
}): { ok: true; descriptor: CanonicalFinalizationTransitionDescriptor } | { ok: false; reasonCode: string } {
  const runId = required(input.runId, "Run ID")
...
function buildReceipt(input: {
  ...
  evidence: unknown
  evidenceRefs: string[]
  evidenceMap?: CanonicalFinalizationEvidenceMap | undefined
  terminalCause?: CanonicalTerminalCause | undefined
}): CanonicalFinalizationReceiptDescriptor {
  const evidenceFingerprint = `sha256:${hash(JSON.stringify(input.evidence))}` as const
  return {
...
function sameTerminalCause(
  left: CanonicalTerminalCause | undefined,
  right: CanonicalTerminalCause | undefined,
): boolean {
...
function buildCompletionEvidenceMap(
  review: CompletionReviewResult | null,
): CanonicalFinalizationEvidenceMap | undefined {
  ...
  if (criterionEvidenceRefs.length === 0 && conditionEvidenceRefs.length === 0) return undefined
  return {
...
function validateLlmDiagnosisContext(input: {
  review: CompletionReviewResult | null
  expected: CompletionReviewContextReceipt | undefined
}): string | null {
  ...
  if (
    actual.contextFingerprint !== input.expected.contextFingerprint
...
export function buildCanonicalCompletionOutcomeDescriptor(input: {
  ...
  | { ok: true; descriptor: CanonicalFinalizationTransitionDescriptor | null }
  | { ok: false; reasonCode: string } {
  const runId = required(input.runId, "Run ID")
  if (input.application.kind === "retry") return { ok: true, descriptor: null }
  ...
  if (input.application.kind === "complete") {
    ...
    if (input.requiresLlmResultDiagnosis) {
      ...
      if (contextFailure) return { ok: false, reasonCode: contextFailure }
...
export function buildCanonicalDeliveryDescriptor(input: {
  ...
  delivery: Pick<
    ...
    "status" | "deliveryKey" | "idempotencyKey" | "existingEventId"
  >
}):
  | { ok: true; descriptor: CanonicalFinalizationTransitionDescriptor }
  | { ok: false; reasonCode: string } {
...
export function recordCanonicalFinalizationTransition(
  descriptor: CanonicalFinalizationTransitionDescriptor,
  dependencies: CanonicalFinalizationTransitionDependencies,
