...
import {
  ...
  type WebEvidencePipelineResult,
...
export function projectValidatedWebToolResultForAgent(
...
function normalizeFactKeys(values: readonly string[]): readonly string[] {
...
function pipelineFailure(reasonCode: string): ToolResult {
  return {
    success: false,
    output: "",
    error: reasonCode,
...
function projectVerification(
  verification: Extract<WebEvidencePipelineResult, { ok: true }>["value"],
): ToolResult {
  const details = {
...
export async function projectWebSearchResultForAgent(
  input: Readonly<{
    requestGoal: string
...
export async function projectWebFetchResultForAgent(
  input: Readonly<{
    requestGoal: string
    requiredFactKeys: readonly string[]
    modelContextTokens: number
