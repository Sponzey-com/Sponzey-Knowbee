...
import {
  ...
  type RequestDeliveryOutcomeStatus,
  type RequestExecutionOutcome,
} from "./flow-contract.js"
import {
  finalizeDeliveryForRun,
  messageLedgerEventHasRequiredDeliveryEvidence,
...
import {
  ...
  type CanonicalWorkTransitionUseCaseInput,
  type CanonicalWorkTransitionUseCaseResult,
} from "./canonical-work-transition-use-case.js"
...
function runtimeManifestMatches(run: RootRun): boolean {
...
export function recoverActiveRunsOnStartup(): RootRun[] {
  ...
  const pendingResponseRepository = new SqliteCanonicalPendingResponseRepository(
    getDb(),
    () => Date.now(),
  )
  ...
  for (const aggregate of recoverableAggregates) {
    ...
    if (run) activeById.set(run.id, run)
  }
  ...
  for (const run of activeRuns) {
    ...
    const responseArtifactAvailable = (() => {
      ...
      try {
        ...
        if (!pending || pending.workId !== aggregate.workId) return false
        if (!pending.reviewEnvelope || pending.reviewIssue) return false
    ...
    const committedDelivery = aggregate && aggregate.revision > 0
      ? listMessageLedgerEvents({ runId: run.id, limit: 1_000 }).find((event) =>
    ...
    if (canonicalDecision?.kind === "resume_delivery"
      ...
      if (aggregateRevision === undefined) continue
      ...
      ? canonicalDecision.kind === "resume_waiting"
        ...
        : canonicalDecision.kind === "resume_delivery"
          ? {
              ...
              nextRunStatus: "awaiting_user" as const,
          ...
          : {
              ...
              nextRunStatus: "interrupted" as const,
...
function resolveStoredDeliveryOutcome(runId: string): RequestDeliveryOutcomeStatus {
  ...
  const pendingResponse = new SqliteCanonicalPendingResponseRepository(
    getDb(),
    () => Date.now(),
  ).loadPending(runId)
  if (pendingResponse) return "pending"
  if (
    events.some(
      (event) =>
  ...
  if (events.some((event) => event.event_kind === "final_answer_generated")) return "pending"
  return "not_started"
}

export function getRequestExecutionOutcome(
  runId: string,
): RequestExecutionOutcome | undefined {
...
export function createRootRun(params: {
  ...
  const tx = db.transaction(() => {
    ...
    if (identifiers.runScope === "root") {
      ...
      if (!created.created) throw new Error("Canonical root work aggregate already exists.")
    }
  })
  ...
  if (!run) throw new Error(`Created root run could not be reloaded: ${params.id}`)
  recordMessageLedgerEvent({
    runId: params.id,
  ...
  if (run.runScope === "root") {
    recordCanonicalRequestReceivedObservability({
      ...
      onDegraded: (error) => log.fieldDebug(
        `Typed ingress observability write degraded: ${redactLogText(
...
export type CanonicalRunTransitionStoreResult =
  | (Extract<CanonicalWorkTransitionUseCaseResult, { status: "applied" }> & { run: RootRun })
  | Exclude<CanonicalWorkTransitionUseCaseResult, { status: "applied" }>
  | { status: "receipt_rejected"; reasonCode: "receipt_not_found" | "receipt_invalid" | "receipt_scope_mismatch" | "receipt_kind_mismatch" | "receipt_already_consumed" }
  | { status: "persistence_failed"; reasonCode: "canonical_run_transition_persistence_failed" }

export function applyCanonicalRunTransition(
  command: CanonicalWorkTransitionUseCaseInput & { runId: string },
): CanonicalRunTransitionStoreResult {
  const db = getDb()
  let result: CanonicalRunTransitionStoreResult
  try {
    result = db.transaction((): CanonicalRunTransitionStoreResult => {
      const run = getRootRun(command.runId)
      if (!run) return { status: "rejected", reasonCode: "aggregate_not_found" }
      ...
      if (!receipt) return { status: "receipt_rejected", reasonCode: "receipt_not_found" }
      const receiptValidation = validateCanonicalWorkReceiptForEvent({
        receipt,
        workId: canonicalWorkIdForRootRun(command.runId),
        event: command.event,
      })
      if (!receiptValidation.ok) return { status: "receipt_rejected", reasonCode: receiptValidation.reasonCode }
      ...
      if (!consumed.consumed) throw new Error("Canonical transition receipt consumption failed.")
      ...
      if (update.changes !== 1) throw new Error("Canonical RootRun projection target is missing.")
      const updated = getRootRun(command.runId)
      if (!updated) throw new Error("Canonical RootRun projection could not be loaded.")
  ...
  if (result.status === "applied") {
    ...
    if (result.run.status === "completed") eventBus.emit("run.completed", { run: result.run })
    if (result.run.status === "failed") eventBus.emit("run.failed", { run: result.run })
    if (result.run.status === "cancelled") eventBus.emit("run.cancelled", { run: result.run })
  }
  return result
}
...
export function clearHistoricalRunHistory(): { deletedRunCount: number } {
  ...
  const requestGroupIds = [...new Set(deletableRows.map((row) => row.request_group_id).filter((value): value is string => typeof value === "string" && value.length > 0))]
