...
} from "../contracts/web-research-observation.js"
import {
...
} from "./web-evidence-pack.js"
import {
  verifyWebEvidencePack,
  type WebEvidenceVerifierPort,
} from "./web-evidence-verifier.js"
...
export type WebEvidencePipelineResult =
  ...
  | Readonly<{
      ok: false
      reasonCode:
        | "web_evidence_pipeline_cancelled"
        | "web_evidence_pipeline_budget_failed"
        | "web_evidence_pipeline_search_observation_failed"
        | "web_evidence_pipeline_source_selection_failed"
        | "web_evidence_pipeline_fetch_failed"
        | "web_evidence_pipeline_chunk_failed"
        | "web_evidence_pipeline_chunk_selection_failed"
        | "web_evidence_pipeline_compression_failed"
        | "web_evidence_pipeline_pack_failed"
        | "web_evidence_pipeline_verification_failed"
    }>

type FailureReason = Extract<WebEvidencePipelineResult, { ok: false }>["reasonCode"]

function failure(reasonCode: FailureReason): WebEvidencePipelineResult {
...
export async function runWebEvidencePipeline(
  ...
  if (cancelled()) return failure("web_evidence_pipeline_cancelled")
  ...
  if (!budget.ok) return failure("web_evidence_pipeline_budget_failed")
  ...
  if (!sourceSnapshot.ok) return failure("web_evidence_pipeline_search_observation_failed")
  ...
  if (cancelled()) return failure("web_evidence_pipeline_cancelled")
  if (!selectedSources.ok) return failure("web_evidence_pipeline_source_selection_failed")

  const compressionResults = []
  for (const selection of selectedSources.value.selections) {
    const candidate = sourceSnapshot.value.candidates.find(
      (item) => item.candidateRef === selection.candidateRef,
    )
    if (!candidate) return failure("web_evidence_pipeline_source_selection_failed")

    let fetchedResult: ToolResult
    try {
    ...
    if (!chunked.ok) return failure("web_evidence_pipeline_chunk_failed")
    const chunkSnapshot = createWebChunkSelectionSnapshot(chunked.value)
    if (!chunkSnapshot.ok) return failure("web_evidence_pipeline_chunk_failed")
    ...
    const chunks = selectedChunks.value.selections.map((selected) =>
      chunkSnapshot.value.chunks.find((chunk) => chunk.chunkRef === selected.chunkRef))
    ...
    const compressed = await compressWebResearchEvidence({
      ...
      selectedChunks: chunks as typeof chunkSnapshot.value.chunks,
    }, dependencies.compressionPort)
    if (cancelled()) return failure("web_evidence_pipeline_cancelled")
    if (!compressed.ok) return failure("web_evidence_pipeline_compression_failed")
  ...
  if (cancelled()) return failure("web_evidence_pipeline_cancelled")
  if (!evidencePack.ok) return failure("web_evidence_pipeline_pack_failed")
  ...
  if (cancelled()) return failure("web_evidence_pipeline_cancelled")
  if (!verification.ok) return failure("web_evidence_pipeline_verification_failed")
  return Object.freeze({ ok: true, value: verification.value })
}

export async function runDirectWebEvidencePipeline(
  input: Readonly<{
    requestGoal: string
    requiredFactKeys: readonly string[]
    modelContextTokens: number
  ...
  const documentObservation = projectWebToolResultObservation(
    "web_fetch",
  ...
  if (cancelled()) return failure("web_evidence_pipeline_cancelled")
  if (!selectedChunks.ok) return failure("web_evidence_pipeline_chunk_selection_failed")
  const chunks = selectedChunks.value.selections.map((selected) =>
    chunkSnapshot.value.chunks.find((chunk) => chunk.chunkRef === selected.chunkRef))
  if (chunks.some((chunk) => !chunk)) {
    return failure("web_evidence_pipeline_chunk_selection_failed")
