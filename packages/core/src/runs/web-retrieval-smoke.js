...
function sanitizeText(value) {
    ...
    if (/(<!doctype\s+html|<html\b|<script\b|<body\b)/iu.test(text))
...
function validateFixture(fixture) {
    ...
    if (!expectation)
        failures.push("llm_diagnosis_expectation_missing");
    ...
    const successfulSourceCount = fixture.sources.filter((source) => (source.status ?? "succeeded") === "succeeded").length;
...
export function isLiveWebSmokeEnabled(env = EMPTY_ENV) {
...
export function validateWebRetrievalLiveSmokeTrace(scenario, trace) {
    ...
    for (const method of scenario.minimumMethods) {
        if (!trace.attemptedMethods.includes(method))
            failures.push(`minimum_method_missing:${method}`);
    }
    const receipt = trace.resultDiagnosis;
    if (!receipt)
        failures.push("llm_result_diagnosis_receipt_missing");
    ...
    if (/(<!doctype\s+html|<html\b|<script\b|Bearer\s+|\bsk-|\/Users\/|\/tmp\/)/iu.test(JSON.stringify(trace))) {
...
export async function runWebRetrievalLiveSmokeScenarios(input = {}) {
    ...
    const execute = input.executeScenario ??
...
export function writeWebRetrievalSmokeArtifact(summary, artifactStorage) {
...
export function buildWebRetrievalReleaseGateSummary(input = {}) {
    ...
    const blockingFailures = fixtureRegression?.status === "failed"
        ? fixtureRegression.results.flatMap((result) => result.failures.map((failure) => `${result.fixtureId}:${failure}`))
        : [];
    if (liveSmoke?.status === "failed")
        blockingFailures.push("live_smoke_failed");
...
export function buildFixtureRegressionFromWorkspace(rootDir) {
    const fixtures = loadWebRetrievalFixturesFromDir(resolve(rootDir, "tests", "fixtures", "web-retrieval"));
