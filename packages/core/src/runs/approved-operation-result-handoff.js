import { buildPersistedToolResultBlock, } from "../agent/index.js";
import { getMessagesForRun, getDb, getArtifactMetadata, insertMessageIfAbsent, listArtifactReceiptsForRun, } from "../db/index.js";
...
import { buildCanonicalRecoveredAttemptEvidenceDescriptor, recordCanonicalAttemptEvidence, } from "./canonical-attempt-evidence.js";
import { appendRunEvent, applyCanonicalRunTransition, getRootRun, } from "./store.js";
function handoffMessageId(continuationId) {
    return `${continuationId}:tool-result`;
}
function hasPersistedToolResultBlock(value) {
    if (!value)
        return false;
    try {
...
function boundedResultEvidenceRefs(result) {
...
function recordRecoveredAttempt(input) {
    ...
    return recordCanonicalAttemptEvidence(descriptor, {
        ...
        applyAttemptTransition: ({ runId, workId, receiptRef }) => applyCanonicalRunTransition({
            runId,
            workId,
...
export function handoffApprovedOperationResult(input) {
    ...
    const existing = getMessagesForRun(run.sessionId, run.id).find((message) => message.id === id);
    if (existing) {
    ...
    if (!recorded.ok)
        return recorded;
    appendRunEvent(run.id, `approved_operation_result_handed_off:${input.continuation.toolName}`);
    return { ok: true, inserted: true };
}
export function loadRecoveredApprovedOperationAttempt(runId) {
    ...
    const database = getDb();
    const aggregate = new SqliteCanonicalWorkRepository(database, () => Date.now()).load(`work:root:${run.id}`);
    ...
    const receipt = new SqliteCanonicalWorkReceiptRepository(database, () => Date.now()).findLatestConsumedByKind(aggregate.workId, "attempt");
    const evidenceRefs = receipt?.evidenceRefs.filter((ref) => ref.startsWith("side-effect-operation:")
    ...
    const deliveredArtifacts = evidenceRefs.flatMap((ref) => {
        ...
        if (!artifact)
            return [];
        const delivery = listArtifactReceiptsForRun(run.id).find((candidate) => candidate.delivered_at !== null
            && candidate.artifact_path === artifact.artifact_path
            && candidate.channel === run.source);
        if (!delivery)
            return [];
        return [{
                toolName: run.source === "telegram"
                    ? "telegram_send_file"
    ...
    const resultMessageExists = getMessagesForRun(run.sessionId, run.id).some((message) => message.id.startsWith("approval-continuation:")
...
//# sourceMappingURL=approved-operation-result-handoff.js.map