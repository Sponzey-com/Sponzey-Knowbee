...
export type CanonicalPlanPolicyReasonCode =
  | "plan_bindings_allowed"
  | "invalid_policy_input"
  | "exclusive_method_unavailable"
...
function decision(
  ...
  outcome: CanonicalPlanPolicyDecision["outcome"],
  reasonCode: CanonicalPlanPolicyReasonCode,
...
function structurallyValid(input: CanonicalPlanPolicyInput): boolean {
  if (!normalized(input.runId) || !normalized(input.workId)) return false
  if (!normalized(input.capabilitySnapshot.snapshotId)) return false
  if (!SHA256_PATTERN.test(input.planFingerprint)) return false
  if (!SHA256_PATTERN.test(input.capabilitySnapshot.fingerprint)) return false
  const bindingsValid = input.capabilitySnapshot.bindings.every(
    (binding) => Boolean(normalized(binding.capabilityId)) && Boolean(normalized(binding.targetId)),
  )
  const exclusionsValid = (input.capabilitySnapshot.exclusions ?? []).every(
    (exclusion) =>
      Boolean(normalized(exclusion.capabilityId)) &&
...
export function evaluateCanonicalPlanPolicy(
  input: CanonicalPlanPolicyInput,
): CanonicalPlanPolicyDecision {
  if (!structurallyValid(input)) return decision(input, "denied", "invalid_policy_input", [])
  ...
  const availableIds = new Set(
    input.capabilitySnapshot.bindings.map((binding) => normalized(binding.capabilityId)),
  )
  ...
  const selectedBindings = input.capabilitySnapshot.bindings.filter((binding) =>
    selected.includes(normalized(binding.capabilityId)),
  )
  if (
    targetId &&
    targetScopedMethods
  ...
  const missingApproval = selected.filter((capabilityId) =>
    applicableBindings.some(
      (binding) =>
        normalized(binding.capabilityId) === capabilityId &&
