...
export async function recoverCanonicalPendingResponsesOnStartup(options = {}) {
    ...
    const results = await replayCanonicalPendingResponses({
        ...
        findCommittedDelivery: (item) => {
            ...
            if (!existing?.delivery_key || !existing.idempotency_key)
                return undefined;
            return {
        ...
        recordCanonicalDelivery: async (descriptor) => {
            ...
            if (!aggregate)
                return { ok: false, reasonCode: "canonical_replay_aggregate_not_found" };
...
//# sourceMappingURL=canonical-pending-response-recovery-runtime.js.map