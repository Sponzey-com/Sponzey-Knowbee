...
function formatBulletLines(values: string[]): string {
  return normalizeList(values).map((value) => `- ${value}`).join("\n")
}

function formatOptionalBlock(title: string, values: string[]): string {
  const lines = normalizeList(values)
  if (lines.length === 0) return ""
  return [title, ...lines.map((line) => `- ${line}`)].join("\n")
}

function formatOriginalRequestBlock(originalRequest: string | undefined): string {
...
function buildChecklistLines(params: {
  ...
  const lines = [
    ...
    ...params.completeConditionLines.map((line) =>
      loadPromptValue("structured_execution_checklist_complete_condition_user", { completeCondition: line })),
