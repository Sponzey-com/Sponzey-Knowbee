...
function safeDeliveryErrorMessage(error: unknown): string {
...
const SUCCESSFUL_EXTERNAL_DELIVERY_STATUSES = new Set<DeliveryReceipt["status"]>([
  "sent",
  "delivered",
])
...
function hasMatchingExternalTextDeliveryEvidence(input: {
  ...
  text: string
  receipts: Array<ChunkDeliveryReceipt | undefined>
}): boolean {
  return input.receipts.some((receipt) =>
    (receipt?.textDeliveries ?? []).some(
      (delivery) =>
        delivery.channel === input.source &&
        delivery.text.trim() === input.text &&
        (delivery.deliveryReceipts ?? []).some(
          (providerReceipt) =>
            providerReceipt.provider === input.source &&
...
export async function emitAssistantTextDelivery(params: {
  ...
  let deliveryAdmissionEventId: string | undefined
  const isCancelled =
    params.isCancelled ??
    (() => {
  ...
  const cancellationReportAuthorized = Boolean(
    cancellationAuthorization &&
      cancellationAuthorization.runId === params.runId &&
      cancellationAuthorization.finalOutcome === "cancelled" &&
      cancellationAuthorization.receiptRef.startsWith(
        `receipt:cancellation:${params.runId}:`,
      ),
  )
  ...
  if (!params.force && deliveryKind === "final") {
    ...
    if (admission.status !== "admitted") {
      const admissionStatus =
        admission.status === "persistence_unavailable"
          ? "persistence_unavailable"
  ...
  const guardedOnChunk: RunChunkDeliveryHandler = params.onChunk
    ? async (chunk) => {
        if (cancellationBlocksDelivery()) {
  ...
  let doneChunkReceipt: ChunkDeliveryReceipt | undefined
  ...
  const externalEvidenceConfirmed = isExternalChannelProvider(params.source)
    ...
    : true
  const textDelivered =
    !textDeliveryFailed && !cancelledBeforeProvider && externalEvidenceConfirmed
  const doneDelivered =
    params.emitDone !== false &&
    !doneDeliveryFailed &&
  ...
  let receipt: AssistantTextDeliveryReceipt = {
...
function chunkNoticeForOutbox(chunk: AgentChunk): Record<string, unknown> | undefined {
  if (chunk.type !== "text" || !chunk.notice) return undefined
  return chunk.notice as unknown as Record<string, unknown>
}
