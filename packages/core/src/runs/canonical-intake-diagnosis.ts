...
export interface CanonicalIntakeDiagnosisRecorderDependencies {
  issueReceipt: (
    descriptor: Omit<CanonicalIntakeDiagnosisDescriptor, "runId">,
...
export function recordCanonicalIntakeDiagnosis(
  descriptor: CanonicalIntakeDiagnosisDescriptor,
  dependencies: CanonicalIntakeDiagnosisRecorderDependencies,
): CanonicalIntakeDiagnosisRecordResult {
  ...
  if (transition.status === "applied") return { ok: true }
  return {
    ok: false,
...
export function buildCanonicalIntakeDiagnosisDescriptor(input: {
  ...
  if (!runId) throw new Error("Run ID is required for canonical intake diagnosis.")
