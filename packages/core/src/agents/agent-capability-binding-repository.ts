...
} from "./agent-capability-binding-command.js"
import type {
  AgentCapabilityBindingSource,
  AgentCapabilityCatalogSource,
  AgentCapabilityKind,
} from "./agent-capability-binding-projection.js"
import { createAgentPublicRef } from "./agent-public-reference.js"

function publicRef(kind: AgentCapabilityKind, internalId: string): string {
  if (kind === "skill") return createSkillPublicRef(internalId)
  if (kind === "mcp_server") return createMcpPublicRef(internalId)
  return createYeonjangPublicRef(internalId)
}

function parseReceipt(value: string | null): AgentCapabilityBindingReceipt | null {
  if (!value) return null
  try {
    return JSON.parse(value) as AgentCapabilityBindingReceipt
  } catch {
    return null
  }
}

export function listAgentCapabilityCatalogSources(
  now = Date.now(),
): AgentCapabilityCatalogSource[] {
  const skills: AgentCapabilityCatalogSource[] = listSkillCatalogEntries({
  ...
  const mcpServers: AgentCapabilityCatalogSource[] = listMcpServerCatalogEntries({
    includeArchived: true,
  }).map((row) => ({
  ...
  const yeonjang: AgentCapabilityCatalogSource[] = listYeonjangRegistryInstances({ now }).map(
    (instance) => ({
...
export function listAgentCapabilityBindingSources(): AgentCapabilityBindingSource[] {
...
function kindRevision(kind: AgentCapabilityKind, now: number): number {
...
export function resolveInternalAgentId(agentRef: string): string | null {
...
export function createSqliteAgentCapabilityBindingCommandPorts(
  input: {
    now?: () => number
  } = {},
): AgentCapabilityBindingCommandPorts {
  const now = input.now ?? Date.now
  const resolveCapability = (kind: AgentCapabilityKind, capabilityRef: string) => {
    const matches = listAgentCapabilityCatalogSources(now()).filter(
      (item) => item.kind === kind && publicRef(kind, item.internalId) === capabilityRef,
    )
    ...
    upsertAgentCapabilityBinding(
      {
        ...(existing
          ? {
              ...
              enabledToolNames: JSON.parse(existing.enabled_tool_names_json) as string[],
              disabledToolNames: JSON.parse(existing.disabled_tool_names_json) as string[],
  ...
  return {
    ...
    receiptByNonce(nonce) {
      ...
      if (!row?.request_fingerprint || !receipt) return null
      return {
    ...
    persist(persist) {
      let result: { ok: boolean; revision: number; reasonCode?: string } = {
        ok: false,
...
export function capabilityPublicRef(kind: AgentCapabilityKind, internalId: string): string {
