...
import {
  ...
  compareAndUpdateAgentIdentity,
  getAgentIdentityMutationReceiptByNonce,
...
import type {
  ...
  AgentIdentityRecord,
} from "./agent-identity-command.js"
import { createAgentPublicRef } from "./agent-public-reference.js"

function parseReceipt(value: string): AgentIdentityMutationReceipt | null {
  try {
    return JSON.parse(value) as AgentIdentityMutationReceipt
  } catch {
    return null
  }
}
...
function defaultSubAgent(input: {
  ...
  const owner = { ownerType: "sub_agent" as const, ownerId: input.agentId }
  return {
...
export function createSqliteAgentIdentityCommandRepository(input: {
  ...
  const internalIdForRef = (agentRef: string) =>
    configs().find((candidate) => createAgentPublicRef(candidate.agentId) === agentRef)?.agentId
  const toRecord = (config: AgentConfig): AgentIdentityRecord => ({
    ...
    activeChildCount: listAgentRelationships().filter(
      (edge) => edge.parent_agent_id === config.agentId && edge.status === "active",
    ).length,
    activeBindingCount: listAgentCapabilityBindings({ includeArchived: false }).filter(
      (binding) => binding.agent_id === config.agentId && binding.status === "enabled",
    ).length,
  })
  return {
    ...
    recordByNormalizedName(normalizedName) {
      const config = configs().find(
        (candidate) =>
          candidate.agentName.normalize("NFKC").trim().toLocaleLowerCase() === normalizedName,
      )
      return config ? toRecord(config) : null
    },
    create(createInput) {
      ...
      try {
        upsertAgentConfig(
          defaultSubAgent({ agentId, name: createInput.name, role: createInput.role, now: now() }),
    ...
    compareAndUpdate(updateInput) {
      ...
      if (!agentId) return { reasonCode: "agent_ref_not_found" }
      try {
        ...
        if (result !== "updated")
          return {
            reasonCode:
              result === "revision_conflict" ? "agent_revision_conflict" : "agent_ref_not_found",
          }
      } catch (error) {
        if (error instanceof AgentNameNamespaceError)
          return { reasonCode: error.details.reasonCode }
