...
import {
  getAgentRelationshipMutationReceiptByNonce,
  getDb,
  listAgentConfigs,
  listAgentRelationships,
  reserveAgentRelationshipMutationReceipt,
...
} from "./agent-relationship-command.js"
import {
  type AgentRelationshipProjection,
  buildAgentRelationshipProjection,
} from "./agent-relationship-projection.js"

const RELATIONSHIP_PUBLIC_REF_NAMESPACE = "knowbee:agent-relationship:v1:"

export function createAgentRelationshipPublicRef(edgeId: string): string {
  if (!edgeId.trim()) throw new Error("agent_relationship_public_ref_source_invalid")
...
function graphRevision(): number {
  return listAgentRelationships().reduce(
    (revision, relationship) => Math.max(revision, relationship.updated_at),
    0,
  )
}

function parseReceipt(value: string | null): AgentRelationshipMutationReceipt | null {
  if (!value) return null
  try {
    return JSON.parse(value) as AgentRelationshipMutationReceipt
  } catch {
    return null
  }
}
...
export function createSqliteAgentRelationshipCommandPorts(input: {
  ...
  const hierarchy = () =>
    createAgentHierarchyService({ config: input.config, storage: input.storage, now })
  const currentRelationship = (internalChildAgentId: string) => {
  ...
  const writeRelationship = (write: {
    ...
    current: ReturnType<typeof currentRelationship>
    revision: number
  }) => {
    ...
    const relationship: AgentRelationship = {
      edgeId,
      parentAgentId:
        write.internalParentAgentId ?? write.current?.internalParentAgentId ?? rootAgentId,
  ...
  return {
    ...
    currentRelationship,
    validate(validation) {
      if (validation.kind === "disconnect") return { ok: true }
      const result = hierarchy().validate({
        edgeId:
          validation.current?.internalEdgeId ?? `relationship:${validation.internalChildAgentId}`,
    ...
    verify(verification) {
      ...
      const row = listAgentRelationships({ childAgentId: verification.internalChildAgentId }).find(
        (relationship) => relationship.updated_at === verification.targetRevision,
      )
      return {
