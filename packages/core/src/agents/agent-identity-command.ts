...
export type AgentIdentityMutationState =
  | "draft"
  | "validating"
  | "persisting"
  | "verifying"
...
export interface AgentIdentityCommandRepository {
  ...
  create(input: {
    ...
    role: string
    modelName?: string
  }): AgentIdentityRecord | { reasonCode: string }
...
function signature(command: AgentIdentityCommand): string {
...
function terminalReceipt(input: {
  ...
  requestSignature: string
  state: "failed" | "conflict" | "cancelled"
  reasonCode: string
  transitions: AgentIdentityMutationState[]
...
export function executeAgentIdentityCommand(
  command: AgentIdentityCommand,
  repository: AgentIdentityCommandRepository,
): AgentIdentityMutationReceipt {
  const requestSignature = signature(command)
  const existingReceipt = repository.receiptByNonce(command.envelope.nonce)
  if (existingReceipt) {
    if (
      existingReceipt.mutationId === command.envelope.mutationId &&
  ...
  const transitions: AgentIdentityMutationState[] = ["draft", "validating"]
  const envelope = command.envelope
  if (
    !envelope.mutationId.trim() ||
    !envelope.nonce.trim() ||
  ...
  let current: AgentIdentityRecord | null = null
  if (command.kind !== "create") {
  ...
  const receipt: AgentIdentityMutationReceipt = {
...
export function publicAgentIdentityReceipt(
  receipt: AgentIdentityMutationReceipt,
): Omit<AgentIdentityMutationReceipt, "nonce" | "requestSignature"> {
