...
export function buildReleasePipelinePlan(
  ...
  const steps: ReleasePipelineStep[] = [
    ...
    step(
      ...
      true,
      false,
      "Verify topology feature flag matrix, workspace route/layer/no-typing usability, contracts/validator-only rollout, dry-run/shadow, gated mode, opt-in routing, single Nobie fallback, sub-agent and channel finalizer regressions, WebUI build gate, runtime smoke, and rollback smoke.",
    ),
    step(
