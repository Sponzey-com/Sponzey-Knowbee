...
import { DEFAULT_CONFIG, type KnowbeeConfig, type OrchestrationConfig } from "../config/types.js"
...
import {
  type WebRetrievalLiveSmokeSummary,
...
import {
  type ChannelLiveEvidenceRejection,
  produceChannelLiveAcceptanceEvidence,
} from "./channel-live-acceptance-evidence.js"
import {
  type ConversationProcessReleaseCandidate,
  type ConversationProcessReleaseEvidence,
  buildConversationProcessReleaseEvidence,
} from "./conversation-process-release-evidence.js"
import {
...
import {
  type ExtensionLiveEvidenceRejection,
  produceExtensionLiveAcceptanceEvidence,
} from "./extension-live-acceptance-evidence.js"
import {
  type LiveAcceptanceAdmissionResult,
...
import {
  type LivePerformanceAcceptanceRunSelector,
  collectLivePerformanceAcceptanceEvidence,
} from "./live-performance-acceptance-collection.js"
import {
  type MemoryCompactionReleaseGateSummary,
  buildMemoryCompactionReleaseGateSummary,
} from "./memory-compaction-gate.js"
import {
  type OperationalRehearsalEvidenceInput,
  type OperationalRehearsalEvidenceSummary,
  verifyOperationalRehearsalEvidence,
} from "./operational-rehearsal-evidence.js"
import type {
...
import {
  ...
  type ReleaseRolloutPolicySelector,
...
import {
  type WebLiveEvidenceRejection,
  produceWebLiveAcceptanceEvidence,
} from "./web-live-acceptance-evidence.js"
import {
  type YeonjangLiveEvidenceRejection,
  produceYeonjangLiveAcceptanceEvidence,
} from "./yeonjang-live-acceptance-evidence.js"
import {
  type VerifiedYeonjangAcceptanceEvidenceInput,
  type VerifiedYeonjangEvidenceRejection,
  produceVerifiedYeonjangAcceptanceEvidence,
} from "./yeonjang-verified-acceptance-evidence.js"
import {
  type YeonjangPlatformAcceptanceMatrix,
  type YeonjangPlatformCapabilityReceipt,
  type YeonjangPlatformDeterministicReceipt,
  type YeonjangPlatformLiveRecord,
  buildYeonjangPlatformAcceptanceMatrix,
} from "./yeonjang-platform-acceptance.js"
...
import {
  type YeonjangBrowserActiveTabInfoReleaseGateSummary,
} from "./yeonjang-browser-active-tab-info-release-gate-summary.js"
import {
  ACTIVE_TAB_INFO_RELEASE_GATE_EVIDENCE_REQUIREMENTS,
  type YeonjangBrowserActiveTabInfoReleaseGateEvidenceCollection,
  type YeonjangBrowserActiveTabInfoReleaseGateModuleEvidence,
  type YeonjangBrowserActiveTabInfoReleaseGateRepositoryEvidenceCollection,
  type YeonjangBrowserActiveTabInfoReleaseGateRepositoryEvidencePort,
  type YeonjangBrowserActiveTabInfoReleaseGateTestEvidence,
...
} from "./yeonjang-browser-active-tab-info-release-gate-evidence-collector.js"
import {
  type YeonjangMultiInstanceReleaseGateSummary,
  type YeonjangProfileSmokeEvidence,
  buildYeonjangMultiInstanceReleaseGateSummary,
} from "./yeonjang-multi-instance-gate.js"
import {
  type YeonjangBrowserActiveTabInfoLiveEnableReviewProjection,
  buildYeonjangBrowserActiveTabInfoLiveEnableReviewProjection,
} from "./yeonjang-browser-active-tab-info-live-enable-review.js"
import {
  type YeonjangBrowserActiveTabInfoLiveEnableProjection,
  buildYeonjangBrowserActiveTabInfoLiveEnableProjection,
} from "./yeonjang-browser-active-tab-info-live-enable-state-machine.js"
import {
  type YeonjangBrowserActiveTabInfoLiveEnablePrerequisitesProjection,
...
export interface ReleaseManifest {
  ...
  extensionLiveAcceptanceProduction: {
    acceptedCounts: { skill: number; mcp: number }
...
function buildReleaseManifestActiveTabInfoReleaseGate(
  options: ReleaseManifestOptions,
): {
  releaseGateSummary: YeonjangBrowserActiveTabInfoReleaseGateSummary
  evidenceCompleteness: YeonjangBrowserActiveTabInfoEvidenceCompleteness
} {
  ...
  return {
    ...
    evidenceCompleteness: buildActiveTabInfoEvidenceCompleteness({
      ...collected,
      missingSourcePaths: ACTIVE_TAB_INFO_RELEASE_GATE_EVIDENCE_REQUIREMENTS.map(
        (requirement) => requirement.modulePath,
      ),
      missingTestPaths: ACTIVE_TAB_INFO_RELEASE_GATE_EVIDENCE_REQUIREMENTS.map(
        (requirement) => requirement.testPath,
      ),
    }),
  }
}

function buildActiveTabInfoEvidenceCompleteness(
  collected: YeonjangBrowserActiveTabInfoReleaseGateEvidenceCollection &
    Partial<YeonjangBrowserActiveTabInfoReleaseGateRepositoryEvidenceCollection> & {
      staleTestPaths?: readonly string[]
      rejectedSkippedTestPaths?: readonly string[]
      rejectedUnknownTestPaths?: readonly string[]
      rejectedPublicRawReportPaths?: readonly string[]
    },
  ...
  const rejectedSkippedTestPaths = sanitizeReleaseRelativePaths(
    collected.rejectedSkippedTestPaths ?? [],
  )
  const rejectedUnknownTestPaths = sanitizeReleaseRelativePaths(
    collected.rejectedUnknownTestPaths ?? [],
  )
  const rejectedPublicRawReportPaths = sanitizeReleaseRelativePaths(
    collected.rejectedPublicRawReportPaths ?? [],
  )
...
function buildYeonjangBrowserActiveTabInfoAuditArtifact(
  completeness: YeonjangBrowserActiveTabInfoEvidenceCompleteness,
): ReleaseGeneratedAuditArtifact {
...
function buildYeonjangBrowserActiveTabInfoAuditArtifactContent(
  completeness: YeonjangBrowserActiveTabInfoEvidenceCompleteness,
): string {
...
function sanitizeReleaseRelativePaths(paths: readonly string[]): string[] {
...
export type ReleaseApprovalEvidenceValidationReasonCode =
  | "release_approval_evidence_required"
  | "release_approval_evidence_raw_data"
  | "release_approval_evidence_invalid"

export type ReleaseApprovalEvidenceValidationResult =
  | { status: "valid"; evidence: ReleaseApprovalEvidenceProjection }
  | { status: "rejected"; reasonCode: ReleaseApprovalEvidenceValidationReasonCode }

export type ReleaseProjectionSurface =
  | "release_summary"
...
export type ReleaseActiveTabInfoAuditArtifactVerificationReasonCode =
  ...
  | "active_tab_info_audit_artifact_json_invalid"
  | "active_tab_info_audit_artifact_schema_invalid"
  | "active_tab_info_audit_artifact_raw_data_allowed"
  | "active_tab_info_audit_artifact_completeness_invalid"
  | "active_tab_info_audit_artifact_detail_path_unsafe"
  | "active_tab_info_audit_artifact_raw_data_detected"
...
export type ReleaseActiveTabInfoAuditPackageVerification =
...
export function buildReleaseManifest(options: ReleaseManifestOptions = {}): ReleaseManifest {
  ...
  const runtimePaths =
    options.runtimePaths ?? createRuntimePaths({ KNOWBEE_STATE_DIR: join(rootDir, ".knowbee") })
  const db = getDb({ paths: runtimePaths })
  const now = options.now ?? new Date()
  const releaseAudience = options.releaseAudience ?? "public"
  const channelProductions = (options.channelLiveSmokeRuns ?? []).map(
    produceChannelLiveAcceptanceEvidence,
  )
  ...
  const latestWebRunByScenario = new Map<string, number>()
  ...
  const webAccepted = webProductions
    .flatMap((item) =>
      item.production.accepted.filter(
        (evidence) => latestWebRunByScenario.get(evidence.scenarioId) === item.index,
      ),
    )
    .sort((left, right) => right.executedAt - left.executedAt)
  ...
  const latestExtensionRunByScenario = new Map<string, number>()
  for (const item of [...extensionProductions].sort(
    (left, right) => right.run.finishedAt - left.run.finishedAt || right.index - left.index,
  )) {
    for (const result of item.run.results) {
  ...
  const extensionAccepted = extensionProductions
    .flatMap((item) =>
      item.production.accepted.filter(
        (evidence) =>
          latestExtensionRunByScenario.get(`${evidence.capability}:${evidence.scenarioId}`) ===
          item.index,
      ),
    )
    .sort((left, right) => right.executedAt - left.executedAt)
  ...
  const latestYeonjangRunByScenario = new Map<string, number>()
  for (const item of [...yeonjangProductions].sort(
    (left, right) => right.run.finishedAt - left.run.finishedAt || right.index - left.index,
  )) {
    for (const result of item.run.results) {
  ...
  const yeonjangAccepted = yeonjangProductions
    .flatMap((item) =>
      item.production.accepted.filter(
        (evidence) => latestYeonjangRunByScenario.get(evidence.scenarioId) === item.index,
      ),
    )
    .sort((left, right) => right.executedAt - left.executedAt)
  ...
  const yeonjangVerifiedAcceptanceProduction = produceVerifiedYeonjangAcceptanceEvidence(
    options.yeonjangVerifiedAcceptanceEvidence ?? [],
  )
  const externallyProvidedEvidence = (options.liveAcceptanceEvidence ?? []).filter(
    (evidence) =>
      !(
        evidence.capability === "yeonjang" &&
  ...
  const yeonjangPlatformRequiredCapabilityMethods =
    options.yeonjangPlatformRequiredCapabilityMethods ?? []
  const yeonjangAutoCapabilityReceipts =
    options.yeonjangAutoCollectPlatformCapabilityReadiness &&
    yeonjangPlatformRequiredCapabilityMethods.length > 0
      ...
      : []
  const explicitYeonjangPlatformCapabilityReceipts =
    options.yeonjangPlatformCapabilityReceipts ?? []
  const explicitYeonjangCapabilityKeys = new Set(
    explicitYeonjangPlatformCapabilityReceipts.map(
      (receipt) => `${receipt.platform}:${receipt.method.trim()}`,
    ),
  )
  const mergedYeonjangPlatformCapabilityReceipts = [
    ...explicitYeonjangPlatformCapabilityReceipts,
    ...yeonjangAutoCapabilityReceipts.filter(
      (receipt) => !explicitYeonjangCapabilityKeys.has(`${receipt.platform}:${receipt.method.trim()}`),
    ),
  ]
  const yeonjangPlatformAcceptance = buildYeonjangPlatformAcceptanceMatrix({
    requiredPlatforms: targetPlatforms,
    availablePlatforms:
      options.availableYeonjangPlatforms ??
  ...
  const operationalRehearsalEvidence = verifyOperationalRehearsalEvidence({
    ...
    artifactCleanupSmokeReceipt:
      options.operationalRehearsalEvidence?.artifactCleanupSmokeReceipt ?? null,
  })
  ...
  const acceptanceEvidence = resolveLivePerformanceAcceptanceEvidence(
    options.livePerformanceAcceptanceSelection,
  )
  ...
  const yeonjangBrowserActiveTabInfoReleaseGateEvidence =
    buildReleaseManifestActiveTabInfoReleaseGate(options)
  const yeonjangBrowserActiveTabInfoReleaseGate =
    yeonjangBrowserActiveTabInfoReleaseGateEvidence.releaseGateSummary
  const yeonjangBrowserActiveTabInfoEvidenceCompleteness =
    yeonjangBrowserActiveTabInfoReleaseGateEvidence.evidenceCompleteness
  const yeonjangBrowserActiveTabInfoAuditArtifact =
    buildYeonjangBrowserActiveTabInfoAuditArtifact(
      yeonjangBrowserActiveTabInfoEvidenceCompleteness,
    )
  const yeonjangBrowserActiveTabInfoLiveEnableReview =
    buildYeonjangBrowserActiveTabInfoLiveEnableReviewProjection(
      options.yeonjangBrowserActiveTabInfoLiveEnableReviewRecord,
      { now },
    )
  const yeonjangBrowserActiveTabInfoRuntimeTransition =
    options.yeonjangBrowserActiveTabInfoLiveEnableReviewRecord === undefined
      ? buildYeonjangBrowserActiveTabInfoLiveEnableProjection({
  ...
  const activeTabInfoEvidenceClean =
    yeonjangBrowserActiveTabInfoReleaseGate.gateStatus !== "blocked" &&
  ...
  const activeTabInfoLivePathsClosed =
    !yeonjangBrowserActiveTabInfoReleaseGate.liveIntegrationState.rustLiveHandlerEnabled &&
  ...
  const yeonjangBrowserActiveTabInfoLiveEnablePrerequisites =
    evaluateYeonjangBrowserActiveTabInfoLiveEnablePrerequisites({
...
export function evaluateReleaseReadiness(manifest: ReleaseManifest): ReleaseReadinessDecision {
  const blockerCodes: ReleaseReadinessBlockerCode[] = []
  if (manifest.requiredMissing.length > 0) blockerCodes.push("required_artifact_missing")
  if (!manifest.updatePreflight.ok) blockerCodes.push("update_preflight_failed")
  if (!manifest.migrationPreflight.ok) blockerCodes.push("migration_preflight_failed")
  if (manifest.performanceEvidence.gateStatus === "failed") {
    blockerCodes.push("performance_gate_failed")
  }
  if (manifest.benchmarkEvidence.gateStatus === "failed") blockerCodes.push("benchmark_gate_failed")
  ...
  if (manifest.uiModeEvidence.gateStatus === "failed") blockerCodes.push("ui_mode_gate_failed")
  if (manifest.yeonjangMultiInstanceEvidence.gateStatus === "failed") {
    blockerCodes.push("yeonjang_multi_instance_gate_failed")
  }
  if (
    manifest.yeonjangBrowserActiveTabInfoReleaseGate.gateStatus === "blocked" ||
    hasActiveTabInfoEvidenceCompletenessBlocker(
...
export function validateReleaseApprovalEvidenceProjection(
  value: unknown,
): ReleaseApprovalEvidenceValidationResult {
  ...
  if (
    ...
    readiness.blockerCodes.some(
      (code) => typeof code !== "string" || !code.trim() || !/^[a-z0-9_.:-]+$/u.test(code),
    )
  ) {
    return rejectedReleaseApprovalEvidence("release_approval_evidence_invalid")
  }
  ...
  const counts: Record<(typeof countKeys)[number], number> = {
...
function releaseApprovalEvidenceContainsRawData(value: unknown): boolean {
  const serialized = JSON.stringify(value)
  return (
    !serialized ||
    /stdout|stderr|stack trace|raw output|https?:\/\/|token=|\/Users\/|\/private\//iu.test(
      serialized,
    )
  )
}

function rejectedReleaseApprovalEvidence(
...
export function buildReleaseActiveTabInfoAuditAccessProjectionMatrix(input: {
  ...
  const entry = (
    surface: ReleaseProjectionSurface,
    audience: ReleaseActiveTabInfoAuditAccessProjectionEntry["audience"],
    visibility: ReleaseActiveTabInfoAuditAccessProjectionEntry["visibility"],
    allowedFields: readonly string[],
    auditDetailPathsIncluded: boolean,
  ): ReleaseActiveTabInfoAuditAccessProjectionEntry =>
    Object.freeze({
      surface,
      audience,
      visibility,
...
export function verifyReleaseActiveTabInfoAuditArtifactPayload(input: {
  ...
  if (
    payload.schemaVersion !== "yeonjang-browser-active-tab-info-audit-artifact-v1" ||
  ...
  if (!releaseObjectRecord(completeness)) {
    return rejectedActiveTabInfoAuditArtifact(
      "active_tab_info_audit_artifact_completeness_invalid",
    )
  }
  const countKeys = [
  ...
  if (
    completeness.schemaVersion !== "yeonjang-browser-active-tab-info-evidence-completeness-v1" ||
  ...
  return Object.freeze({
    ...
    summary: Object.freeze({
      ...
      evidenceCountSummary: Object.freeze({
        missingSourceCount: completeness.missingSourceCount as number,
        missingTestCount: completeness.missingTestCount as number,
        staleTestCount: completeness.staleTestCount as number,
        rejectedSkippedTestCount: completeness.rejectedSkippedTestCount as number,
        rejectedUnknownTestCount: completeness.rejectedUnknownTestCount as number,
        rejectedPublicRawReportCount: completeness.rejectedPublicRawReportCount as number,
        failingTestCount: completeness.failingTestCount as number,
      }),
    }),
  })
}

function releaseObjectRecord(value: unknown): value is Record<string, unknown> {
  return Boolean(value) && typeof value === "object" && !Array.isArray(value)
}

function rejectedActiveTabInfoAuditArtifact(
  reasonCode: ReleaseActiveTabInfoAuditArtifactVerificationReasonCode,
): ReleaseActiveTabInfoAuditArtifactVerificationResult {
  return Object.freeze({
    status: "rejected",
...
function hasActiveTabInfoEvidenceCompletenessBlocker(
  completeness: YeonjangBrowserActiveTabInfoEvidenceCompleteness,
): boolean {
  return (
    completeness.missingSourceCount > 0 ||
...
function resolveLivePerformanceAcceptanceEvidence(
  selection: ReleaseLivePerformanceAcceptanceSelection | undefined,
): ReleasePerformanceAcceptanceEvidence | undefined {
  if (!selection) return undefined
  const candidate = selection as Partial<ReleaseLivePerformanceAcceptanceSelection>
  const selector = candidate.selector as Partial<PerformanceAcceptanceMatrixSelector> | undefined
  if (
    !selector ||
    ...
    return {
      ...
      baselineVersion:
        typeof selector?.baselineVersion === "string" ? selector.baselineVersion : null,
  ...
  if (targetPlatforms.has("windows")) {
    definitions.push(
      optionalArtifact(
        ...
        "yeonjang/windows/knowbee-yeonjang.exe",
  ...
  if (targetPlatforms.has("linux")) {
    definitions.push(
      optionalArtifact(
        ...
        "yeonjang_linux_binary",
        rootDir,
        "Yeonjang/target/release/knowbee-yeonjang",
        "yeonjang/linux/knowbee-yeonjang",
...
export function buildReleasePipelinePlan(
  ...
  const steps: ReleasePipelineStep[] = [
    step(
      ...
      true,
      false,
      "Validate version, artifact definitions, migration state, backup inventory, readiness blockers, and sanitized releaseApprovalEvidence without mutating runtime state.",
    ),
    step(
      "release-approval-candidate-preparation",
      "Release approval candidate preparation",
      [
        "pnpm",
        "run",
      ...
      true,
      false,
      "Prepare performance or rollout approval candidates by attaching releaseApprovalEvidence from release-package --dry-run --json, then run release:authorize. Output must show only artifact id, checksum, packagePath, count summary, readiness blocker state, and no threshold/baseline raw detail, raw audit JSON, URL, or local absolute path.",
    ),
    step(
      "clean-build",
      "Clean build",
    ...
    step(
      ...
      true,
      false,
      "Verify topology feature flag matrix, workspace route/layer/Executor-first usability, contracts/validator-only rollout, dry-run/shadow, gated mode, opt-in routing, single main-agent fallback, sub-agent and channel finalizer regressions, WebUI build gate, runtime smoke, and rollback smoke.",
    ),
    step(
    ...
    step(
      "yeonjang-browser-active-tab-info-release-gate",
      "Yeonjang browser active tab info release gate",
      [
        "pnpm",
        "exec",
    ...
    step(
      "artifact-cleanup-cli-smoke",
      "Artifact cleanup CLI smoke",
      ["pnpm", "run", "smoke:artifact-cleanup-cli"],
      true,
      true,
      "Verify artifact cleanup preview and confirmation failure paths through the installed CLI. This non-destructive smoke must not run the destructive fixture success path.",
    ),
  ]
  if (targetPlatforms.has("macos"))
    steps.push(
  ...
  steps.push(
    step(
      ...
      "Web retrieval live smoke",
      ["pnpm", "run", "smoke:web:live"],
      false,
      true,
      "Opt-in DuckDuckGo search and public document fetch provider smoke; provider limits are recorded without exposing raw content.",
    ),
  )
  steps.push(
    step(
      "live-smoke-gate",
      "Live smoke gate",
      ["pnpm", "exec", "knowbee", "smoke", "channels", "--live"],
      audience === "public",
      true,
...
export function buildCleanMachineInstallChecklist(): ReleaseChecklistItem[] {
  return [
    ...
    },
    {
      ...
      required: true,
      description:
        "Enterprise Topology feature flag matrix, single main-agent fallback, sub-agent and channel finalizer regressions, WebUI build gate, runtime smoke, active topology rollback, and compiled snapshot restore evidence pass before opt-in routing.",
    },
    {
      id: "plan-drift",
      required: true,
    ...
    },
    {
      ...
      required: true,
      description:
        "Artifact cleanup preview and confirmation-failure paths pass through the installed CLI using the non-destructive default smoke.",
    },
  ]
}
...
function writeReleaseManifest(options: {
  ...
  let activeTabInfoAuditVerification: ReleaseActiveTabInfoAuditPackageVerification =
    buildPendingActiveTabInfoAuditVerification(manifest)
  if (options.copyPayload !== false) {
    ...
    const activeTabInfoAuditTargetPath = join(
      payloadDir,
      ...manifest.yeonjangBrowserActiveTabInfoAuditArtifact.packagePath.split("/"),
    )
    mkdirSync(dirname(activeTabInfoAuditTargetPath), { recursive: true })
    const activeTabInfoAuditContent = buildYeonjangBrowserActiveTabInfoAuditArtifactContent(
      manifest.yeonjangBrowserActiveTabInfoEvidenceCompleteness,
    )
    ...
    if (activeTabInfoAuditVerification.status === "rejected") {
      throw new Error(
        `active_tab_info_audit_artifact_verification_failed:${activeTabInfoAuditVerification.reasonCode}`,
      )
    }
...
function buildPendingActiveTabInfoAuditVerification(
  manifest: ReleaseManifest,
): ReleaseActiveTabInfoAuditPackageVerification {
...
function releaseArtifactHandling(kind: ReleaseArtifactKind): ReleaseArtifactHandlingPolicy {
...
export function buildReleaseOrchestrationEvidence(input: {
  ...
  const offParitySnapshot = resolveOrchestrationModeSnapshotSync({
    ...
    loadRegistry: () => {
      ...
      return {
        activeSubAgents: [
          { agentId: "agent:unexpected", agentName: "Unexpected", source: "config" as const },
        ],
  ...
  checks.push(
    buildOrchestrationCheck({
      ...
      pass:
        offParitySnapshot.mode === "single_knowbee" &&
  ...
  checks.push(
    buildOrchestrationCheck({
      ...
      pass:
        noAgentFallbackSnapshot.mode === "single_knowbee" &&
