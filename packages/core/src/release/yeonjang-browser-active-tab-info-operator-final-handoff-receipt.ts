...
} from "./yeonjang-browser-active-tab-info-final-handoff-closure-marker.js"

export type YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceiptStatus = "ready"

export type YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceiptBlockingReasonCode =
...
const SAFE_OPERATOR_FINAL_HANDOFF_RECEIPT_REF_PATTERN =
  /^operator-final-handoff-receipt:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_OPERATOR_FINAL_HANDOFF_ACK_REF_PATTERN =
  /^operator-final-handoff:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractFinalHandoffClosureMarkerId(
  marker: YeonjangBrowserActiveTabInfoFinalHandoffClosureMarker,
): string | undefined {
  if (
    marker.status !== "final_handoff_closure_marker_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoOperatorFinalHandoffReceipt(
  input: YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceiptInput,
): YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoOperatorFinalHandoffReceiptBlockingReasonCode[] = []
  const finalHandoffClosureMarkerId =
    extractFinalHandoffClosureMarkerId(input.finalHandoffClosureMarker)
  if (finalHandoffClosureMarkerId === undefined) {
    blockingReasonCodes.push("operator_final_handoff_receipt_marker_not_ready")
  }
  const sanitizedOperatorFinalHandoffReceiptRef =
    input.sanitizedOperatorFinalHandoffReceiptRef.trim()
  ...
  const operatorFinalHandoffAcknowledgementRef =
    input.operatorFinalHandoffAcknowledgementRef.trim()
  ...
  return baseResult({
    ...
    receipt: Object.freeze({
      operatorFinalHandoffReceiptId:
        buildOperatorFinalHandoffReceiptId({
