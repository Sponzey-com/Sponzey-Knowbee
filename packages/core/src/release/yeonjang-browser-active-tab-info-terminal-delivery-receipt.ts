...
} from "./yeonjang-browser-active-tab-info-terminal-report-projection.js"

export type YeonjangBrowserActiveTabInfoTerminalDeliveryStatus = "delivered"

export type YeonjangBrowserActiveTabInfoTerminalDeliveryReceiptBlockingReasonCode =
...
export type YeonjangBrowserActiveTabInfoTerminalDeliveryReceipt = Readonly<{
  ...
  reasonCode:
    | "active_tab_info_terminal_delivery_receipt_ready"
    | "active_tab_info_terminal_delivery_receipt_blocked"
...
const SAFE_TERMINAL_OUTPUT_CHANNEL_ACK_REF_PATTERN =
  /^terminal-output-channel:active-tab-info:ack:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_TERMINAL_DELIVERY_EVENT_REF_PATTERN =
  /^terminal-delivery-event:active-tab-info:sanitized:[a-z0-9._:-]+$/u

function extractTerminalReportProjectionId(
  projection: YeonjangBrowserActiveTabInfoTerminalReportProjection,
): string | undefined {
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoTerminalDeliveryReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoTerminalDeliveryReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoTerminalDeliveryReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoTerminalDeliveryReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoTerminalDeliveryReceipt(
  input: YeonjangBrowserActiveTabInfoTerminalDeliveryReceiptInput,
): YeonjangBrowserActiveTabInfoTerminalDeliveryReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoTerminalDeliveryReceiptBlockingReasonCode[] = []
