export type YeonjangBrowserActiveTabInfoRustSourceDriftSection =
  | "methods_inventory"
  | "dispatch"
  | "capability_matrix"
  ...
  | "tool_health"
  | "method_metadata"
  | "permission_setting_allow_browser_read"
...
const REQUIRED_SECTIONS: readonly {
  section: YeonjangBrowserActiveTabInfoRustSourceDriftSection
  pattern: RegExp
}[] = [
...
export function hasYeonjangBrowserActiveTabInfoRuntimeInventoryExposure(source: string): boolean {
  return (
    /"name"\s*:\s*"browser\.active_tab_info"/u.test(source) ||
    source
      .split(/\r?\n/u)
      .some((line) => (
        /"browser\.active_tab_info"\s*=>/u.test(line) &&
