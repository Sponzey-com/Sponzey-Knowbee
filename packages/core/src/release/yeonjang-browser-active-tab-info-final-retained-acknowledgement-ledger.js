...
const SAFE_FINAL_RETAINED_ACKNOWLEDGEMENT_LEDGER_REF_PATTERN = /^final-retained-acknowledgement-ledger:active-tab-info:sanitized:[a-z0-9._:-]+$/u;
const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN = /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u;
const SAFE_FINAL_RETAINED_ACKNOWLEDGEMENT_REF_PATTERN = /^final-retained-acknowledgement:active-tab-info:ack:[a-z0-9._:-]+$/u;
function extractOperatorRetainedTransferIndexAcknowledgementReceiptId(receipt) {
    if (receipt.status !==
...
export function buildYeonjangBrowserActiveTabInfoFinalRetainedAcknowledgementLedger(input) {
    const blockingReasonCodes = [];
    const operatorRetainedTransferIndexAcknowledgementReceiptId = extractOperatorRetainedTransferIndexAcknowledgementReceiptId(input.operatorRetainedTransferIndexAcknowledgementReceipt);
    if (operatorRetainedTransferIndexAcknowledgementReceiptId === undefined) {
        blockingReasonCodes.push("final_retained_acknowledgement_ledger_receipt_not_ready");
    }
    const sanitizedFinalRetainedAcknowledgementLedgerRef = input.sanitizedFinalRetainedAcknowledgementLedgerRef.trim();
    ...
    if (!SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN.test(productLogEvidenceRef)) {
        blockingReasonCodes.push("final_retained_acknowledgement_ledger_product_log_evidence_ref_invalid");
    }
    const finalRetainedAcknowledgementRef = input.finalRetainedAcknowledgementRef.trim();
    ...
    return baseResult({
        ...
        ledger: Object.freeze({
            finalRetainedAcknowledgementLedgerId: buildFinalRetainedAcknowledgementLedgerId({
...
//# sourceMappingURL=yeonjang-browser-active-tab-info-final-retained-acknowledgement-ledger.js.map