import { activatePerformanceAcceptanceMatrix, evaluateMeasuredFlowWithAcceptanceMatrix, } from "../maintenance/performance-acceptance-matrix.js";
...
export function buildPerformanceAcceptanceEvidence(input) {
    ...
    const snapshotMatches = !input.baseline ||
        REQUIRED_REPRESENTATIVE_FLOW_IDS.every((flowId) => {
            const approved = input.selected.candidate.baselineSnapshot.flows.find((flow) => flow.flowId === flowId);
            const supplied = input.baseline?.flows.find((flow) => flow.flowId === flowId);
            return (approved?.latencyP95Ms === supplied?.latencyP95Ms &&
    ...
    for (const flowId of REQUIRED_REPRESENTATIVE_FLOW_IDS) {
        ...
        if (evaluation.status === "baseline_only")
            status = "baseline_only";
        else if (evaluation.status === "rejected" && status !== "baseline_only")
            status = "rejected";
    }
    return {
        status,
...
//# sourceMappingURL=performance-acceptance-evidence.js.map