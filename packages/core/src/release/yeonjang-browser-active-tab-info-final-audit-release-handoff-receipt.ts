...
} from "./yeonjang-browser-active-tab-info-archival-release-evidence-index.js"
...
export type YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceipt = Readonly<{
  ...
  reasonCode:
    | "active_tab_info_final_audit_release_handoff_receipt_ready"
...
const SAFE_RELEASE_HANDOFF_RECEIPT_REF_PATTERN =
  /^release-handoff:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_MANUAL_AUDIT_QUEUE_ACK_REF_PATTERN =
  /^manual-audit-queue:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractArchivalReleaseEvidenceIndexId(
  index: YeonjangBrowserActiveTabInfoArchivalReleaseEvidenceIndex,
): string | undefined {
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceipt(
  input: YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceiptInput,
): YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoFinalAuditReleaseHandoffReceiptBlockingReasonCode[] = []
  const archivalReleaseEvidenceIndexId = extractArchivalReleaseEvidenceIndexId(
    input.archivalReleaseEvidenceIndex,
  )
