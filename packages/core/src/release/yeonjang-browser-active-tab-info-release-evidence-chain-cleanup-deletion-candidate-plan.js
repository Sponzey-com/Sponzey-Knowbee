const SAFE_DELETION_CANDIDATE_REF = /^cleanup-deletion-candidate:active-tab-info:sanitized:[a-z0-9][a-z0-9:-]{0,96}$/u;
...
export function buildYeonjangBrowserActiveTabInfoReleaseEvidenceChainCleanupDeletionCandidatePlan(input) {
    if (input.cleanupBranchExecutionAdmission.admissionStatus !== "accepted" ||
        input.cleanupBranchExecutionAdmission.nextAllowedAction !==
            "prepare_cleanup_deletion_candidate_after_branch_admission") {
        return blocked({
            reasonCode: "active_tab_info_release_evidence_chain_cleanup_deletion_candidate_plan_admission_not_accepted",
    ...
    if (candidateRefs.length === 0) {
        return blocked({
            reasonCode: "active_tab_info_release_evidence_chain_cleanup_deletion_candidate_plan_no_candidates",
    ...
    if (candidateRefs.some((ref) => !SAFE_DELETION_CANDIDATE_REF.test(ref))) {
        return blocked({
            reasonCode: "active_tab_info_release_evidence_chain_cleanup_deletion_candidate_plan_candidate_ref_invalid",
            reviewedAdmissionStatus: "accepted",
            nextAllowedAction: "provide_cleanup_deletion_candidate_refs",
        });
    }
...
//# sourceMappingURL=yeonjang-browser-active-tab-info-release-evidence-chain-cleanup-deletion-candidate-plan.js.map