...
function pushSample(target, candidate) {
    if (!candidate)
        return;
    const existingIndex = target.findIndex((item) => item.runId === candidate.runId && item.stage === candidate.stage);
...
function terminalEvent(events, kind) {
    return [...events]
        .filter((event) => event.kind === kind)
        .sort((left, right) => left.at - right.at)
        .at(-1);
}
function firstEvent(events, kinds) {
    return events
        .filter((event) => kinds.includes(event.kind))
        .sort((left, right) => left.at - right.at)[0];
}
function appendTypedEventSamples(samples, counters, runId, events, terminalAt) {
    ...
    const executionEnd = terminalEvent(events, "execution_completed") ??
    ...
    const evidence = terminalEvent(events, "evidence_recorded") ?? terminalEvent(events, "execution_completed");
...
function countSourceIssues(counts) {
    return [...counts.entries()].map(([code, count]) => ({ code, count }));
}
function terminalInvocationReceipts(receipts) {
...
function appendLlmInvocationMetrics(samples, counters, runId, receipts) {
...
export class SqliteReleaseMetricRecordPort {
    loadWindow(window) {
        const db = getDb();
        const samples = [];
        const counters = [];
        ...
        const selectedRuns = db
            .prepare(`SELECT id, request_group_id, status, created_at, updated_at
       FROM root_runs
       ...
       LIMIT 501`)
        ...
        if (selectedRuns.length > runs.length)
            issueCounts.set("window_limit_reached", 1);
        const runIds = new Set(runs.map((run) => run.id));
        const requestGroupOwner = new Map();
        ...
        const steps = db
            .prepare(`SELECT s.run_id, s.step_key, s.status, s.started_at, s.finished_at
       FROM run_steps s
       ...
       ORDER BY s.run_id ASC, s.step_index ASC`)
        ...
        for (const run of runs) {
            ...
            for (const issue of typedSnapshot.issues) {
                const code = issue.code === "stored_event_invalid" ? "stored_event_invalid" : "source_record_invalid";
                issueCounts.set(code, (issueCounts.get(code) ?? 0) + 1);
            }
            const events = typedSnapshot.events.filter((event) => event.at >= window.startAt && event.at <= window.endAt);
            ...
            const receipts = [
                ...
                ...(run.request_group_id && requestGroupOwner.get(run.request_group_id) === run.id
                    ...
                    : []),
            ].filter((receipt, index, all) => receipt.at >= window.startAt &&
                receipt.at <= window.endAt &&
                all.findIndex((candidate) => candidate.invocationId === receipt.invocationId && candidate.phase === receipt.phase) === index);
        ...
        const audits = db
            .prepare(`SELECT id, timestamp, run_id, source, tool_name, result, duration_ms, retry_count
       FROM audit_logs
       WHERE timestamp >= ? AND timestamp <= ? AND run_id IS NOT NULL
       ORDER BY timestamp ASC, id ASC`)
            .all(window.startAt, window.endAt);
        for (const audit of audits) {
            if (!audit.run_id || !runIds.has(audit.run_id))
                continue;
        ...
        const ledgers = db
            .prepare(`SELECT id, run_id, event_kind, status, created_at
       FROM message_ledger
       WHERE created_at >= ? AND created_at <= ? AND run_id IS NOT NULL
       ORDER BY created_at ASC, id ASC`)
            .all(window.startAt, window.endAt);
        for (const ledger of ledgers) {
            if (!ledger.run_id || !runIds.has(ledger.run_id))
                continue;
        ...
        const approvals = db
            .prepare(`SELECT id, run_id, status, requested_at, consumed_at
       FROM approval_registry
       WHERE requested_at <= ? AND COALESCE(consumed_at, requested_at) >= ?
       ORDER BY requested_at ASC, id ASC`)
            .all(window.endAt, window.startAt);
        for (const approval of approvals) {
            if (!runIds.has(approval.run_id) || approval.consumed_at === null)
                continue;
            pushSample(samples, sample({
        ...
        const queueRows = db
            .prepare(`SELECT id, run_id, event_kind, created_at, retry_count
...
//# sourceMappingURL=sqlite-release-metric-record-port.js.map