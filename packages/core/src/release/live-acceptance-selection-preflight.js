...
function parseExactNameList(value, options = {}) {
    try {
        const parsed = JSON.parse(value);
        if (parsed === null && options.allowCanonicalEmpty)
...
function oneExact(items, predicate) {
...
function resolveExtension(input) {
    ...
    const binding = oneExact(input.snapshot.extensions, (item) => item.bindingId === input.selection.bindingId);
    if (!binding)
        return "live_preflight_binding_missing";
    if (binding === "ambiguous")
        return "live_preflight_binding_ambiguous";
    if (binding.agentId !== input.selection.agentId)
        return "live_preflight_binding_owner_mismatch";
    if (binding.capabilityKind !== expectedKind)
        return "live_preflight_binding_kind_mismatch";
    if (binding.catalogId !== input.selection.catalogId) {
        return "live_preflight_binding_catalog_mismatch";
    }
    if (binding.bindingStatus !== "enabled")
        return "live_preflight_binding_not_enabled";
    ...
    if (!enabledNames || !disabledNames)
        return "live_preflight_binding_tool_list_invalid";
    ...
    const catalog = oneExact(input.snapshot.catalogs, (item) => item.capability === input.selection.capability &&
        item.catalogId === input.selection.catalogId);
    if (!catalog)
        return "live_preflight_catalog_missing";
    if (catalog === "ambiguous")
        return "live_preflight_catalog_ambiguous";
    if (catalog.status !== "enabled")
        return "live_preflight_catalog_not_enabled";
    if (catalog.risk !== "safe")
        return "live_preflight_catalog_not_safe";
    const catalogNames = parseExactNameList(catalog.toolNamesJson);
    if (!catalogNames)
        return "live_preflight_catalog_tool_list_invalid";
    ...
    if (!tool)
        return "live_preflight_tool_missing";
    if (tool === "ambiguous")
        return "live_preflight_tool_ambiguous";
...
function resolveYeonjang(input) {
    const instance = oneExact(input.snapshot.yeonjangInstances, (item) => item.instanceId === input.selection.instanceId);
    if (!instance)
        return "live_preflight_yeonjang_missing";
    if (instance === "ambiguous")
        return "live_preflight_yeonjang_ambiguous";
    if (instance.state !== "online")
        return "live_preflight_yeonjang_not_online";
    if (instance.trustState !== "trusted")
        return "live_preflight_yeonjang_untrusted";
    if (instance.scopeAccess !== "allowed")
        return "live_preflight_yeonjang_scope_denied";
    if (!instance.runnableTarget)
        return "live_preflight_yeonjang_not_runnable";
    if (instance.duplicateLiveSessionDetected || instance.liveSessionCount !== 1) {
        return "live_preflight_yeonjang_duplicate";
    }
    if (!instance.session)
        return "live_preflight_yeonjang_session_missing";
    if (instance.session.sessionId !== input.selection.sessionId) {
        return "live_preflight_yeonjang_session_mismatch";
    }
    if (instance.session.endedAt !== null ||
        ["offline", "disconnected", "duplicate_instance_conflict", "session_replaced"].includes(instance.session.state.trim().toLowerCase())) {
        return "live_preflight_yeonjang_session_inactive";
    }
    if (instance.session.stale ||
...
function unavailableSelection(capability) {
    const reasonCode = capability === "skill"
...
export function inspectLiveAcceptanceSelectionAvailability(input) {
    const inputValid = Number.isSafeInteger(input.now) &&
    ...
    const skillReady = inputValid && extensionSelectionAvailable({ capability: "skill", snapshot: input.snapshot });
    const mcpReady = inputValid && extensionSelectionAvailable({ capability: "mcp", snapshot: input.snapshot });
    const yeonjangReady = inputValid &&
    ...
    return Object.freeze([
        skillReady
            ? Object.freeze({ capability: "skill", status: "ready" })
            : unavailableSelection("skill"),
        mcpReady
            ? Object.freeze({ capability: "mcp", status: "ready" })
            : unavailableSelection("mcp"),
        yeonjangReady
            ? Object.freeze({ capability: "yeonjang", status: "ready" })
...
export function resolveLiveAcceptanceExecutionSelections(input) {
    ...
    if (input.selection.extensions.length !== 2 ||
        new Set(capabilities).size !== 2 ||
        !capabilities.includes("skill") ||
        !capabilities.includes("mcp") ||
        input.selection.extensions.some((item) => item.readOnly !== true ||
            !exact(item.agentId) ||
    ...
    if (typeof yeonjang === "string")
        return rejected(yeonjang);
    return Object.freeze({
        status: "verified",
        snapshotCapturedAt: input.snapshot.capturedAt,
...
//# sourceMappingURL=live-acceptance-selection-preflight.js.map