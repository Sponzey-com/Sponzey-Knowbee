...
} from "./yeonjang-browser-active-tab-info-final-retained-completion-acknowledgement-ledger.js"

export type YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceiptStatus =
  "ready"
...
export type YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceipt = Readonly<{
  ...
  status:
    | "operator_retained_ledger_acknowledgement_receipt_ready"
    | "blocked"
  reasonCode:
...
const SAFE_OPERATOR_RETAINED_LEDGER_ACKNOWLEDGEMENT_RECEIPT_REF_PATTERN =
  /^operator-retained-ledger-acknowledgement-receipt:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_OPERATOR_RETAINED_LEDGER_ACKNOWLEDGEMENT_REF_PATTERN =
  /^operator-retained-ledger:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractFinalRetainedCompletionAcknowledgementLedgerId(
  ledger: YeonjangBrowserActiveTabInfoFinalRetainedCompletionAcknowledgementLedger,
): string | undefined {
  if (
    ledger.status !== "final_retained_completion_acknowledgement_ledger_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceipt(
  input: YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceiptInput,
): YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoOperatorRetainedLedgerAcknowledgementReceiptBlockingReasonCode[] = []
  const finalRetainedCompletionAcknowledgementLedgerId =
    extractFinalRetainedCompletionAcknowledgementLedgerId(
      input.finalRetainedCompletionAcknowledgementLedger,
    )
  ...
  const sanitizedOperatorRetainedLedgerAcknowledgementReceiptRef =
    input.sanitizedOperatorRetainedLedgerAcknowledgementReceiptRef.trim()
  ...
  const operatorRetainedLedgerAcknowledgementRef =
    input.operatorRetainedLedgerAcknowledgementRef.trim()
  ...
  return baseResult({
    ...
    receipt: Object.freeze({
      operatorRetainedLedgerAcknowledgementReceiptId:
        buildOperatorRetainedLedgerAcknowledgementReceiptId({
