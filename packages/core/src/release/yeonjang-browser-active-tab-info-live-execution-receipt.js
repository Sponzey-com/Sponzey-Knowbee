...
export function buildYeonjangBrowserActiveTabInfoLiveExecutionReceipt(input, options) {
    const blockingReasonCodes = [];
    const authorization = input.liveExecutionAuthorization.authorization;
    if (input.liveExecutionAuthorization.status !== "live_execution_authorization_ready" ||
    ...
    if (startsAt !== undefined && expiresAt !== undefined) {
        ...
        else if (startsAt.getTime() > options.now.getTime() ||
...
//# sourceMappingURL=yeonjang-browser-active-tab-info-live-execution-receipt.js.map