...
} from "./yeonjang-browser-active-tab-info-final-completion-ledger.js"

export type YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceiptStatus =
  "ready"
...
const SAFE_OPERATOR_FINAL_ACKNOWLEDGEMENT_RECEIPT_REF_PATTERN =
  /^operator-final-acknowledgement-receipt:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_OPERATOR_FINAL_ACKNOWLEDGEMENT_REF_PATTERN =
  /^operator-final-acknowledgement:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractFinalCompletionLedgerId(
  ledger: YeonjangBrowserActiveTabInfoFinalCompletionLedger,
): string | undefined {
  if (
    ledger.status !== "final_completion_ledger_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceipt(
  input: YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceiptInput,
): YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoOperatorFinalAcknowledgementReceiptBlockingReasonCode[] = []
  const finalCompletionLedgerId = extractFinalCompletionLedgerId(
    input.finalCompletionLedger,
  )
  ...
  const sanitizedOperatorFinalAcknowledgementReceiptRef =
    input.sanitizedOperatorFinalAcknowledgementReceiptRef.trim()
  ...
  const operatorFinalAcknowledgementRef =
    input.operatorFinalAcknowledgementRef.trim()
  ...
  return baseResult({
    ...
    receipt: Object.freeze({
      operatorFinalAcknowledgementReceiptId:
        buildOperatorFinalAcknowledgementReceiptId({
