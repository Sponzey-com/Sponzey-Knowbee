...
function packageDigestRecord(value) {
    return (objectRecord(value) &&
        typeof value.name === "string" &&
        typeof value.version === "string" &&
        typeof value.digestSha256 === "string");
}
function runtimeIdentityRecord(value) {
    return (objectRecord(value) &&
...
function validateRuntime(runtime) {
    return (runtime.nodeVersion.trim().length > 0 &&
...
function normalizePackages(packages) {
    ...
    const required = new Set(REQUIRED_NPM_RELEASE_PACKAGE_NAMES);
    for (const item of packages) {
    ...
    for (const name of REQUIRED_NPM_RELEASE_PACKAGE_NAMES) {
        if (!seen.has(name))
            return { status: "rejected", reasonCode: `package_set_missing:${name}` };
    }
    ...
    const normalized = REQUIRED_NPM_RELEASE_PACKAGE_NAMES.map((name) => {
        ...
        if (!item)
            throw new Error("validated package set is incomplete");
...
export function verifyNpmCleanInstallReceipt(input) {
    ...
    if (receipt.kind !== "knowbee.release.npm_clean_install_receipt" ||
...
//# sourceMappingURL=npm-install-receipt.js.map