...
const SAFE_FINAL_RETAINED_SEALED_COMPLETION_LEDGER_REF_PATTERN = /^final-retained-sealed-completion-ledger:active-tab-info:sanitized:[a-z0-9._:-]+$/u;
const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN = /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u;
const SAFE_FINAL_RETAINED_SEALED_COMPLETION_REF_PATTERN = /^final-retained-sealed-completion:active-tab-info:ack:[a-z0-9._:-]+$/u;
function extractOperatorFinalRetainedSealedCompletionReceiptId(receipt) {
    if (receipt.status !== "operator_final_retained_sealed_completion_receipt_ready" ||
...
export function buildYeonjangBrowserActiveTabInfoFinalRetainedSealedCompletionLedger(input) {
    const blockingReasonCodes = [];
    const operatorFinalRetainedSealedCompletionReceiptId = extractOperatorFinalRetainedSealedCompletionReceiptId(input.operatorFinalRetainedSealedCompletionReceipt);
    if (operatorFinalRetainedSealedCompletionReceiptId === undefined) {
        blockingReasonCodes.push("final_retained_sealed_completion_ledger_receipt_not_ready");
    }
    const sanitizedFinalRetainedSealedCompletionLedgerRef = input.sanitizedFinalRetainedSealedCompletionLedgerRef.trim();
    ...
    if (!SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN.test(productLogEvidenceRef)) {
        blockingReasonCodes.push("final_retained_sealed_completion_ledger_product_log_evidence_ref_invalid");
    }
    const finalRetainedSealedCompletionRef = input.finalRetainedSealedCompletionRef.trim();
    ...
    return baseResult({
        ...
        ledger: Object.freeze({
            finalRetainedSealedCompletionLedgerId: buildFinalRetainedSealedCompletionLedgerId({
...
//# sourceMappingURL=yeonjang-browser-active-tab-info-final-retained-sealed-completion-ledger.js.map