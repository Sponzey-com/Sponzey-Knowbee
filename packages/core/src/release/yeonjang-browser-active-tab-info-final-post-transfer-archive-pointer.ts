...
} from "./yeonjang-browser-active-tab-info-operator-final-transfer-acknowledgement-receipt.js"

export type YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointerStatus = "ready"

export type YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointerBlockingReasonCode =
...
const SAFE_POST_TRANSFER_ARCHIVE_POINTER_REF_PATTERN =
  /^post-transfer-archive-pointer:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_ARCHIVE_TRANSFER_ACK_REF_PATTERN =
  /^archive-transfer:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractOperatorFinalTransferAcknowledgementReceiptId(
  receipt: YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceipt,
): string | undefined {
  if (
    receipt.status !== "operator_final_transfer_acknowledgement_receipt_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointer["status"]
  reasonCode: YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointer["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointerBlockingReasonCode[]
  pointer?: YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointer["pointer"]
...
export function buildYeonjangBrowserActiveTabInfoFinalPostTransferArchivePointer(
  input: YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointerInput,
): YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointer {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoFinalPostTransferArchivePointerBlockingReasonCode[] = []
  const operatorFinalTransferAcknowledgementReceiptId =
    extractOperatorFinalTransferAcknowledgementReceiptId(input.operatorFinalTransferAcknowledgementReceipt)
  ...
  const sanitizedPostTransferArchivePointerRef =
    input.sanitizedPostTransferArchivePointerRef.trim()
  ...
  const archiveTransferAcknowledgementRef =
    input.archiveTransferAcknowledgementRef.trim()
  ...
  return baseResult({
    ...
    pointer: Object.freeze({
      finalPostTransferArchivePointerId:
        buildFinalPostTransferArchivePointerId({
