...
import {
  type BackupRestoreRehearsalReceipt,
  verifyBackupRestoreRehearsalReceipt,
} from "./backup-restore-receipt.js"
import {
  type NpmCleanInstallReceipt,
  type StagedNpmPackageDigest,
  verifyNpmCleanInstallReceipt,
} from "./npm-install-receipt.js"
...
export type ArtifactCleanupSmokeCheck =
  | "preview"
...
const ARTIFACT_CLEANUP_REQUIRED_CHECKS: readonly ArtifactCleanupSmokeCheck[] = [
  "preview",
  "confirmation_failure",
]
const ARTIFACT_CLEANUP_ALLOWED_CHECKS = new Set<ArtifactCleanupSmokeCheck>([
  "preview",
  "confirmation_failure",
  "destructive_fixture_success",
])

function verifyArtifactCleanupSmokeReceipt(
  receipt: Readonly<ArtifactCleanupSmokeReceipt> | unknown | null | undefined,
): {
  summary: OperationalRehearsalEvidenceSummary["artifactCleanupSmoke"]
  reasonCode: string | null
} {
  ...
  const candidate = receipt as Partial<ArtifactCleanupSmokeReceipt>
  ...
  if (
    checked.length !== candidate.checked.length ||
...
export function verifyOperationalRehearsalEvidence(
  input: OperationalRehearsalEvidenceInput,
): OperationalRehearsalEvidenceSummary {
  const reasonCodes: string[] = []
  let npmInstall: OperationalRehearsalEvidenceSummary["npmInstall"]
  if (input.npmReceipt === null || input.npmReceipt === undefined) {
    ...
    if (verification.status === "rejected") {
      ...
      const receipt = input.npmReceipt as Readonly<NpmCleanInstallReceipt>
  ...
  let backupRestore: OperationalRehearsalEvidenceSummary["backupRestore"]
  if (input.backupReceipt === null || input.backupReceipt === undefined) {
    ...
    if (verification.status === "rejected") {
      ...
      const receipt = input.backupReceipt as Readonly<BackupRestoreRehearsalReceipt>
      if (
        receipt.snapshot.appVersion !== input.candidate.appVersion ||
  ...
  const artifactCleanupSmoke = verifyArtifactCleanupSmokeReceipt(
    input.artifactCleanupSmokeReceipt,
  )
  if (artifactCleanupSmoke.reasonCode) {
    reasonCodes.push(
      artifactCleanupSmoke.summary.reasonCode ?? artifactCleanupSmoke.reasonCode,
    )
  }
