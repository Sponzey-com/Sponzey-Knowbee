...
} from "./yeonjang-browser-active-tab-info-operator-sealed-archive-handoff-receipt.js"

export type YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndexStatus = "ready"

export type YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndexBlockingReasonCode =
...
const SAFE_FINAL_SEALED_ARCHIVE_HANDOFF_COMPLETION_INDEX_REF_PATTERN =
  /^final-sealed-archive-handoff-completion-index:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_FINAL_SEALED_ARCHIVE_HANDOFF_COMPLETION_ACK_REF_PATTERN =
  /^final-sealed-archive-handoff-completion:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractOperatorSealedArchiveHandoffReceiptId(
  receipt: YeonjangBrowserActiveTabInfoOperatorSealedArchiveHandoffReceipt,
): string | undefined {
  if (
    receipt.status !== "operator_sealed_archive_handoff_receipt_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndex["status"]
  reasonCode: YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndex["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndexBlockingReasonCode[]
  index?: YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndex["index"]
...
export function buildYeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndex(
  input: YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndexInput,
): YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndex {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoFinalSealedArchiveHandoffCompletionIndexBlockingReasonCode[] = []
  const operatorSealedArchiveHandoffReceiptId =
    extractOperatorSealedArchiveHandoffReceiptId(input.operatorSealedArchiveHandoffReceipt)
  ...
  const sanitizedFinalSealedArchiveHandoffCompletionIndexRef =
    input.sanitizedFinalSealedArchiveHandoffCompletionIndexRef.trim()
  ...
  const finalSealedArchiveHandoffCompletionAcknowledgementRef =
    input.finalSealedArchiveHandoffCompletionAcknowledgementRef.trim()
  ...
  return baseResult({
    ...
    index: Object.freeze({
      finalSealedArchiveHandoffCompletionIndexId:
        buildFinalSealedArchiveHandoffCompletionIndexId({
