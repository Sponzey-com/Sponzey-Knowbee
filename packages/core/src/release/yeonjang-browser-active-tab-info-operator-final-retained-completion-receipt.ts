...
} from "./yeonjang-browser-active-tab-info-final-retained-sealed-completion-ledger.js"

export type YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceiptStatus =
  "ready"

export type YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceiptBlockingReasonCode =
...
export type YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceipt = Readonly<{
  ...
  reasonCode:
    | "active_tab_info_operator_final_retained_completion_receipt_ready"
...
const SAFE_OPERATOR_FINAL_RETAINED_COMPLETION_RECEIPT_REF_PATTERN =
  /^operator-final-retained-completion-receipt:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_OPERATOR_FINAL_RETAINED_COMPLETION_REF_PATTERN =
  /^operator-final-retained-completion:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractFinalRetainedSealedCompletionLedgerId(
  ledger: YeonjangBrowserActiveTabInfoFinalRetainedSealedCompletionLedger,
): string | undefined {
  if (
    ledger.status !== "final_retained_sealed_completion_ledger_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceipt(
  input: YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceiptInput,
): YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoOperatorFinalRetainedCompletionReceiptBlockingReasonCode[] = []
  const finalRetainedSealedCompletionLedgerId =
    extractFinalRetainedSealedCompletionLedgerId(
      input.finalRetainedSealedCompletionLedger,
    )
  ...
  const sanitizedOperatorFinalRetainedCompletionReceiptRef =
    input.sanitizedOperatorFinalRetainedCompletionReceiptRef.trim()
  ...
  const operatorFinalRetainedCompletionRef =
    input.operatorFinalRetainedCompletionRef.trim()
  ...
  return baseResult({
    ...
    receipt: Object.freeze({
      operatorFinalRetainedCompletionReceiptId:
        buildOperatorFinalRetainedCompletionReceiptId({
