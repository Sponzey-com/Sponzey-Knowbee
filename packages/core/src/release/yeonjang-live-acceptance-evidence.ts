...
export type YeonjangLiveEvidenceRejectionCode =
  | "yeonjang_smoke_not_live"
  | "yeonjang_smoke_run_not_passed"
  | "yeonjang_smoke_scenario_duplicate"
  | "yeonjang_smoke_result_not_verified"
  ...
  | "yeonjang_smoke_run_correlation_invalid"
  | "yeonjang_smoke_command_missing"
...
export function produceYeonjangLiveAcceptanceEvidence(input: {
  ...
  const rejected: YeonjangLiveEvidenceRejection[] = []
  ...
  for (const result of input.run.results) {
    ...
    let reasonCode: YeonjangLiveEvidenceRejectionCode | undefined

    if (input.run.mode !== "live-run") reasonCode = "yeonjang_smoke_not_live"
    else if (input.run.status !== "passed") reasonCode = "yeonjang_smoke_run_not_passed"
    else if (seen.has(scenario.id)) reasonCode = "yeonjang_smoke_scenario_duplicate"
    ...
    } else if (!trace || !instance) reasonCode = "yeonjang_smoke_trace_missing"
    ...
    } else if (!command) reasonCode = "yeonjang_smoke_command_missing"
    else if (
      command.runId !== input.run.runId ||
    ...
    } else if (!observed) reasonCode = "yeonjang_smoke_observed_result_missing"
    else if (
      observed.runId !== input.run.runId ||
    ...
    } else if (!diagnosis) reasonCode = "yeonjang_smoke_llm_diagnosis_missing"
    else if (
    ...
    } else if (!trace.auditEventId?.trim()) reasonCode = "yeonjang_smoke_audit_missing"
    else if (trace.redactionStatus !== "verified") reasonCode = "yeonjang_smoke_unredacted"
    else {
      accepted.push({
    ...
    if (reasonCode) rejected.push({ scenarioId: scenario.id, reasonCode })
  }
