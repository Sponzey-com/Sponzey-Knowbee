...
} from "./yeonjang-browser-active-tab-info-release-evidence-chain-cleanup-deletion-review-receipt.ts"
...
export type YeonjangBrowserActiveTabInfoReleaseEvidenceChainCleanupDeletionExecutionAdmission = {
  ...
  readonly admissionDecision:
    | "manual_cleanup_deletion_execution_admitted"
    | "complete_cleanup_deletion_review_receipt"
  readonly nextAllowedAction:
    | "prepare_cleanup_deletion_dry_run_after_execution_admission"
    | "complete_cleanup_deletion_review_receipt"
    | "provide_cleanup_deletion_execution_admission_ref"
...
const SAFE_OPERATOR_EXECUTION_ADMISSION_REF =
  /^cleanup-deletion-execution-admission:active-tab-info:operator:[a-z0-9][a-z0-9:-]{0,96}$/u

function buildExecutionAdmissionId(admissionRef: string): string {
  return `cleanup-deletion-execution-admission:active-tab-info:sha256:${
    createHash("sha256").update(admissionRef).digest("hex")
  }`
}
...
function blocked(input: {
  ...
  nextAllowedAction: Extract<
    ...
    "complete_cleanup_deletion_review_receipt" | "provide_cleanup_deletion_execution_admission_ref"
  >
...
export function buildYeonjangBrowserActiveTabInfoReleaseEvidenceChainCleanupDeletionExecutionAdmission(
  input: YeonjangBrowserActiveTabInfoReleaseEvidenceChainCleanupDeletionExecutionAdmissionInput,
): YeonjangBrowserActiveTabInfoReleaseEvidenceChainCleanupDeletionExecutionAdmission {
  if (
    input.cleanupDeletionReviewReceipt.receiptStatus !== "accepted" ||
