...
export type YeonjangBrowserActiveTabInfoVerificationStatus =
  | "verified"
  | "unverifiable"
  | "failed"
...
export type YeonjangBrowserActiveTabInfoFinalResultProjectionResult =
  | { ok: true; projection: YeonjangBrowserActiveTabInfoFinalResultProjection }
  | {
      ok: false
...
export type YeonjangBrowserActiveTabInfoProductLogProjectionResult =
  | { ok: true; projection: YeonjangBrowserActiveTabInfoProductLogProjection }
  | {
      ok: false
...
export function isSafeYeonjangBrowserActiveTabInfoEvidenceRef(value: string): boolean {
...
export function buildYeonjangBrowserActiveTabInfoFinalResultProjection(input: {
  ...
  if (!evidenceRef) return { ok: false, reasonCode: "evidence_ref_required" }
  ...
  if (!validation.ok) return { ok: false, reasonCode: "final_result_redaction_required" }
...
export function buildYeonjangBrowserActiveTabInfoProductLogProjection(input: {
  ...
  if (!validation.ok) return { ok: false, reasonCode: "product_log_evidence_ref_only" }
...
function normalizePublicTargetName(value: string): string {
