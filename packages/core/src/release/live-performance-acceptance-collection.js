import { collectLivePerformanceEvidence, } from "../maintenance/live-performance-evidence.js";
import { REQUIRED_REPRESENTATIVE_FLOW_IDS, } from "../maintenance/performance-baseline.js";
import { selectPerformanceAcceptanceMatrix, } from "./performance-acceptance-authorization.js";
import { buildPerformanceAcceptanceEvidence } from "./performance-acceptance-evidence.js";
function baselineOnly(selector, reasonCode, authorizationId = null) {
    return {
...
export function collectLivePerformanceAcceptanceEvidence(input) {
    ...
    if (selected.status === "baseline_only") {
        return baselineOnly(input.selector, selected.reasonCodes[0] ?? "performance_matrix_selection_failed");
    }
    ...
    for (const run of input.runs) {
        ...
        if (collected.status === "rejected") {
            return baselineOnly(input.selector, `flow:${run.flowId}:collection:${collected.reasonCode}`, authorizationId ?? null);
        }
        samples.push(collected.sample);
    }
    return buildPerformanceAcceptanceEvidence({ selected, samples });
}
//# sourceMappingURL=live-performance-acceptance-collection.js.map