...
const SAFE_OPERATOR_FINAL_RETENTION_ACKNOWLEDGEMENT_RECEIPT_REF_PATTERN = /^operator-final-retention-acknowledgement-receipt:active-tab-info:sanitized:[a-z0-9._:-]+$/u;
const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN = /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u;
const SAFE_OPERATOR_FINAL_RETENTION_ACKNOWLEDGEMENT_REF_PATTERN = /^operator-final-retention-acknowledgement:active-tab-info:ack:[a-z0-9._:-]+$/u;
function extractFinalRetentionClosureLedgerId(ledger) {
    if (ledger.status !== "final_retention_closure_ledger_ready" ||
...
export function buildYeonjangBrowserActiveTabInfoOperatorFinalRetentionAcknowledgementReceipt(input) {
    const blockingReasonCodes = [];
    const finalRetentionClosureLedgerId = extractFinalRetentionClosureLedgerId(input.finalRetentionClosureLedger);
    ...
    const sanitizedOperatorFinalRetentionAcknowledgementReceiptRef = input.sanitizedOperatorFinalRetentionAcknowledgementReceiptRef.trim();
    ...
    const operatorFinalRetentionAcknowledgementRef = input.operatorFinalRetentionAcknowledgementRef.trim();
    ...
    if (blockingReasonCodes.length > 0 || finalRetentionClosureLedgerId === undefined) {
        return baseResult({
            status: "blocked",
            reasonCode: "active_tab_info_operator_final_retention_acknowledgement_receipt_blocked",
            blockingReasonCodes,
        });
    }
    ...
    return baseResult({
        ...
        reasonCode: "active_tab_info_operator_final_retention_acknowledgement_receipt_ready",
        receipt: Object.freeze({
            operatorFinalRetentionAcknowledgementReceiptId: buildOperatorFinalRetentionAcknowledgementReceiptId({
...
//# sourceMappingURL=yeonjang-browser-active-tab-info-operator-final-retention-acknowledgement-receipt.js.map