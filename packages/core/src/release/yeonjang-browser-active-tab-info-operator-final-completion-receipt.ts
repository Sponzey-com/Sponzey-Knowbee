...
} from "./yeonjang-browser-active-tab-info-final-retained-completion-ledger.js"

export type YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceiptStatus =
  "ready"

export type YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceiptBlockingReasonCode =
...
export type YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceipt = Readonly<{
  ...
  reasonCode:
    | "active_tab_info_operator_final_completion_receipt_ready"
...
const SAFE_OPERATOR_FINAL_COMPLETION_RECEIPT_REF_PATTERN =
  /^operator-final-completion-receipt:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_OPERATOR_FINAL_COMPLETION_REF_PATTERN =
  /^operator-final-completion:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractFinalRetainedCompletionLedgerId(
  ledger: YeonjangBrowserActiveTabInfoFinalRetainedCompletionLedger,
): string | undefined {
  if (
    ledger.status !== "final_retained_completion_ledger_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoOperatorFinalCompletionReceipt(
  input: YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceiptInput,
): YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceiptBlockingReasonCode[] = []
  const finalRetainedCompletionLedgerId = extractFinalRetainedCompletionLedgerId(
    input.finalRetainedCompletionLedger,
  )
  if (finalRetainedCompletionLedgerId === undefined) {
    blockingReasonCodes.push("operator_final_completion_receipt_ledger_not_ready")
  }
  const sanitizedOperatorFinalCompletionReceiptRef =
    input.sanitizedOperatorFinalCompletionReceiptRef.trim()
