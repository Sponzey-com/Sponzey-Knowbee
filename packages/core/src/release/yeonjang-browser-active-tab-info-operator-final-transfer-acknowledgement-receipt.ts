...
} from "./yeonjang-browser-active-tab-info-final-transfer-closeout-ledger.js"

export type YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceiptStatus = "ready"

export type YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceiptBlockingReasonCode =
...
const SAFE_OPERATOR_FINAL_TRANSFER_ACKNOWLEDGEMENT_RECEIPT_REF_PATTERN =
  /^operator-final-transfer-acknowledgement-receipt:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_OPERATOR_FINAL_TRANSFER_ACKNOWLEDGEMENT_REF_PATTERN =
  /^operator-final-transfer:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractFinalTransferCloseoutLedgerId(
  ledger: YeonjangBrowserActiveTabInfoFinalTransferCloseoutLedger,
): string | undefined {
  if (
    ledger.status !== "final_transfer_closeout_ledger_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceipt["status"]
  reasonCode: YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceipt["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceiptBlockingReasonCode[]
  receipt?: YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceipt["receipt"]
...
export function buildYeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceipt(
  input: YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceiptInput,
): YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceipt {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoOperatorFinalTransferAcknowledgementReceiptBlockingReasonCode[] = []
  const finalTransferCloseoutLedgerId =
    extractFinalTransferCloseoutLedgerId(input.finalTransferCloseoutLedger)
  ...
  const sanitizedOperatorFinalTransferAcknowledgementReceiptRef =
    input.sanitizedOperatorFinalTransferAcknowledgementReceiptRef.trim()
  ...
  const operatorFinalTransferAcknowledgementRef =
    input.operatorFinalTransferAcknowledgementRef.trim()
  ...
  return baseResult({
    ...
    receipt: Object.freeze({
      operatorFinalTransferAcknowledgementReceiptId:
        buildOperatorFinalTransferAcknowledgementReceiptId({
