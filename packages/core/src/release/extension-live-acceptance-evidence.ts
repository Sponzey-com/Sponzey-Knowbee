...
export type ExtensionLiveEvidenceRejectionCode =
  | "extension_smoke_not_live"
  | "extension_smoke_run_not_passed"
  | "extension_smoke_scenario_duplicate"
  | "extension_smoke_result_not_verified"
  | "extension_smoke_read_only_required"
  | "extension_smoke_selection_mismatch"
  | "extension_smoke_run_correlation_invalid"
  | "extension_smoke_discovery_only"
  | "extension_smoke_tool_receipt_missing"
  | "extension_smoke_tool_receipt_mismatch"
  | "extension_smoke_tool_not_succeeded"
  | "extension_smoke_llm_diagnosis_missing"
...
export function produceExtensionLiveAcceptanceEvidence(
  run: ExtensionLiveSmokeSummary,
): ExtensionLiveEvidenceProductionResult {
  const accepted: LiveAcceptanceEvidence[] = []
  const rejected: ExtensionLiveEvidenceRejection[] = []
  ...
  for (const result of run.results) {
    ...
    let reasonCode: ExtensionLiveEvidenceRejectionCode | undefined

    if (run.mode !== "live-run") reasonCode = "extension_smoke_not_live"
    else if (run.status !== "passed") reasonCode = "extension_smoke_run_not_passed"
    else if (seen.has(scenario.id)) reasonCode = "extension_smoke_scenario_duplicate"
    else if (result.state !== "verified" || result.status !== "passed") {
      reasonCode = "extension_smoke_result_not_verified"
    } else if (!scenario.readOnly) reasonCode = "extension_smoke_read_only_required"
    else if (
      trace?.selectedCapability !== scenario.capability ||
    ...
    } else if (trace.discoveryOnly) reasonCode = "extension_smoke_discovery_only"
    else if (!tool) reasonCode = "extension_smoke_tool_receipt_missing"
    else if (
    ...
    } else if (!diagnosis) reasonCode = "extension_smoke_llm_diagnosis_missing"
    else if (
    ...
    } else if (!trace.auditEventId?.trim()) reasonCode = "extension_smoke_audit_missing"
    else if (trace.redactionStatus !== "verified") reasonCode = "extension_smoke_unredacted"
    else {
      accepted.push({
