...
export type LiveAcceptanceBundleSignatureVerifier = (
  input: Readonly<LiveAcceptanceBundleSignatureVerification>,
) => boolean

export type LiveAcceptanceBundleParseResult =
  | { status: "verified"; bundle: Readonly<LiveAcceptanceBundle> }
...
function canonicalJson(value: unknown): string {
  ...
  if (encoded === undefined) throw new Error("live_acceptance_bundle_canonical_value_invalid")
  return encoded
}

export function buildLiveAcceptanceBundleSigningBytes(
  payload: LiveAcceptanceBundlePayload,
): Uint8Array {
  return new TextEncoder().encode(canonicalJson(payload))
}

export function buildLiveAcceptanceBundleChecksum(
  payload: LiveAcceptanceBundlePayload,
): `sha256:${string}` {
...
function reject(reasonCode: string): LiveAcceptanceBundleParseResult {
  return { status: "rejected", reasonCode }
}

export type LiveAcceptanceBundlePayloadValidationResult =
  | { status: "verified"; payload: Readonly<LiveAcceptanceBundlePayload> }
  | { status: "rejected"; reasonCode: string }
...
function validatePayloadFields(input: {
  ...
  if (
    input.value.kind !== "knowbee.release.live_acceptance_bundle" ||
  ...
  if (
    candidate.appVersion !== input.expectedCandidate.appVersion ||
  ...
  if (
    approval.principalType !== "authenticated_user" ||
  ...
  if (
    approval.authorizationStatus !== "active" ||
    ...
    (approval.approvedAt as number) > input.now ||
    (approval.approvedAt as number) >= (approval.expiresAt as number) ||
    approval.redactionStatus !== "verified"
  ) {
    return { status: "rejected", reasonCode: "live_acceptance_bundle_approval_invalid" }
  }
  if ((approval.expiresAt as number) <= input.now) {
  ...
  for (const item of input.value.evidence) {
    ...
    evidence.push(Object.freeze({ ...item }) as unknown as LiveAcceptanceEvidence)
  }
  const payload = {
    kind: "knowbee.release.live_acceptance_bundle",
    schemaVersion: 2,
    candidate: Object.freeze({ ...candidate }) as unknown as LiveAcceptanceBundleCandidate,
    approval: Object.freeze({
      ...approval,
      roles: Object.freeze([...approval.roles]),
    }) as unknown as LiveAcceptanceBundleApproval,
    evidence: Object.freeze(evidence),
  } as unknown as LiveAcceptanceBundlePayload
...
export function parseLiveAcceptanceBundle(input: {
  ...
  if (payloadResult.status === "rejected") return payloadResult
  const payload = payloadResult.payload as LiveAcceptanceBundlePayload
  if (
    typeof input.value.payloadSha256 !== "string" ||
  ...
  try {
    if (
      !input.verifySignature?.({
        algorithm: "ed25519",
        keyId: signature.keyId as `sha256:${string}`,
  ...
  } as unknown as LiveAcceptanceBundle
