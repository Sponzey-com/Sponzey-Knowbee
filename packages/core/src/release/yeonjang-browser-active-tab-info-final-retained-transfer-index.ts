...
} from "./yeonjang-browser-active-tab-info-operator-post-transfer-archive-acknowledgement-receipt.js"

export type YeonjangBrowserActiveTabInfoFinalRetainedTransferIndexStatus = "ready"

export type YeonjangBrowserActiveTabInfoFinalRetainedTransferIndexBlockingReasonCode =
...
const SAFE_RETAINED_TRANSFER_INDEX_REF_PATTERN =
  /^retained-transfer-index:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_RETENTION_TRANSFER_ACKNOWLEDGEMENT_REF_PATTERN =
  /^retention-transfer:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractOperatorPostTransferArchiveAcknowledgementReceiptId(
  receipt: YeonjangBrowserActiveTabInfoOperatorPostTransferArchiveAcknowledgementReceipt,
): string | undefined {
  if (
    receipt.status !==
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoFinalRetainedTransferIndex["status"]
  reasonCode: YeonjangBrowserActiveTabInfoFinalRetainedTransferIndex["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoFinalRetainedTransferIndexBlockingReasonCode[]
  index?: YeonjangBrowserActiveTabInfoFinalRetainedTransferIndex["index"]
...
export function buildYeonjangBrowserActiveTabInfoFinalRetainedTransferIndex(
  input: YeonjangBrowserActiveTabInfoFinalRetainedTransferIndexInput,
): YeonjangBrowserActiveTabInfoFinalRetainedTransferIndex {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoFinalRetainedTransferIndexBlockingReasonCode[] = []
  const operatorPostTransferArchiveAcknowledgementReceiptId =
    extractOperatorPostTransferArchiveAcknowledgementReceiptId(
      input.operatorPostTransferArchiveAcknowledgementReceipt,
    )
  ...
  const sanitizedRetainedTransferIndexRef =
    input.sanitizedRetainedTransferIndexRef.trim()
  ...
  const retentionTransferAcknowledgementRef =
    input.retentionTransferAcknowledgementRef.trim()
