...
} from "./yeonjang-browser-active-tab-info-operator-final-completion-receipt.js"

export type YeonjangBrowserActiveTabInfoFinalCompletionLedgerStatus = "ready"

export type YeonjangBrowserActiveTabInfoFinalCompletionLedgerBlockingReasonCode =
...
export type YeonjangBrowserActiveTabInfoFinalCompletionLedger = Readonly<{
  ...
  reasonCode:
    | "active_tab_info_final_completion_ledger_ready"
...
const SAFE_FINAL_COMPLETION_LEDGER_REF_PATTERN =
  /^final-completion-ledger:active-tab-info:sanitized:[a-z0-9._:-]+$/u

const SAFE_PRODUCT_LOG_EVIDENCE_REF_PATTERN =
  /^product-log:active-tab-info:evidence:[a-z0-9._:-]+$/u

const SAFE_FINAL_COMPLETION_REF_PATTERN =
  /^final-completion:active-tab-info:ack:[a-z0-9._:-]+$/u

function extractOperatorFinalCompletionReceiptId(
  receipt: YeonjangBrowserActiveTabInfoOperatorFinalCompletionReceipt,
): string | undefined {
  if (
    receipt.status !== "operator_final_completion_receipt_ready" ||
...
function baseResult(input: {
  status: YeonjangBrowserActiveTabInfoFinalCompletionLedger["status"]
  reasonCode: YeonjangBrowserActiveTabInfoFinalCompletionLedger["reasonCode"]
  blockingReasonCodes?: readonly YeonjangBrowserActiveTabInfoFinalCompletionLedgerBlockingReasonCode[]
  ledger?: YeonjangBrowserActiveTabInfoFinalCompletionLedger["ledger"]
...
export function buildYeonjangBrowserActiveTabInfoFinalCompletionLedger(
  input: YeonjangBrowserActiveTabInfoFinalCompletionLedgerInput,
): YeonjangBrowserActiveTabInfoFinalCompletionLedger {
  const blockingReasonCodes: YeonjangBrowserActiveTabInfoFinalCompletionLedgerBlockingReasonCode[] = []
  const operatorFinalCompletionReceiptId = extractOperatorFinalCompletionReceiptId(
    input.operatorFinalCompletionReceipt,
  )
  if (operatorFinalCompletionReceiptId === undefined) {
    blockingReasonCodes.push("final_completion_ledger_receipt_not_ready")
  }
  const sanitizedFinalCompletionLedgerRef =
    input.sanitizedFinalCompletionLedgerRef.trim()
