...
export function produceWebLiveAcceptanceEvidence(input) {
    const accepted = [];
    const rejected = [];
    const seen = new Set();
    for (const result of input.run.results) {
        ...
        if (input.run.mode !== "live-run")
            ...
            reasonCode = "web_smoke_result_not_passed";
        else if (trace?.answerProduced !== true)
            reasonCode = "web_smoke_answer_missing";
        else if (!diagnosis)
            reasonCode = "web_smoke_llm_diagnosis_missing";
        ...
        else if (!receipt?.sourceEvidence.length ||
            receipt.sourceEvidence.some((source) => !source.evidenceRef.trim() ||
                !source.sourceDomain.trim() ||
                !diagnosis.evidenceRefs.includes(source.evidenceRef)) ||
            diagnosis.evidenceRefs.some((evidenceRef) => !receipt.sourceEvidence.some((source) => source.evidenceRef === evidenceRef))) {
            ...
            else if (!receipt.auditEventId.trim())
                reasonCode = "web_smoke_audit_missing";
            else if (receipt.redactionStatus !== "verified")
                reasonCode = "web_smoke_unredacted";
            else {
                accepted.push({
...
//# sourceMappingURL=web-live-acceptance-evidence.js.map