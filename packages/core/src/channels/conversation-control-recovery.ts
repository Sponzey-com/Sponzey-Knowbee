...
export type ConversationInteractionAdmission =
  | "accepted"
  | "duplicate_rejected"
  | "expired_rejected"
...
export function validateConversationControlRecoveryParity(
  observations: readonly ConversationControlRecoveryObservation[],
  ...
  for (const scenarioId of scenarioIds) {
    ...
    for (const channel of REQUIRED_CHANNELS) {
      const candidates = observations.filter(
        (item) => item.scenarioId === scenarioId && item.channel === channel,
      )
      ...
      const expectedTransitionCount =
        observation.interactionAdmission === "accepted" ? 1 : 0
      ...
      if (
        observation.restartDelivery.attempted
