...
function scopedReceipts(receipts, started) {
    return (receipts ?? []).filter((receipt) => receipt.runId === started.runId && receipt.requestGroupId === started.requestGroupId);
}
function projectScenarioEvidence(scenario, started, observation) {
    ...
    if (!expectedTool)
        throw new Error("telegram_live_smoke_expected_tool_missing");
    const toolReceipt = scopedReceipts(observation.toolReceipts, started).find((receipt) => receipt.toolName === expectedTool);
    if (!toolReceipt)
        throw new Error("telegram_live_smoke_tool_receipt_missing");
    ...
    for (const artifact of artifacts) {
        if (artifact.mode === "download_link" &&
    ...
    if (scenario.kind === "artifact_delivery" || scenario.kind === "web_skill")
        return projected;
    const approval = scopedReceipts(observation.approvalReceipts, started).find((receipt) => receipt.toolName === expectedTool);
    if (!approval)
        throw new Error("telegram_live_smoke_approval_receipt_missing");
    if (approval.status === "requested") {
        throw new Error("telegram_live_smoke_approval_unresolved");
    }
    if (approval.status === "denied")
        throw new Error("telegram_live_smoke_approval_denied");
    if (approval.status === "expired")
        throw new Error("telegram_live_smoke_approval_timed_out");
    if (!approval.uiVisible)
        throw new Error("telegram_live_smoke_approval_ui_missing");
    return {
        ...projected,
        approval: {
            requested: true,
...
export function createTelegramLiveSmokeExecutor(ports) {
    return async (scenario) => {
        ...
        if (capabilityAdmissionRequired &&
            ...
            throw new Error("telegram_live_smoke_capability_admission_receipt_missing");
        }
        ...
        return {
            ...
            latency: {
                ...
                terminalResponseLatencyMs: observation.latencyEvidence.terminalResponseLatencyMs,
            },
            finalDelivery: {
                delivered: true,
                targetChannel: "telegram",
...
//# sourceMappingURL=telegram-live-smoke-executor.js.map