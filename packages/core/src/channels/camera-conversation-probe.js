...
export class CameraConversationProbeAdapter {
    dependencies;
...
function freezeConversation(observation) {
...
function freezeCamera(observation) {
    return Object.freeze({
        ...observation,
...
export function projectCameraConversationPreEffectSnapshot(facts) {
    ...
    if (approval.toolName !== CAMERA_TOOL_NAME
    ...
    if (!SHA256_FINGERPRINT_PATTERN.test(approval.executionTargetFingerprint)
    ...
    if (!["in_progress", "awaiting_approval"].includes(facts.requestOutcome.executionStatus)
    ...
    if (!facts.receipts.requestDiagnosisReceiptId.trim()
    ...
    const camera = freezeCamera({
        ...
        deliveryApproval: {
            ...
            status: facts.deliveryTarget.channel === "telegram"
                ? "not_started"
                : "not_required",
        },
        capture: {
...
export function projectCameraConversationPostEffectSnapshot(facts) {
    const binding = facts.binding;
    const approval = facts.approval;
    const admission = facts.capabilityAdmission;
    if (!binding.runId.trim()
    ...
    if (facts.effect.startEffectCount !== 1
    ...
    if (!/^artifact:[0-9a-f-]{36}$/iu.test(facts.artifact.artifactRef)
        || !["image/jpeg", "image/png", "image/webp"].includes(facts.artifact.mimeType)
...
export function projectCameraConversationDeliveryApprovalSnapshot(facts) {
    ...
    if (delivery.runId !== binding.runId
        || delivery.requestGroupId !== binding.requestGroupId
        || !delivery.approvalRequestRef.trim()
        || !SHA256_FINGERPRINT_PATTERN.test(delivery.executionTargetFingerprint)
...
export function projectCameraConversationCompletedSnapshot(facts) {
    const captured = projectCameraConversationPostEffectSnapshot(facts.capture);
    if (captured.status !== "success")
        return captured;
    const binding = facts.capture.binding;
    ...
    if (deliveryApproval.runId !== binding.runId
        || deliveryApproval.requestGroupId !== binding.requestGroupId
        || !deliveryApproval.approvalRequestRef.trim()
        || !SHA256_FINGERPRINT_PATTERN.test(deliveryApproval.executionTargetFingerprint)
    ...
    if (delivery.providerSendCount !== 1
    ...
    if (facts.completionReview.invocationCount !== 1
...
//# sourceMappingURL=camera-conversation-probe.js.map