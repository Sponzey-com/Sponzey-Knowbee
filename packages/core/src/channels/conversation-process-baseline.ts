export type ConversationBaselineClassification =
  | "product_defect"
  | "missing_coverage"
  | "stale_test"
  | "live_prerequisite"
...
export type ConversationProcessBaselineProjection =
  | { status: "ready"; evidence: ConversationProcessBaselineEvidence }
  | {
      status: "rejected"
      reasonCode:
        | "invalid_command"
...
const UNSAFE_EVIDENCE_TEXT =
  /(?:\/Users\/|\/private\/|\/tmp\/|[A-Za-z]:\\|Bearer\s+|raw[_ -]?request|provider[_ -]?payload|chat[_ -]?id|prompt|token|secret|authorization|cookie)/iu
const SAFE_COMMAND =
  /^pnpm exec vitest run --cache=false(?: tests\/[A-Za-z0-9._/-]+\.test\.ts)+$/u
...
export function projectConversationProcessBaseline(
  input: ConversationProcessBaselineInput,
  ...
  const files: ConversationBaselineTestFile[] = []
  for (const file of input.testFiles) {
    if (
      !SAFE_TEST_PATH.test(file.path)
