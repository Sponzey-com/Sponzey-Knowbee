...
interface TelegramReplyContext {
  reply(text: string, options?: { parse_mode: "Markdown" }): Promise<unknown>
}
...
export type TelegramCommandReplyResolution =
  | { status: "ready"; text: string; parseMode?: "Markdown" | undefined; textSource: "llm_reviewed" }
  | { status: "blocked"; reason: string }

export function resolveTelegramCommandResponseLanguage(languageCode: string | undefined): TelegramCommandResponseLanguage {
  return languageCode?.toLowerCase().startsWith("ko") ? "ko" : "en"
}

export function buildTelegramCommandResponseNotice(
  command: TelegramCommandName,
  language: TelegramCommandResponseLanguage = "en",
): TelegramCommandResponseNotice {
  return {
    kind: "telegram_command_response_notice",
    command,
    language,
...
function commandResponse(
  input: Pick<TelegramCommandResponse, "command" | "language" | "text" | "parseMode">,
...
export function buildTelegramCommandResponse(input: TelegramCommandResponseInput): TelegramCommandResponse {
...
function commandOriginalRequest(response: TelegramCommandResponse): string {
...
export async function resolveTelegramCommandReply(
  response: TelegramCommandResponse,
  dependencies: TelegramCommandReplyRenderDependencies = {},
): Promise<TelegramCommandReplyResolution> {
  const originalRequest = commandOriginalRequest(response)
  const config = dependencies.config
  if (!config) return { status: "blocked", reason: "telegram_command_reply_config_missing" }
  const workDir = dependencies.workDir?.trim() || config?.profile.workspace.trim()
  if (!workDir) return { status: "blocked", reason: "telegram_command_reply_work_dir_missing" }
  const identityContext = dependencies.identityContext
    ?? (config
      ...
      : undefined)
  if (!identityContext) return { status: "blocked", reason: "telegram_command_reply_identity_context_missing" }

  const model = dependencies.getDefaultModel?.() ?? (config ? getDefaultModel(config) : "")
  if (!model.trim()) return { status: "blocked", reason: "telegram_command_reply_model_missing" }
  ...
  if (!text) return { status: "blocked", reason: "telegram_command_reply_render_failed" }
...
export async function replyTelegramCommandResponse(
