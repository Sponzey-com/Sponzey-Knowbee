...
function isVerifiedCameraArtifact(artifact) {
...
export function validateCameraChannelAcceptance(observation) {
    const failures = [];
    const fail = (reasonCode) => {
        if (!failures.includes(reasonCode))
    ...
    const deliveryApproved = !observation.deliveryApproval.required ||
    ...
    if (!observation.capabilityAdmission.receiptId ||
    ...
    if (observation.capture.status === "failed" &&
        observation.delivery.status !== "not_started") {
    ...
    if (!deliveryApproved) {
        ...
        if (observation.deliveryApproval.status === "awaiting" &&
    ...
    if (observation.delivery.status === "delivered") {
        ...
        if (!observation.delivery.targetBound ||
    ...
    const terminalReportRequired = observation.delivery.status === "delivered" ||
    ...
    else if (observation.captureApproval.status === "awaiting" ||
    ...
    if (observation.completionReview.outcome === "complete" &&
        (observation.capture.status !== "succeeded" ||
            !verifiedArtifact ||
...
//# sourceMappingURL=camera-acceptance.js.map