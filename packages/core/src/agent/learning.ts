  ...
  if (inserted && policy.autoApply) {
    ...
    const stored = await storeOwnerScopedMemory({
      ...
      longTermWriteGate: {
        ...
        category: "agent_role_knowledge",
        storageNeed: "agent_learning",
        sensitivity: "not_sensitive",
        userIntent: "learning_event_approved",
        sourceEvidenceRefs: [
          ...event.evidenceRefs,
          `learning_event:${learningEventId}`,
        ],
        retentionPurpose: "approved agent learning event persistence",
      },
...
export async function approveLearningEvent(
  ...
  if (historyInserted && event.learningTarget === "memory") {
    const stored = await storeOwnerScopedMemory({
      ...
      retentionPolicy: "long_term",
      longTermWriteGate: {
        targetOwner: event.identity.owner,
        category: "agent_role_knowledge",
        storageNeed: "agent_learning",
        sensitivity: "not_sensitive",
        userIntent: "learning_event_approved",
        sourceEvidenceRefs: [
          ...event.evidenceRefs,
          `learning_event:${event.learningEventId}`,
        ],
        retentionPurpose: "approved learning event review persistence",
      },
      rawText: event.afterSummary,
