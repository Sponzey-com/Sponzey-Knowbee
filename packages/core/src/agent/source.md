...
- intake AI 호출의 성공과 실패는 모두 opaque `providerInvocationRef`를 보존합니다. provider contract rejection, transport failure, provider unavailable, deadline, cancellation, invalid response는 raw provider payload 없이 typed reason·retryable·invocation ref의 closed outcome으로 Gateway 복구 경계에 전달합니다.
...
- intake의 `preferred_methods`는 LLM이 현재 capability 계약에서 선택한 exact 실행 방법을 사용자 선호 우선순위로 기록하고, `exclusive_methods`는 사용자가 대안을 금지한 exact 방법만 기록합니다. purpose-specific approval Tool과 non-empty method 제약이 서로 다르거나, privileged `external_action`이 exact method를 선택하지 않으면 typed repair 대상으로 거절합니다. 제약된 메서드가 하나의 명시적이고 활성화된 safe Skill binding에 속할 때만 harness가 의미 추론 없이 그 binding 안의 안전하고 사용 가능한 companion Tool을 같은 실행 범위로 투영하며, moderate-risk Yeonjang 같은 side-effect bundle이나 다른 Skill bundle로는 확장하지 않습니다.
...
- intake의 typed failure도 성공 결과처럼 exact provider invocation ref를 보존합니다. 후속 Gateway 복구는 이 durable LLM receipt ref를 사용하며 provider 오류를 장치·권한 오류로 재해석하지 않습니다.
- intake의 첫 provider 결과가 `retryable`이면 fatal finalization 전에 현재 요청만 포함한
  compact context와 별도 `task_intake_adapter_recovery` invocation으로 다시 분석합니다.
  영구 contract rejection과 cancellation은 반복하지 않으며, 복구 호출도 실패하면 마지막
  typed reason과 invocation ref를 그대로 상위 canonical failure 경계에 반환합니다.
...
- intake 성공과 terminal failure는 모두 해당 observed provider 호출의 opaque `providerInvocationRef`를 보존합니다. 실패 outcome은 raw prompt, provider payload 또는 예외 문구를 포함하지 않고 phase/reason/retryable과 receipt 결속에 필요한 ref만 상위 실행 경계에 전달합니다.
- terminal Tool output은 기본적으로 sanitization을 거칩니다. 단, `screen_permission_denied + rejected + change_strategy`가 Yeonjang side-effect 계약과 정확히 결속되고 Gateway가 고정한 안전 안내문과 일치할 때만 deterministic terminal notice로 유지합니다. 이 예외는 임의 remote output이나 다른 `remote_rejected` 사유를 신뢰하는 통로가 아닙니다.
