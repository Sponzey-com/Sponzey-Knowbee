...
export type SecurityBoundaryKind =
  | "external_input"
  | "network_input"
  | "filesystem_input"
  | "process_input"
...
export type SecurityThreatInventoryDiagnosticCode =
  | "duplicate_surface_id"
  | "required_threat_class_missing"
  | "owner_source_not_declared"
  | "owner_source_missing"
  | "owner_marker_missing"
  | "mitigation_test_not_declared"
  | "mitigation_test_missing"
  | "mitigation_marker_missing"
...
export const DEFAULT_SECURITY_THREAT_SURFACES: readonly SecurityThreatSurface[] = [
  {
...
function verifyEvidenceRefs(input: {
  surfaceId: string
  refs: readonly SecurityThreatEvidenceRef[]
  kind: "owner" | "mitigation"
...
export function auditSecurityThreatInventory(input: {
  ...
  const diagnostics: SecurityThreatInventoryDiagnostic[] = []
  const idCounts = new Map<string, number>()
  const coveredThreatClasses = new Set<SecurityThreatClass>()
  ...
  for (const [surfaceId, count] of idCounts) {
    if (count > 1) diagnostics.push({ code: "duplicate_surface_id", surfaceId, artifactId: null })
  }
  ...
  const incompleteSurfaceIds = new Set(
    diagnostics
