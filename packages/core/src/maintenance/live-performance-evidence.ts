...
export interface LivePerformanceEvidenceRecords {
  run: { status: string; startedAt: number; finishedAt: number }
  llmReceipts: readonly LlmInvocationReceipt[]
  events: ReadonlyArray<{ eventKind: string; payloadBytes: number }>
...
export type LivePerformanceEvidenceReadResult =
  | { status: "ready"; records: LivePerformanceEvidenceRecords }
...
export type CollectLivePerformanceEvidenceResult =
  | { status: "ready"; sample: MeasuredRepresentativeFlowSample }
  | { status: "rejected"; reasonCode: string }
...
export function collectLivePerformanceEvidence(input: {
  ...
  if (!REQUIRED_REPRESENTATIVE_FLOW_IDS.includes(input.flowId as RepresentativeFlowId)) {
  ...
  const orderedTransitions = [...loaded.records.queueTransitions].sort(
    (left, right) => left.sequence - right.sequence,
  )
  const queuedAtByKey = new Map<string, number[]>()
  let queueWaitMs = 0
  for (const transition of orderedTransitions) {
    if (transition.eventKind !== "queued" && transition.eventKind !== "running") continue
  ...
  return {
    ...
    sample: buildMeasuredRepresentativeFlowSample({
      flowId: input.flowId as RepresentativeFlowId,
      ...
      attemptCount: records.events.filter(
        (event) => event.eventKind === "typed_observability:execution_started",
      ).length,
      queueWaitMs,
