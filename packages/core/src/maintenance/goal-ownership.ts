export type GoalOwnershipResponsibilityKind =
  | "document_ownership"
  | "product_behavior"
...
export interface GoalOwnershipDiagnostic {
  code:
    ...
    | "ownership_chapter_missing"
    | "ownership_chapter_kind_mismatch"
    | "ownership_artifact_missing"
    | "ownership_canonical_artifact_repeated_as_reference"
  responsibilityId: string
  chapter: string
  artifact: string
}
...
export type GoalDocumentRuleOwnershipIssueCode =
  | "rule_definition_duplicate"
  | "rule_wrong_owner_chapter"
  | "chapter4_responsibility_leak"
...
export const GOAL_OWNERSHIP_CATALOG: readonly GoalOwnershipEntry[] = [
...
export function auditGoalRuleOwnership(input: {
  ...
  const chapterByKind = new Map<GoalOwnershipResponsibilityKind, string>()
  ...
  const diagnostics: GoalDocumentRuleOwnershipDiagnostic[] = []
  const definitionsByRule = new Map<string, GoalDocumentRuleOccurrence[]>()
  for (const occurrence of input.occurrences) {
    if (occurrence.occurrenceKind !== "definition") continue
  ...
  const add = (
    code: GoalDocumentRuleOwnershipIssueCode,
    occurrence: GoalDocumentRuleOccurrence,
    expectedChapter: string,
  ): void => {
    diagnostics.push({
      code,
...
const EXPECTED_KIND_BY_CHAPTER: Readonly<Record<string, GoalOwnershipResponsibilityKind>> = {
...
function markdownChapters(markdown: string): Set<string> {
  const chapters = new Set<string>()
  for (const line of markdown.split(/\r?\n/u)) {
    const match = line.trim().match(/^#{2,4}\s+(\d+(?:\.\d+)*)\b/u)
    if (match?.[1]) chapters.add(match[1])
  }
  return chapters
}
...
export function auditGoalOwnership(input: {
  ...
  const diagnostics: GoalOwnershipDiagnostic[] = []
...
export function validateGoalOwnershipCatalog(
  catalog: readonly GoalOwnershipEntry[] = GOAL_OWNERSHIP_CATALOG,
): GoalOwnershipAuditResult {
