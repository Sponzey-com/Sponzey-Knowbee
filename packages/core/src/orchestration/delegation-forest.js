...
import { findAgentNameNamespaceConflict, normalizeAgentNameSnapshot, validateAgentRelationship, } from "../contracts/sub-agent-orchestration.js";
...
function assertNoCycles(agentIds, childrenByParent) {
    const visiting = new Set();
    const visited = new Set();
    const visit = (agentId) => {
        if (visiting.has(agentId))
            throw new Error("Delegation forest contains a cycle.");
...
export function validateDelegationForestSnapshot(input) {
    ...
    if (input.agents.length === 0)
        throw new Error("Delegation forest requires at least one agent.");
    ...
    for (const item of agents) {
        if (agentById.has(item.agentId))
            throw new Error(`Agent ID must be unique: ${item.agentId}.`);
    ...
    if (nameConflict)
        throw new Error(`Agent name must be unique: ${nameConflict.normalizedAgentName}.`);
    ...
    for (const relationship of active) {
        ...
        if (edgeIds.has(relationship.edgeId))
            throw new Error(`Delegation forest edge ID is duplicate: ${relationship.edgeId}.`);
        ...
        if (relationshipPairs.has(pair))
            throw new Error("Delegation forest contains a duplicate relationship.");
    ...
    if (parentByChild.has(rootAgentId))
        throw new Error("The main root agent must remain parentless.");
    ...
    const directChildAgentIdsByParent = Object.fromEntries([...childrenByParent.entries()]
        .map(([parentAgentId, children]) => [parentAgentId, [...new Set(children)].sort()])
        .sort(([left], [right]) => left.localeCompare(right)));
    const relationships = active.map((relationship) => structuredClone(relationship)).sort((left, right) => left.parentAgentId.localeCompare(right.parentAgentId)
...
export function authorizeDelegationInForest(input) {
    ...
    if (!caller)
        return { ok: false, reasonCode: "caller_unknown" };
    if (!target)
        return { ok: false, reasonCode: "target_unknown" };
    if (caller.status !== "enabled")
        return { ok: false, reasonCode: "caller_inactive" };
    if (target.status !== "enabled")
        return { ok: false, reasonCode: "target_inactive" };
    const directChildren = input.snapshot.directChildAgentIdsByParent[caller.agentId] ?? [];
    if (!directChildren.includes(target.agentId))
        return { ok: false, reasonCode: "target_not_direct_child" };
    if (caller.agentType === "sub_agent") {
        const policy = caller.delegationPolicy;
        if (!policy?.enabled)
            return { ok: false, reasonCode: "delegation_disabled" };
        if (policy.redelegationAllowed !== true)
            return { ok: false, reasonCode: "redelegation_denied" };
        if (policy.directChildOnly !== true)
            return { ok: false, reasonCode: "direct_child_policy_required" };
...
//# sourceMappingURL=delegation-forest.js.map