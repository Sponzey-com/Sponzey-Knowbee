...
function executionHarnessFallbackText(key: string): string {
  ...
  if (!value) throw new Error(`execution harness fallback text missing: ${key}`)
  return value
}

export function executionHarnessPolicyContextLabel(key: string): string {
...
function executionHarnessErrorDetail(error: unknown, fallback: string): string {
...
export function buildAgentExecutionDecisionPrompt(
  ...
  return loadPromptTemplate({
    ...
    variables: {
      policyBlock,
      allowedActions: AGENT_EXECUTION_DECISION_V2_ACTIONS.join(", "),
      contextJson,
    },
  })
}
...
function isSubAgentDelegationRoute(route: AgentExecutionRoute): boolean {
  return route === "delegate_to_child" || route === "sub_agent"
}
...
function fallbackReasonForValidationStatus(
  ...
  if (isRootCurrentExecutor(context)) return AgentExecutionFallbackReason.RootKnowbeeDirect
  return context.requester?.requester_type === "executor"
    ? AgentExecutionFallbackReason.AskParent
    : AgentExecutionFallbackReason.AskUser
}
