...
export {
  ...
  WEB_EVIDENCE_PIPELINE_PROMPT_SOURCE_IDS,
} from "../ai/web-evidence-pipeline-factory.js"
export type {
  FileBackedWebEvidencePipelineAdapterInput,
} from "../ai/web-evidence-pipeline-factory.js"
...
export type AgentPromptStageId =
  | (typeof CANONICAL_AGENT_PROMPT_SOURCE_IDS)[number]
  | "sub_agent_base"
  | "agent_persona"
  | "work_handoff"
...
export type AgentPromptSourceCompositionIssueCode =
  | `source_missing:${string}`
  | `source_duplicate:${string}`
...
export function buildAgentPromptStageIds(input: AgentPromptStageInput): AgentPromptStageId[] {
  return [
    ...CANONICAL_AGENT_PROMPT_SOURCE_IDS.slice(0, -2),
    ...(input.agentType === "sub_agent" ? ["sub_agent_base" as const] : []),
    ...(input.agentType === "sub_agent" && input.hasExplicitUserTraits
      ? ["agent_persona" as const]
      : []),
    "work_handoff",
...
export function auditAgentPromptSourceComposition(
  input: AgentPromptSourceCompositionInput,
): AgentPromptSourceCompositionAudit {
  const stageIds = buildAgentPromptStageIds(input)
  const issueCodes: AgentPromptSourceCompositionIssueCode[] = []
  for (const sourceId of stageIds) {
    if (sourceId === "work_handoff") continue
    const eligibleCount = input.sources.filter(
      (candidate) =>
    ...
    if (eligibleCount === 0) issueCodes.push(`source_missing:${sourceId}`)
    if (eligibleCount > 1) issueCodes.push(`source_duplicate:${sourceId}`)
  }
  return {
...
export function composeAgentPromptSources(
  input: AgentPromptSourceCompositionInput,
): LoadedPromptSource[] {
  const orderedIds = buildAgentPromptStageIds(input).filter(
    (sourceId) => sourceId !== "work_handoff",
  )
...
export const DIAGNOSIS_SUPPORT_SOURCE_IDS = ["work_record"] as const
...
export type RuntimePromptSourceId =
  ...
  | (typeof SUB_AGENT_PROMPT_BUNDLE_SOURCE_IDS)[number]
...
export interface FileBackedDiagnosisProviderInput {
  ...
  observabilityContext?: Pick<
    NonNullable<import("../ai/types.js").ChatParams["observability"]>,
...
export const SOLUTION_PLAN_PROMPT_SOURCE_IDS = ["work_record", "workflow"] as const
export const CAPABILITY_SELECTION_PROMPT_SOURCE_IDS = ["capability_selection"] as const
export const WEB_RESEARCH_METHOD_PROMPT_SOURCE_IDS = ["web_research_method"] as const

export function selectRuntimePromptSources(
  input: PromptSourceSelectionInput,
): LoadedPromptSource[] {
...
export function selectAgentPromptBundleSources(
  input: PromptSourceSelectionInput,
): LoadedPromptSource[] {
...
export function selectExecutionHarnessPolicySources(
  input: PromptSourceSelectionInput,
): LoadedPromptSource[] {
  return selectRuntimePromptSources({
    ...input,
...
export function selectDiagnosisPromptSources(
  input: PromptSourceSelectionInput,
): LoadedPromptSource[] {
...
export function selectSolutionPlanPromptSources(
  input: PromptSourceSelectionInput,
): LoadedPromptSource[] {
  return selectRuntimePromptSources({ ...input, sourceIds: SOLUTION_PLAN_PROMPT_SOURCE_IDS })
}

export function selectCapabilitySelectionPromptSources(
  input: PromptSourceSelectionInput,
): LoadedPromptSource[] {
  return input.sources
    .filter((source) => source.locale === "en")
    ...
    .filter((source) => source.enabled)
    .filter((source) =>
      CAPABILITY_SELECTION_PROMPT_SOURCE_IDS.includes(
        source.sourceId as (typeof CAPABILITY_SELECTION_PROMPT_SOURCE_IDS)[number],
      ),
    )
    .sort((left, right) => left.priority - right.priority)
}

export function selectWebResearchMethodPromptSources(
  input: PromptSourceSelectionInput,
): LoadedPromptSource[] {
  return input.sources
    .filter((source) => source.locale === "en")
    ...
    .filter((source) => source.enabled)
    .filter((source) =>
      WEB_RESEARCH_METHOD_PROMPT_SOURCE_IDS.includes(
        source.sourceId as (typeof WEB_RESEARCH_METHOD_PROMPT_SOURCE_IDS)[number],
      ),
    )
    .sort((left, right) => left.priority - right.priority)
}

export function loadRuntimePromptPolicySources(
  input: PromptSourceLoadInput = {},
): LoadedPromptSource[] {
  try {
...
export function renderDiagnosisPromptSourceBlock(input: PromptPolicyBlockInput): string {
...
export function createFileBackedDiagnosisProvider(
  input: FileBackedDiagnosisProviderInput,
): AiChatDiagnosisProviderAdapter {
...
export function createFileBackedSolutionPlanProvider(
  input: FileBackedSolutionPlanProviderInput,
): AiChatSolutionPlanProviderAdapter {
  const sources = loadPromptSourceRegistry(input.workDir)
...
export function createFileBackedCapabilitySelectionProvider(
  input: FileBackedCapabilitySelectionProviderInput,
): AiChatCapabilitySelectionProviderAdapter {
  const sources = loadPromptSourceRegistry(input.workDir)
  const selected = selectCapabilitySelectionPromptSources({ sources, locale: "en" })
  const selectedIds = new Set(selected.map((source) => source.sourceId))
  const missing = CAPABILITY_SELECTION_PROMPT_SOURCE_IDS.filter(
...
export function createFileBackedWebResearchMethodProvider(
  ...
  const selectedIds = new Set(selected.map((source) => source.sourceId))
  const missing = WEB_RESEARCH_METHOD_PROMPT_SOURCE_IDS.filter(
    (sourceId) => !selectedIds.has(sourceId),
  )
  if (missing.length > 0) {
