...
export type ParentResultDisposition =
  | { outcome: "accept"; reviewId: string; sourceResultReportId: string }
  | { outcome: "correct"; correction: ParentCorrectionPackage }
...
export type RedelegationAuthorizationDecision =
  ...
  | { ok: true; reasonCode: RedelegationReasonCode; authorizationReceiptId: `redelegation:${string}`; authorizationFingerprint: `sha256:${string}` }
...
export function fingerprintStructuredTaskScope(scope: StructuredTaskScope): `sha256:${string}` {
  return fingerprint(scope)
}

export function isRedelegationReasonCode(value: string): value is RedelegationReasonCode {
...
export function buildParentResultDisposition(input: {
  ...
  if (input.review.verdict === "accept") return { outcome: "accept", reviewId, sourceResultReportId }
  ...
  if (missingItems.length === 0 && requiredChanges.length === 0) throw new Error("A correction package requires missing items or required changes.")
  const preservedEvidenceRefs = unique(input.preservedEvidenceRefs, "Preserved evidence reference")
  if (preservedEvidenceRefs.some((value) => !TYPED_REFERENCE.test(value))) throw new Error("Preserved evidence must use typed references.")
  const correctedScope = structuredClone(input.correctedScope)
  const base = { reviewId, sourceResultReportId, verdict: input.review.verdict as ParentCorrectionPackage["verdict"], missingItems, requiredChanges, preservedEvidenceRefs, correctedScope, correctedScopeFingerprint: fingerprintStructuredTaskScope(correctedScope) }
  return { outcome: "correct", correction: { ...base, correctionFingerprint: fingerprint(base) } }
}

function reasonMatchesReview(reasonCode: RedelegationReasonCode, correction: ParentCorrectionPackage, evidenceRefs: string[]): boolean {
  if (reasonCode === "missing_evidence") return correction.verdict === "insufficient_evidence"
  if (reasonCode === "low_quality") return correction.verdict === "needs_revision" || correction.verdict === "reject"
  if (reasonCode === "incomplete_scope") return correction.verdict === "needs_revision" || correction.verdict === "limited_success"
  if (reasonCode === "execution_failure") return correction.verdict === "reject"
  if (reasonCode === "permission_boundary") return evidenceRefs.some((ref) => ref.startsWith("evidence:permission:"))
  return evidenceRefs.some((ref) => ref.startsWith("evidence:user-goal-changed:"))
}

export function authorizeEvidenceBackedRedelegation(input: RedelegationAuthorizationInput): RedelegationAuthorizationDecision {
  ...
  if (parentAgentName === nextTargetAgentName) return { ok: false, reasonCode: "self_redelegation_denied" }
  const evidenceRefs = unique(input.reasonEvidenceRefs, "Redelegation evidence reference")
  if (evidenceRefs.length === 0 || evidenceRefs.some((ref) => !TYPED_REFERENCE.test(ref))) return { ok: false, reasonCode: "redelegation_evidence_invalid" }
  if (!reasonMatchesReview(input.reasonCode, input.correction, evidenceRefs)) return { ok: false, reasonCode: "redelegation_reason_review_mismatch" }
  if (originalScopeFingerprint === input.correction.correctedScopeFingerprint) return { ok: false, reasonCode: "redelegation_scope_unchanged" }
  if (
    input.previousStrategyFingerprint &&
