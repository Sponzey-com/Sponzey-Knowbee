...
function outputsFor(unitOutputs, decisionOutputs, successCriteria) {
...
function validateStepOutputs(scopes) {
    const reasons = [];
    for (const scope of scopes) {
        const outputIds = new Set();
        for (const output of scope.expectedOutputs) {
            const outputId = output.outputId.trim();
            if (!outputId || !output.description.trim())
                reasons.push("workflow_output_invalid");
            if (outputId && outputIds.has(outputId))
                reasons.push("workflow_output_id_duplicate");
            if (outputId)
...
function topologicalOrder(nodeCount, dependencies) {
...
export function authorWorkflowFromExecutionDecision(decision) {
    if (!decision || decision.task_profile.task_units.length <= 1)
        return undefined;
    const units = decision.task_profile.task_units;
    const successCriteria = unique(decision.task_profile.success_criteria);
    const reasons = [];
    if (successCriteria.length === 0)
        reasons.push("completion_contract_missing");
    const indexById = new Map();
    units.forEach((unit, index) => {
        const id = unit.id.trim();
        if (!id || indexById.has(id))
            reasons.push("workflow_unit_id_invalid");
        else
    ...
    units.forEach((unit, toScopeIndex) => {
        for (const dependencyId of unique(unit.depends_on_unit_ids ?? [])) {
            ...
            if (fromScopeIndex === undefined)
                reasons.push("dependency_missing");
            else
                dependencies.push({ fromScopeIndex, toScopeIndex, reasonCode: "workflow_unit_dependency" });
        }
    });
    const order = topologicalOrder(units.length, dependencies);
    if (!order)
        reasons.push("dependency_cycle");
    const sourceTaskScopes = units.map((unit) => ({
        goal: unit.goal.trim(),
    ...
    if (sourceTaskScopes.some((scope) => !scope.goal || !scope.actionType))
        reasons.push("workflow_unit_invalid");
    if (sourceTaskScopes.some((scope) => scope.expectedOutputs.length === 0))
        reasons.push("completion_contract_missing");
...
//# sourceMappingURL=workflow-authoring.js.map