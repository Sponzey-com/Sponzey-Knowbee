...
export function buildNodeDefinitionLlmNotConfiguredResult(): NodeDefinitionSuggestionErrorResponse {
  return {
    ok: false,
...
export async function createNodeDefinitionSuggestion(
  ...
  const prompt = await buildNodeDefinitionPrompt(redacted.request)
...
async function buildNodeDefinitionPrompt(request: NodeDefinitionSuggestionRequest): Promise<string> {
  const { loadPromptTemplate } = await import("../memory/knowbee-md.js")
  return loadPromptTemplate({
    sourceId: "node_definition_suggestion",
    variables: {
      forbiddenInternalTerms: INTERNAL_TERMS.join(", "),
      inputBlock: buildNodeDefinitionPromptInput(request),
    },
  })
}
...
function normalizeAlternativePatch(
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (isEmptyImportantPatch(key, cleaned)) continue
    ;(output as Record<string, unknown>)[key] = cleaned
  }
  return output
}
...
function cleanPatchFieldValue(
  value: string | string[],
  warnings: NodeDefinitionSuggestionWarning[],
): string | string[] {
  if (Array.isArray(value)) return value.map((item) => cleanUserFacingText(item, warnings))
...
function suggestionErrorMessage(error: unknown): string {
  if (error instanceof Error && error.message.trim()) return suggestionErrorUserMessage(error)
  return "AI provider 응답을 처리할 수 없습니다."
}

function suggestionErrorUserMessage(error: Error): string {
