...
- `packages/core/src/topology`는 토폴로지 저장, 변환, 검증, 서브 에이전트 그래프 projection을 다루는 영역이다.

## 현재 source-of-truth 기준

- 기본 제품 모델은 서브 에이전트와 위임 연결선이다.
- `ExecutorGraph`와 `ExecutorTopologyV2` 계열은 신규 source-of-truth 후보이다.
- 서브 에이전트의 핵심 저장 필드는 내부 id, 사용자-facing `agent_name`, 역할명, 성격과 하는 일, 위치, 상태이다.
- 연결선의 핵심 저장 필드는 source sub-agent, target sub-agent, `delegates_to` 관계, 상태이다.
...
- 신규 저장과 실행 결정은 V2 서브 에이전트 그래프를 기준으로 해야 한다.
- V1이 필요하면 `legacy-enterprise-topology-adapter.ts` 같은 compatibility adapter 또는 diagnostic 경로로 격리한다.
- 런타임 실행 그래프가 V1 envelope를 읽어야 할 때도 먼저 V2 서브 에이전트 read model로 투영한 뒤 사용한다.
- compatibility adapter는 기존 row/fixture의 endpoint 오류나 `children` 불일치 같은 이관 진단만 보존하고, V1 조직/도구/권한 모델을 실행 판단 기준으로 되살리지 않는다.

## 자연어 판단 경계

- topology 코드가 사용자 원문이나 서브 에이전트 설명을 키워드/regex로 읽어 서브 에이전트를 선택하면 안 된다.
- 의미 판단은 prompt/harness가 구조화된 direct child 후보와 서브 에이전트 정의를 보고 수행한다.
...
- 서브 에이전트 그래프 런타임 경로는 `pnpm run test:architecture:runtime`으로 확인한다.
