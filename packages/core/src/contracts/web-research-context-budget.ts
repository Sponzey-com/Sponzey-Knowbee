...
export type WebResearchContextBudgetResult =
  | Readonly<{ ok: true; value: WebResearchContextBudget }>
  | Readonly<{
      ok: false
      reasonCode:
        | "context_budget_input_invalid"
        | "context_budget_allocation_invalid"
        | "context_budget_web_cap_exceeded"
        | "context_budget_answer_reserve_too_small"
        | "context_budget_total_exceeded"
        | "context_budget_estimator_invalid"
        | "context_budget_input_exceeds_allocation"
    }>
...
function failure(
  reasonCode: Exclude<WebResearchContextBudgetResult, { ok: true }>["reasonCode"],
): WebResearchContextBudgetResult {
  return Object.freeze({ ok: false, reasonCode })
}

function defaultAllocations(
  modelContextTokens: number,
  measuredSystemToolTokens: number,
  measuredConversationTokens: number,
): WebResearchContextAllocations {
  ...
  const availableForWeb = Math.max(
    0,
    modelContextTokens
      - systemToolTokens
      - conversationTokens
...
export function createWebResearchContextBudget(
  input: CreateWebResearchContextBudgetInput,
  ...
  if (
    !isNonNegativeInteger(systemToolTokens) ||
  ...
  if (
    systemToolTokens > allocations.systemToolTokens ||
