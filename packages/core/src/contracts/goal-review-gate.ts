...
export type GoalReviewGateIssueCode =
  | "review_category_missing"
  | "review_gate_missing"
  | "review_gate_failed"
  | "review_evidence_missing"
  | "review_gate_duplicate"
...
function hasEvidence(item: GoalReviewGateItem): boolean {
...
export function validateGoalReviewGateReport(
  report: Partial<GoalReviewGateReport>,
): GoalReviewGateValidationResult {
  const issues: GoalReviewGateIssue[] = []

  for (const category of Object.keys(GOAL_REVIEW_GATE_REQUIRED_KEYS) as GoalReviewGateCategory[]) {
    ...
    if (!Array.isArray(items)) {
      addIssue(
        ...
        category,
        category,
        `${category} review category is required.`,
      )
      continue
    }

    const seen = new Set<string>()
    const byKey = new Map<string, GoalReviewGateItem>()
    for (const item of items) {
      if (seen.has(item.key)) {
        addIssue(
          ...
          category,
          `${category}.${item.key}`,
          `${item.key} review gate is duplicated.`,
          item.key,
        )
      }
      seen.add(item.key)
    ...
    for (const requiredKey of GOAL_REVIEW_GATE_REQUIRED_KEYS[category]) {
      ...
      if (!item) {
        addIssue(
          ...
          `${requiredKey} review gate is required.`,
          requiredKey,
        )
        continue
      }
      if (!item.passed) {
        addIssue(
          ...
          category,
          `${category}.${requiredKey}`,
          `${requiredKey} review gate did not pass.`,
          requiredKey,
        )
      }
      if (!hasEvidence(item)) {
        addIssue(
          ...
          "review_evidence_missing",
          category,
          `${category}.${requiredKey}.evidenceRefs`,
          `${requiredKey} review gate requires evidence.`,
          requiredKey,
        )
      }
    }
