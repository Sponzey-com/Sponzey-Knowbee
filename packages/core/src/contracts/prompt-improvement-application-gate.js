...
function validateSummary(summary, proposalFingerprint) {
...
export function authorizePromptImprovementApplication(input) {
    ...
    if (input.sourceInputs.length === 0)
        return { status: "blocked", reasonCode: "prompt_source_missing" };
    ...
    if (new Set(sourceRefs).size !== sourceRefs.length)
        throw new Error("Prompt source references must be unique.");
    ...
    for (const review of input.invariantReviews) {
        if (reviews.has(review.invariant))
    ...
    if (!summary)
        return { status: "blocked", reasonCode: "behavior_summary_missing" };
    if (!validateSummary(summary, proposalFingerprint))
        return { status: "blocked", reasonCode: "behavior_summary_invalid" };
    const confirmation = input.confirmation;
    if (!confirmation)
        return { status: "blocked", reasonCode: "confirmation_missing" };
    if (confirmation.schemaVersion !== 1)
        throw new Error("Unsupported prompt behavior confirmation schema version.");
    required(confirmation.confirmationId, "Confirmation ID");
    if (confirmation.decision !== "confirmed")
        return { status: "blocked", reasonCode: "confirmation_denied" };
    timestamp(confirmation.confirmedAt, "Confirmation time");
    timestamp(confirmation.expiresAt, "Confirmation expiry");
    if (confirmation.confirmedAt > now || confirmation.expiresAt <= now)
        return { status: "blocked", reasonCode: "confirmation_expired" };
...
export async function applyConfirmedPromptImprovement(input) {
    if (input.decision.status !== "authorized")
        return input.decision;
    return { status: "applied", result: await input.apply(input.decision) };
}
//# sourceMappingURL=prompt-improvement-application-gate.js.map