...
export const PROMPT_MEMORY_EXCHANGE_METHODS = ["message_payload", "approved_handoff_package"] as const
...
export type PromptMemoryExchangeDecision =
  | { status: "verified"; exchangeId: string; method: PromptMemoryExchangeMethod; sourceAgentId: string; targetAgentId: string; payloadFingerprint: string }
  | { status: "blocked"; reasonCode: "exchange_receipt_invalid" | "exchange_method_invalid" | "exchange_owner_same" | "message_evidence_missing" | "handoff_approval_missing" | "handoff_receipt_missing" | "handoff_not_eligible" | "handoff_scope_mismatch" }
...
export type PromptImprovementMemoryInvariantDecision =
  | { status: "authorized"; receipt: PromptImprovementMemoryInvariantReceipt }
  | { status: "blocked"; reasonCode:
      | "memory_ownership_incomplete"
      | "memory_namespace_receipt_invalid"
      | "memory_namespace_mixed"
      | "memory_namespace_coverage_incomplete"
      | "memory_exchange_invalid"
      | "compaction_preservation_incomplete"
      | "long_term_policy_incomplete"
      | "long_term_mutation_ineligible"
      | "memory_review_lineage_invalid" }

export type MemoryIsolationInvariantProjectionDecision =
  | { status: "authorized"; review: PlatformPromptInvariantReview }
  | { status: "blocked"; reasonCode: "memory_review_receipt_invalid" | "memory_review_expired" | "memory_review_scope_mismatch" | "goal_section3_lineage_mismatch" }
...
function unique(values: string[]): string[] | undefined {
  ...
  if (normalized.length !== values.length || new Set(normalized).size !== normalized.length) return undefined
  return normalized
}

export function evaluatePromptMemoryExchangeReceipt(receipt: PromptMemoryExchangeReceipt): PromptMemoryExchangeDecision {
  ...
  if (!(PROMPT_MEMORY_EXCHANGE_METHODS as readonly string[]).includes(receipt.method)) {
    return { status: "blocked", reasonCode: "exchange_method_invalid" }
  }
  if (sourceAgentId === targetAgentId) return { status: "blocked", reasonCode: "exchange_owner_same" }
  if (receipt.method === "message_payload") {
    if (!exact(receipt.messageEvidenceRef)) return { status: "blocked", reasonCode: "message_evidence_missing" }
  } else {
    if (!exact(receipt.approvalRef)) return { status: "blocked", reasonCode: "handoff_approval_missing" }
    if (!receipt.handoff || !receipt.handoffDecision) return { status: "blocked", reasonCode: "handoff_receipt_missing" }
    if (receipt.handoffDecision.status !== "eligible") return { status: "blocked", reasonCode: "handoff_not_eligible" }
...
function verifyNamespaceSeparation(
  receipt: MemoryNamespaceSeparationReceipt,
  activeAgentIds: string[],
): "verified" | "invalid" | "mixed" | "incomplete" {
  ...
  if (receipt.schemaVersion !== 1 || receipt.status !== "verified" || !exact(receipt.evidenceRef)
    || !agentNamespaces || agentNamespaces.length === 0 || !userNamespaces) return "invalid"
  const userSet = new Set(userNamespaces)
  if (agentNamespaces.some((namespaceId) => userSet.has(namespaceId))) return "mixed"
  if (agentNamespaces.length !== activeAgentIds.length * 3
    || activeAgentIds.some((agentId) => ["short_term", "long_term", "history"]
      .some((kind) => !agentNamespaces.includes(`${agentId}:${kind}`)))) return "incomplete"
  return "verified"
}
...
export function authorizePromptImprovementMemoryInvariant(input: {
  ...
  if (namespaceStatus === "invalid") return { status: "blocked", reasonCode: "memory_namespace_receipt_invalid" }
  if (namespaceStatus === "mixed") return { status: "blocked", reasonCode: "memory_namespace_mixed" }
  if (namespaceStatus === "incomplete") return { status: "blocked", reasonCode: "memory_namespace_coverage_incomplete" }
...
export function projectMemoryIsolationInvariantReview(input: {
  ...
  if (receipt.expiresAt <= input.now) return { status: "blocked", reasonCode: "memory_review_expired" }
