...
export type RecursiveHarnessAddendumIssueCode =
  | "addendum_header_missing"
  | "addendum_header_duplicate"
  | "addendum_sentence_missing"
  | "addendum_sentence_duplicate"

export interface RecursiveHarnessAddendumIssue {
  code: RecursiveHarnessAddendumIssueCode
  sentence?: string
  occurrences: number
}

export interface RecursiveHarnessAddendumAudit {
  status: "valid" | "invalid"
  sentenceCount: number
  issues: RecursiveHarnessAddendumIssue[]
}

const ADDENDUM_HEADER = "## Harness System Prompt Addendum"

function literalOccurrences(content: string, value: string): number {
  let count = 0
  let offset = 0
...
export function auditRecursiveHarnessAddendum(content: string): RecursiveHarnessAddendumAudit {
  const issues: RecursiveHarnessAddendumIssue[] = []
