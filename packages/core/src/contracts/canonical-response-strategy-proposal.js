...
export function buildCanonicalResponseStrategyProposal(input) {
    if (input.intake.schemaVersion !== 1)
        throw new Error("Unsupported response-strategy intake schema version.");
    ...
    if (input.validationCriteria.length === 0)
        throw new Error("At least one validation criterion is required.");
    ...
    return {
        ...
        proposal: {
            ...
            harnessProjection: {
                ...
                requiredTests: validationCriteria.map((criterion) => `${criterion.inputCondition} => ${criterion.expectedResult}; pass=${criterion.passCondition}`),
            },
        },
    };
}
//# sourceMappingURL=canonical-response-strategy-proposal.js.map