...
import { validateRecoveryCandidateAgainstFailure, } from "./work-record.js";
const TRANSITIONS = {
...
function requireTextList(values, field) {
    if (!values?.length)
        throw new Error(`${field} requires at least one evidence value.`);
...
function validateFailure(failure) {
    ...
    if (!failure.recoverable)
        throw new Error("Failure diagnosis must be recoverable before a recovery action is selected.");
}
function validateStop(input, receiptId) {
    const stop = input.stop;
    if (!stop)
        throw new Error("A structured stop condition is required before recovery can stop.");
    ...
    if (stop.condition === "goal_achieved") {
        ...
        if (stop.unresolvedScope.length > 0)
            throw new Error("Goal-achieved recovery cannot retain unresolved scope.");
        return {
            state: "stopped",
            outcome: "completed",
            receiptId,
    ...
    if (stop.condition === "alternatives_exhausted" && input.recoveryCandidates.length > 0) {
        for (let index = 0; index < input.recoveryCandidates.length; index += 1) {
            ...
            if (!review)
                throw new Error("alternatives_exhausted requires a constraint review for every recovery candidate.");
...
function validatePartialReport(input, receiptId) {
    const report = input.partialReport;
    if (!report)
        throw new Error("A structured partial report is required for partial_report.");
    return {
        state: "report_ready",
        outcome: "partial",
        receiptId,
...
function validateRetry(input, receiptId) {
    ...
    if (!selected)
        throw new Error("Selected recovery candidate is not in recovery_candidates.");
    ...
    if (!validation.ok)
        throw new Error(`Recovery candidate must have a valid changed dimension: ${validation.issues[0]?.message ?? "invalid candidate"}`);
    const review = input.constraintReviews.find((candidate) => candidate.candidateIndex === selectedIndex);
    if (!review)
        throw new Error("Selected recovery candidate requires a constraint review.");
...
export function decideStructuredFailureRecovery(input) {
    ...
    const terminalEvent = decision.state === "retry_ready"
        ? "retry_selected"
        : decision.state === "report_ready"
            ? "partial_report_selected"
            : "stop_selected";
    const stateTrace = ["diagnosing"];
    stateTrace.push(transitionFailureRecovery(stateTrace.at(-1), "diagnosis_recorded"));
    stateTrace.push(transitionFailureRecovery(stateTrace.at(-1), "candidates_generated"));
    stateTrace.push(transitionFailureRecovery(stateTrace.at(-1), "constraints_reviewed"));
    stateTrace.push(transitionFailureRecovery(stateTrace.at(-1), terminalEvent));
    return { ...decision, stateTrace };
}
export function transitionFailureRecovery(state, event) {
    if (state === "retry_ready" || state === "report_ready" || state === "stopped") {
        throw new Error(`Failure recovery state ${state} is terminal.`);
    }
    const next = TRANSITIONS[state]?.[event];
    if (!next)
        throw new Error(`Invalid failure recovery transition: ${state} + ${event}.`);
    return next;
}
//# sourceMappingURL=failure-recovery-decision.js.map