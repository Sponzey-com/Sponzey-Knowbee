...
export interface BlockedUserReport {
  ...
  directCause: { text: string; evidenceSummaries: string[] }
...
export type UserResponseAction =
  | { status: "continue_now"; candidateId: string }
  | { status: "request_user_input" }
  | { status: "report_blocked" }
  | { status: "reassess" }

const INTERNAL_DETAIL =
  /(?:\b(?:work|receipt|diagnosis|prompt|request_id|work_id|attempt_id):|[{}[\]])/iu
...
export function buildBlockedUserReport(input: {
  ...
  if (
    new Set(attemptedPaths.map((path) => path.strategyFingerprint)).size !== attemptedPaths.length
  ) {
...
export function decideUserResponseAction(input: {
  ...
  if (input.clarificationRequired) return { status: "request_user_input" }
  if (input.exhaustionAuthorized) return { status: "report_blocked" }
