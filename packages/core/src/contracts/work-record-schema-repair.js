...
function validateCandidateIdentity(candidate, baseline) {
    const issues = [];
    if (candidate.work_id !== baseline.work_id)
        issues.push(issue("$.work_id", "Repaired work_id must match the baseline."));
    if (candidate.parent_work_id !== baseline.parent_work_id)
        issues.push(issue("$.parent_work_id", "Repaired parent_work_id must match the baseline."));
    if (candidate.owner_agent_name !== baseline.owner_agent_name)
        issues.push(issue("$.owner_agent_name", "Repaired owner_agent_name must match the baseline."));
    if (candidate.source !== baseline.source)
        issues.push(issue("$.source", "Repaired source must match the baseline."));
    return issues;
}
...
function blockedRecord(baseline, failedStepId, validationIssues) {
...
export async function resolveWorkRecordWithOneShotRepair(input) {
    const baselineValidation = validateWorkRecord(input.baseline);
    if (!baselineValidation.ok)
        throw new Error("Baseline WorkRecord must pass canonical validation before repair.");
    ...
    if (initial.ok)
        return { status: "valid", repairAttempted: false, record: initial.value };
    let repaired;
    ...
    const repairedValidation = providerFailed
        ? { ok: false, issues: [issue("$", "WorkRecord schema repair provider failed.")] }
...
//# sourceMappingURL=work-record-schema-repair.js.map