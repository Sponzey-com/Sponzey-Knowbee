...
export const STRUCTURED_WORK_TEXT_LIMIT = 500 as const
...
function requireTextList(values: string[], field: string): string[] {
...
function validateDiagnosisText(
  request: LlmRequestDiagnosisRecord,
  result: LlmResultDiagnosisRecord,
): void {
...
function validateFailureBundleText(bundle: WorkRecordFailureBundle): void {
  ...
  for (const candidate of bundle.recoveryCandidates) {
    ...
    if (candidate.required_permission !== undefined) requireText(candidate.required_permission, "Recovery required permission")
  }
}

function statusForStepResult(result: WorkStepResult): WorkStepPlanItem["status"] {
...
function assembleSteps(
  plan: StructuredWorkPlanDecision,
  results: WorkStepResult[],
): { steps: WorkStepPlanItem[]; results: WorkStepResult[] } {
  if (results.length !== plan.steps.length) throw new Error("Work record requires one result for every planned step.")
  const planned = new Map(plan.steps.map((step) => [step.step_id, step]))
  const seen = new Set<string>()
  for (const result of results) {
    if (!planned.has(result.step_id)) throw new Error(`Step result references unknown planned step ${result.step_id}.`)
    if (seen.has(result.step_id)) throw new Error("Step result references must be unique.")
    seen.add(result.step_id)
    if (result.output_ref !== undefined) requireText(result.output_ref, "Step output reference")
    requireTextList(result.evidence_refs, "Step evidence references")
    if (result.error !== undefined) requireText(result.error, "Step error summary")
  }
  return {
    steps: plan.steps.map((step) => {
      ...
      return { ...step, input_refs: [...step.input_refs], status: statusForStepResult(results.find((result) => result.step_id === step.step_id) as WorkStepResult) }
    }),
    results: results.map((result) => ({ ...result, evidence_refs: [...result.evidence_refs] })),
  }
}

function requireFailureBundle(
  status: WorkRecordStatus,
  action: ActionDecision["selected_action"],
  bundle: WorkRecordFailureBundle | undefined,
): void {
  if (status !== "failed" && status !== "partial") return
  if (!bundle) throw new Error(`Failure bundle is required for ${status} work.`)
  if (bundle.recoveryCandidates.length === 0) throw new Error("Failure bundle requires at least one recovery candidate.")
...
export function assembleCanonicalWorkRecord(input: AssembleCanonicalWorkRecordInput): WorkRecord {
  ...
  if (input.failureBundle) validateFailureBundleText(input.failureBundle)
