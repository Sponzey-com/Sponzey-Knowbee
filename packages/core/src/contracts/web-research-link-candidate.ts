import type {
  WebResearchFetchCandidateDiscovery,
  WebResearchFingerprint,
  WebResearchFingerprintPort,
...
export type WebResearchLinkTargetAdmission =
  | Readonly<{
      observedUrl: string
      status: "allowed"
      canonicalUrl: string
...
export type WebResearchLinkCandidate = Extract<
  WebResearchMethodCandidate,
  { kind: "fetch" }
> & {
...
function text(value: unknown, maxLength = 8_192): string | null {
  if (typeof value !== "string") return null
  const normalized = value.trim()
  return normalized.length > 0 && normalized.length <= maxLength ? normalized : null
}

function createCheckedFingerprint(
...
export function projectWebResearchLinkCandidates(
  input: {
    runId: string
  ...
  createFingerprint: WebResearchFingerprintPort,
): WebResearchLinkCandidateProjection {
  ...
  const candidates: WebResearchLinkCandidate[] = []
  const exclusions: WebResearchLinkCandidateExclusion[] = []
  const canonicalUrls = new Set<string>()
  for (const observation of [...input.observations].sort(
    (left, right) => left.ordinal - right.ordinal,
  )) {
    const admissions = input.targetAdmissions.filter(
      (admission) => admission.observedUrl === observation.url,
    )
    ...
    const discoveryFingerprint = createCheckedFingerprint(
      createFingerprint,
      "web-research-link-discovery:v1",
      discoveryPayload,
    )
    const strategyFingerprint = createCheckedFingerprint(
      createFingerprint,
      "web-research-link-strategy:v1",
      {
        runId,
        parentEvidenceRef,
        canonicalUrl,
    ...
    candidates.push(
      Object.freeze({
        ...
        discovery: Object.freeze({
          origin: "fetched_document_link" as const,
          parentEvidenceRef,
          parentProvenanceRef,
          documentFinalUrl,
          observationOrdinal: observation.ordinal,
