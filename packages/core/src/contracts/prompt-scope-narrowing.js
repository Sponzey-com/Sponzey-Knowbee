...
export function evaluatePromptScopeNarrowing(input) {
    ...
    const issues = [];
    const add = (code, subjectId, dimension) => {
    ...
    for (const rule of input.rules) {
        ...
        if (rule.kind !== "reference" && !owned.has(responsibilityId))
            add("module_rule_out_of_scope", ruleKey);
        if (rule.kind !== "reference" && canonicalOwnerModuleId !== moduleId)
            add("module_rule_owner_mismatch", ruleKey);
    }
    for (const receipt of input.consolidations) {
        ...
        if (receipt.activeDefinitionModuleIds.length !== 1 || receipt.activeDefinitionModuleIds[0] !== owner)
            add("consolidation_definition_count_invalid", key);
        if (receipt.unresolvedConflictModuleIds.length > 0)
            add("consolidation_conflict_unresolved", key);
        ...
        if ([...removed].some((moduleId) => !updated.has(moduleId)))
            add("consolidation_reference_update_missing", key);
    }
    ...
    if (confidence(baseline.parserConfidence, "Baseline parser confidence") < input.minimumParserConfidence ||
    ...
    if (!Number.isFinite(baseline.conditionStrictness) || !Number.isFinite(proposed.conditionStrictness))
        throw new Error("Condition strictness must be finite.");
    if (proposed.conditionStrictness < baseline.conditionStrictness)
        add("semantic_condition_weakened", proposedFingerprint, "condition");
...
export async function writeNarrowedPromptScope(input) {
    if (input.decision.status !== "eligible")
        return input.decision;
    return { status: "written", result: await input.write(input.decision) };
}
//# sourceMappingURL=prompt-scope-narrowing.js.map