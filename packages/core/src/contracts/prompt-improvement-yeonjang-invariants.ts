...
import { validateYeonjangBroadcastIntent, type YeonjangBroadcastIntent } from "./yeonjang-broadcast.js"
import {
...
import {
  ...
  type ExactYeonjangSelector,
  type YeonjangExactTargetDecision,
} from "./yeonjang-target-resolution.js"
...
export type PromptImprovementYeonjangInvariantDecision =
  | { status: "authorized"; receipt: PromptImprovementYeonjangInvariantReceipt }
  | { status: "blocked"; reasonCode:
      | "host_instance_duplicate"
      | "exact_target_required"
      | "target_capability_missing"
      | "all_instances_broadcast_invalid"
      | "all_instances_user_request_missing"
      | "all_instances_user_request_invalid"
      | "all_instances_scope_mismatch"
      | "no_yeonjang_scope_invalid"
      | "no_yeonjang_result_invalid"
      | "sensitive_authorization_missing"
      | "sensitive_authorization_scope_mismatch"
      | "yeonjang_review_lineage_invalid" }

export type YeonjangToolBoundaryInvariantProjectionDecision =
  | { status: "authorized"; review: PlatformPromptInvariantReview }
  | { status: "blocked"; reasonCode: "yeonjang_review_receipt_invalid" | "yeonjang_review_expired" | "yeonjang_review_scope_mismatch" | "goal_section3_lineage_mismatch" }
...
function validateNoYeonjangResult(
  decision: NoYeonjangCapabilityGapDecision,
  result: TruthfulNoYeonjangResult,
): boolean {
  if (decision.schemaVersion !== 1 || result.schemaVersion !== 1 || decision.blockedSteps.length === 0) return false
  const expectedStatus = decision.selfSolveSteps.length > 0 ? "partial" : "blocked"
  if (result.status !== expectedStatus || JSON.stringify(result.blockedSteps) !== JSON.stringify(decision.blockedSteps)) return false
...
export function authorizePromptImprovementYeonjangInvariant(input: {
  ...
  const instancesByHost = new Map<string, number>()
  ...
  if (input.scope.kind === "single") {
    if (input.scope.targetDecision.status !== "resolved") return { status: "blocked", reasonCode: "exact_target_required" }
    try {
    ...
    if (!userRequest) return { status: "blocked", reasonCode: "all_instances_user_request_missing" }
...
export function projectYeonjangToolBoundaryInvariantReview(input: {
  ...
  if (receipt.expiresAt <= input.now) return { status: "blocked", reasonCode: "yeonjang_review_expired" }
