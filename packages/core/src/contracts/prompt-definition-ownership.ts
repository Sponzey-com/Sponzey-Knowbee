...
export type PromptDefinitionOwnershipIssueCode =
  | "abstract_criterion_missing"
  | "abstract_criterion_not_immediate"
  | "abstract_criterion_rule_mismatch"
  | "sentence_multiple_responsibilities"
  | "sentence_multiple_execution_contexts"
  | "sentence_parser_confidence_low"
...
export type PromptDefinitionOwnershipDecision =
  | { status: "eligible"; definitionKeys: string[] }
  | { status: "blocked"; issues: PromptDefinitionOwnershipIssue[] }
...
export function evaluatePromptDefinitionOwnership(input: {
  ...
  const issues: PromptDefinitionOwnershipIssue[] = []
  const add = (code: PromptDefinitionOwnershipIssueCode, subjectId: string): void => { issues.push({ code, subjectId }) }
  ...
  for (const binding of input.abstractBindings) {
    ...
    if (!binding.criterionText.trim() || !binding.testOrFixtureRef.trim()) add("abstract_criterion_missing", term)
    if (binding.criterionSegmentIndex !== binding.termSegmentIndex + 1) add("abstract_criterion_not_immediate", term)
    if (binding.criterionRuleId !== ruleId) add("abstract_criterion_rule_mismatch", term)
  }

  for (const sentence of input.sentences) {
    const sentenceId = required(sentence.sentenceId, "Prompt sentence ID")
    required(sentence.ruleId, "Prompt sentence rule ID")
    if (sentence.primaryResponsibilityIds.length !== 1) add("sentence_multiple_responsibilities", sentenceId)
  ...
  const ownerByKey = new Map<string, PromptDefinitionOwner>()
  for (const owner of input.owners) {
    ...
    if (ownerByKey.has(key)) add("definition_duplicate", key)
    ownerByKey.set(key, owner)
  }
  const definitionsByKey = new Map<string, PromptDefinitionOccurrence[]>()
  for (const occurrence of input.occurrences) {
    ...
    if (!owner) { add("definition_key_unknown", key); continue }
    if (occurrence.occurrenceKind === "definition") {
      ...
      if (sourceId !== owner.canonicalSourceId) add("definition_owner_mismatch", key)
  ...
  for (const [key, owner] of ownerByKey) {
    ...
    if (!definitions.some((item) => item.sourceId === owner.canonicalSourceId)) add("canonical_definition_missing", key)
    if (definitions.length > 1) add("definition_duplicate", key)
  }
...
export async function writeOwnershipEligiblePrompt<T>(input: {
  decision: PromptDefinitionOwnershipDecision
  write: (decision: Extract<PromptDefinitionOwnershipDecision, { status: "eligible" }>) => Promise<T>
}): Promise<{ status: "written"; result: T } | Extract<PromptDefinitionOwnershipDecision, { status: "blocked" }>> {
