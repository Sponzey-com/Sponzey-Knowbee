...
function timestamp(value, field, capturedAt, maxAgeMs) {
    if (!Number.isSafeInteger(value) || value < 0)
        throw new Error(`${field} must be a non-negative integer.`);
    if (value > capturedAt)
        throw new Error(`${field} cannot be later than capturedAt.`);
    if (capturedAt - value > maxAgeMs)
        throw new Error(`${field} is stale.`);
    return value;
}
function uniqueIds(values, id, field) {
    const result = new Map();
    for (const value of values) {
        const key = required(id(value), field);
        if (result.has(key))
            throw new Error(`${field} must be unique: ${key}.`);
...
export function validateYeonjangIdentityBoundarySnapshot(input) {
    ...
    if (snapshot.schemaVersion !== 1)
        throw new Error("Unsupported Yeonjang identity schema version.");
    ...
    if (!computers.has(hostComputerId))
        throw new Error("Runtime host computer association is missing.");
    ...
    for (const instance of snapshot.instances) {
        ...
        if (!computers.has(computerId))
            throw new Error(`Yeonjang instance computer association is missing: ${instance.instanceId}.`);
        const expectedLocality = computerId === hostComputerId ? "local" : "remote";
...
export function projectYeonjangUserFacingIdentities(snapshot) {
    ...
    return snapshot.instances.map((instance) => {
        ...
        if (!computer)
            throw new Error("Validated computer association is required for user projection.");
        const operatingSystem = operatingSystems.get(computer.operatingSystemId);
        if (!operatingSystem)
            throw new Error("Validated operating system association is required for user projection.");
        return {
...
//# sourceMappingURL=yeonjang-identity-boundary.js.map