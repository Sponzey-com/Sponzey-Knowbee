...
export type SolutionPathDisposition =
  | "available"
  | "attempted"
...
export function assessSolutionPathExhaustion(
  reviews: readonly SolutionPathReview[],
): SolutionPathExhaustionAssessment {
  const byPath = new Map<SolutionPath, SolutionPathReview>()

  for (const review of reviews) {
    if (byPath.has(review.path)) throw new Error(`duplicate solution path review: ${review.path}`)
    if (!review.reasonCode.trim()) throw new Error(`solution path review requires reasonCode: ${review.path}`)
  ...
  const partialResultRefs = uniqueNonEmpty(
    reviews.flatMap((review) => review.disposition === "completed_partial" ? review.resultRefs ?? [] : []),
  )
  const workaroundGuidance = uniqueNonEmpty(
    reviews.flatMap((review) => review.disposition === "guidance_ready" ? [review.guidance ?? ""] : []),
  )
...
export function assessAuthorizedSolutionPathExhaustion(input: {
  ...
  for (const review of input.reviews) {
    ...
    if (review.disposition === "attempted") {
      ...
      if (!signature) throw new Error(`Attempted solution path requires attemptSignature: ${review.path}.`)
      if (attemptSignatures.has(signature)) throw new Error(`Duplicate unchanged attempt: ${signature}.`)
...
export function buildTerminalFailurePayload(input: {
  ...
  if (!conciseReason) throw new Error("Terminal failure payload requires a concise reason.")
  const unresolvedScope = uniqueNonEmpty(input.unresolvedScope)
  if (unresolvedScope.length === 0) throw new Error("Terminal failure payload requires unresolved scope.")
  const userActions = uniqueNonEmpty(input.userActions)
  if (userActions.length === 0) throw new Error("Terminal failure payload requires at least one user action.")
...
export function buildPartialCompletionPayload(input: {
  ...
  const unresolvedScope = uniqueNonEmpty(input.unresolvedScope)
  if (unresolvedScope.length === 0) throw new Error("Partial completion payload requires unresolved scope.")
  const nextActions = uniqueNonEmpty(input.nextActions)
  if (nextActions.length === 0) throw new Error("Partial completion payload requires next actions.")
  return {
    status: "partial",
    partialResultRefs: [...input.assessment.partialResultRefs],
    unresolvedScope,
...
function assertAssessmentPartialResultsPreserved(
  assessment: AuthorizedSolutionPathExhaustionAssessment,
): void {
  const expected = uniqueNonEmpty(assessment.reviews.flatMap((review) =>
    review.disposition === "completed_partial" ? review.resultRefs ?? [] : [],
  ))
