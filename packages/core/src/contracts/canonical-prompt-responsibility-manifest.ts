...
export type CanonicalPromptManifestIssueCode =
  | "module_missing"
  | "module_duplicate"
  | "module_unknown"
  | "purpose_missing"
  | "owned_responsibility_count_invalid"
...
export type CanonicalPromptManifestDecision =
  | { status: "eligible"; moduleIds: CanonicalPromptModuleId[]; responsibilityIds: string[] }
  | { status: "blocked"; issues: CanonicalPromptManifestIssue[] }
...
export const CANONICAL_PROMPT_RESPONSIBILITY_MANIFEST: readonly CanonicalPromptResponsibilityManifestEntry[] = [
...
export function validateCanonicalPromptResponsibilityManifest(
  entries: readonly CanonicalPromptResponsibilityManifestEntry[],
): CanonicalPromptManifestDecision {
  const issues: CanonicalPromptManifestIssue[] = []
  const add = (code: CanonicalPromptManifestIssueCode, subjectId: string): void => { issues.push({ code, subjectId }) }
  const expected = new Set<string>(CANONICAL_PROMPT_MODULE_IDS)
  ...
  for (const manifest of entries) {
    ...
    if (!expected.has(moduleId)) add("module_unknown", moduleId)
    if (!manifest.purpose.trim()) add("purpose_missing", moduleId)
    if (manifest.ownedResponsibilityIds.length !== 1 || !manifest.ownedResponsibilityIds[0]?.trim()) {
      ...
      if (responsibilityOwners.has(responsibilityId)) add("responsibility_duplicate", responsibilityId)
      else responsibilityOwners.set(responsibilityId, moduleId)
    }
    ...
    if (moduleId !== "system" && manifest.dependencyModuleIds.length === 0) add("dependency_missing", moduleId)
    for (const dependencyId of manifest.dependencyModuleIds) {
      if (dependencyId === moduleId) add("dependency_self", moduleId)
      else if (!expected.has(dependencyId)) add("dependency_unknown", `${moduleId}:${dependencyId}`)
    }
  }

  for (const moduleId of CANONICAL_PROMPT_MODULE_IDS) {
    const count = moduleCounts.get(moduleId) ?? 0
    if (count === 0) add("module_missing", moduleId)
    if (count > 1) add("module_duplicate", moduleId)
  }

  const uniqueIssues = [...new Map(issues.map((issue) => [`${issue.code}\u0000${issue.subjectId}`, issue])).values()]
  if (uniqueIssues.length > 0) return { status: "blocked", issues: uniqueIssues }
  return {
    status: "eligible",
