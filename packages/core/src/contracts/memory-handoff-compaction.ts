...
export type MemoryHandoffIssueCode =
  | "handoff_owner_same"
  | "handoff_work_mismatch"
  | "handoff_purpose_missing"
  | "handoff_payload_field_not_allowed"
  | "handoff_context_ref_not_allowed"
  | "handoff_provenance_missing"
  | "handoff_raw_memory_forbidden"
  | "handoff_unrelated_history_forbidden"
  | "handoff_long_term_grant_forbidden"
  | "handoff_expired"
...
export type ShortTermCompactionDecision =
  | { status: "eligible"; candidateSegmentIds: string[]; policyVersion: string }
  | { status: "no_op"; reasonCode: "threshold_not_exceeded" }
  | { status: "blocked"; reasonCode: "no_safe_compaction_candidate" }

export type MemoryHandoffDecision =
  | { status: "eligible"; handoffId: string }
  | { status: "blocked"; issueCodes: MemoryHandoffIssueCode[] }
...
function uniqueSet(values: string[], field: string): Set<string> {
...
export function evaluateWorkBoundMemoryHandoff(input: WorkBoundMemoryHandoff): MemoryHandoffDecision {
  ...
  const issueCodes = new Set<MemoryHandoffIssueCode>()
  if (sourceAgentId === recipientAgentId) issueCodes.add("handoff_owner_same")
  if (assignedWorkId !== receiptWorkId) issueCodes.add("handoff_work_mismatch")
  if (!input.purpose.trim()) issueCodes.add("handoff_purpose_missing")
  const allowedFields = uniqueSet(input.allowedPayloadFieldNames, "Allowed payload field")
  if ([...uniqueSet(input.payloadFieldNames, "Payload field")].some((field) => !allowedFields.has(field))) issueCodes.add("handoff_payload_field_not_allowed")
  const allowedRefs = uniqueSet(input.allowedContextRefs, "Allowed context ref")
  if ([...uniqueSet(input.contextRefs, "Context ref")].some((ref) => !allowedRefs.has(ref))) issueCodes.add("handoff_context_ref_not_allowed")
  if (uniqueSet(input.provenanceRefs, "Provenance ref").size === 0) issueCodes.add("handoff_provenance_missing")
  if (input.containsRawMemory) issueCodes.add("handoff_raw_memory_forbidden")
  if (input.containsUnrelatedHistory) issueCodes.add("handoff_unrelated_history_forbidden")
  if (input.grantsLongTermRetention) issueCodes.add("handoff_long_term_grant_forbidden")
  if (!Number.isFinite(input.expiresAt) || !Number.isFinite(input.evaluatedAt) || input.expiresAt <= input.evaluatedAt) issueCodes.add("handoff_expired")
...
export function evaluateShortTermCompaction(input: {
  ...
  if (!Number.isSafeInteger(input.currentTokenEstimate) || input.currentTokenEstimate < 0) throw new Error("Current token estimate must be a non-negative integer.")
  if (!Number.isSafeInteger(input.currentMessageCount) || input.currentMessageCount < 0) throw new Error("Current message count must be a non-negative integer.")
  if (input.currentTokenEstimate <= tokenThreshold && input.currentMessageCount <= messageThreshold) return { status: "no_op", reasonCode: "threshold_not_exceeded" }
  ...
  const candidates = ordered.filter((segment) =>
    !protectedOrdinals.has(segment.ordinal) && !segment.pinned && !segment.activeWork && !segment.activeDelegation &&
...
export async function runEligibleMemoryOperation<T>(input: {
  eligible: boolean
  run: () => Promise<T>
