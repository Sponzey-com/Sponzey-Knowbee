import type { ContractValidationIssue, ContractValidationResult, JsonValue } from "./index.js"
...
const WORK_RECORD_SOURCES = new Set<WorkRecordSource>(["user", "parent_agent", "system", "scheduled"])
const WORK_RECORD_STATUSES = new Set<WorkRecordStatus>(["intake", "planned", "running", "waiting", "completed", "partial", "blocked", "failed", "cancelled"])
...
const STEP_ACTION_TYPES = new Set<WorkStepActionType>(["direct_answer", "plan", "delegate", "use_tool", "use_yeonjang", "ask_clarification", "validate", "report"])
const STEP_STATUSES = new Set<WorkStepStatus>(["pending", "running", "completed", "blocked", "failed", "skipped"])
const STEP_RESULT_STATUSES = new Set<WorkStepResultStatus>(["completed", "partial", "blocked", "failed"])
const RESULT_SUFFICIENCIES = new Set<ResultSufficiency>(["sufficient", "partial", "insufficient", "unknown"])
const RECOVERY_CHANGED_DIMENSIONS = new Set<RecoveryChangedDimension>(["input", "strategy", "tool", "delegation_target", "permission", "scope", "validation_method"])
const WORK_BLOCKER_KINDS = new Set<WorkBlockerKind>(["missing_information", "permission", "resource", "safety"])
const CHILD_WORK_RESULT_STATUSES = new Set<ChildWorkResultStatus>(["completed", "partial", "blocked", "failed"])
const WORK_HANDOFF_MEMORY_VISIBILITIES = new Set<WorkHandoffMemoryVisibility>(["explicit_handoff_only"])
const WORK_HANDOFF_RETURN_FORMATS = new Set<WorkHandoffReturnFormat>(["ChildWorkResult"])
...
const INTERNAL_AGENT_NAME_PREFIXES = ["agent:", "team:", "session:", "sub_session:"] as const
const BLOCKED_ACTIONS = new Set<RecommendedAction>(["ask_clarification", "stop_blocked"])
...
export const WORK_RECORD_STATUS_TRANSITIONS: Readonly<Record<WorkRecordStatus, readonly WorkRecordStatus[]>> =
  Object.freeze({
  ...
  } satisfies Record<WorkRecordStatus, readonly WorkRecordStatus[]>)

export function isDeclaredWorkRecordStatusTransition(
...
function handoffIssue(
  path: string,
  code: "handoff_text_limit_exceeded" | "handoff_array_limit_exceeded" | "handoff_text_not_executable",
  message: string,
): ContractValidationIssue {
  return { path, code, message }
}

function structuredTextIssue(
  path: string,
  code: "structured_text_limit_exceeded" | "structured_text_aggregate_exceeded",
  message: string,
): ContractValidationIssue {
  return { path, code, message }
}

function validateStructuredTextScalar(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (typeof value !== "string") return
  if (value.length > STRUCTURED_INTERNAL_TEXT_LIMITS.scalarCharacters) {
    issues.push(structuredTextIssue(
      path,
      "structured_text_limit_exceeded",
      `Structured text exceeds ${STRUCTURED_INTERNAL_TEXT_LIMITS.scalarCharacters} characters.`,
    ))
  }
}

function validateStructuredTextArray(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  ...
  if (value.length > STRUCTURED_INTERNAL_TEXT_LIMITS.arrayItems) {
    issues.push(structuredTextIssue(
      path,
      "structured_text_aggregate_exceeded",
      `Structured text array exceeds ${STRUCTURED_INTERNAL_TEXT_LIMITS.arrayItems} items.`,
    ))
  }
  let aggregate = 0
  value.forEach((item, index) => {
    ...
    if (item.length > STRUCTURED_INTERNAL_TEXT_LIMITS.arrayItemCharacters) {
      issues.push(structuredTextIssue(
        `${path}[${index}]`,
        "structured_text_limit_exceeded",
        `Structured text item exceeds ${STRUCTURED_INTERNAL_TEXT_LIMITS.arrayItemCharacters} characters.`,
      ))
    }
  })
  if (aggregate > STRUCTURED_INTERNAL_TEXT_LIMITS.arrayAggregateCharacters) {
    issues.push(structuredTextIssue(
      path,
      "structured_text_aggregate_exceeded",
      `Structured text array exceeds ${STRUCTURED_INTERNAL_TEXT_LIMITS.arrayAggregateCharacters} aggregate characters.`,
    ))
  }
}

function validateDiagnosisTextBounds(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!isRecord(value)) return
  for (const field of ["diagnosis_summary", "intent", "goal", "risk", "confidence", "reason"] as const) {
    validateStructuredTextScalar(value[field], `${path}.${field}`, issues)
  }
  for (const field of ["constraints", "missing_information", "conflicts", "risks"] as const) {
    validateStructuredTextArray(value[field], `${path}.${field}`, issues)
  }
}

function validateRecoveryTextBounds(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!isRecord(value)) return
  for (const field of ["changed_input_or_strategy", "expected_benefit", "risk", "required_permission"] as const) {
    validateStructuredTextScalar(value[field], `${path}.${field}`, issues)
  }
}

function validateWorkRecordTextBounds(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  ...
  if (Array.isArray(value.step_plan)) value.step_plan.forEach((step, index) => {
  ...
  if (Array.isArray(value.step_results)) value.step_results.forEach((result, index) => {
  ...
  if (Array.isArray(value.recovery_candidates)) value.recovery_candidates.forEach((candidate, index) => {
  ...
  if (isRecord(value.blocker_resolution)) {
    ...
    validateStructuredTextArray(
      value.blocker_resolution.resolution_evidence_refs,
      "$.blocker_resolution.resolution_evidence_refs",
      issues,
    )
  }
...
function validateChildWorkResultTextBounds(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  for (const field of ["task_goal", "summary", "result", "recommended_next_step"] as const) {
    validateStructuredTextScalar(value[field], `$.${field}`, issues)
  }
  for (const field of ["evidence", "assumptions", "risks", "missing_information", "actions_taken"] as const) {
  ...
  if (Array.isArray(value.recovery_attempts)) value.recovery_attempts.forEach((candidate, index) => {
    validateRecoveryTextBounds(candidate, `$.recovery_attempts[${index}]`, issues)
  })
}

function validateHandoffScalarText(value: unknown, path: string, executable: boolean, issues: ContractValidationIssue[]): void {
  if (typeof value !== "string") return
...
function validateHandoffTextArray(value: unknown, path: string, executable: boolean, issues: ContractValidationIssue[]): void {
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) return
...
function validateHandoffTextBounds(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
...
function validateRequiredString(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (typeof value === "string" && value.trim().length > 0) return
  issues.push(issue(path, `Expected non-empty string at ${path}.`))
}

function looksLikeInternalAgentIdentifier(value: unknown): boolean {
  if (typeof value !== "string") return false
  const normalized = value.trim().toLowerCase()
  return INTERNAL_AGENT_NAME_PREFIXES.some((prefix) => normalized.startsWith(prefix))
}

function validateUserFacingAgentName(value: unknown, path: string, message: string, issues: ContractValidationIssue[]): void {
  if (!looksLikeInternalAgentIdentifier(value)) return
  issues.push(issue(path, message))
}

function validateOptionalNonEmptyString(value: unknown, path: string, message: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) return
  if (typeof value === "string" && value.trim().length > 0) return
  issues.push(issue(path, message))
}

function validateNumber(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (typeof value === "number" && Number.isFinite(value)) return
  issues.push(issue(path, `Expected number at ${path}.`))
}

function validateNonNegativeInteger(value: unknown, path: string, message: string, issues: ContractValidationIssue[]): void {
  if (Number.isInteger(value) && typeof value === "number" && value >= 0) return
  issues.push(issue(path, message))
}

function validateBoolean(value: unknown, path: string, issues: ContractValidationIssue[]): void {
...
function validateNonEmptyStringArray(value: unknown, path: string, message: string, issues: ContractValidationIssue[]): void {
  validateStringArray(value, path, issues)
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) return
  if (value.some((item) => item.trim().length > 0)) return
  issues.push(issue(path, message))
}

function validateOptionalStringArray(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) return
  validateStringArray(value, path, issues)
}
...
function validateMetadataObjectEntries(value: Record<string, unknown>, path: string, issues: ContractValidationIssue[]): void {
...
function validateMetadataJsonValue(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) {
...
function validateRecoveryCandidateMetadata(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) return
  if (!isPlainJsonObject(value)) {
...
function validateEnum<T extends string>(value: unknown, allowed: ReadonlySet<T>, path: string, issues: ContractValidationIssue[]): void {
  if (typeof value === "string" && allowed.has(value as T)) return
  issues.push(issue(path, `Unsupported enum value at ${path}.`))
}

function validateRequestDiagnosis(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!isRecord(value)) {
...
function validateResultDiagnosis(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!isRecord(value)) {
    issues.push(issue(path, `Expected object at ${path}.`))
    return
  }
...
function validateStepPlan(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!Array.isArray(value)) {
  ...
  value.forEach((item, index) => {
    ...
    validateUserFacingAgentName(
      item.owner_agent_name,
      `${itemPath}.owner_agent_name`,
      "step_plan.owner_agent_name must use a user-facing agent name, not an internal ID.",
      issues,
    )
...
function validateStepResults(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!Array.isArray(value)) {
    issues.push(issue(path, `Expected array at ${path}.`))
    return
  }
  value.forEach((item, index) => {
    ...
    validateOptionalNonEmptyString(
      item.output_ref,
      `${itemPath}.output_ref`,
      "step_results.output_ref must be non-empty when present.",
      issues,
    )
    ...
    validateOptionalNonEmptyString(
      item.error,
      `${itemPath}.error`,
      "step_results.error must be non-empty when present.",
      issues,
    )
    if (
    ...
    if (item.completed_at !== undefined) validateNumber(item.completed_at, `${itemPath}.completed_at`, issues)
  })
}

function validateStepPlanUniqueIds(value: unknown, path: string, issues: ContractValidationIssue[]): Set<string> | undefined {
  if (!Array.isArray(value)) return undefined
  const plannedStepIds = new Set<string>()
  value.forEach((item, index) => {
    if (!isRecord(item) || typeof item.step_id !== "string") return
...
function validateWorkRecordStepReferences(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  const plannedStepIds = validateStepPlanUniqueIds(value.step_plan, "$.step_plan", issues)
  if (!plannedStepIds) return

  const stepStatusById = new Map<string, unknown>()
  ...
  if (Array.isArray(value.step_results)) {
    value.step_results.forEach((item, index) => {
      ...
      if (
        ...
        issues.push(issue(
          `$.step_results[${index}].status`,
          "step_results.status must match terminal step_plan.status for completed, failed, and blocked results.",
        ))
      }
      if (
        item.status === "partial" &&
        ...
        issues.push(issue(
          `$.step_results[${index}].status`,
          "partial step_results.status requires a running or terminal step_plan.status.",
        ))
      }
    })
  }
  ...
  if (
    ...
    if (!plannedStepIds.has(failedStepId)) {
      ...
      if (plannedStatus === "completed" || plannedStatus === "skipped") {
        issues.push(issue(
          "$.failure_diagnosis.failed_step_id",
          "failure_diagnosis.failed_step_id must not reference a completed or skipped step.",
        ))
      }
    }
  }
  ...
  if (
    isRecord(value.action_decision) &&
...
function validateFailureDiagnosis(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) return
  if (!isRecord(value)) {
    issues.push(issue(path, `Expected object at ${path}.`))
    return
...
function validateActiveWorkBlocker(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) return
  if (!isRecord(value)) {
    issues.push(issue(path, `Expected object at ${path}.`))
    return
  ...
  validateNonEmptyStringArray(
    value.evidence_refs,
    `${path}.evidence_refs`,
    "active blocker requires at least one evidence reference.",
    issues,
  )
...
function validateBlockerResolutionReceipt(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) return
  if (!isRecord(value)) {
    issues.push(issue(path, `Expected object at ${path}.`))
    return
  ...
  validateRequiredString(value.blocker_ref, `${path}.blocker_ref`, issues)
  validateNonEmptyStringArray(
    value.resolution_evidence_refs,
    `${path}.resolution_evidence_refs`,
    "blocker resolution requires at least one evidence reference.",
    issues,
  )
  validateNonEmptyStringArrayItems(
    value.resolution_evidence_refs,
    `${path}.resolution_evidence_refs`,
    "blocker resolution evidence refs must be non-empty.",
    issues,
  )
  validateUniqueStringArrayItems(
    value.resolution_evidence_refs,
    `${path}.resolution_evidence_refs`,
    "blocker resolution evidence refs must be unique.",
    issues,
  )
  validateBoolean(value.verified, `${path}.verified`, issues)
}

function validateRecoveryCandidate(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!isRecord(value)) {
    issues.push(issue(path, `Expected object at ${path}.`))
    return
  }
  ...
  validateOptionalNonEmptyString(
    ...
    "required_permission must be non-empty when present.",
    issues,
  )
  ...
  if (!Array.isArray(value.changed_dimensions)) {
    ...
    if (value.changed_dimensions.length === 0) {
      issues.push(issue(
        ...
        "Recovery candidate must change input, strategy, tool, delegation target, permission, scope, or validation method.",
      ))
    }
...
function sameWorkStepPlanItem(left: Record<string, unknown>, right: Record<string, unknown>): boolean {
...
function sameChangedDimensions(left: readonly RecoveryChangedDimension[], right: readonly RecoveryChangedDimension[]): boolean {
  if (left.length !== right.length) return false
...
function definedObjectKeys(value: Record<string, JsonValue | undefined>): string[] {
...
function sameMetadata(
  left: Record<string, JsonValue | undefined> | undefined,
  right: Record<string, JsonValue | undefined> | undefined,
): boolean {
  return sameJsonValue(left ?? {}, right ?? {})
}

function recoveryCandidateMatches(left: RecoveryCandidate, right: RecoveryCandidate): boolean {
...
function validateRecoveryCandidates(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (value === undefined) return
  if (!Array.isArray(value)) {
    issues.push(issue(path, `Expected array at ${path}.`))
    return
  ...
  const validCandidates: RecoveryCandidate[] = []
  value.forEach((candidate, index) => {
    ...
    const duplicate = validCandidates.some((validCandidate) =>
      recoveryCandidateMatches(validCandidate, candidate as unknown as RecoveryCandidate))
    if (duplicate) {
      issues.push(issue(`${path}[${index}]`, "recovery_candidates items must be unique."))
      return
    }
    validCandidates.push(candidate as unknown as RecoveryCandidate)
  })
}

function validateActionDecision(value: unknown, path: string, issues: ContractValidationIssue[]): void {
  if (!isRecord(value)) {
    issues.push(issue(path, `Expected object at ${path}.`))
    return
  }
  ...
  validateOptionalNonEmptyString(
    value.next_step_id,
    `${path}.next_step_id`,
    "action_decision.next_step_id must be non-empty when present.",
    issues,
  )
}

function validateSelectedRecoveryActionReference(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  if (value.selected_recovery_action === undefined) return
  if (!isRecord(value.selected_recovery_action)) return
  const selectedIssues: ContractValidationIssue[] = []
  ...
  const selected = value.selected_recovery_action as unknown as RecoveryCandidate
  const hasMatchingCandidate = value.recovery_candidates.some((candidate) => {
    ...
    if (candidateIssues.length > 0) return false
    return recoveryCandidateMatches(candidate as unknown as RecoveryCandidate, selected)
  })
  if (hasMatchingCandidate) return
  issues.push(issue("$.selected_recovery_action", "selected_recovery_action must match one recovery_candidates item."))
}

function validateSelectedRecoveryActionAgainstFailure(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  if (!isRecord(value.failure_diagnosis) || !isRecord(value.selected_recovery_action)) return
  const failureIssues: ContractValidationIssue[] = []
  validateFailureDiagnosis(value.failure_diagnosis, "$.failure_diagnosis", failureIssues)
  const selectedIssues: ContractValidationIssue[] = []
  validateRecoveryCandidate(value.selected_recovery_action, "$.selected_recovery_action", selectedIssues)
  if (failureIssues.length > 0 || selectedIssues.length > 0) return

  const result = validateRecoveryCandidateAgainstFailure(
    value.failure_diagnosis as unknown as FailureDiagnosis,
    value.selected_recovery_action as unknown as RecoveryCandidate,
  )
  if (result.ok) return
...
function validateFailedWorkRecordRecoveryBundle(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
...
function completedWorkRecordRequiredStepsAreComplete(value: Record<string, unknown>): boolean {
  if (!Array.isArray(value.step_plan) || !Array.isArray(value.step_results)) return true
  const requiredStepIds: string[] = []
  for (const step of value.step_plan) {
    if (!isRecord(step) || typeof step.step_id !== "string") continue
  ...
  const completedResultIds = new Set(
    value.step_results
      .filter((result): result is Record<string, unknown> => isRecord(result))
      .filter((result) => result.status === "completed" && typeof result.step_id === "string")
      .map((result) => result.step_id as string),
  )
  return requiredStepIds.every((stepId) => completedResultIds.has(stepId))
}

function validateWorkRecordStatusGate(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  const diagnosis = value.result_diagnosis
  const decision = value.action_decision
  if (!isRecord(diagnosis) || !isRecord(decision)) return

  if (value.status === "completed" && (
    diagnosis.sufficiency !== "sufficient" ||
    diagnosis.recommended_action !== "final_report" ||
    decision.selected_action !== "final_report"
  )) {
    issues.push(issue(
      "$.status",
      "completed work record requires sufficient final_report diagnosis and action decision.",
    ))
  }
  if (value.status === "completed" && !completedWorkRecordRequiredStepsAreComplete(value)) {
    issues.push(issue(
      "$.status",
      "completed work record requires completed required steps and completed step results.",
    ))
  }

  if (value.status === "partial" && (
    diagnosis.sufficiency !== "partial" ||
    diagnosis.recommended_action === "final_report" ||
    decision.selected_action === "final_report" ||
    ...
    issues.push(issue(
      "$.status",
      "partial work record requires partial diagnosis, a matching non-final action decision, failure_diagnosis, and at least one recovery candidate.",
    ))
  }
  if (
    ...
    issues.push(issue(
      "$.selected_recovery_action",
      "partial retry or redelegate work record requires selected_recovery_action.",
    ))
  }
  ...
  if (value.status === "failed" && (
    ...
    issues.push(issue(
      "$.status",
      "failed work record requires non-sufficient diagnosis and non-final action decision.",
    ))
  }

  if (value.status === "blocked" && (
    diagnosis.sufficiency === "sufficient" ||
    !BLOCKED_ACTIONS.has(diagnosis.recommended_action as RecommendedAction) ||
    !BLOCKED_ACTIONS.has(decision.selected_action as RecommendedAction)
  )) {
    issues.push(issue(
      "$.status",
      "blocked work record requires non-sufficient diagnosis and blocked or clarification action.",
    ))
  }
}

function validateChildResultActionGate(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  const diagnosis = value.result_diagnosis
  const decision = value.action_decision
  if (!isRecord(diagnosis) || !isRecord(decision)) return
  ...
  if (
    ...
    RECOMMENDED_ACTIONS.has(expectedAction as RecommendedAction) &&
    RECOMMENDED_ACTIONS.has(selectedAction as RecommendedAction) &&
    selectedAction !== expectedAction
  ) {
    issues.push(issue(
      "$.action_decision.selected_action",
      "Child work result action decision must match result_diagnosis.recommended_action.",
    ))
  }
}

function validateChildResultStatusGate(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  const diagnosis = value.result_diagnosis
  const decision = value.action_decision
  if (!isRecord(diagnosis) || !isRecord(decision)) return
  ...
  if (value.status === "completed" && (
    ...
    issues.push(issue(
      "$.status",
      "completed child work result requires sufficient final_report diagnosis and action decision.",
    ))
  }
  ...
  if (value.status === "partial" && (
    ...
    issues.push(issue(
      "$.status",
      "partial child work result requires partial diagnosis and a non-final next action.",
    ))
  }
  ...
  if (value.status === "failed" && (
    ...
    issues.push(issue(
      "$.status",
      "failed child work result requires non-sufficient diagnosis and non-final action decision.",
    ))
  }
  ...
  if (value.status === "blocked" && (
    ...
    issues.push(issue(
      "$.status",
      "blocked child work result requires non-sufficient diagnosis and blocked or clarification action.",
    ))
  }
}

function validateChildResultStepStatusGate(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  if (!Array.isArray(value.completed_steps) || !Array.isArray(value.failed_steps)) return
  ...
  value.completed_steps.forEach((step, index) => {
    ...
    issues.push(issue(
      ...
      "completed_steps items must be non-empty step ids.",
    ))
  })
  value.failed_steps.forEach((step, index) => {
    ...
    issues.push(issue(
      `$.failed_steps[${index}]`,
      "failed_steps items must be non-empty step ids.",
    ))
  })
  ...
  value.failed_steps.forEach((step, index) => {
    ...
    if (completedSteps.has(step.trim())) {
      issues.push(issue(
        `$.failed_steps[${index}]`,
        "Child work result step cannot be both completed and failed.",
      ))
    }
  })
...
function validateNonEmptyStringArrayItems(value: unknown, path: string, message: string, issues: ContractValidationIssue[]): void {
...
function validateUniqueStringArrayItems(value: unknown, path: string, message: string, issues: ContractValidationIssue[]): void {
  if (!Array.isArray(value)) return
...
function validateUniqueNormalizedStringArrayItems(value: unknown, path: string, message: string, issues: ContractValidationIssue[]): void {
  if (!Array.isArray(value)) return
  const seen = new Set<string>()
  value.forEach((item, index) => {
    if (typeof item !== "string") return
...
function validateFailedChildResultRecoveryBundle(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  if (value.status !== "failed") return
...
function hasNonEmptyStringItem(value: unknown): boolean {
  return Array.isArray(value) && value.some((item) => typeof item === "string" && item.trim().length > 0)
}

function validateCompletedChildResultReviewEvidence(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
...
function validatePartialChildResultReviewContext(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  if (value.status !== "partial") return
...
function validateBlockedChildResultReviewContext(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
...
function validateChildResultRecoveryGate(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  if (!isRecord(value.failure_diagnosis) || !Array.isArray(value.recovery_attempts)) return
  const failureValidationIssues: ContractValidationIssue[] = []
  validateFailureDiagnosis(value.failure_diagnosis, "$.failure_diagnosis", failureValidationIssues)
  if (failureValidationIssues.length > 0) return

  value.recovery_attempts.forEach((candidate, index) => {
    const result = validateRecoveryCandidateAgainstFailure(value.failure_diagnosis as unknown as FailureDiagnosis, candidate as RecoveryCandidate)
    if (result.ok) return
    result.issues.forEach((candidateIssue) => {
...
function validateWorkHandoffActionGate(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  ...
  if (isRecord(diagnosis)) {
    ...
    if (
      ...
      RECOMMENDED_ACTIONS.has(recommendedAction as RecommendedAction) &&
      recommendedAction !== "delegate"
    ) {
      issues.push(issue(
        "$.request_diagnosis.recommended_action",
        "Work handoff request diagnosis must recommend delegate.",
      ))
    }
  }
  ...
  if (isRecord(currentStep)) {
    ...
    if (
      ...
      STEP_ACTION_TYPES.has(actionType as WorkStepActionType) &&
      actionType !== "delegate"
    ) {
      issues.push(issue(
        "$.current_step.action_type",
        "Work handoff current step action_type must be delegate.",
      ))
    }
  }
}

function validateWorkHandoffStepReferences(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  ...
  if (isRecord(currentStep) && Array.isArray(value.step_plan) && typeof currentStep.step_id === "string") {
    ...
    const matchingStep = value.step_plan.find((item) => (
      isRecord(item) &&
  ...
  if (
    ...
    issues.push(issue(
      "$.current_step.owner_agent_name",
      "current_step.owner_agent_name must match target_agent_name.",
    ))
  }
}

function validateWorkHandoffToolBoundary(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  if (!Array.isArray(value.allowed_tools) || !Array.isArray(value.disallowed_actions)) return
  ...
  value.disallowed_actions.forEach((item, index) => {
    if (typeof item !== "string") return
    const normalized = item.trim().toLowerCase()
    if (normalized.length === 0 || !allowed.has(normalized)) return
    issues.push(issue(
      `$.disallowed_actions[${index}]`,
      "disallowed_actions must not repeat allowed_tools items.",
    ))
  })
}

function validateWorkHandoffFailureRecoveryPolicy(value: Record<string, unknown>, issues: ContractValidationIssue[]): void {
  validateRequiredString(value.failure_recovery_policy, "$.failure_recovery_policy", issues)
  if (typeof value.failure_recovery_policy !== "string") return
  const normalized = value.failure_recovery_policy.trim().toLowerCase()
  if (normalized.length === 0) return
  if (RECOVERY_CHANGED_DIMENSION_TERMS.some((term) => normalized.includes(term))) return
  issues.push(issue(
    "$.failure_recovery_policy",
...
export function validateLlmRequestDiagnosisRecord(
  value: unknown,
): ContractValidationResult<LlmRequestDiagnosisRecord> {
  const issues: ContractValidationIssue[] = []
  validateRequestDiagnosis(value, "$", issues)
  return issues.length === 0
    ? { ok: true, value: value as unknown as LlmRequestDiagnosisRecord, issues: [] }
    : { ok: false, issues }
}

export function validateLlmResultDiagnosisRecord(
  value: unknown,
): ContractValidationResult<LlmResultDiagnosisRecord> {
  const issues: ContractValidationIssue[] = []
  validateResultDiagnosis(value, "$", issues)
  return issues.length === 0
    ? { ok: true, value: value as unknown as LlmResultDiagnosisRecord, issues: [] }
    : { ok: false, issues }
}

export function validateWorkRecord(value: unknown): ContractValidationResult<WorkRecord> {
  const issues: ContractValidationIssue[] = []
  if (!isRecord(value)) {
    return { ok: false, issues: [issue("$", "Work record must be an object.")] }
  }
  for (const field of Object.keys(value)) {
    if (!WORK_RECORD_FIELDS.has(field)) issues.push(issue(`$.${field}`, "Work record contains an unsupported field."))
  }
  ...
  validateUserFacingAgentName(
    value.owner_agent_name,
    "$.owner_agent_name",
    "owner_agent_name must use a user-facing agent name, not an internal ID.",
    issues,
  )
  ...
  if (value.selected_recovery_action !== undefined) validateRecoveryCandidate(value.selected_recovery_action, "$.selected_recovery_action", issues)
  ...
  return issues.length === 0 ? { ok: true, value: value as unknown as WorkRecord, issues: [] } : { ok: false, issues }
}

function requiredStepIds(record: WorkRecord): Set<string> {
  return new Set(record.step_plan.filter((step) => step.status !== "skipped").map((step) => step.step_id))
}

function completedStepResultIds(record: WorkRecord): Set<string> {
  return new Set(record.step_results.filter((result) => result.status === "completed").map((result) => result.step_id))
}

function verifiedCompletedStepResults(record: WorkRecord): WorkStepResult[] {
  return record.step_results.filter((result) =>
    result.status === "completed" &&
    Boolean(result.output_ref?.trim()) &&
    result.evidence_refs.some((reference) => reference.trim().length > 0))
}

function requiredStepPlanStatusesAreCompleted(record: WorkRecord): boolean {
  return record.step_plan.every((step) => step.status === "skipped" || step.status === "completed")
}

function completionCriteriaMet(record: WorkRecord): boolean {
  if (record.result_diagnosis.sufficiency !== "sufficient") return false
  if (record.result_diagnosis.recommended_action !== "final_report") return false
  if (record.action_decision.selected_action !== "final_report") return false
  if (!requiredStepPlanStatusesAreCompleted(record)) return false
...
function hasBlockerResolutionEvidence(record: WorkRecord): boolean {
...
function failureDiagnosisStepReferenceIsValid(record: WorkRecord): boolean {
  if (!record.failure_diagnosis) return false
  if (record.failure_diagnosis.recoverable !== true) return false
...
const PARTIAL_NEXT_ACTIONS = new Set<RecommendedAction>([
  "retry",
  "redelegate",
  "partial_report",
  "stop_blocked",
...
function partialCriteriaMet(record: WorkRecord): boolean {
...
export function decideWorkRecordRunningExit(
  record: WorkRecord,
  targetStatus: "completed" | "partial",
): WorkRecordRunningExitDecision {
...
function hasValidRecoveryCandidate(record: WorkRecord): boolean {
  if (!record.failure_diagnosis || !record.recovery_candidates?.length) return false
  return record.recovery_candidates.some((candidate) =>
    validateRecoveryCandidateAgainstFailure(record.failure_diagnosis as FailureDiagnosis, candidate).ok)
}

function selectedRecoveryActionIsValid(record: WorkRecord): boolean {
  if (!record.failure_diagnosis || !record.selected_recovery_action || !record.recovery_candidates?.length) return false
  if (!failureDiagnosisStepReferenceIsValid(record)) return false
  if (!record.recovery_candidates.some((candidate) => recoveryCandidateMatches(candidate, record.selected_recovery_action as RecoveryCandidate))) {
    return false
  }
...
export function decideWorkRecordRecoveryReentry(record: WorkRecord): WorkRecoveryReentryDecision {
...
export function canTransitionWorkRecordStatus(record: WorkRecord, nextStatus: WorkRecordStatus): WorkRecordTransitionResult {
...
export function validateRecoveryCandidateAgainstFailure(
  failure: FailureDiagnosis,
  candidate: RecoveryCandidate,
): ContractValidationResult<RecoveryCandidate> {
  const issues: ContractValidationIssue[] = []
  ...
  const repeatsFailedInput = failure.failed_input_refs.some((inputRef) =>
    changedDescription === normalizeComparisonText(inputRef))
...
export function validateWorkHandoffPackage(value: unknown): ContractValidationResult<WorkHandoffPackage> {
  const issues: ContractValidationIssue[] = []
  if (!isRecord(value)) {
  ...
  validateUserFacingAgentName(
    value.parent_agent_name,
    "$.parent_agent_name",
    "parent_agent_name must use a user-facing agent name, not an internal ID.",
    issues,
  )
  validateUserFacingAgentName(
    value.target_agent_name,
    "$.target_agent_name",
    "target_agent_name must use a user-facing agent name, not an internal ID.",
    issues,
  )
  if (
    typeof value.parent_agent_name === "string" &&
  ...
  validateUniqueNormalizedStringArrayItems(
    value.allowed_tools,
    "$.allowed_tools",
    "allowed_tools items must be unique after trim and lowercase normalization.",
    issues,
  )
  validateUniqueNormalizedStringArrayItems(
    value.disallowed_actions,
    "$.disallowed_actions",
    "disallowed_actions items must be unique after trim and lowercase normalization.",
    issues,
  )
  ...
  return issues.length === 0 ? { ok: true, value: value as unknown as WorkHandoffPackage, issues: [] } : { ok: false, issues }
}

export function validateChildWorkResult(value: unknown): ContractValidationResult<ChildWorkResult> {
  const issues: ContractValidationIssue[] = []
  if (!isRecord(value)) {
    return { ok: false, issues: [issue("$", "Child work result must be an object.")] }
  }
  for (const field of Object.keys(value)) {
    if (!CHILD_WORK_RESULT_FIELDS.has(field)) issues.push(issue(`$.${field}`, "Child work result contains an unsupported field."))
  }
  if (value.schemaVersion !== WORK_RECORD_SCHEMA_VERSION) {
  ...
  validateUserFacingAgentName(
    value.agent_name,
    "$.agent_name",
    "agent_name must use a user-facing agent name, not an internal ID.",
    issues,
  )
  validateRequiredString(value.task_goal, "$.task_goal", issues)
  ...
  return issues.length === 0 ? { ok: true, value: value as unknown as ChildWorkResult, issues: [] } : { ok: false, issues }
}

export function validateWorkRecordActionGate(
  value: unknown,
  phase: WorkRecordActionGatePhase,
): ContractValidationResult<WorkRecord> {
  ...
  if (record.action_decision.selected_action !== expectedAction) {
    return {
      ...
      issues: [issue(
        "$.action_decision.selected_action",
        `Action decision must match ${phase}_diagnosis.recommended_action.`,
      )],
    }
  }
  return validation
