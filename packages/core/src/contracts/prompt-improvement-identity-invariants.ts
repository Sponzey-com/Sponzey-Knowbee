...
export type PromptImprovementIdentityAuditDecision =
  | { status: "preserved"; effectiveMainAgentName: string; normalizedAgentNames: string[] }
  | { status: "blocked"; reasonCode:
      | "product_identity_mismatch"
      | "main_agent_identity_invalid"
      | "main_agent_name_mismatch"
      | "agent_identity_invalid"
      | "agent_identity_field_invalid"
      | "agent_name_duplicate"
      | "user_agent_name_collision"
      | "user_facing_identity_exposed"
      | "response_attribution_incomplete"
...
export type PromptImprovementIdentityReviewDecision =
  | { status: "authorized"; receipt: PromptImprovementIdentityReviewReceipt }
  | Extract<PromptImprovementIdentityAuditDecision, { status: "blocked" }>
  | { status: "blocked"; reasonCode: "identity_review_lineage_invalid" }

export type ProductIdentityInvariantProjectionDecision =
  | { status: "authorized"; review: PlatformPromptInvariantReview }
  | { status: "blocked"; reasonCode: "identity_review_receipt_invalid" | "identity_review_expired" | "identity_review_scope_mismatch" | "goal_section3_lineage_mismatch" }

const IDENTITY_FIELDS = ["agentId", "agentName"] as const
...
function hasOnlyIdentityFields(value: PromptImprovementAgentIdentity): boolean {
  const keys = Object.keys(value).sort()
  return keys.length === IDENTITY_FIELDS.length && IDENTITY_FIELDS.every((field) => keys.includes(field))
}

export function auditPromptImprovementIdentitySnapshot(
  ...
  if (!configuredMainAgentId) return { status: "blocked", reasonCode: "main_agent_identity_invalid" }
  if (snapshot.agents.length === 0) return { status: "blocked", reasonCode: "agent_identity_invalid" }

  const ids = new Set<string>()
  const names = new Set<string>()
  const agents = new Map<string, string>()
  const normalizedAgentNames: string[] = []
  for (const agent of snapshot.agents) {
    if (!hasOnlyIdentityFields(agent)) return { status: "blocked", reasonCode: "agent_identity_field_invalid" }
    const agentId = exact(agent.agentId)
    const agentName = exact(agent.agentName)
    if (!agentId || !agentName || ids.has(agentId)) return { status: "blocked", reasonCode: "agent_identity_invalid" }
    const normalized = normalizedName(agentName)
    if (names.has(normalized)) return { status: "blocked", reasonCode: "agent_name_duplicate" }
    ids.add(agentId)
  ...
  if (!mainAgentName) return { status: "blocked", reasonCode: "main_agent_identity_invalid" }
  ...
  if (mainAgentName !== effectiveMainAgentName) return { status: "blocked", reasonCode: "main_agent_name_mismatch" }
  ...
  for (const attribution of snapshot.responseAttributions) {
    if (!hasOnlyIdentityFields(attribution)) return { status: "blocked", reasonCode: "response_attribution_mismatch" }
  ...
  if (attributed.size !== agents.size) return { status: "blocked", reasonCode: "response_attribution_incomplete" }
...
export function createPromptImprovementIdentityReview(input: {
  ...
  if (audit.status === "blocked") return audit
...
export function projectProductIdentityInvariantReview(input: {
  ...
  if (receipt.expiresAt <= input.now) return { status: "blocked", reasonCode: "identity_review_expired" }
