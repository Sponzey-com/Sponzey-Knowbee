...
export const PROMPT_ROLLBACK_SOURCE_MANIFEST: readonly PromptRollbackSourceManifestEntry[] = [
...
export type PromptRollbackSourceIssueCode =
...
const manifestByType = new Map(
  PROMPT_ROLLBACK_SOURCE_MANIFEST.map((entry) => [entry.sourceType, entry]),
)

export function validatePromptImprovementRollbackSource(
  source: Partial<PromptImprovementRollbackSource>,
  pathPrefix = "",
): PromptImprovementRollbackSourceValidationResult {
  const issues: PromptRollbackSourceIssue[] = []
