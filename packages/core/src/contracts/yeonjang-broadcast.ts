...
export type YeonjangBroadcastToolName =
  | "screen_capture"
  | "screen_find_text"
  | "shell_exec"
  | "mouse_action"
...
export type YeonjangBroadcastIntentValidationResult =
  | { ok: true; value: YeonjangBroadcastIntent; issues: [] }
  | { ok: false; issues: YeonjangBroadcastIntentValidationIssue[] }

export type YeonjangBroadcastRetryReceiptValidationResult =
  | { ok: true; value: YeonjangBroadcastRetryReceipt; issues: [] }
  | { ok: false; issues: YeonjangBroadcastIntentValidationIssue[] }
...
export function validateYeonjangBroadcastIntent(value: unknown): YeonjangBroadcastIntentValidationResult {
  const issues: YeonjangBroadcastIntentValidationIssue[] = []
  if (!isRecord(value)) {
    return {
      ok: false,
  ...
  return {
    ok: true,
    value: normalizeYeonjangBroadcastIntent(value as unknown as YeonjangBroadcastIntent),
    issues: [],
  }
}

export function normalizeYeonjangBroadcastIntent(intent: YeonjangBroadcastIntent): YeonjangBroadcastIntent {
  return {
    confirm: true,
...
export function buildYeonjangBroadcastIntentSchemaProperty(): Record<string, unknown> {
  return {
    type: "object",
    properties: {
      confirm: {
...
export function normalizeYeonjangBroadcastRetryReceipt(
  receipt: YeonjangBroadcastRetryReceipt,
): YeonjangBroadcastRetryReceipt {
  const targetReceipts = Array.isArray(receipt.targetReceipts)
    ? receipt.targetReceipts
      .map((item) => ({
        ...
        status: normalizeString(item?.status) as YeonjangBroadcastRetryTargetStatus,
...
export function validateYeonjangBroadcastRetryReceipt(
  value: unknown,
): YeonjangBroadcastRetryReceiptValidationResult {
  if (value == null) {
  ...
  return {
    ok: true,
    value: normalizeYeonjangBroadcastRetryReceipt(value as YeonjangBroadcastRetryReceipt),
    issues: [],
  }
}

export function buildYeonjangBroadcastRetryReceiptSchemaProperty(): Record<string, unknown> {
  return {
    type: "object",
    properties: {
...
export function isYeonjangBroadcastSelector(selector: YeonjangTargetSelector): boolean {
  return selector.type === "all_online" || selector.type === "filtered_group"
}

export function validateYeonjangBroadcastSelector(value: unknown): {
  ok: true
