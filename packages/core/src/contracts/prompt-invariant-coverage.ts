...
} from "./prompt-improvement-application-gate.js"
...
export type PromptInvariantCoverageReasonCode =
  | "impact_scope_mismatch"
  | "invariant_review_missing"
  | "invariant_review_duplicate"
  | "invariant_not_preserved"
  | "invariant_review_scope_mismatch"
  | "goal_section3_lineage_mismatch"
  | "invariant_review_expired"
  | "application_review_missing"
  | "application_review_duplicate"
  | "harness_projection_missing"
  | "harness_projection_orphan"
  | "goal_section3_conflict"
  | "common_policy_final_review_missing"
  | "common_policy_final_review_blocked"
  | "common_policy_final_review_scope_mismatch"
  | "agent_ownership_review_missing"
...
const REQUIRED_INVARIANT_BY_IMPACT: Record<PromptImprovementImpactKind, PlatformPromptProtectedInvariant> = {
...
function impacts(values: PromptImprovementImpactKind[]): PromptImprovementImpactKind[] | undefined {
  if (values.length === 0 || new Set(values).size !== values.length) return undefined
  if (values.some((value) => !PROMPT_IMPROVEMENT_IMPACT_KINDS.includes(value))) return undefined
...
function validateRuleSnapshots(input: {
...
}): { corrections: GoalInvariantProjectionCorrection[] } | PromptInvariantCoverageReasonCode {
  const canonical = new Map<string, GoalProductInvariantRuleSnapshot>()
  for (const rule of input.canonicalRules) {
    ...
    if (canonical.has(ruleId)) return "harness_projection_orphan"
    canonical.set(ruleId, rule)
  }
  if (canonical.size === 0) return "harness_projection_missing"

  const projectionsByCanonical = new Map<string, HarnessInvariantProjectionRuleSnapshot[]>()
  ...
  for (const projection of input.harnessProjectionRules) {
    ...
    if (projectionIds.has(projectionRuleId) || !canonical.has(canonicalRuleId)) return "harness_projection_orphan"
  ...
  const corrections: GoalInvariantProjectionCorrection[] = []
...
export function authorizePromptInvariantCoverage(input: {
  ...
  if (expiresAt <= reviewedAt) return { status: "blocked", reasonCode: "invariant_review_expired" }
  ...
  const coverageByImpact = new Map<PromptImprovementImpactKind, PromptInvariantCoverageEvidence>()
  for (const evidence of input.coverage) {
    if (!analyzed.includes(evidence.impact)) return { status: "blocked", reasonCode: "impact_scope_mismatch" }
    if (coverageByImpact.has(evidence.impact)) return { status: "blocked", reasonCode: "invariant_review_duplicate" }
  ...
  const applicationReviews = new Map<PlatformPromptProtectedInvariant, PlatformPromptInvariantReview>()
  const coverageEvidenceRefs: string[] = []
  ...
  if (typeof ruleValidation === "string") return { status: "blocked", reasonCode: ruleValidation }
  ...
  let mainReviewId: string | undefined
  if (input.ownershipMode === "common_policy") {
    ...
    if (!decision) return { status: "blocked", reasonCode: "common_policy_final_review_missing" }
    ...
    if (!ownershipReview) return { status: "blocked", reasonCode: "agent_ownership_review_missing" }
