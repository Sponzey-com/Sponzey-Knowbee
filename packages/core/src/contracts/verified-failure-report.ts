...
export function buildVerifiedFailureReportFacts(input: {
  ...
  if (failedScope.length === 0) throw new Error("Failure report requires at least one failed scope.")
  if (evidenceRefs.length === 0) throw new Error("Failure report requires verified failure evidence.")
  ...
  if (nextActions.length === 0) throw new Error("Failure report requires at least one verified next action.")
