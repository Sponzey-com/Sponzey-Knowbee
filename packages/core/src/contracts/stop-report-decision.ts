export type AttemptLimitPolicy =
  | { kind: "bounded"; maxTurns: number; maxRetries: number; policyVersion: string }
  | { kind: "unbounded"; policyVersion: string }
  | { kind: "strategy_guarded"; policyVersion: string }
...
export function normalizeStartupAttemptLimitPolicy(input: {
  ...
  if (maxTurns === 0) return { kind: "unbounded", policyVersion }
...
export type StopReportDecision =
  | { status: "continue"; nextTurn: number }
  | {
  ...
  | { status: "blocked_pending_input"; reasonCode: "completion_evidence_incomplete"; missingCriterionIds: string[] }
  | { status: "stop_and_report"; reasonCode: StopReportInput["reasonCode"]; reportInput: StopReportInput }
...
export type BlockedStopReportDecision =
  | { status: "continue"; reasonCode: "solution_paths_remaining"; remainingPathIds: string[] }
  | { status: "blocked_pending_input"; reasonCode: "recoverable_condition"; requiredChanges: string[] }
  | { status: "stop_and_report"; reasonCode: BlockedStopReportInput["reasonCode"]; reportInput: BlockedStopReportInput }
...
export function evaluateBlockedStopReportDecision(input: {
  ...
  if (unresolvedItemIds.length === 0) throw new Error("Blocked stop requires unresolved items.")

  const exhaustionEvidence = unique(input.exhaustion.evidenceRefs, "Solution-path evidence")
  if (exhaustionEvidence.length === 0) throw new Error("Exhausted solution paths require evidence.")
  ...
  if (input.permissionDenial) {
    ...
    if (permissionEvidence.length === 0) throw new Error("Permission denial requires evidence.")
  ...
  if (input.impossibility) {
    ...
    if (facts.length === 0 || evidence.length === 0) throw new Error("Concrete impossibility requires verified facts and evidence.")
    const requiredChanges = unique(input.impossibility.requiredChanges, "Required change")
    if (input.impossibility.recoverable) {
      if (requiredChanges.length === 0) throw new Error("Recoverable impossibility requires at least one required change.")
...
export function evaluateStopReportDecision(input: {
  ...
  attempts: { currentTurn: number; currentRetry: number }
  ...
  if (input.policy.kind === "bounded") {
    if (!Number.isSafeInteger(input.policy.maxTurns) || input.policy.maxTurns <= 0) throw new Error("maxTurns must be a positive integer.")
    if (!Number.isSafeInteger(input.policy.maxRetries) || input.policy.maxRetries < 0) throw new Error("maxRetries must be a non-negative integer.")
  }
...
export async function executeContinuingAction<T>(input: {
  decision: StopReportDecision | BlockedStopReportDecision
  execute: () => Promise<T>
}): Promise<
  | { status: "executed"; result: T }
  | Exclude<StopReportDecision | BlockedStopReportDecision, { status: "continue" }>
> {
  if (input.decision.status !== "continue") return input.decision
