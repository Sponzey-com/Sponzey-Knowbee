...
export type StructuredWorkLifecyclePhase =
  | "input"
  | "decision"
  | "execution"
  | "validation"
...
function requireUniqueText(values: string[], field: string): string[] {
...
function classify(complexity: StructuredWorkComplexitySignals): StructuredWorkClassification {
...
function validateSteps(
  ...
  return steps.map((step) => {
    ...
    if (ids.has(stepId)) throw new Error("Step IDs must be unique.")
    ids.add(stepId)
    ...
    if (inputRefs.length === 0) throw new Error("Step input references require at least one value.")
    if (step.status !== "pending") throw new Error("A new lifecycle plan requires pending steps.")
    return {
      ...step,
      step_id: stepId,
...
function requestPlanningStates(): DiagnosisRoutingState[] {
  const states: DiagnosisRoutingState[] = ["received"]
  states.push(
    transitionDiagnosisRouting(states.at(-1) as DiagnosisRoutingState, "diagnosis_requested"),
  )
  states.push(
    transitionDiagnosisRouting(states.at(-1) as DiagnosisRoutingState, "request_diagnosed"),
  )
  states.push(transitionDiagnosisRouting(states.at(-1) as DiagnosisRoutingState, "route_selected"))
  return states
}
...
export function planStructuredWorkLifecycle(input: {
  ...
  if (!solutionPlanReceipt) throw new Error("solution plan receipt missing")
  const requestIntent = requireText(input.diagnosis.intent, "Request diagnosis intent")
  const missingInformation = requireUniqueText(
    input.diagnosis.missing_information,
...
function validateStepResults(
  plan: StructuredWorkPlanDecision,
  results: StructuredWorkLifecycleStepResult[],
): { stepIds: string[]; outputRefs: string[]; evidenceRefs: string[] } {
  ...
  for (const result of results) {
    ...
    if (evidence.length === 0) throw new Error("Each step result requires evidence.")
...
function finalLifecycleState(
  states: DiagnosisRoutingState[],
  action: LlmResultDiagnosisRecord["recommended_action"],
): { state: DiagnosisRoutingState; status: StructuredWorkLifecycleProjection["status"] } {
  const current = states.at(-1) as DiagnosisRoutingState
...
export function projectStructuredWorkLifecycle(input: {
  ...
  states.push(
    transitionDiagnosisRouting(states.at(-1) as DiagnosisRoutingState, "execution_started"),
  )
  states.push(
    transitionDiagnosisRouting(states.at(-1) as DiagnosisRoutingState, "execution_result_received"),
  )
  states.push(
    transitionDiagnosisRouting(states.at(-1) as DiagnosisRoutingState, "result_diagnosed"),
  )
  states.push(
    transitionDiagnosisRouting(states.at(-1) as DiagnosisRoutingState, "next_action_selected"),
  )
