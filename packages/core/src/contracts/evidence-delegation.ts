...
export type DelegationBenefitKind =
  | "specialty"
  | "parallelism"
  | "independent_review"
  | "verification"
...
export interface EvidenceBasedDelegationInput {
  ...
  baseEligibility: { state: "eligible" | "rejected"; reasonCodes: string[] }
...
function validCost(value: number, field: string): void {
  if (!Number.isFinite(value) || value < 0) throw new Error(`${field} must be a non-negative finite number.`)
}

function isInternalAgentName(value: string): boolean {
  return /^(?:agent|team|session|sub_session):/i.test(value)
}

function rejected(input: EvidenceBasedDelegationInput, reasonCode: string): EvidenceBasedDelegationDecision {
  return { outcome: "rejected", reasonCode, benefitKinds: [], targetAgentName: input.targetAgentName.trim() }
}

export function decideEvidenceBasedDelegation(input: EvidenceBasedDelegationInput): EvidenceBasedDelegationDecision {
  ...
  if (isInternalAgentName(parentAgentName) || isInternalAgentName(targetAgentName)) return rejected(input, "user_facing_name_required")
  if (parentAgentName === targetAgentName) return rejected(input, "self_delegation_denied")
  if (!input.targetIsDirectChild) return rejected(input, "target_not_direct_child")
  if (!input.targetActive) return rejected(input, "target_inactive")
  ...
  const benefitKinds: DelegationBenefitKind[] = []
  for (const benefit of input.benefits) {
    ...
    if (!benefitKinds.includes(benefit.kind)) benefitKinds.push(benefit.kind)
  }
...
function requireExplicitContextRefs(values: string[]): string[] {
...
function validationError(label: string, issues: Array<{ path: string; message: string }>): Error {
...
export function createStructuredDelegationHandoff(input: {
  ...
  if (input.decision.outcome !== "delegate") throw new Error("A delegate decision is required before handoff creation.")
  const parentValidation = validateWorkRecord(input.parentRecord)
  if (!parentValidation.ok) throw validationError("Parent work record", parentValidation.issues)
  ...
  if (!parentStep) throw new Error("Parent step ID must reference the parent WorkRecord step plan.")
  if (input.decision.targetAgentName === parent.owner_agent_name) throw new Error("Delegation target must differ from parent agent name.")
  const childWorkId = requireText(input.childWorkId, "Child work ID")
  if (childWorkId === parent.work_id) throw new Error("Child work ID must differ from parent work ID.")
  const targetStep: WorkStepPlanItem = {
    ...structuredClone(parentStep),
    owner_agent_name: input.decision.targetAgentName,
    status: "pending",
  }
  const handoff: WorkHandoffPackage = {
    schemaVersion: 1,
  ...
  if (!validation.ok) throw validationError("Work handoff package", validation.issues)
...
export type StructuredChildResultMergeFailureReason =
  | "invalid_parent_record"
  | "invalid_handoff"
  | "invalid_child_result"
  | "linkage_mismatch"
  | "duplicate_child_result"
  | "invalid_merged_record"
...
function childStepProjection(status: ChildWorkResult["status"]): {
  stepStatus: WorkStepPlanItem["status"]
  resultStatus: WorkRecord["step_results"][number]["status"]
} {
  if (status === "completed") return { stepStatus: "completed", resultStatus: "completed" }
  if (status === "partial") return { stepStatus: "running", resultStatus: "partial" }
  if (status === "blocked") return { stepStatus: "blocked", resultStatus: "blocked" }
...
export function mergeStructuredChildResultIntoParent(input: {
  ...
  try {
    ...
    return mergeFailure(
      "linkage_mismatch",
      "$.childResult",
      error instanceof Error ? error.message : "Child result linkage validation failed.",
    )
  }
  if (parent.step_results.some((result) => result.step_id === handoff.parent_step_id)) {
    return mergeFailure(
      "duplicate_child_result",
      "$.parentRecord.step_results",
      "The parent step already has a result and cannot merge the child result twice.",
    )
  }
  ...
  const {
    failure_diagnosis: _previousFailureDiagnosis,
    recovery_candidates: _previousRecoveryCandidates,
    selected_recovery_action: _previousSelectedRecoveryAction,
    ...parentBase
...
export function validateStructuredDelegationRoundTrip(input: {
  ...
  if (!parentValidation.ok) throw validationError("Parent work record", parentValidation.issues)
  const handoffValidation = validateWorkHandoffPackage(input.handoff)
  if (!handoffValidation.ok) throw validationError("Work handoff package", handoffValidation.issues)
  const childValidation = validateChildWorkResult(input.childResult)
  if (!childValidation.ok) throw validationError("Child work result", childValidation.issues)
  const parent = parentValidation.value
  const handoff = handoffValidation.value
  const child = childValidation.value
  if (handoff.parent_work_id !== parent.work_id) throw new Error("Handoff parent work ID does not match parent WorkRecord.")
  if (handoff.parent_agent_name !== parent.owner_agent_name) throw new Error("Handoff parent agent name does not match parent WorkRecord.")
  const parentStep = parent.step_plan.find((step) => step.step_id === handoff.parent_step_id)
  if (!parentStep) throw new Error("Handoff parent step does not exist in parent WorkRecord.")
  if (child.work_id !== handoff.work_id) throw new Error("Child work ID does not match handoff work ID.")
  if (child.agent_name !== handoff.target_agent_name) throw new Error("Child agent name does not match handoff target agent name.")
  ...
  for (const stepId of [...child.completed_steps, ...child.failed_steps]) {
    if (!handoffSteps.has(stepId)) throw new Error("Child result step does not exist in the handoff step plan.")
  }
  const evidenceRefs = uniqueText(child.evidence, "Child result evidence")
  if (evidenceRefs.length === 0) throw new Error("Child result requires evidence before parent review.")
  return {
    ok: true,
    parentWorkId: parent.work_id,
    childWorkId: child.work_id,
