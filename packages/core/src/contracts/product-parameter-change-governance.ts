...
const CANONICAL_PROMPT_BY_PARAMETER: Record<ProductParameterKey, string> = {
...
export type ProductParameterChangeReasonCode =
  | "parameter_key_invalid"
  | "parameter_value_invalid"
  | "parameter_value_unchanged"
  | "decision_approval_invalid"
  | "change_source_invalid"
  | "canonical_prompt_source_mismatch"
  | "test_fixture_invalid"
  | "change_revision_mismatch"
  | "runtime_activation_invalid"

export type ProductParameterChangeDecision =
  | { status: "authorized"; receipt: ProductParameterChangeReceipt }
  | { status: "blocked"; reasonCode: ProductParameterChangeReasonCode }

function exact(value: string | undefined): string {
  return value?.trim() ?? ""
}

function validSource(source: ProductParameterChangeSourceReceipt): boolean {
  return Boolean(exact(source.sourceRef) && exact(source.revisionFingerprint) && exact(source.evidenceRef))
}

function cloneSource(source: ProductParameterChangeSourceReceipt): ProductParameterChangeSourceReceipt {
...
export function authorizeProductParameterChange(
  input: ProductParameterChangeInput,
...
export async function applyAuthorizedProductParameterChange<T>(input: {
  decision: ProductParameterChangeDecision
  apply: (receipt: ProductParameterChangeReceipt) => Promise<T> | T
}): Promise<
  | { status: "applied"; receipt: ProductParameterChangeReceipt; result: T }
  | { status: "blocked"; reasonCode: "product_parameter_change_not_authorized" }
> {
