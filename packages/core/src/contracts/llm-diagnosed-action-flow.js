...
function receiptReferenced(projection, reasonCode, receiptId) {
    return projection.trace.some((event) => event.reasonCode === reasonCode && event.referenceIds.includes(receiptId));
}
function traceReasonIndex(reasonCodes, expected) {
    return expected.endsWith("_")
        ? reasonCodes.findIndex((reason) => reason.startsWith(expected))
        : reasonCodes.indexOf(expected);
}
export function decideLlmDiagnosedActionFlowAcceptance(input) {
    const workId = normalized(input.workId);
    const runId = normalized(input.runId);
    const issues = [];
    if (!workId)
        addIssue(issues, "work_id_required", "$.workId");
    if (!runId)
        addIssue(issues, "run_id_required", "$.runId");
    const request = input.requestDiagnosis;
    if (!request) {
        addIssue(issues, input.rawInputRefs.length > 0 ? "raw_input_not_authoritative" : "request_diagnosis_missing", "$.requestDiagnosis");
    }
    else {
        ...
        if (!receiptReferenced(input.projection, "request_diagnosis_received", request.receipt.receiptId)) {
            addIssue(issues, "request_receipt_reference_mismatch", "$.projection.trace");
        }
    }
    const result = input.resultDiagnosis;
    if (!result) {
        addIssue(issues, input.rawResultRefs.length > 0 ? "raw_result_not_authoritative" : "result_diagnosis_missing", "$.resultDiagnosis");
    }
    else {
        ...
        if (!receiptReferenced(input.projection, "result_diagnosis_received", result.receipt.receiptId)) {
    ...
    if (!receiptReferenced(input.projection, "solution_plan_received", input.plan.solutionPlanReceiptId)) {
    ...
    const requiredReasons = input.plan.classification === "simple"
    ...
    else if (indexes.some((index, position) => position > 0 && index <= (indexes[position - 1] ?? -1))) {
        addIssue(issues, "trace_phase_order_invalid", "$.projection.trace");
    }
    const selectedActionRecorded = input.plan.classification === "simple"
...
//# sourceMappingURL=llm-diagnosed-action-flow.js.map