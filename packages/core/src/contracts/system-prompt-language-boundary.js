...
export function validateSystemPromptLanguageSource(source) {
    const sourceId = required(source.sourceId, "Prompt source ID");
    if (source.sourceClassification !== "system_prompt")
        return { status: "not_applicable", sourceId };
    ...
    if (expectedChecksum !== actualChecksum)
        return { status: "blocked", reasonCode: "checksum_mismatch" };
    if (source.segments.length === 0)
        return { status: "blocked", reasonCode: "segment_missing" };
    const ids = new Set();
    for (const segment of source.segments) {
        ...
        if (ids.has(segmentId))
            return { status: "blocked", reasonCode: "segment_duplicate" };
        ids.add(segmentId);
        if (!SYSTEM_PROMPT_SEGMENT_KINDS.includes(segment.kind))
            return { status: "blocked", reasonCode: "segment_unclassified" };
    ...
    for (const segment of source.segments) {
        if (segment.kind !== "product_name_literal" && segment.kind !== "user_input_example_literal")
            continue;
        const expectedPurpose = segment.kind === "product_name_literal" ? "korean_product_name" : "korean_user_input_example";
        if (segment.literalPurpose !== expectedPurpose)
            return { status: "blocked", reasonCode: "literal_purpose_invalid" };
        ...
        if (!surrounding || surrounding.kind !== "instruction")
            return { status: "blocked", reasonCode: "literal_binding_invalid" };
...
export async function registerLanguageEligibleSystemPrompt(input) {
    if (input.decision.status !== "eligible")
        return input.decision;
    return { status: "registered", result: await input.register(input.decision) };
}
//# sourceMappingURL=system-prompt-language-boundary.js.map