...
import { validateWebDocument, type WebDocument } from "./web-retrieval.js"
...
export type WebDocumentChunkResult =
  | Readonly<{ ok: true; value: readonly WebDocumentChunk[] }>
  | Readonly<{
      ok: false
      reasonCode:
        | "web_chunk_document_invalid"
        | "web_chunk_budget_fingerprint_invalid"
        | "web_chunk_estimator_invalid"
        | "web_chunk_content_unbreakable"
    }>
...
function estimate(estimator: TokenEstimatorPort, text: string): number | null {
  try {
...
function headingEvents(markdown: string): readonly HeadingEvent[] {
...
function preferredParagraphEnds(markdown: string): ReadonlySet<number> {
  ...
  for (const match of markdown.matchAll(separator)) {
    ...
    if (rawEnd - trailingWhitespace > start) ends.add(rawEnd - trailingWhitespace)
  ...
  if (finalEnd > start) ends.add(finalEnd)
  return ends
}

function pathAt(events: readonly HeadingEvent[], offset: number): readonly string[] {
  let path: readonly string[] = Object.freeze([])
...
export function chunkWebDocument(
  ...
  while (wordIndex < words.length) {
    ...
    while (scan < words.length) {
      ...
      if (tokenCount > MAX_CHUNK_TOKENS) break
    ...
    for (let candidate = maximumWordIndex; candidate >= wordIndex; candidate -= 1) {
      ...
      if (!paragraphEnds.has(end)) continue
