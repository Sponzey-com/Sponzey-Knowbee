 ...
 */
export function fencePersistedYeonjangMqttV2LivenessAtStartup(input) {
    ...
    const candidates = db
        .prepare("SELECT instance_id, protocol_version, transport_json FROM yeonjang_instances")
        .all()
        .filter((candidate) => {
        const transport = parseJson(candidate.transport_json);
    ...
    const transaction = db.transaction(() => {
        for (const candidate of candidates) {
            const instanceWrite = db.prepare(`UPDATE yeonjang_instances
         ...
         WHERE instance_id = ?
           AND (connection_state <> 'offline'
             ...
             OR method_count <> 0)`).run(input.observedAt, candidate.instance_id);
            const sessionWrite = db.prepare(`UPDATE yeonjang_instance_sessions
         SET client_id = NULL,
         ...
         WHERE session_id = (
           ...
           AND (client_id IS NOT NULL
             ...
             OR ended_at IS NULL)`).run(input.observedAt, input.observedAt, input.observedAt, candidate.instance_id);
