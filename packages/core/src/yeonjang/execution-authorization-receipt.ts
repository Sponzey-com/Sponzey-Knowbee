import type {
  YeonjangExecutionAdmissionKeyPort,
...
export type YeonjangExecutionAuthorizationIssueResult =
  ...
  | {
      readonly ok: false
      readonly reasonCode:
        | "execution_authorization_input_invalid"
        | "execution_authorization_key_unavailable"
        | "execution_authorization_expired"
        | "execution_authorization_proof_invalid"
    }
...
export function createYeonjangExecutionAuthorizationIssuer(input: {
  ...
  return Object.freeze({
    issue: (
      request: Parameters<YeonjangExecutionAuthorizationIssuerPort["issue"]>[0],
    ): YeonjangExecutionAuthorizationIssueResult => {
...
export function canonicalizeYeonjangAuthorizationReceipt(
  input: Omit<YeonjangAuthorizationReceipt, "proof">,
): string {
  return [
