...
export type YeonjangSideEffectGoalValidationCandidateReasonCode =
  | "operation_id_missing"
  | "operation_not_found"
  | "operation_run_scope_mismatch"
  | "operation_work_scope_mismatch"
  | "operation_not_manual"
...
export function loadYeonjangSideEffectGoalValidationCandidate(input: {
  ...
  if (!operationId) return { status: "not_ready", reasonCode: "operation_id_missing" }
  const row = input.db
    .prepare<[string], OperationScopeRow>(
      "SELECT operation_id, scope_id FROM side_effect_operations WHERE operation_id = ?",
    )
    .get(operationId)
  if (!row) return { status: "not_ready", reasonCode: "operation_not_found" }
