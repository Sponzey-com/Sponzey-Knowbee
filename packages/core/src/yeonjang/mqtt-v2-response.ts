...
export type YeonjangMqttV2TerminalAdmission =
  | { readonly ok: true; readonly terminal: YeonjangMqttV2TerminalResult }
  | { readonly ok: false; readonly reasonCode:
      | "yeonjang_v2_response_payload_invalid"
      | "yeonjang_v2_response_identity_mismatch"
      | "yeonjang_v2_response_expired"
      | "yeonjang_v2_response_signature_rejected" }
...
export function admitYeonjangMqttV2TerminalResponse(input: {
  ...
  if (!envelope) return { ok: false, reasonCode: "yeonjang_v2_response_payload_invalid" }
  ...
  if (
    envelope.response_digest !== responseDigest
...
interface TerminalEnvelope {
  ...
  readonly schema_id: "yeonjang.response.v2"
  ...
  readonly payload: {
    ...
    readonly target_fingerprint: string
    readonly idempotency_key: string
    readonly target_scope_digest: string
    readonly terminal: {
      ...
      readonly operation_id: string
      readonly requester_id: string
      readonly target: { readonly platform: string; readonly instance_id: string; readonly session_id: string; readonly fingerprint: string }
      readonly method: "camera.capture" | "screen.capture"
      readonly resource: "camera" | "screen"
      readonly idempotency_key: string
      readonly binding_digest: string
      readonly execution_outcome: YeonjangMqttV2TerminalResult["executionOutcome"]
      readonly delivery_outcome: YeonjangMqttV2TerminalResult["deliveryOutcome"]
      readonly terminal_revision: number
      readonly failure?: Readonly<Record<string, unknown>>
    }
    readonly artifact?: YeonjangMqttV2ArtifactDescriptor
  }
...
function parseTerminalEnvelope(value: unknown): TerminalEnvelope | null {
  ...
  ]) || value.protocol_version !== 2 || value.schema_id !== "yeonjang.response.v2" || value.message_kind !== "response") return null
  ...
  ]) || value.payload.schema_version !== 3 || !isRecord(value.payload.terminal)) return null
  ...
  if (!exact(terminal, [...terminalKeys, ...(terminal.failure === undefined ? [] : ["failure"])]) || terminal.schema_version !== 1 || !isRecord(terminal.target)) return null
  if (!isRecord(value.authorization) || !exact(value.authorization, ["schema_version", "issuer", "key_id", "audience", "scope", "requester_id", "request_id", "command_id", "operation_id", "target_instance_id", "target_session_id", "target_fingerprint", "idempotency_key", "expires_at", "nonce", "signature"])) return null
  ...
  if (!strings.every(isBoundedText) || !Number.isSafeInteger(value.issued_at) || !Number.isSafeInteger(value.expires_at) || !Number.isSafeInteger(value.sequence) || Number(value.sequence) < 1) return null
  const auth = value.authorization
  if (auth.schema_version !== 1 || auth.scope !== "response.publish" || !isLowerHex(auth.signature) || !Number.isSafeInteger(auth.expires_at)) return null
  if (!isBoundedText(auth.issuer) || !isBoundedText(auth.key_id) || !isBoundedText(auth.audience) || !isBoundedText(auth.nonce)) return null
  ...
  if (!outcomes.includes(String(terminal.execution_outcome)) || !delivery.includes(String(terminal.delivery_outcome)) || !Number.isSafeInteger(terminal.terminal_revision) || Number(terminal.terminal_revision) < 1 || Number(value.sequence) !== Number(terminal.terminal_revision)) return null
  if (terminal.execution_outcome !== "succeeded" && !isRecord(terminal.failure)) return null
  if (value.payload.artifact !== undefined && !parseArtifact(value.payload.artifact)) return null
  if (auth.issuer !== value.target_instance_id || auth.audience !== value.requester_id || auth.expires_at !== value.expires_at) return null
  return value as unknown as TerminalEnvelope
}

function parseArtifact(value: unknown): value is YeonjangMqttV2ArtifactDescriptor {
  if (!isRecord(value) || !exact(value, ["schemaVersion", "artifactRef", "kind", "mediaType", "sizeBytes", "fullDigest", "createdAtMs", "expiresAtMs", "lifecycleRevision"])) return false
...
function contentIdentityMatches(content: TerminalEnvelope["payload"], envelope: TerminalEnvelope): boolean {
...
function responseSigningBytes(envelope: TerminalEnvelope, payloadDigest: Buffer): Buffer {
  const output: Buffer[] = []
  ...
  for (const [name, value] of values) append(output, name, Buffer.isBuffer(value) ? value : typeof value === "number" ? u64(value) : Buffer.from(value))
  return Buffer.concat(output)
}

function append(output: Buffer[], name: string, value: Buffer): void {
