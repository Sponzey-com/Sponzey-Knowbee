...
export function admitYeonjangMqttV2ResponseAckResult(input) {
    ...
    try {
        ...
        return { ok: false, reasonCode: "yeonjang_v2_response_ack_result_invalid" };
    }
    if (!record(value)
    ...
    if ((payload.outcome !== "accepted" && payload.outcome !== "duplicate")
        ...
        || auth.scope !== "response.ack.result" || auth.issuer !== expected.enrollment.instanceId || auth.audience !== expected.enrollment.requesterId || typeof auth.signature !== "string" || !/^[0-9a-f]{64}$/u.test(auth.signature))
        return { ok: false, reasonCode: "yeonjang_v2_response_ack_result_invalid" };
    ...
    for (const [name, nested] of [["domain", "yeonjang.response-ack-result.authorization.v2"], ["payload_sha256", payloadDigest], ...["message_id", "request_id", "command_id", "operation_id", "correlation_id", "causation_id", "requester_id", "target_instance_id", "target_session_id", "target_fingerprint", "idempotency_key"].map((key) => [key, value[key]]), ["authorization_issuer", auth.issuer], ["authorization_key_id", auth.key_id], ["authorization_audience", auth.audience], ["authorization_scope", auth.scope], ["authorization_nonce", auth.nonce], ["issued_at", value.issued_at], ["expires_at", value.expires_at], ["sequence", value.sequence]])
        field(bytes, name, Buffer.isBuffer(nested) ? nested : typeof nested === "number" ? u64(nested) : Buffer.from(String(nested)));
    ...
    if (observed.length !== calculated.length || !timingSafeEqual(observed, calculated))
        return { ok: false, reasonCode: "yeonjang_v2_response_ack_result_signature_rejected" };
...
function exact(value, keys) { const actual = Object.keys(value); return actual.length === keys.length && keys.every((key) => Object.hasOwn(value, key)); }
function isBoundedText(value) { return typeof value === "string" && value.trim().length > 0 && Buffer.byteLength(value, "utf8") <= 256; }
function signingBytes(envelope, ack) {
    const payloadParts = [];
    for (const [name, value] of [
        ...
        field(payloadParts, name, typeof value === "number" ? u64(value) : Buffer.from(String(value)));
    ...
    for (const [name, value] of values)
        field(output, name, Buffer.isBuffer(value) ? value : typeof value === "number" ? u64(value) : Buffer.from(String(value)));
    return Buffer.concat(output);
}
function field(output, name, value) { const key = Buffer.from(name); output.push(u64(key.length), key, u64(value.length), value); }
function u64(value) { const bytes = Buffer.alloc(8); bytes.writeBigUInt64BE(BigInt(value)); return bytes; }
//# sourceMappingURL=mqtt-v2-response-ack.js.map