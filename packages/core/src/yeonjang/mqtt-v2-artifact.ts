...
import { type YeonjangMqttV2Enrollment, buildYeonjangMqttV2Topics } from "./mqtt-v2-contract.js"
...
export function createYeonjangMqttV2ArtifactControl(input: {
  ...
  readonly descriptor: { readonly artifactRef: string; readonly fullDigest: string; readonly lifecycleRevision: number }
...
}): { readonly topic: string; readonly envelope: YeonjangMqttV2ArtifactControlEnvelope } {
  ...
  const envelope: YeonjangMqttV2ArtifactControlEnvelope = {
...
function validateArtifactControl(envelope: YeonjangMqttV2ArtifactControlEnvelope, key: Uint8Array): void {
...
function artifactSigningBytes(envelope: YeonjangMqttV2ArtifactControlEnvelope): Buffer {
  const payload = envelope.payload as { artifact: string; params: Record<string, unknown> }
  const params = payload.params
  const payloadBytes: Buffer[] = []
  appendField(payloadBytes, "artifact", Buffer.from(payload.artifact))
  for (const name of ["artifact_ref", "owner_request_id", "owner_operation_id", "transfer_id"] as const) appendField(payloadBytes, name, Buffer.from(String(params[name])))
  ...
  const output: Buffer[] = []
  const values: ReadonlyArray<readonly [string, string | number | Buffer]> = [
  ...
  for (const [name, value] of values) appendField(output, name, Buffer.isBuffer(value) ? value : typeof value === "number" ? u64(value) : Buffer.from(value))
...
function appendOptionalText(output: Buffer[], name: string, value: string | null): void {
  ...
  if (value !== null) appendField(output, name, Buffer.from(value))
}
function appendOptionalU32(output: Buffer[], name: string, value: number | null): void {
  appendField(output, `${name}_present`, u64(value === null ? 0 : 1))
  ...
  if (value !== null) appendField(output, name, u64(value))
}
function appendField(output: Buffer[], name: string, value: Buffer): void {
  const key = Buffer.from(name); output.push(u64(key.length), key, u64(value.length), value)
}
function u64(value: number): Buffer {
  const bytes = Buffer.alloc(8); bytes.writeBigUInt64BE(BigInt(value)); return bytes
}

export type YeonjangMqttV2ArtifactFetchRejectReason =
  | "missing"
  | "wrong_owner"
  | "wrong_transfer"
  | "revision_conflict"
...
export type YeonjangMqttV2ArtifactFetchRejectionAdmission =
  | {
      readonly ok: true
      readonly rejection: { readonly reason: YeonjangMqttV2ArtifactFetchRejectReason }
    }
  | {
      readonly ok: false
      readonly reasonCode:
        | "yeonjang_v2_artifact_fetch_payload_invalid"
        | "yeonjang_v2_artifact_fetch_identity_mismatch"
        | "yeonjang_v2_artifact_fetch_expired"
        | "yeonjang_v2_artifact_fetch_signature_rejected"
    }
...
interface ArtifactFetchRejectionEnvelope {
  readonly protocol_version: 2
  readonly schema_id: "yeonjang.artifact-fetch-result.v2"
  ...
  readonly payload: {
    ...
    readonly owner_request_id: string
...
export function admitYeonjangMqttV2ArtifactFetchRejection(input: {
  ...
  if (!envelope) return { ok: false, reasonCode: "yeonjang_v2_artifact_fetch_payload_invalid" }
  ...
  if (
    envelope.issued_at > input.nowMs
...
function parseArtifactFetchRejection(value: unknown): ArtifactFetchRejectionEnvelope | null {
  if (!isRecord(value) || !exact(value, [
    ...
    || value.message_kind !== "response" || value.sequence !== 1) return null
  const identities = [
  ...
  if (!identities.every(isProtocolIdentity)
    ...
    || !Number.isSafeInteger(value.expires_at) || Number(value.expires_at) < 1) return null
  ...
  const reasons: readonly YeonjangMqttV2ArtifactFetchRejectReason[] = [
  ...
  if (typeof value.payload.artifact_ref !== "string"
    ...
    || !reasons.includes(value.payload.reason as YeonjangMqttV2ArtifactFetchRejectReason)) return null
  ...
  if (auth.schema_version !== 1 || auth.scope !== "response.publish"
    ...
    || typeof auth.signature !== "string" || !/^[0-9a-f]{64}$/u.test(auth.signature)) return null
  return value as unknown as ArtifactFetchRejectionEnvelope
}

function artifactFetchRejectionSigningBytes(
  envelope: ArtifactFetchRejectionEnvelope,
...
function isProtocolIdentity(value: unknown): value is string {
...
export type YeonjangMqttV2ArtifactAssemblyResult =
  | { readonly ok: true; readonly state: "pending" }
  | { readonly ok: true; readonly state: "complete"; readonly bytes: Buffer }
  | { readonly ok: false; readonly reasonCode:
      | "yeonjang_v2_artifact_frame_invalid"
      | "yeonjang_v2_artifact_binding_mismatch"
      | "yeonjang_v2_artifact_digest_mismatch"
      | "yeonjang_v2_artifact_expired"
      | "yeonjang_v2_artifact_already_terminal" }
...
export function createYeonjangMqttV2ArtifactAssembler(input: {
  ...
  const chunks = new Map<number, { readonly offset: number; readonly bytes: Buffer }>()
  let expectedCount: number | null = null
  let terminal = false
  return {
    accept(frame): YeonjangMqttV2ArtifactAssemblyResult {
      if (terminal) return { ok: false, reasonCode: "yeonjang_v2_artifact_already_terminal" }
      ...
      if (
        header.transfer_id !== input.transferId || header.artifact_ref !== input.artifactRef
      ...
      if (expectedCount === null) expectedCount = header.count
      ...
      if (chunks.size !== expectedCount) return { ok: true, state: "pending" }
...
interface ArtifactChunkHeader {
  readonly schema_version: 1
  readonly transfer_id: string
  readonly artifact_ref: string
  readonly owner_requester_id: string
  readonly owner_request_id: string
...
function decodeFrame(frameInput: Uint8Array): { readonly header: ArtifactChunkHeader; readonly bytes: Buffer } | null {
  const frame = Buffer.from(frameInput)
  if (frame.length < 9 || frame.subarray(0, 4).toString("ascii") !== "YAC2") return null
  const headerLength = frame.readUInt32BE(4)
  if (headerLength < 1 || headerLength > 65_536 || 8 + headerLength >= frame.length) return null
  let header: unknown
  try {
    header = JSON.parse(frame.subarray(8, 8 + headerLength).toString("utf8")) as unknown
  } catch {
    return null
  }
  if (!isRecord(header) || !exact(header, ["schema_version", "transfer_id", "artifact_ref", "owner_requester_id", "owner_request_id", "index", "count", "offset", "chunk_size", "total_size", "payload_digest", "full_digest", "expires_at_ms"])) return null
  const bytes = frame.subarray(8 + headerLength)
  if (
    header.schema_version !== 1 || !isIdentity(header.transfer_id) || !isIdentity(header.owner_requester_id) || !isIdentity(header.owner_request_id)
  ...
  return { header: header as unknown as ArtifactChunkHeader, bytes }
}
