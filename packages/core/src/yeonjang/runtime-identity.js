...
function normalizeGatewayOs(platform = process.platform) {
    switch (platform) {
        case "darwin":
            return "macos";
        case "win32":
...
function normalizeGatewayArch(arch = process.arch) {
    switch (arch) {
        case "x64":
            return "x86_64";
        case "arm64":
...
export function getYeonjangRuntimeHostname() {
    return YEONJANG_RUNTIME_IDENTITY.hostname;
}
export function getDefaultYeonjangWorkspaceScopeId() {
    return YEONJANG_RUNTIME_IDENTITY.workspaceScopeId;
}
export function getDefaultYeonjangOwnerUserId() {
    return YEONJANG_RUNTIME_IDENTITY.ownerUserId;
}
export function getYeonjangGatewayHostFingerprint() {
...
export function previewYeonjangFingerprint(value) {
    return value.length <= 12 ? value : `${value.slice(0, 6)}...${value.slice(-4)}`;
}
export function getYeonjangGatewayHostFingerprintPreview() {
