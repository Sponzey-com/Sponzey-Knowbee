export function admitYeonjangEvidenceForReview(input) {
    const evidence = readEvidence(input.result);
    if (evidence == null) {
        return blocked("YEONJANG_EVIDENCE_MISSING", "Yeonjang result details do not contain normalized evidence.");
    }
    if (!isYeonjangEvidenceEnvelope(evidence)) {
        return blocked("YEONJANG_EVIDENCE_INVALID", "Yeonjang result evidence is not a valid yeonjang-evidence-v1 envelope.");
    }
    const expectedToolName = input.expectedToolName.trim();
    if (!expectedToolName || evidence.toolName !== expectedToolName) {
        return blocked("YEONJANG_EVIDENCE_TOOL_MISMATCH", `Yeonjang evidence tool mismatch. expected=${expectedToolName} actual=${evidence.toolName}`);
    }
    ...
    if (evidence.postCheck.kind === "goal_validated") {
        if (evidence.postCheck.verified === true &&
        ...
        return blocked("YEONJANG_GOAL_VALIDATION_RECEIPT_INVALID", "Yeonjang goal validation post-check does not contain a valid result diagnosis receipt binding.");
    }
    return blocked("YEONJANG_POST_CHECK_UNVERIFIED", `Yeonjang evidence post-check is not verified. kind=${evidence.postCheck.kind}`);
}
function readEvidence(result) {
    if (!isRecord(result.details))
        return undefined;
    return result.details.evidence;
}
function isYeonjangEvidenceEnvelope(value) {
    if (!isRecord(value))
        return false;
    if (value.schemaVersion !== "yeonjang-evidence-v1")
        return false;
    if (!isNonEmptyString(value.targetRef))
        return false;
    if (!isNonEmptyString(value.toolName))
        return false;
    if (!Array.isArray(value.methodIds) || value.methodIds.length === 0)
        return false;
    if (!value.methodIds.every(isNonEmptyString))
        return false;
    if (!isNonEmptyString(value.group))
        return false;
    if (!isNonEmptyString(value.riskLevel))
        return false;
    if (typeof value.requiresApproval !== "boolean")
        return false;
    if (typeof value.collectedAt !== "number" || !Number.isFinite(value.collectedAt))
        return false;
    if (!isNonEmptyString(value.summary))
        return false;
    if (value.rawPayloadVisibility !== "audit_only")
        return false;
    return isPostCheck(value.postCheck);
}
function isPostCheck(value) {
    if (!isRecord(value))
        return false;
    if (value.kind === "not_required")
        return true;
