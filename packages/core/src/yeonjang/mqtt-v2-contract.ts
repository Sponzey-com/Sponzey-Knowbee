...
export type YeonjangMqttV2EnrollmentParseResult =
  | { readonly ok: true; readonly enrollment: YeonjangMqttV2Enrollment }
  | { readonly ok: false; readonly reasonCode: "yeonjang_v2_enrollment_invalid" }
...
export type YeonjangMqttV2ObservationAdmissionResult =
  ...
  | {
      readonly ok: false
      readonly reasonCode:
        | "yeonjang_v2_observation_topic_invalid"
        | "yeonjang_v2_observation_non_retained"
        | "yeonjang_v2_observation_payload_invalid"
        | "yeonjang_v2_observation_identity_mismatch"
        | "yeonjang_v2_observation_expired"
        | "yeonjang_v2_observation_signature_rejected"
    }
 ...
 */
export function mapYeonjangMqttV2WireIdentity(label: string, canonical: string): string {
  if (isMqttV2Identifier(canonical)) return canonical
 ...
 */
export function parseYeonjangMqttV2Enrollment(
  value: YeonjangMqttV2Enrollment,
): YeonjangMqttV2EnrollmentParseResult {
  if (
    !isMqttV2Identifier(value.instanceId)
...
export function buildYeonjangMqttV2Topics(
  value: YeonjangMqttV2Enrollment,
): YeonjangMqttV2Topics {
...
export function parseYeonjangMqttV2ObservationTopic(
  topic: string,
): YeonjangMqttV2ObservationTopic | null {
...
export function admitYeonjangMqttV2Observation(input: {
  ...
  if (!topic) return { ok: false, reasonCode: "yeonjang_v2_observation_topic_invalid" }
  if (!input.retained) return { ok: false, reasonCode: "yeonjang_v2_observation_non_retained" }
  if (
  ...
  const envelope = parseStatusEnvelope(
    value,
    text.match(/"expires_at"\s*:\s*9223372036854775807/gu)?.length === 1,
  )
  if (!envelope) return { ok: false, reasonCode: "yeonjang_v2_observation_payload_invalid" }
  if (
    envelope.target_instance_id !== topic.instanceId
    || envelope.target_session_id !== topic.sessionId
  ) return { ok: false, reasonCode: "yeonjang_v2_observation_identity_mismatch" }
  if (
    envelope.observed_at > input.nowMs
    || (envelope.payload.state === "online" && (
...
function admitCapabilitiesObservation(input: {
  readonly topic: YeonjangMqttV2ObservationTopic
  ...
  if (!envelope) return { ok: false, reasonCode: "yeonjang_v2_observation_payload_invalid" }
  if (
    envelope.target_instance_id !== input.topic.instanceId
...
interface CapabilitiesEnvelope {
  readonly protocol_version: 2
  readonly schema_id: "yeonjang.capabilities.v2"
  readonly message_kind: "capabilities"
  readonly message_id: string
  readonly target_instance_id: string
  readonly target_session_id: string
  readonly target_fingerprint: string
  ...
  readonly sequence: number
  readonly payload: {
    readonly targetPlatform: YeonjangMqttV2CapabilitiesObservation["platform"]
    readonly policyRevision: number
    readonly advertisedMethods: ("camera.capture" | "screen.capture")[]
    readonly capabilities: Array<YeonjangMqttV2CapabilityRow & {
      readonly policyResource: { readonly kind: "any" | "exact_camera" | "exact_display" }
    }>
  }
  readonly authorization: ProjectionAuthorization<"capabilities.publish">
}

function parseCapabilitiesEnvelope(value: unknown): CapabilitiesEnvelope | null {
  ...
  const typedRows = rows as CapabilitiesEnvelope["payload"]["capabilities"]
  if (typedRows[0]?.method !== "camera.capture" || typedRows[1]?.method !== "screen.capture") return null
  const executable = typedRows.filter((row) => row.implementationStatus === "executable").map((row) => row.method)
  if (
    payload.advertisedMethods.length !== executable.length
    || payload.advertisedMethods.some((method, index) => method !== executable[index])
  ) return null
  return value as unknown as CapabilitiesEnvelope
}

function parseCapabilityRow(value: unknown): CapabilitiesEnvelope["payload"]["capabilities"][number] | null {
  ...
  if (!hasExactKeys(value, value.knownLimitation === undefined ? required : [...required, "knownLimitation"])) return null
  if (!isRecord(value.policyResource) || !hasExactKeys(value.policyResource, ["kind"])) return null
  if (
    !["camera.capture", "screen.capture"].includes(String(value.method))
  ...
  void keys
  return value as unknown as CapabilitiesEnvelope["payload"]["capabilities"][number]
}

function capabilitiesAuthorizationSigningBytes(envelope: CapabilitiesEnvelope): Buffer {
  const chunks: Buffer[] = []
...
interface ProjectionAuthorization<Scope extends string> {
  readonly schema_version: 1
  readonly issuer: string
...
interface StatusEnvelope {
  readonly protocol_version: 2
  readonly schema_id: "yeonjang.status.v2"
  readonly message_kind: "status"
  readonly message_id: string
  readonly target_instance_id: string
  readonly target_session_id: string
  readonly target_fingerprint: string
  ...
  readonly payload:
    ...
    | { readonly state: "offline"; readonly reason: "unexpected_disconnect" | "graceful_shutdown" }
  readonly authorization: ProjectionAuthorization<"status.publish">
}

function parseStatusEnvelope(value: unknown, offlineI64Max: boolean): StatusEnvelope | null {
  if (!isRecord(value) || !hasExactKeys(value, [
    "protocol_version", "schema_id", "message_kind", "message_id",
    "target_instance_id", "target_session_id", "target_fingerprint",
    "observed_at", "expires_at", "sequence", "payload", "authorization",
  ...
  if (!isRecord(value.payload) || !(
    (hasExactKeys(value.payload, ["state"]) && value.payload.state === "online")
    || (
  ...
  return value as unknown as StatusEnvelope
}

function statusAuthorizationSigningBytes(envelope: StatusEnvelope): Buffer {
  const chunks: Buffer[] = []
  appendBytes(chunks, "domain", Buffer.from("yeonjang.status.authorization.v2"))
  appendBytes(
    chunks,
    "payload_sha256",
    createHash("sha256").update(JSON.stringify(envelope.payload)).digest(),
  )
  for (const [name, value] of [
  ...
  appendI64(
    chunks,
    "expires_at",
    envelope.payload.state === "offline" ? 9_223_372_036_854_775_807n : envelope.expires_at,
  )
  appendU64(chunks, "sequence", envelope.sequence)
  return Buffer.concat(chunks)
}
 ...
 */
export function deriveYeonjangMqttV2HmacKey(secret: Uint8Array): Buffer {
...
export function createYeonjangMqttV2Command(input: {
  ...
  readonly sequence: number
  readonly hmacKey: Uint8Array
}): { readonly topic: string; readonly envelope: YeonjangMqttV2CommandEnvelope } {
  ...
  const unsigned: YeonjangMqttV2CommandEnvelope = {
    protocol_version: 2,
  ...
  const signature = createHmac("sha256", input.hmacKey)
    ...
    .digest("hex")
  const envelope: YeonjangMqttV2CommandEnvelope = {
    ...unsigned,
...
function commandAuthorizationSigningBytes(envelope: YeonjangMqttV2CommandEnvelope): Buffer {
  const chunks: Buffer[] = []
...
function commandPayloadDigest(payload: YeonjangMqttV2CommandEnvelope["payload"]): Buffer {
  const chunks: Buffer[] = []
...
function isValidCommandParams(
  method: YeonjangMqttV2CommandMethod,
...
function appendText(output: Buffer[], name: string, value: string): void {
  appendBytes(output, name, Buffer.from(value, "utf8"))
}

function appendBytes(output: Buffer[], name: string, value: Uint8Array): void {
  const nameBytes = Buffer.from(name, "utf8")
  output.push(u64(nameBytes.length), nameBytes, u64(value.byteLength), Buffer.from(value))
}

function appendU64(output: Buffer[], name: string, value: number): void {
  appendBytes(output, name, u64(value))
}

function appendI64(output: Buffer[], name: string, value: number | bigint): void {
  const bytes = Buffer.alloc(8)
  bytes.writeBigInt64BE(BigInt(value))
  appendBytes(output, name, bytes)
}

function appendOptionalText(output: Buffer[], name: string, value: unknown): void {
  appendBytes(output, `${name}_present`, Buffer.from([value === undefined ? 0 : 1]))
  if (typeof value === "string") appendText(output, name, value)
}

function appendOptionalU64(output: Buffer[], name: string, value: unknown): void {
  appendBytes(output, `${name}_present`, Buffer.from([value === undefined ? 0 : 1]))
  if (typeof value === "number") appendU64(output, name, value)
}
...
function isSha256Fingerprint(value: string): boolean {
  if (!value.startsWith("sha256:") || value.length !== 71) return false
  return [...value.slice(7)].every((character) =>
...
function isMqttV2Identifier(value: string): boolean {
  const bytes = Buffer.from(value, "utf8")
  if (bytes.length < 1 || bytes.length > MAX_IDENTIFIER_BYTES) return false
...
function isLowercaseAlphanumeric(byte: number): boolean {
