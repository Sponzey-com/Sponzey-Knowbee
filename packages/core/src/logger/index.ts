...
const PURPOSE_VISIBILITY: Record<LogPurpose, number> = { product: 0, debug: 1, development: 2 }
...
const SECRET_KEY_PATTERN =
  /api[_-]?key|auth[_-]?token|authorization|bearer|cookie|credential|password|private[_-]?key|refresh[_-]?token|secret|token/iu
...
const LOCAL_PATH_PATTERN =
  /(?:\/Users\/[\w .@+-]+(?:\/[^\s"'`<>]*)+|\/private\/[\w .@+-]+(?:\/[^\s"'`<>]*)+|\/tmp\/[\w .@+-]+(?:\/[^\s"'`<>]*)+|\/var\/folders\/[\w .@+-]+(?:\/[^\s"'`<>]*)+|[A-Za-z]:\\[^\s"'`<>]+(?:\\[^\s"'`<>]+)*)/gu
const PRODUCT_IDENTIFIER_KEY_PATTERN =
  /^(?:runId|requestGroupId|requestId|sessionId|channelId|chatId|threadId|threadTs|messageId|messageTs|userId|agentId|instanceId|extensionId|workerSessionId|subSessionId|parentRunId|rootRunId|externalChatId|externalThreadId|externalMessageId)$/u
const PRODUCT_IDENTIFIER_TEXT_PATTERN =
  /\b(runId|requestGroupId|requestId|sessionId|channelId|channel|chatId|chat|threadId|threadTs|thread|messageId|messageTs|ts|userId|user|agentId|instanceId|extensionId|workerSessionId|subSessionId|parentRunId|rootRunId|externalChatId|externalThreadId|externalMessageId)=([^\s,]+)/giu
...
export function normalizeLogPurposeVisibility(
  value: string | undefined,
  fallback: LogPurpose = "product",
): LogPurpose {
...
const LOGGER_PROCESS: NodeJS.Process | undefined =
...
const LOG_POLICY = {
  minLevel,
  purposeVisibility: normalizeLogPurposeVisibility(
...
export function redactLogText(value: string, purpose: LogPurpose = "product"): string {
  if (!value) return value
  if (containsInternalLlmStructuredDataText(value)) return INTERNAL_LLM_DATA_MASK
  if (RAW_HTML_PATTERN.test(value)) return LOG_RAW_PAYLOAD_MASK
...
function redactLogValue(
  ...
  key = "",
  depth = 0,
  purpose: LogPurpose = "product",
): unknown {
  if (value == null) return value
  if (depth > 8) return "[truncated]"
  if (isInternalLlmStructuredDataKey(key)) return INTERNAL_LLM_DATA_MASK
  if (RAW_PAYLOAD_KEY_PATTERN.test(key)) return LOG_RAW_PAYLOAD_MASK
  if (purpose === "product" && PRODUCT_IDENTIFIER_KEY_PATTERN.test(key)) return "[id-redacted]"
  if (typeof value === "string") {
    if (SECRET_KEY_PATTERN.test(key)) return LOG_SECRET_MASK
...
function serializeArg(value: unknown, purpose: LogPurpose): string {
  if (value instanceof Error) {
    return JSON.stringify(redactLogValue(value, "", 0, purpose))
  }

  if (typeof value === "string") return redactLogText(value, purpose)
...
export function createLogger(namespace: string): Logger {
  ...
  function log(level: LogLevel, purpose: LogPurpose, message: string, ...args: unknown[]) {
    if (LEVELS[level] < minLevelValue) return
    if (PURPOSE_VISIBILITY[purpose] > maxPurposeVisibility) return
