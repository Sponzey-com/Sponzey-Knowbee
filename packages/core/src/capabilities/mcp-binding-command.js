...
export async function executeMcpBindingCommand(input, ports) {
    const now = ports.now();
    ...
    if (!mcp)
        return rejected({ ...input, revision: baseRevision, reasonCode: "mcp_ref_not_found" });
...
//# sourceMappingURL=mcp-binding-command.js.map