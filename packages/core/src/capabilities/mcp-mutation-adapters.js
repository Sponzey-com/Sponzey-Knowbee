...
function canonicalServerKey(internalMcpId) {
    const key = internalMcpId.trim().replace(/^mcp:/u, "");
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u.test(key))
        throw new Error("mcp_internal_id_invalid");
    return key;
}
function canonicalCatalogKey(internalMcpId) {
    return internalMcpId.trim().replace(/^mcp:/u, "");
}
function serverConfigFromDraft(draft) {
    const base = {
        enabled: true,
...
function draftFromConfig(displayName, config) {
...
export function createMcpConfigurationStore(input) {
    const readRaw = () => {
        if (!input.fileSystem.exists(input.paths.configFile))
            return {};
    ...
    const mcpObject = (raw) => object(raw.mcp);
    const persistedServers = (raw) => {
        const rawServers = object(mcpObject(raw).servers);
        if (Object.keys(rawServers).length > 0)
            return cloneRecord(rawServers);
        return cloneRecord(input.initialConfig.mcp?.servers ?? {});
    };
    const revisionFor = (raw) => {
        const rawRevision = mcpObject(raw).revision;
        const persistedRevision = typeof rawRevision === "number" && Number.isSafeInteger(rawRevision) && rawRevision >= 0
            ? rawRevision
            : 0;
        return Math.max(persistedRevision, ...catalogRows().map((row) => row.updatedAt), input.externalRevision?.() ?? 0, 0);
    };
    const listEntries = () => {
        const rowsByKey = new Map(catalogRows()
    ...
    return Object.freeze({
        ...
        runtimeConfigurationSnapshot: () => Object.freeze({ servers: Object.freeze(persistedServers(readRaw())) }),
        persist: ({ mode, internalMcpId, draft, expectedRevision, targetRevision, }) => {
            const raw = readRaw();
            ...
            const previousCatalog = catalogRows().find((row) => row.internalMcpId === internalMcpId) ?? null;
            ...
            try {
                ...
                if (rollbackSnapshot.token.fileExisted)
                    writePersistedRawConfig(cloneRecord(raw), input.paths, input.fileSystem);
                else
                    input.fileSystem.remove(input.paths.configFile);
                try {
                    if (previousCatalog)
        ...
        persistLifecycle: ({ internalMcpId, action, expectedRevision, targetRevision, }) => {
            const raw = readRaw();
            const actualRevision = revisionFor(raw);
            if (actualRevision !== expectedRevision)
                return { ok: false, revision: actualRevision, reasonCode: "capability_revision_conflict" };
            ...
            if (action === "delete")
                ...
                nextServers[serverKey] = { ...previousServer, enabled: action === "enable" };
            const nextRaw = cloneRecord(raw);
        ...
        persistRecovery: ({ internalMcpId, expectedRevision, targetRevision, }) => {
            const raw = readRaw();
            const actualRevision = revisionFor(raw);
            if (actualRevision !== expectedRevision)
                return { ok: false, revision: actualRevision, reasonCode: "capability_revision_conflict" };
            ...
            try {
                writePersistedRawConfig(nextRaw, input.paths, input.fileSystem);
                if (previousCatalog)
                    input.catalog.write({ ...previousCatalog, updatedAt: targetRevision });
                return { ok: true, revision: targetRevision, rollbackSnapshot };
            }
            catch {
                try {
        ...
        rollback: (snapshot) => {
            ...
            try {
                if (token.fileExisted)
                    ...
                    input.fileSystem.remove(input.paths.configFile);
                if (token.previousCatalog)
                    input.catalog.write(token.previousCatalog);
                else {
...
export function createMcpRegistryApplyAdapter(input) {
    ...
    const sameEvidence = (left, right) => JSON.stringify([...left].sort((a, b) => a.name.localeCompare(b.name))) ===
    ...
    return Object.freeze({
        ...
        apply: async ({ configuration, targetRevision, }) => {
            if (!configuration || typeof configuration !== "object" || Array.isArray(configuration))
                return { ok: false, reasonCode: "mcp_runtime_snapshot_invalid" };
            const nextConfig = {
                ...activeConfig,
                mcp: cloneRecord(configuration),
            };
            try {
        ...
        verify: async ({ internalMcpId, targetRevision, }) => {
            const serverKey = canonicalServerKey(internalMcpId);
        ...
        verifyLifecycle: async ({ internalMcpId, action, targetRevision, }) => {
            const serverKey = canonicalServerKey(internalMcpId);
            const matches = input.registry.statuses().filter((status) => status.name === serverKey);
        ...
        captureTarget: (internalMcpId) => Object.freeze({
            token: Object.freeze({
                kind: "target",
                internalMcpId,
        ...
        applyTarget: async ({ internalMcpId, configuration, targetRevision, }) => {
            if (!configuration || typeof configuration !== "object" || Array.isArray(configuration))
                return { ok: false, reasonCode: "mcp_runtime_snapshot_invalid" };
            const nextConfig = {
                ...activeConfig,
            ...
            if (!serverConfig)
                return { ok: false, reasonCode: "mcp_ref_not_found" };
            try {
        ...
        verifyTarget: async ({ internalMcpId, targetRevision, }) => {
            const serverKey = canonicalServerKey(internalMcpId);
            const matches = input.registry.statuses().filter((status) => status.name === serverKey);
            const match = matches.length === 1 ? matches[0] : undefined;
        ...
        rollbackTarget: async (snapshot) => {
            ...
            if (!previous)
                return { ok: false, reasonCode: "mcp_target_rollback_config_missing" };
            try {
                await input.registry.reloadTarget({
                    name: serverKey,
                    config: previous,
                ...
                const peerEvidence = (rows) => rows.filter((status) => status.name !== serverKey);
        ...
        rollback: async (snapshot) => {
            ...
            try {
                ...
                if (!restored)
                    return { ok: false, reasonCode: "mcp_runtime_rollback_verification_failed" };
                activeConfig = token.config;
                activeRevision = token.revision;
                return { ok: true };
            }
...
//# sourceMappingURL=mcp-mutation-adapters.js.map