...
import {
  ...
  type YeonjangCapabilityRiskLevel,
  type YeonjangCapabilitySideEffectClass,
} from "./yeonjang-capability-schema.js"

export type YeonjangSideEffectPostCheckMode =
  | "artifact_verified"
...
export interface YeonjangSideEffectMethodContract {
  ...
  readonly sideEffectClass: Exclude<
    YeonjangCapabilitySideEffectClass,
    "none" | "read_local" | "network"
  >
...
export const YEONJANG_SIDE_EFFECT_METHOD_CONTRACTS: readonly YeonjangSideEffectMethodContract[] =
  Object.freeze(
...
const CONTRACT_BY_METHOD = new Map(
  YEONJANG_SIDE_EFFECT_METHOD_CONTRACTS.map((contract) => [contract.method, contract]),
)

export function getYeonjangSideEffectMethodContract(
  method: string,
): YeonjangSideEffectMethodContract | undefined {
  return CONTRACT_BY_METHOD.get(method.trim().toLowerCase())
}

export function isYeonjangSideEffectMethod(method: string): boolean {
  return getYeonjangSideEffectMethodContract(method) != null
}

export type YeonjangSideEffectToolContractValidation =
  | { ok: true; contract: YeonjangSideEffectMethodContract }
  | {
      ok: false
      reasonCode:
        | "method_not_side_effect"
        | "tool_missing_runtime_method"
        | "tool_name_not_bound"
        | "tool_requires_approval_missing"
        | "tool_risk_too_low"
    }
...
export function validateYeonjangSideEffectToolContract(input: {
  ...
  if (!contract) return { ok: false, reasonCode: "method_not_side_effect" }
