...
function validateHttpUrl(value) {
    ...
    if (endpoint.username || endpoint.password)
        reasons.push("mcp_url_credentials_forbidden");
    if (endpoint.hash)
        reasons.push("mcp_url_fragment_forbidden");
    return reasons;
}
export function validateMcpConnectionDraft(input) {
    ...
    if (!displayName)
        reasons.push("mcp_display_name_missing");
    else if (displayName.length > 80)
        reasons.push("mcp_display_name_too_long");
    const transport = record.transport;
    if (transport !== "stdio" && transport !== "http")
        reasons.push("mcp_transport_invalid");
    const command = typeof record.command === "string" ? record.command.trim() : "";
    if (transport === "stdio" && !command)
        reasons.push("mcp_command_missing");
    else if (command.includes("\0") || /[\r\n]/.test(command) || command.length > 4096)
        reasons.push("mcp_command_invalid");
    const rawArgs = record.args;
    const argsValid = Array.isArray(rawArgs) &&
        rawArgs.every((value) => typeof value === "string" && !value.includes("\0") && value.length <= 4096) &&
        rawArgs.length <= 128;
    if (!argsValid)
        reasons.push("mcp_args_invalid");
    const args = argsValid ? rawArgs.map((value) => value.trim()).filter(Boolean) : [];
    ...
    if (record.url !== undefined && typeof record.url !== "string")
        reasons.push("mcp_url_invalid");
    if (transport === "http") {
        reasons.push(...validateHttpUrl(url));
        if (command || args.length > 0 || cwd)
            reasons.push("mcp_transport_fields_mixed");
    }
    else if (transport === "stdio" && url)
        reasons.push("mcp_transport_fields_mixed");
    if (typeof record.required !== "boolean")
        reasons.push("mcp_required_invalid");
...
//# sourceMappingURL=mcp-connection-validation.js.map