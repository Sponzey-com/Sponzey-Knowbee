...
export function isYeonjangWebSearchCapabilityMethod(method) {
...
export function classifyYeonjangCapabilityMethod(method) {
    const normalized = normalizeMethodName(method);
...
function supportStateFromMatrixEntry(input) {
    ...
    if (input.entry.supported === true)
        return "supported";
    if (input.entry.supported === false)
        return "unsupported";
    return "unknown";
}
...
function normalizeLegacyMethodCapability(method, implemented) {
...
export function normalizeYeonjangCapabilityMatrix(input) {
    const issues = [];
...
//# sourceMappingURL=yeonjang-capability-schema.js.map