...
import {
  ...
  defaultMainAgentNameForLanguage,
...
function controlPlaneModelDiscoveryErrorMessage(error: unknown): string {
...
function readRawConfig(paths: SetupPersistencePaths): JsonObject {
...
function writeRawConfig(raw: JsonObject, paths: SetupPersistencePaths): void {
...
export function readSetupState(paths: SetupPersistencePaths): SetupState {
...
export function writeSetupState(state: SetupState, paths: SetupPersistencePaths): SetupState {
...
export function buildSetupDraft(config: KnowbeeConfig, paths: SetupPersistencePaths | null): SetupDraft {
...
function cloneSetupDraft(draft: SetupDraft): SetupDraft {
  return JSON.parse(JSON.stringify(draft)) as SetupDraft
}

function maskSetupSecret(value: string | undefined): string {
  return value?.trim() ? SETUP_SECRET_MASK : ""
}

function restoreSetupSecret(value: string, existing: string | undefined): string {
  return value === SETUP_SECRET_MASK ? (existing ?? "") : value
}

export function redactSetupDraftSecrets(input: SetupDraft): SetupDraft {
...
function restoreSetupDraftSecretMasks(input: SetupDraft, existing: SetupDraft): SetupDraft {
  const draft = cloneSetupDraft(input)
  ...
  draft.aiBackends = draft.aiBackends.map((backend) => {
    ...
    return {
      ...
      credentials: {
        ...
        ...(backend.credentials.oauthAuthFilePath !== undefined
          ? {
              oauthAuthFilePath: restoreSetupSecret(
                backend.credentials.oauthAuthFilePath,
                existingBackend?.credentials.oauthAuthFilePath,
              ),
            }
          : {}),
      },
  ...
  draft.channels = {
    ...
    googleChatAppCredentialJson: restoreSetupSecret(
      draft.channels.googleChatAppCredentialJson,
      existing.channels.googleChatAppCredentialJson,
    ),
    googleChatWebhookUrl: restoreSetupSecret(draft.channels.googleChatWebhookUrl, existing.channels.googleChatWebhookUrl),
    googleChatVerificationToken: restoreSetupSecret(
      draft.channels.googleChatVerificationToken,
      existing.channels.googleChatVerificationToken,
    ),
...
export function resetSetupEnvironment(paths: SetupPersistencePaths): { draft: SetupDraft; state: SetupState; checks: SetupChecks } {
  writeRawConfig(JSON.parse(JSON.stringify(DEFAULT_CONFIG)) as JsonObject, paths)
...
export function completeSetup(paths: SetupPersistencePaths): SetupState {
...
export function createSetupChecks(config: KnowbeeConfig, paths: SetupPersistencePaths): SetupChecks {
...
export function redactSetupChecksForApi(checks: SetupChecks): SetupChecks {
  return {
    ...checks,
...
export function parseEnterpriseTopologyBuilderUiEnabled(value: string | undefined): boolean {
  const normalized = value?.trim().toLowerCase()
  return !(normalized === "0" || normalized === "false" || normalized === "no" || normalized === "off")
}

function createEnterpriseTopologyBuilderCapability(options: CapabilityProjectionOptions): FeatureCapability {
...
export function createCapabilities(options: CapabilityProjectionOptions): FeatureCapability[] {
...
export function createCapabilityCounts(options: CapabilityProjectionOptions): CapabilityCounts {
  return countCapabilities(createCapabilities(options))
}

export function getPrimaryAiTarget(config: KnowbeeConfig): string | null {
  ...
  if (providerType === "openai" && authMode === "chatgpt_oauth") {
    ...
    return {
      ...
      capabilityMatrix: buildCheckedCapability(
        "ok",
        "ChatGPT OAuth account model catalog loaded.",
        sourceUrl,
        models,
      ),
    }
