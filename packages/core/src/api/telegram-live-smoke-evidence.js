...
export function createTelegramLiveSmokeEvidenceReader(dependencies) {
    return (run, target) => {
        ...
        const providerMessageIds = scopedEvents.flatMap((event) => {
            const legacyTextDelivery = event.event_kind === "text_delivered" &&
                (event.status === "delivered" || event.status === "succeeded");
            const providerReceipt = event.event_kind === "delivery_receipted" &&
                (event.status === "sent" || event.status === "delivered" || event.status === "succeeded");
            if (!legacyTextDelivery && !providerReceipt)
                return [];
        ...
        const targetMatched = providerMessageIds.some((messageId) => refs.some((ref) => ref.external_chat_id === String(target.chatId) &&
        ...
        const deliveredUserReports = scopedEvents.filter((event) => {
            if (event.event_kind !== "final_answer_delivered" ||
                (event.status !== "delivered" && event.status !== "succeeded")) {
                return false;
            }
        ...
        const capabilitySelectionDecisionTrace = (dependencies.listDecisionTracesForRun?.(run.id) ?? []).find((trace) => {
        ...
        const capabilityReceipts = auditLogs.flatMap((row) => row.run_id === run.id &&
            row.request_group_id === run.requestGroupId &&
            row.channel === "telegram" &&
            row.result === "failed" &&
        ...
        const approvalReceipts = approval &&
            ...
            : [];
        const artifactReceipts = (dependencies.listArtifactReceiptsForRun?.(run.id) ?? []).flatMap((receipt) => receipt.run_id === run.id &&
        ...
        return {
            ...
            ...(capabilitySelectionDecisionTrace
                ? {
                    capabilitySelectionDecisionTraceId: capabilitySelectionDecisionTrace.id,
                }
                : {}),
            toolReceipts,
            approvalReceipts,
...
//# sourceMappingURL=telegram-live-smoke-evidence.js.map