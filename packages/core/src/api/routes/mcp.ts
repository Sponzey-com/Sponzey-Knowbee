...
import {
  type McpBindingUserReceipt,
  executeMcpBindingCommand,
} from "../../capabilities/mcp-binding-command.js"
import {
  type McpBindingRow,
  type McpCatalogQuery,
  type McpCatalogRow,
  type McpRuntimeRow,
  buildMcpCatalogPage,
  buildMcpCatalogSnapshot,
} from "../../capabilities/mcp-catalog-query.js"
import {
  type McpConnectionProbeReceipt,
  probeMcpConnectionDraft,
} from "../../capabilities/mcp-connection-probe.js"
...
const defaultCatalogRepository: McpCatalogRepository = {
  listCatalog: () => listMcpServerCatalogEntries(),
  listBindings: () =>
    listAgentCapabilityBindings({ capabilityKind: "mcp_server", includeArchived: true }),
}

export function registerMcpRoute(app: FastifyInstance, options: McpRouteOptions = {}): void {
  ...
  const probeActorForRequest =
    options.probeActorForRequest ??
    ((request: unknown) => {
  ...
  const runtimeWorkspaceForRequest =
    options.runtimeWorkspaceForRequest ??
    ((request: unknown) => getApiRuntimeConfig(request as never).profile.workspace)
  ...
  const currentMcpRevision = () =>
    Math.max(
      listMcpServerCatalogEntries({ includeArchived: true }).reduce(
        (revision, row) => Math.max(revision, row.updated_at),
        0,
      ),
      listAgentCapabilityBindings({ capabilityKind: "mcp_server", includeArchived: true }).reduce(
        (revision, row) => Math.max(revision, row.updated_at),
        0,
      ),
    )
  const reserveMcpReceipt = ({
    envelope,
    state,
    now: reservedAt,
  }: { envelope: MutationEnvelope; state: CapabilityMutationState; now: number }) =>
    reserveCapabilityMutationReceipt({
  ...
  const resolveMcpIdentity = (mcpRef: string) => {
    const catalog = listMcpServerCatalogEntries({ includeArchived: true }).filter(
      (row) => row.status !== "archived" && publicRefForMcpId(row.mcp_server_id) === mcpRef,
    )
  ...
  const writeBinding = (input: {
    ...
    const existing = listAgentCapabilityBindings({
      ...
      (binding) =>
        binding.agent_id === input.internalAgentId && binding.catalog_id === input.internalMcpId,
    )
    upsertAgentCapabilityBinding(
      {
        ...(existing
        ...
        ...(existing?.permission_profile_json
          ? { permissionProfile: parseObject(existing.permission_profile_json) as never }
          : {}),
        ...(existing?.rate_limit_json
          ? { rateLimit: parseObject(existing.rate_limit_json) as never }
          : {}),
  ...
  const executeBinding =
    ...
    (async (input) =>
      executeMcpBindingCommand(input, {
        now,
        ...
        resolveAgent,
        bindingEnabled: ({ internalMcpId, internalAgentId }) =>
          listAgentCapabilityBindings({ capabilityKind: "mcp_server", includeArchived: true }).some(
            (binding) =>
              binding.catalog_id === internalMcpId &&
              binding.agent_id === internalAgentId &&
              binding.status === "enabled",
          ),
        persist: ({ internalMcpId, internalAgentId, enabled, expectedRevision, targetRevision }) =>
          currentMcpRevision() === expectedRevision &&
        ...
        verify: ({ internalMcpId, internalAgentId, enabled, targetRevision }) => {
          const binding = listAgentCapabilityBindings({
            ...
            (entry) => entry.catalog_id === internalMcpId && entry.agent_id === internalAgentId,
          )
          return {
            ok: Boolean(
              binding &&
  ...
  const executeProbe =
    ...
    (async (input: { actorRef: string; draft: unknown; defaultCwd: string; signal: AbortSignal }) =>
      probeMcpConnectionDraft(
        input.draft,
        {
          now,
  ...
  const projectedMcpId = (
    mcpRef: string,
    rows: readonly McpCatalogRow[],
    statuses: readonly McpRuntimeRow[],
  ): string | null => {
    const catalogMatches = rows.filter(
      (row) => row.status !== "archived" && publicRefForMcpId(row.mcp_server_id) === mcpRef,
    )
    const catalogMatch = catalogMatches.length === 1 ? catalogMatches[0] : undefined
    if (catalogMatch) return catalogMatch.mcp_server_id
    const runtimeMatches = statuses.filter(
      (row) => publicRefForMcpId(`mcp:${row.name.trim()}`) === mcpRef,
    )
    const runtimeMatch = runtimeMatches.length === 1 ? runtimeMatches[0] : undefined
    return runtimeMatch ? `mcp:${runtimeMatch.name.trim()}` : null
  }

  const normalizeQuery = (raw: Record<string, unknown>): McpCatalogQuery => {
    const limit =
      raw.limit === undefined
        ? undefined
        : typeof raw.limit === "number"
          ...
          : Number(raw.limit)
    const transport =
      raw.transport === undefined || raw.transport === "" ? undefined : raw.transport
  ...
  app.get<{ Querystring: McpCatalogQuery }>(
  ...
  app.get<{ Params: { mcpRef: string } }>(
    "/api/capabilities/mcp/:mcpRef",
    { preHandler: authMiddleware },
    async (req, reply) => {
      ...
      try {
        ...
        if (!item) return reply.status(404).send({ error: "mcp_ref_not_found" })
        const internalMcpId = projectedMcpId(req.params.mcpRef, source.rows, source.runtimeStatuses)
        if (!internalMcpId) return reply.status(404).send({ error: "mcp_ref_not_found" })
        ...
        const runtime = source.runtimeStatuses.find(
          (status) => status.name.trim().replace(/^mcp:/u, "") === internalKey,
        )
        ...
        const tools = toolAccess.tools.map((tool) =>
          tool.access.length > 0 ? tool : { name: tool.name, description: tool.description },
        )
  ...
  app.post<{ Body: { draft?: unknown } }>(
    ...
    async (req, reply) => {
      ...
      if (!actorRef) return reply.status(403).send({ error: "mcp_probe_actor_denied" })
      if (!validation.valid) {
        const rejected: McpConnectionProbeReceipt = {
          state: "rejected",
          ready: false,
      ...
      try {
        ...
        if (receipt.state === "rejected") return reply.status(422).send(receipt)
        if (receipt.state === "cancelled") return reply.status(408).send(receipt)
        return receipt
      } catch {
  ...
  const envelopeFrom = (value: unknown, actorRef: string): MutationEnvelope | null => {
    ...
    if (Object.keys(envelope).some((key) => !allowed.has(key))) return null
    if (
  ...
  const mutationStatus = (
    receipt: { state: string; reasonCode: string | null },
    successStatus: number,
  ) => {
    if (receipt.state === "active") return successStatus
  ...
  if (typeof app.patch === "function")
    app.patch<{
      Params: { mcpRef: string; agentRef: string }
      Body: { envelope?: unknown; bound?: unknown }
    }>(
      "/api/capabilities/mcp/:mcpRef/bindings/:agentRef",
      { preHandler: authMiddleware },
      async (req, reply) => {
        if (
          !/^mcp_v1_[a-f0-9]{24}$/u.test(req.params.mcpRef) ||
        ...
        if (!actorRef) return reply.status(403).send({ error: "mcp_binding_actor_denied" })
        const envelope = envelopeFrom(body.envelope, actorRef)
        if (!envelope) return reply.status(400).send({ error: "mcp_binding_request_invalid" })
        try {
          const startedAt = now()
  ...
  if (mutationRuntime) {
    app.post<{ Body: { envelope?: unknown; draft?: unknown } }>(
      "/api/capabilities/mcp",
      { preHandler: authMiddleware },
      async (req, reply) => {
        const body = req.body
        ...
        if (draftValidation.reasonCodes.includes("mcp_draft_field_unknown"))
          return reply.status(400).send({ error: "mcp_create_request_invalid" })
        const actorRef = mutationActorForRequest(req)
        if (!actorRef) return reply.status(403).send({ error: "mcp_create_actor_denied" })
        const envelope = envelopeFrom(body.envelope, actorRef)
        if (!envelope) return reply.status(400).send({ error: "mcp_create_request_invalid" })
        const controller = new AbortController()
        const raw = (
          req as {
            raw?: {
    ...
    app.post<{ Params: { mcpRef: string }; Body?: Record<string, never> }>(
      ...
      async (req, reply) => {
        ...
        const actorRef = mutationActorForRequest(req)
        if (!actorRef) return reply.status(403).send({ error: "mcp_existing_probe_actor_denied" })
        const controller = new AbortController()
        const raw = (
          req as {
            raw?: {
        ...
        try {
          ...
          if (receipt.state === "not_found") return reply.status(404).send(receipt)
          if (receipt.state === "cancelled") return reply.status(408).send(receipt)
          return receipt
        } catch {
    ...
    app.post<{ Params: { mcpRef: string }; Body: { envelope?: unknown } }>(
      "/api/capabilities/mcp/:mcpRef/recover",
      { preHandler: authMiddleware },
      async (req, reply) => {
        if (!/^mcp_v1_[a-f0-9]{24}$/u.test(req.params.mcpRef))
          return reply.status(400).send({ error: "mcp_ref_invalid" })
        ...
        const actorRef = mutationActorForRequest(req)
        if (!actorRef) return reply.status(403).send({ error: "mcp_recovery_actor_denied" })
        const envelope = envelopeFrom(body.envelope, actorRef)
        if (!envelope) return reply.status(400).send({ error: "mcp_recovery_request_invalid" })
        const controller = new AbortController()
        const raw = (
          req as {
            raw?: {
    ...
    if (typeof app.patch === "function")
      app.patch<{
        Params: { mcpRef: string }
        Body: { envelope?: unknown; draft?: unknown; change?: unknown }
      }>("/api/capabilities/mcp/:mcpRef", { preHandler: authMiddleware }, async (req, reply) => {
        if (!/^mcp_v1_[a-f0-9]{24}$/u.test(req.params.mcpRef))
          return reply.status(400).send({ error: "mcp_ref_invalid" })
        const body = req.body
        if (
          ...
          Object.keys(body).some(
            (key) => key !== "envelope" && key !== "draft" && key !== "change",
          ) ||
          !("envelope" in body) ||
        ...
        if (
          shapeReason === "mcp_update_change_field_unknown" ||
          shapeReason === "mcp_replacement_invalid" ||
          draftValidation?.reasonCodes.includes("mcp_draft_field_unknown")
        )
          return reply.status(400).send({ error: "mcp_update_request_invalid" })
        const actorRef = mutationActorForRequest(req)
        if (!actorRef) return reply.status(403).send({ error: "mcp_update_actor_denied" })
        const envelope = envelopeFrom(body.envelope, actorRef)
        if (!envelope) return reply.status(400).send({ error: "mcp_update_request_invalid" })
        const controller = new AbortController()
        const raw = (
          req as {
            raw?: {
        ...
        try {
          ...
          mcpRouteLogger.fieldDebug("MCP update receipt", {
            mutationId: receipt.mutationId,
            transport:
              draftValidation?.draft?.transport ?? (protectedChange ? "protected" : "invalid"),
            durationMs: Math.max(0, now() - startedAt),
            reasonCode: receipt.reasonCode,
          })
    ...
    if (typeof app.patch === "function")
      app.patch<{ Params: { mcpRef: string }; Body: { envelope?: unknown; enabled?: unknown } }>(
        "/api/capabilities/mcp/:mcpRef/status",
        { preHandler: authMiddleware },
        async (req, reply) => {
          if (!/^mcp_v1_[a-f0-9]{24}$/u.test(req.params.mcpRef))
            return reply.status(400).send({ error: "mcp_ref_invalid" })
          ...
          const actorRef = mutationActorForRequest(req)
          if (!actorRef) return reply.status(403).send({ error: "mcp_status_actor_denied" })
          const envelope = envelopeFrom(body.envelope, actorRef)
          if (!envelope) return reply.status(400).send({ error: "mcp_status_request_invalid" })
          const controller = new AbortController()
          const raw = (
            req as {
              raw?: {
    ...
    if (typeof app.delete === "function")
      app.delete<{ Params: { mcpRef: string }; Body: { envelope?: unknown } }>(
        "/api/capabilities/mcp/:mcpRef",
        { preHandler: authMiddleware },
        async (req, reply) => {
          if (!/^mcp_v1_[a-f0-9]{24}$/u.test(req.params.mcpRef))
          ...
          const actorRef = mutationActorForRequest(req)
          if (!actorRef) return reply.status(403).send({ error: "mcp_delete_actor_denied" })
          const envelope = envelopeFrom(body.envelope, actorRef)
          if (!envelope) return reply.status(400).send({ error: "mcp_delete_request_invalid" })
          const controller = new AbortController()
          const raw = (
            req as {
              raw?: {
  ...
  app.post("/api/mcp/reload", { preHandler: authMiddleware }, async (req, reply) => {
    ...
    try {
      ...
      else await mcpRegistry.reloadFromConfig(config)
      return {
