...
import { executeYeonjangBindingCommand, } from "../../capabilities/yeonjang-binding-command.js";
import { buildYeonjangCapabilityProjection } from "../../capabilities/yeonjang-capability-projection.js";
import { queryYeonjangCapabilityCatalog, resolveYeonjangCapabilityDetail, } from "../../capabilities/yeonjang-capability-query.js";
...
import { executeYeonjangRecoveryCommand, } from "../../capabilities/yeonjang-recovery-command.js";
import { getCapabilityMutationReceiptByNonce, listAgentCapabilityBindings, listAgentConfigs, reserveCapabilityMutationReceipt, updateCapabilityMutationReceipt, upsertAgentCapabilityBinding, } from "../../db/index.js";
import { createLogger } from "../../logger/index.js";
import { buildYeonjangBrowserActiveTabInfoBackendAcceptanceCriteria, } from "../../release/yeonjang-browser-active-tab-info-backend-acceptance-criteria.js";
...
import { buildYeonjangBrowserActiveTabInfoRustInventoryContract, } from "../../release/yeonjang-browser-active-tab-info-rust-inventory-contract.js";
import { buildYeonjangBrowserActiveTabInfoPreDispatchBridgePlan, } from "../../release/yeonjang-browser-active-tab-info-pre-dispatch-bridge.js";
import { assembleYeonjangBrowserActiveTabInfoReadinessObservationsFromRegistry, selectYeonjangBrowserActiveTabInfoRedactedObservationFromRegistry, } from "../../release/yeonjang-browser-active-tab-info-readiness-source-adapter.js";
...
import { approveYeonjangInstancePairing, assignYeonjangLocalMarker, listYeonjangGovernanceHistory, renameYeonjangRegistryInstance, updateYeonjangInstanceTrustState, verifyYeonjangInstancePairing, } from "../../yeonjang/registry.js";
...
function yeonjangCameraPermissionProfile(agentId) {
    return {
...
async function waitForRecoveryObservation(intervalMs, signal) {
    if (signal.aborted)
        return;
...
function currentYeonjangBindingRevision() {
    return listAgentCapabilityBindings({ capabilityKind: "yeonjang", includeArchived: true }).reduce((revision, binding) => Math.max(revision, binding.updated_at), 0);
}
function envelopeFrom(value, actorRef) {
...
function mutationStatus(receipt) {
    if (receipt.state === "active")
        return 200;
    if (receipt.reasonCode === "yeonjang_ref_not_found" ||
    ...
    if (receipt.reasonCode === "mutation_scope_denied" ||
        ...
        return 403;
    if (receipt.reasonCode === "mutation_revision_conflict" ||
...
function queryInput(value) {
    ...
    if (Object.keys(query).some((key) => !allowed.has(key)))
        return null;
    const string = (key) => query[key] === undefined || typeof query[key] === "string";
    if (!["search", "location", "platform", "status", "cursor", "limit"].every(string))
        return null;
    ...
    if (location !== undefined && location !== "local" && location !== "remote")
        return null;
...
function publicActiveTabInfoPreDispatchPreview(plan) {
...
export function registerYeonjangInstancesRoute(app, options = {}) {
    const fleetProjection = options.fleetProjection ?? buildYeonjangFleetProjection;
    const now = options.now ?? Date.now;
    const currentRevision = options.currentRevision ?? (options.fleetProjection ? () => 0 : currentYeonjangBindingRevision);
    ...
    const bindingProjectionRepository = options.bindingProjectionRepository ??
        (options.fleetProjection
            ...
            : {
                ...
                listBindings: () => listAgentCapabilityBindings({ capabilityKind: "yeonjang", includeArchived: true }),
            });
    const browserActiveTabInfoReadinessRecords = options.browserActiveTabInfoReadinessRecords ?? (() => []);
    const browserActiveTabInfoRedactedObservationForTarget = options.browserActiveTabInfoRedactedObservationForTarget ??
        ((publicTargetName) => {
    ...
    const browserActiveTabInfoReadinessSummary = (audience) => buildYeonjangBrowserActiveTabInfoPublicReadinessSummary({
        audience,
        observations: assembleYeonjangBrowserActiveTabInfoReadinessObservationsFromRegistry({
            records: browserActiveTabInfoReadinessRecords(),
        }),
    });
    const mutationActorForRequest = options.mutationActorForRequest ??
        ((request) => getApiAuthenticationPrincipal(request)?.principalRef ?? null);
    const reserveReceipt = (input) => reserveCapabilityMutationReceipt({
    ...
    const resolveIdentity = (yeonjangRef) => {
        const fleet = fleetProjection();
        const matches = fleet.instances.filter((instance) => createYeonjangPublicRef(instance.instanceId) === yeonjangRef);
        if (matches.length !== 1)
            return null;
        const instance = matches[0];
        if (!instance)
            return null;
        const projection = resolveYeonjangCapabilityDetail(publicProjection({ fleet, now: now() }), yeonjangRef);
        return projection ? { instance, projection } : null;
    };
    const recoveryExecutor = options.recoveryExecutor ??
        (async (input) => executeYeonjangRecoveryCommand(input, {
            now,
            currentRevision,
            ...
            persistIntent: async ({ expectedRevision, targetRevision }) => currentRevision() === expectedRevision
            ...
            applyAction: async ({ internalInstanceId, action }, signal) => {
                ...
                try {
                    await invokeYeonjangMethod(action === "reconnect" ? "node.ping" : "node.capabilities", {}, {
    ...
    const writeBinding = (input) => {
        ...
        }).find((binding) => binding.catalog_id === input.internalInstanceId &&
        ...
        upsertAgentCapabilityBinding({
            ...
            permissionProfile: parseRecord(existing?.permission_profile_json)
                ?? yeonjangCameraPermissionProfile(input.internalAgentId),
            ...(parseRecord(existing?.rate_limit_json) ? { rateLimit: parseRecord(existing?.rate_limit_json) } : {}),
    ...
    const bindingExecutor = options.bindingExecutor ??
        (async (input) => executeYeonjangBindingCommand(input, {
            now,
            currentRevision,
            nonceUsed: (nonce) => Boolean(getCapabilityMutationReceiptByNonce(nonce)),
            reserveReceipt,
            ...
            resolveYeonjang: (yeonjangRef) => {
                ...
                return identity
                    ? {
                        internalInstanceId: identity.instance.instanceId,
                        runnable: identity.projection.runnable,
                        scopeAllowed: identity.instance.scopeAccess === "allowed" &&
            ...
            bindingEnabled: ({ internalInstanceId, internalAgentId }) => listAgentCapabilityBindings({ capabilityKind: "yeonjang", includeArchived: true }).some((binding) => binding.catalog_id === internalInstanceId &&
                binding.agent_id === internalAgentId &&
                binding.status === "enabled"),
            persist: ({ internalInstanceId, internalAgentId, enabled, expectedRevision, targetRevision, }) => currentRevision() === expectedRevision &&
            ...
            verify: ({ internalInstanceId, internalAgentId, enabled, targetRevision }) => {
                const binding = listAgentCapabilityBindings({
                    capabilityKind: "yeonjang",
                    includeArchived: true,
                }).find((entry) => entry.catalog_id === internalInstanceId && entry.agent_id === internalAgentId);
                return {
                    ok: Boolean(binding &&
    ...
    app.get("/api/capabilities/yeonjang", { preHandler: authMiddleware }, async (request, reply) => {
        ...
        if (!input)
            return reply.status(400).send({ error: "yeonjang_query_invalid" });
        const startedAt = now();
        const result = queryYeonjangCapabilityCatalog(publicProjection({ fleet: fleetProjection(), now: startedAt }), input);
        if (!result.cursorValid)
            return reply.status(400).send({ error: "yeonjang_cursor_invalid" });
    ...
    app.get("/api/capabilities/yeonjang/:yeonjangRef", { preHandler: authMiddleware }, async (request, reply) => {
        if (!/^yeonjang_v1_[a-f0-9]{24}$/u.test(request.params.yeonjangRef))
            return reply.status(400).send({ error: "yeonjang_ref_invalid" });
        const detail = resolveYeonjangCapabilityDetail(publicProjection({ fleet: fleetProjection(), now: now() }), request.params.yeonjangRef);
        if (!detail)
            return reply.status(404).send({ error: "yeonjang_ref_not_found" });
    ...
    app.post("/api/capabilities/yeonjang/:yeonjangRef/recovery", { preHandler: authMiddleware }, async (request, reply) => {
        if (!/^yeonjang_v1_[a-f0-9]{24}$/u.test(request.params.yeonjangRef))
        ...
        if (!actorRef)
            return reply.status(403).send({ error: "yeonjang_recovery_actor_denied" });
    ...
    if (typeof app.patch === "function")
        app.patch("/api/capabilities/yeonjang/:yeonjangRef/bindings/:agentRef", { preHandler: authMiddleware }, async (request, reply) => {
            if (!/^yeonjang_v1_[a-f0-9]{24}$/u.test(request.params.yeonjangRef) ||
            ...
            const actorRef = mutationActorForRequest(request);
            if (!actorRef)
                return reply.status(403).send({ error: "yeonjang_binding_actor_denied" });
            const envelope = envelopeFrom(request.body.envelope, actorRef);
            if (!envelope)
                return reply.status(400).send({ error: "yeonjang_binding_request_invalid" });
            const startedAt = now();
    ...
    app.get("/api/yeonjang/browser-active-tab-info/readiness", { preHandler: authMiddleware }, async () => browserActiveTabInfoReadinessSummary("general"));
    app.get("/api/yeonjang/browser-active-tab-info/readiness/diagnostics", { preHandler: authMiddleware }, async () => browserActiveTabInfoReadinessSummary("advanced"));
    app.post("/api/yeonjang/browser-active-tab-info/pre-dispatch/preview", { preHandler: authMiddleware }, async (request) => {
        ...
        const plan = buildYeonjangBrowserActiveTabInfoPreDispatchBridgePlan({
            ...
            redactedProjection: body.redactedProjection ??
