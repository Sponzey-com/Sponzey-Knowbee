...
import type {
  AgentCapabilityBindingCommandInput,
  AgentCapabilityBindingReceipt,
} from "../../agents/agent-capability-binding-command.js"
import {
  type AgentCapabilityBindingProjection,
  type AgentCapabilityKind,
  queryAgentCapabilityBindings,
} from "../../agents/agent-capability-binding-projection.js"
import {
  type AgentIdentityCommand,
  type AgentIdentityMutationReceipt,
  publicAgentIdentityReceipt,
} from "../../agents/agent-identity-command.js"
import {
  type AgentOperationalSettingsCommand,
  type AgentOperationalSettingsMutationReceipt,
  projectAgentOperationalSettingsMutationLog,
} from "../../agents/agent-operational-settings-command.js"
import type { AgentOperationalSettingsProjection } from "../../agents/agent-operational-settings-projection.js"
import type {
  AgentRelationshipCommandInput,
  AgentRelationshipMutationReceipt,
} from "../../agents/agent-relationship-command.js"
import {
  type AgentRelationshipProjection,
  queryAgentRelationshipProjection,
} from "../../agents/agent-relationship-projection.js"
import type { AgentWorkspaceProjection } from "../../agents/agent-workspace-projection.js"
import {
  type AgentWorkspaceQueryInput,
  projectAgentWorkspaceQueryLog,
  queryAgentWorkspace,
  resolveAgentWorkspaceDetail,
} from "../../agents/agent-workspace-query.js"
...
export interface AgentWorkspaceRouteOptions {
  ...
  capabilityProjection?(
    ...
    agentRef: string,
  ): AgentCapabilityBindingProjection | null
  executeCapabilityBindingCommand?(
    request: FastifyRequest,
  ...
  settingsProjection?(
    request: FastifyRequest,
    agentRef: string,
  ): AgentOperationalSettingsProjection | null
  executeOperationalSettingsCommand?(
    request: FastifyRequest,
...
function capabilityKind(value: unknown): AgentCapabilityKind | null {
  return value === "skill" || value === "mcp_server" || value === "yeonjang" ? value : null
}

function capabilityRefValid(kind: AgentCapabilityKind, value: string): boolean {
  if (kind === "skill") return /^skill_v1_[a-f0-9]{24}$/u.test(value)
  if (kind === "mcp_server") return /^mcp_v1_[a-f0-9]{24}$/u.test(value)
...
function capabilityRiskLevel(value: unknown): value is CapabilityRiskLevel {
  return (
    value === "safe" ||
    value === "moderate" ||
    value === "external" ||
...
function operationalSettingsCommand(input: {
  ...
  now: number
  createMutationId: () => string
}): AgentOperationalSettingsCommand | null {
  const body = record(input.body)
  if (!onlyKeys(body, ["kind", "targetRevision", "value", "confirmElevation"])) return null
  if (!Number.isInteger(body.targetRevision) || (body.targetRevision as number) < 1) return null
  ...
  if (body.kind === "update_permission") {
    if (
      (body.confirmElevation !== undefined && typeof body.confirmElevation !== "boolean") ||
      !onlyKeys(value, [
        "riskCeiling",
...
function operationalSettingsStatus(receipt: AgentOperationalSettingsMutationReceipt): number {
  if (receipt.state === "active") return 200
  if (receipt.state === "conflict") return 409
  if (receipt.reasonCode === "agent_ref_not_found") return 404
  if (receipt.reasonCode === "agent_settings_inactive") return 410
  if (receipt.reasonCode === "mutation_scope_denied") return 403
...
function mutationEnvelope(value: unknown) {
  ...
  if (!mutationId || !nonce || !actorRef || envelope.scope !== "agent_identity") return null
  return { mutationId, nonce, actorRef, scope: "agent_identity" as const }
}

function mutationStatus(receipt: AgentIdentityMutationReceipt): number {
  if (receipt.state === "active") return 200
  if (receipt.state === "conflict") return 409
  return 400
}
...
function executeMutation(
  ...
  options: AgentWorkspaceRouteOptions,
  command: AgentIdentityCommand | null,
) {
...
function queryInput(value: unknown): AgentWorkspaceQueryInput {
...
export function registerAgentWorkspaceRoute(
  app: FastifyInstance,
  options: AgentWorkspaceRouteOptions,
): void {
        ...
        envelope && name !== null && role !== null
          ? {
              ...
              ...(requiredText(body.modelName)
                ? { modelName: requiredText(body.modelName) as string }
                : {}),
            }
          : null,
      )
  ...
  app.get("/api/agent-workspace", { preHandler: authMiddleware }, async (request, reply) => {
    ...
    const logInput = {
      status: result.cursorValid ? ("passed" as const) : ("failed" as const),
    ...
    if (!result.cursorValid) {
      options.logger?.development(
        projectAgentWorkspaceQueryLog({ ...logInput, level: "development" }),
      )
      return reply.status(400).send({ error: "agent_cursor_invalid" })
    }
    return result
  })
  app.get<{ Params: { agentRef: string } }>(
    "/api/agent-workspace/:agentRef/settings",
    { preHandler: authMiddleware },
    async (request, reply) => {
  ...
  app.patch<{ Params: { agentRef: string }; Body: unknown }>(
    "/api/agent-workspace/:agentRef/settings",
    { preHandler: authMiddleware },
    async (request, reply) => {
      if (!/^agent_v1_[a-f0-9]{24}$/u.test(request.params.agentRef))
      ...
      if (!command) return reply.status(400).send({ error: "agent_settings_request_invalid" })
      ...
      if (receipt.state !== "active")
        options.logger?.development(
          projectAgentOperationalSettingsMutationLog("development", receipt),
        )
      return reply.status(operationalSettingsStatus(receipt)).send(receipt)
    },
  )
  app.get<{ Params: { agentRef: string } }>(
    "/api/agent-workspace/:agentRef",
    { preHandler: authMiddleware },
    async (request, reply) => {
      if (!/^agent_v1_[a-f0-9]{24}$/u.test(request.params.agentRef))
        return reply.status(400).send({ error: "agent_ref_invalid" })
      const detail = resolveAgentWorkspaceDetail(
        options.projection(request),
        request.params.agentRef,
      )
      return detail ?? reply.status(404).send({ error: "agent_ref_not_found" })
    },
  )
  app.get<{ Params: { agentRef: string }; Querystring: unknown }>(
    "/api/agent-workspace/:agentRef/capabilities",
    { preHandler: authMiddleware },
    async (request, reply) => {
      if (!/^agent_v1_[a-f0-9]{24}$/u.test(request.params.agentRef))
        return reply.status(400).send({ error: "agent_ref_invalid" })
      ...
      if (!projection) return reply.status(404).send({ error: "agent_ref_not_found" })
  ...
  app.get<{ Querystring: unknown }>(
    "/api/agent-workspace/relationships",
    { preHandler: authMiddleware },
    async (request, reply) => {
  ...
  app.patch<{ Params: { childRef: string }; Body: unknown }>(
    "/api/agent-workspace/:childRef/parent",
    { preHandler: authMiddleware },
    async (request, reply) => {
      ...
      const body = record(request.body)
      const kind =
        body.kind === "connect" || body.kind === "reparent" || body.kind === "disconnect"
          ? body.kind
          : null
      ...
      const receipt = await options.executeRelationshipCommand(request, {
        ...
        parentRef,
        envelope: {
          actorRef: mutation.actorRef as string,
          scope: "agent_relationship:write",
          mutationId: mutation.mutationId as string,
          targetRevision: mutation.targetRevision as number,
          purpose: mutation.purpose as string,
          issuedAt: mutation.issuedAt,
          nonce: mutation.nonce as string,
        },
      })
      const fields = {
        kind: receipt.kind,
  ...
  app.patch<{
    Params: { agentRef: string; capabilityRef: string }
    Body: unknown
  }>(
    "/api/agent-workspace/:agentRef/capabilities/:capabilityRef",
    { preHandler: authMiddleware },
    async (request, reply) => {
      if (!/^agent_v1_[a-f0-9]{24}$/u.test(request.params.agentRef))
        return reply.status(400).send({ error: "agent_ref_invalid" })
      ...
      const receipt = await options.executeCapabilityBindingCommand(request, {
        ...
        bound: body.bound,
        envelope: {
          actorRef: envelope.actorRef as string,
          scope: "capability:write",
          mutationId: envelope.mutationId as string,
          targetRevision: envelope.targetRevision,
          purpose: envelope.purpose as string,
  ...
  app.patch<{ Params: { agentRef: string }; Body: unknown }>(
    ...
    async (request, reply) => {
      ...
      return executeMutation(
        ...
        reply,
        options,
        envelope && name !== null && role !== null && baseRevision !== null
          ? {
              kind: "update",
              envelope,
              agentRef: request.params.agentRef,
  ...
  app.post<{ Params: { agentRef: string }; Body: unknown }>(
    "/api/agent-workspace/:agentRef/archive",
    { preHandler: authMiddleware },
    async (request, reply) => {
      if (!/^agent_v1_[a-f0-9]{24}$/u.test(request.params.agentRef))
        return reply.status(400).send({ error: "agent_ref_invalid" })
