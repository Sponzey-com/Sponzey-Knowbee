...
import { createCapabilities, type CapabilityProjectionOptions } from "../../control-plane/index.js"
...
import {
  buildSkillCatalogPage,
  type SkillBindingRow,
  type SkillCatalogRow,
} from "../../capabilities/skill-catalog-query.js"
...
import { evaluateSkillSourceValidation, type SkillSourceInspection } from "../../capabilities/skill-source-validation.js"
import { executeSkillCreateCommand, type SkillCreateDraft, type SkillCreateUserReceipt } from "../../capabilities/skill-create-command.js"
import { executeSkillUpdateCommand, type SkillUpdateChange, type SkillUpdateSnapshot, type SkillUpdateUserReceipt } from "../../capabilities/skill-update-command.js"
import { executeSkillBindingCommand, type SkillBindingUserReceipt } from "../../capabilities/skill-binding-command.js"
import { executeSkillDeleteCommand, type SkillDeleteUserReceipt } from "../../capabilities/skill-delete-command.js"
...
export interface CapabilitiesRouteOptions extends Omit<CapabilityProjectionOptions, "config"> {
  ...
  skillSourceInspector?: (input: { requestedPath: string; allowedRoots: readonly string[] }) => SkillSourceInspection
  runtimeConfigForRequest?: (request: unknown) => Pick<KnowbeeConfig, "profile" | "security">
  skillCreateExecutor?: (input: { envelope: MutationEnvelope; draft: SkillCreateDraft; allowedRoots: readonly string[] }) => Promise<SkillCreateUserReceipt>
  skillUpdateExecutor?: (input: { envelope: MutationEnvelope; skillRef: string; change: SkillUpdateChange }) => Promise<SkillUpdateUserReceipt>
  skillBindingExecutor?: (input: { envelope: MutationEnvelope; skillRef: string; agentRef: string; action: "bind" | "unbind" }) => Promise<SkillBindingUserReceipt>
  skillDeleteExecutor?: (input: { envelope: MutationEnvelope; skillRef: string }) => Promise<SkillDeleteUserReceipt>
  mutationActorForRequest?: (request: unknown) => string | null
  now?: () => number
}

const defaultSkillCatalogRepository: SkillCatalogReadRepository = {
  listSkills: () => listSkillCatalogEntries(),
  listBindings: () => listAgentCapabilityBindings({ capabilityKind: "skill" }),
}

function buildCapabilitiesResponse(options: CapabilitiesRouteOptions, config: KnowbeeConfig) {
  return {
...
export function registerCapabilitiesRoute(app: FastifyInstance, options: CapabilitiesRouteOptions = {}): void {
  ...
  const runtimeConfigForRequest = options.runtimeConfigForRequest ?? ((request: unknown) => getApiRuntimeConfig(request as never))
  const now = options.now ?? Date.now
  const mutationActorForRequest = options.mutationActorForRequest ?? ((request: unknown) => {
    const principal = getApiAuthenticationPrincipal(request as never)
    if (principal) return principal.principalRef
  ...
  const currentSkillRevision = () => Math.max(
    listSkillCatalogEntries({ includeArchived: true }).reduce((revision, row) => Math.max(revision, row.updated_at), 0),
    listAgentCapabilityBindings({ capabilityKind: "skill", includeArchived: true }).reduce((revision, row) => Math.max(revision, row.updated_at), 0),
  )
  const reserveSkillReceipt = ({ envelope, state, now: reservedAt }: { envelope: MutationEnvelope; state: CapabilityMutationState; now: number }) => reserveCapabilityMutationReceipt({ mutationId: envelope.mutationId, nonce: envelope.nonce, actorRef: envelope.actorRef, scope: envelope.scope, purpose: envelope.purpose, capabilityKind: "skill", targetRevision: envelope.targetRevision, state, now: reservedAt })
  const updateSkillReceipt = (receipt: { mutationId: string; state: CapabilityMutationState; reasonCode: string | null; now: number }) => { updateCapabilityMutationReceipt(receipt) }
  const executeCreate = options.skillCreateExecutor ?? (async (input) => executeSkillCreateCommand({ envelope: input.envelope, draft: input.draft }, {
    ...
    persist: ({ internalSkillId, skillKind, draft, canonicalPath, expectedRevision, targetRevision }) => {
      ...
      if (actualRevision !== expectedRevision) return { ok: false, revision: actualRevision, reasonCode: "mutation_revision_conflict" }
  ...
  const resolveUpdateSnapshot = (skillRef: string): SkillUpdateSnapshot | null => {
  ...
  const writeUpdateSnapshot = (input: { internalSkillId: string; displayName: string; description: string; runtimeStatus: "active" | "inactive"; revision: number }): boolean => {
  ...
  const resolveAgent = (agentRef: string) => {
  ...
  const writeBinding = (input: { internalSkillId: string; internalAgentId: string; enabled: boolean; revision: number }): boolean => {
    ...
    upsertAgentCapabilityBinding({
      ...
      ...(existing?.permission_profile_json ? { permissionProfile: parseObject(existing.permission_profile_json) as never } : {}),
      ...(existing?.rate_limit_json ? { rateLimit: parseObject(existing.rate_limit_json) as never } : {}),
  ...
  const executeDelete = options.skillDeleteExecutor ?? (async (input) => executeSkillDeleteCommand(input, {
    ...
    persistArchive: ({ snapshot, expectedRevision, targetRevision }) => {
      if (currentSkillRevision() !== expectedRevision) return { ok: false, revision: currentSkillRevision(), reasonCode: "mutation_revision_conflict" }
      const row = listSkillCatalogEntries({ includeArchived: true }).find((item) => item.skill_id === snapshot.internalSkillId)
      if (!row) return { ok: false, revision: currentSkillRevision(), reasonCode: "skill_ref_not_found" }
  ...
  app.get<{ Querystring: { limit?: string; cursor?: string; search?: string; source?: string; status?: string; bound?: string } }>(
    ...
    async (req, reply) => {
      try {
        ...
        if (req.query.source && !sourceKind) return reply.status(400).send({ error: "skill_catalog_source_invalid" })
        if (req.query.status && !runtimeStatus) return reply.status(400).send({ error: "skill_catalog_status_invalid" })
        ...
        const queryReason = error instanceof Error && (
          error.message === "skill_catalog_limit_invalid"
          || error.message === "skill_catalog_cursor_invalid"
        ) ? error.message : null
        return queryReason
  ...
  app.get<{ Params: { skillRef: string } }>("/api/capabilities/skills/:skillRef", { preHandler: authMiddleware }, async (req, reply) => {
    try {
      ...
      if (matches.length !== 1) return reply.status(404).send({ error: "skill_ref_not_found" })
      ...
      if (!item) return reply.status(404).send({ error: "skill_ref_not_found" })
  ...
  app.post<{ Body: { displayName?: string; sourceKind?: string; requestedPath?: string } }>(
    "/api/capabilities/skills/validate",
    { preHandler: authMiddleware },
    async (req, reply) => {
  ...
  app.post<{ Body: { envelope?: Partial<MutationEnvelope>; draft?: Partial<SkillCreateDraft> } }>(
    "/api/capabilities/skills",
    { preHandler: authMiddleware },
    async (req, reply) => {
      ...
      try {
        const config = runtimeConfigForRequest(req)
        const receipt = await executeCreate({ envelope: { ...envelope, actorRef: mutationActor } as MutationEnvelope, draft: draft as SkillCreateDraft, allowedRoots: [config.profile.workspace, ...config.security.allowedPaths] })
  ...
  if (typeof app.patch === "function") app.patch<{ Params: { skillRef: string }; Body: { envelope?: Partial<MutationEnvelope>; change?: Record<string, unknown> } }>(
    "/api/capabilities/skills/:skillRef",
    { preHandler: authMiddleware },
    async (req, reply) => {
      const envelope = req.body?.envelope
      ...
      const mutationActor = mutationActorForRequest(req)
      if (!mutationActor) return reply.status(403).send({ error: "skill_update_actor_denied" })
      try {
        const receipt = await executeUpdate({ envelope: { ...envelope, actorRef: mutationActor } as MutationEnvelope, skillRef: req.params.skillRef, change: change as SkillUpdateChange })
  ...
  if (typeof app.patch === "function") app.patch<{ Params: { skillRef: string; agentRef: string }; Body: { envelope?: Partial<MutationEnvelope>; bound?: boolean } }>(
    "/api/capabilities/skills/:skillRef/bindings/:agentRef",
    { preHandler: authMiddleware },
    async (req, reply) => {
      const envelope = req.body?.envelope
      if (!envelope || typeof req.body?.bound !== "boolean" || typeof envelope.scope !== "string" || typeof envelope.mutationId !== "string" || typeof envelope.targetRevision !== "number" || typeof envelope.purpose !== "string" || typeof envelope.issuedAt !== "number" || typeof envelope.nonce !== "string") return reply.status(400).send({ error: "skill_binding_request_invalid" })
      const mutationActor = mutationActorForRequest(req)
      if (!mutationActor) return reply.status(403).send({ error: "skill_binding_actor_denied" })
      try {
        const receipt = await executeBinding({ envelope: { ...envelope, actorRef: mutationActor } as MutationEnvelope, skillRef: req.params.skillRef, agentRef: req.params.agentRef, action: req.body.bound ? "bind" : "unbind" })
  ...
  if (typeof app.delete === "function") app.delete<{ Params: { skillRef: string }; Body: { envelope?: Partial<MutationEnvelope> } }>(
    "/api/capabilities/skills/:skillRef",
    { preHandler: authMiddleware },
    async (req, reply) => {
      const envelope = req.body?.envelope
      if (!envelope || typeof envelope.scope !== "string" || typeof envelope.mutationId !== "string" || typeof envelope.targetRevision !== "number" || typeof envelope.purpose !== "string" || typeof envelope.issuedAt !== "number" || typeof envelope.nonce !== "string") return reply.status(400).send({ error: "skill_delete_request_invalid" })
      const mutationActor = mutationActorForRequest(req)
      if (!mutationActor) return reply.status(403).send({ error: "skill_delete_actor_denied" })
      try {
        const receipt = await executeDelete({ envelope: { ...envelope, actorRef: mutationActor } as MutationEnvelope, skillRef: req.params.skillRef })
