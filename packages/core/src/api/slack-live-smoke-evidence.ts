...
export interface SlackLiveSmokeEvidenceReaderDependencies {
  listMessageLedgerEvents(input: { runId: string; limit?: number }): readonly DbMessageLedgerEvent[]
  listChannelMessageRefsForRun(runId: string): readonly DbChannelMessageRef[]
}

export function createSlackLiveSmokeEvidenceReader(
  dependencies: SlackLiveSmokeEvidenceReaderDependencies,
): (
