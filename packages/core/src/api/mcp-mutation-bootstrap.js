...
import { createMcpConfigurationStore, createMcpRegistryApplyAdapter, } from "../capabilities/mcp-mutation-adapters.js";
import { createMcpMutationRuntime, } from "../capabilities/mcp-mutation-runtime.js";
...
import { getCapabilityMutationReceiptByNonce, listAgentCapabilityBindings, listAgentConfigs, listMcpServerCatalogEntries, reserveCapabilityMutationReceipt, updateCapabilityMutationReceipt, upsertMcpServerCatalogEntry, } from "../db/index.js";
...
export function createApiMcpMutationRuntime(input) {
    ...
    const catalog = {
        list: (_includeArchived) => catalogRows(),
        write: (row) => upsertMcpServerCatalogEntry({
    ...
    const store = createMcpConfigurationStore({
        ...
        catalog,
        externalRevision: () => Math.max(0, ...listAgentCapabilityBindings({ capabilityKind: "mcp_server" }).map((binding) => binding.updated_at)),
    });
    const runtime = createMcpRegistryApplyAdapter({
        ...
        registry: {
            ...
            reloadTarget: ({ name, config, defaultCwd, baseEnv }) => mcpRegistry.reloadServer(name, config, { defaultCwd, baseEnv: { ...baseEnv } }),
        },
    });
    return createMcpMutationRuntime({
        ...
        inspection: {
            inspect: async (draft, signal) => {
                ...
                if (!directory.ok)
                    return directory;
                const receipt = await probeMcpConnectionDraft(directory.draft, {
                    now,
                    probe: async (normalized) => {
                        const result = await testMcpServerConnection({
                            ...
                            status: "planned",
                            tools: [],
                        }, input.config.profile.workspace, { baseEnv: input.mcpProcessEnv, signal });
                        return {
                            ok: result.ok,
        ...
        receipts: {
            ...
            reserveReceipt: ({ envelope, state, now: reservedAt }) => reserveCapabilityMutationReceipt({
        ...
        boundAgentNames: (internalMcpId) => {
            const ids = new Set(listAgentCapabilityBindings({ capabilityKind: "mcp_server", enabledOnly: true })
...
//# sourceMappingURL=mcp-mutation-bootstrap.js.map