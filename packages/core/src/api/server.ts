...
import {
  type ArtifactCleanupEvidenceResolver,
...
import { type MemoryJournalRepository, createMemoryJournalRepository } from "../memory/journal.js"
...
import {
  ...
  markGatewayReady,
  markGatewayStarting,
} from "../runtime/gateway-readiness.js"
...
} from "./routes/live-acceptance.js"
...
let apiMemoryJournal: MemoryJournalRepository | null = null
...
export function createAvailableChannelSmokeLiveExecutor(input: {
  ...
  if (!input.webui && !input.telegram && !input.slack) return undefined
      ...
      scenario.channel === "webui"
        ...
        : scenario.channel === "telegram"
          ...
          : scenario.channel === "slack"
            ? input.slack
            : undefined
    if (!executor) throw new Error(`channel_live_smoke_executor_unavailable:${scenario.channel}`)
    return executor(scenario)
  }
}

const resolveArtifactCleanupEvidence: ArtifactCleanupEvidenceResolver = (artifact) => {
  const referencedRuns = new Map<string, ReturnType<typeof getRootRun>>()
  if (artifact.source_run_id) {
    const sourceRun = getRootRun(artifact.source_run_id)
    if (sourceRun) referencedRuns.set(sourceRun.id, sourceRun)
  }
...
function createUiModeRuntimeInput(runtime: ApiServerRuntimeContext): UiModeRuntimeInput {
...
export async function startServer(
  cfg: KnowbeeConfig,
  paths: RuntimePaths,
  runtime: ApiServerRuntimeContext,
): Promise<void> {
  if (!cfg.webui.enabled) return

  if (!runtime.startupProgress) markGatewayStarting()
  ...
  const capabilityProjectionOptions = {
    enterpriseTopologyBuilderEnabled: parseEnterpriseTopologyBuilderUiEnabled(
      runtime.enterpriseTopologyBuilderUi,
    ),
  }
  ...
  const readLiveSmokeDecisionReceiptRefs = createLiveSmokeDecisionReceiptReader(
    new SqliteLlmInvocationReceiptRepository(),
    new SqliteCapabilityAdmissionEvidenceReader(getDb()),
  )
  const readLiveSmokeFirstResponseLatency =
    createLiveSmokeFirstResponseLatencyReader({ list: listLatencyMetrics })
  ...
  const webUiLiveSmokeExecutor = runtime.channelSmokeLiveEnabled
    ? createWebUiLiveSmokeExecutor(
        createWebUiLiveSmokeRuntimePorts({
          startCanonicalRequest: (request) =>
            startCanonicalLocalRun({
              artifactStorage,
              memoryJournal,
              hierarchyStorage,
              message: request,
    ...
    : undefined
  const telegramLiveSmokeExecutor =
    runtime.channelSmokeLiveEnabled && runtime.telegramLiveSmokeTarget
      ? createTelegramLiveSmokeExecutor(
          createTelegramLiveSmokeRuntimePorts({
            target: runtime.telegramLiveSmokeTarget,
            startCanonicalRequest: async ({ request, target }) => {
              const channel = getActiveTelegramChannel()
              if (!channel) throw new Error("telegram_live_smoke_runtime_unavailable")
      ...
      : undefined
  const slackLiveSmokeExecutor =
    runtime.channelSmokeLiveEnabled && runtime.slackLiveSmokeTarget
      ? createSlackLiveSmokeExecutor(
          createSlackLiveSmokeRuntimePorts({
            target: runtime.slackLiveSmokeTarget,
            startCanonicalRequest: async ({ request, target }) => {
              const channel = getActiveSlackChannel()
              if (!channel) throw new Error("slack_live_smoke_runtime_unavailable")
              return channel.acceptLiveSmokeRequest({ request, target })
            },
            observabilityRepository: new SqliteTypedObservabilityEventRepository(),
            listTopologyRunsForRootRun,
  ...
  const channelSmokeLiveExecutor =
    runtime.channelSmokeLiveExecutor ?? defaultChannelSmokeLiveExecutor
  ...
  const readinessItem = (
    capability: "webui" | "telegram" | "slack" | "web",
    ready: boolean,
  ...
  const inspectLiveAcceptanceReadiness = (): readonly LiveAcceptanceCapabilityReadiness[] =>
    Object.freeze([
      readinessItem("webui", Boolean(runtime.channelSmokeLiveExecutor ?? webUiLiveSmokeExecutor)),
      readinessItem(
        "telegram",
        Boolean(runtime.channelSmokeLiveExecutor ?? telegramLiveSmokeExecutor),
      ),
      readinessItem("slack", Boolean(runtime.channelSmokeLiveExecutor ?? slackLiveSmokeExecutor)),
      readinessItem(
        "web",
        Boolean(runtime.liveAcceptanceExecutor ?? runtime.liveAcceptanceExecutorFactory),
      ),
  ...
  try {
    ...
    startArtifactCleanupScheduler(
      { cleanupEvidence: resolveArtifactCleanupEvidence },
      artifactStorage,
    )
