import { checkPromptSourceLocaleParity, dryRunPromptSourceAssembly, inspectPromptSourceAssemblyCoverage, loadPromptSourceRegistry, promptSourceFileExists, } from "./knowbee-md.js";
import { existsSync, readdirSync, readFileSync } from "node:fs";
import { basename, dirname, join } from "node:path";
import { validateGoalOwnershipCatalog } from "../maintenance/goal-ownership.js";
import { CANONICAL_PROMPT_RESPONSIBILITY_MANIFEST, validateCanonicalPromptResponsibilityManifest, } from "../contracts/canonical-prompt-responsibility-manifest.js";
const EXPECTED_PROMPT_SOURCE_IDS = [
    ...
    "final_response_evidence_review",
    "final_response_evidence_review_user",
    "final_response_user",
...
function validateOrphanPromptSourceFiles(workDir, sources) {
    const registeredEnglishFilenames = new Set(sources
        .filter((source) => source.locale === "en")
        .map((source) => basename(source.path)));
    const promptDirs = new Set(sources.map((source) => dirname(source.path)));
    const workDirPrompts = join(workDir, "prompts");
    if (existsSync(workDirPrompts))
        promptDirs.add(workDirPrompts);
    const issues = [];
    for (const promptDir of promptDirs) {
        ...
        for (const filename of filenames) {
            if (!filename.endsWith(".md") || filename.endsWith(".ko.md"))
                continue;
            if (registeredEnglishFilenames.has(filename))
                continue;
            issues.push(makeIssue({
                severity: "error",
...
const ALLOWED_KOREAN_PROMPT_LITERALS = new Set([
    "깊게 봐줘",
    "다운로드",
    "다운도르",
    "노비",
...
const ALLOWED_KOREAN_OUTPUT_TEMPLATE_LINES = new Map();
function isAllowedKoreanPromptLiteral(value) {
    return ALLOWED_KOREAN_PROMPT_LITERALS.has(value.trim());
}
function stripAllowedKoreanPromptSegments(line) {
    return line
        .replace(/`([^`\n]*[가-힣][^`\n]*)`/gu, (segment, value) => (isAllowedKoreanPromptLiteral(value) ? "" : segment))
        .replace(/"([^"\n]*[가-힣][^"\n]*)"/gu, (segment, value) => (isAllowedKoreanPromptLiteral(value) ? "" : segment))
        .replace(/'([^'\n]*[가-힣][^'\n]*)'/gu, (segment, value) => (isAllowedKoreanPromptLiteral(value) ? "" : segment));
}
function stripAllowedKoreanOutputTemplateLine(sourceId, line) {
    const allowed = ALLOWED_KOREAN_OUTPUT_TEMPLATE_LINES.get(sourceId) ?? [];
    return allowed.some((pattern) => pattern.test(line.trim())) ? "" : line;
}
function validateEnglishPromptLocale(sources) {
    const issues = [];
    for (const source of sources) {
        if (source.locale !== "en")
            continue;
        const line = source.content
            ...
            .find((candidate) => /[가-힣]/u.test(stripAllowedKoreanOutputTemplateLine(source.sourceId, stripAllowedKoreanPromptSegments(candidate))));
        if (!line)
            continue;
        issues.push(makeIssue({
            severity: "error",
...
function validatePlanningArtifactCopies(sources) {
    const issues = [];
    const planningArtifactPattern = /(?:^\s*[-*]\s*\[[ xX]\]\s+|\.tasks\/task\d+\.md\b)/u;
    for (const source of sources) {
        if (source.sourceId.endsWith("_user"))
            continue;
...
function validateCanonicalBoundarySections(sources) {
    const issues = [];
    const canonicalIds = new Set(CANONICAL_BOUNDARY_PROMPT_SOURCE_IDS);
    for (const source of sources) {
        if (source.locale !== "en" || !canonicalIds.has(source.sourceId))
            continue;
...
function validateCanonicalResponsibilityManifest() {
    const decision = validateCanonicalPromptResponsibilityManifest(CANONICAL_PROMPT_RESPONSIBILITY_MANIFEST);
...
function validateAgentNameTerminology(sources) {
    const issues = [];
    for (const source of sources) {
        if (source.locale !== "en")
            continue;
...
function validateAssemblyCoverage(workDir, locales) {
    const results = [];
    for (const locale of locales) {
        const assembly = dryRunPromptSourceAssembly(workDir, locale).assembly;
        if (!assembly)
...
export function runPromptSourceRegression(workDir, options = {}) {
