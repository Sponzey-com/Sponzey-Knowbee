...
import {
  type PromptImprovementActivationState,
  type PromptImprovementHarnessBlockedDecision,
  type PromptImprovementHarnessInput,
  type PromptImprovementHarnessReport,
  type PromptImprovementHarnessValidationResult,
  type PromptImprovementMutableSourceAuditRecord,
  type PromptImprovementSourceWriteState,
...
import {
  PromptSourceContentQualityError,
  validatePromptSourceContentQuality,
} from "./prompt-source-quality.js"
...
export function listPromptSourceDefinitions(): PromptSourceDefinition[] {
...
export const REQUIRED_RUNTIME_PROMPT_SOURCE_IDS = PROMPT_SOURCE_DEFINITIONS.filter(
  (definition) => definition.required && definition.defaultRuntime,
...
function promptSeedFilenames(definition: PromptSourceDefinition, _locale: "ko" | "en"): string[] {
...
const PROMPT_SOURCE_SECRET_PATTERNS: Array<{ marker: string; pattern: RegExp }> = [
  {
    ...
    pattern:
      /\b(?:api[_ -]?key|apikey)\b\s*[:=]\s*["']?(?!unknown|none|미확정|없음)[A-Za-z0-9_./+=-]{16,}/i,
  },
  {
    marker: "oauth_token_assignment",
    pattern:
      /\b(?:oauth[_ -]?token|access[_ -]?token|refresh[_ -]?token)\b\s*[:=]\s*["']?(?!unknown|none|미확정|없음)[A-Za-z0-9_./+=-]{16,}/i,
  },
  {
    marker: "bot_token_assignment",
    pattern:
      /\b(?:bot[_ -]?token|telegram[_ -]?token|slack[_ -]?token)\b\s*[:=]\s*["']?(?!unknown|none|미확정|없음)[A-Za-z0-9_./+=-]{16,}/i,
  },
  {
    marker: "channel_secret_assignment",
    pattern:
      /\b(?:channel[_ -]?secret|client[_ -]?secret|signing[_ -]?secret)\b\s*[:=]\s*["']?(?!unknown|none|미확정|없음)[A-Za-z0-9_./+=-]{16,}/i,
  },
 ...
 */
export function loadKnowbeeMd(workDir: string): string | null {
  let current = workDir
  for (let i = 0; i < 4; i++) {
...
export function detectPromptSourceSecretMarkers(content: string): string[] {
  return PROMPT_SOURCE_SECRET_PATTERNS.filter(({ pattern }) => pattern.test(content)).map(
    ({ marker }) => marker,
  )
}
...
function applyPromptSourceStates(
  sources: LoadedPromptSource[],
...
function selectRuntimePromptSources(
  sources: LoadedPromptSource[],
  locale: "ko" | "en",
): LoadedPromptSource[] {
  ...
  for (const source of sources) {
    ...
    if (
      !PROMPT_SOURCE_DEFINITIONS.find((definition) => definition.sourceId === source.sourceId)
        ?.defaultRuntime
    )
      continue
...
function selectPromptSourcesByUsageScope(
  ...
  const selected: LoadedPromptSource[] = []
  for (const definition of PROMPT_SOURCE_DEFINITIONS.filter(
    (item) => item.usageScope === usageScope,
  )) {
    const candidates = bySourceId.get(definition.sourceId) ?? []
    const preferred = candidates.find((source) => source.locale === "en")
    if (preferred) selected.push(preferred)
...
function buildRequiredPromptSourceDiagnostics(
  ...
  return PROMPT_SOURCE_DEFINITIONS.filter(
    (definition) => definition.required && definition.usageScope === usageScope,
  )
...
function promptSourceAssemblyFragment(
  source: LoadedPromptSource,
  variables: PromptTemplateVariables = {},
): string {
...
function promptSourceAssemblyHeader(source: LoadedPromptSource): string {
...
function promptSourceAssemblyContextLabel(
  key: string,
  variables: PromptTemplateVariables = {},
): string {
...
export function inspectPromptSourceAssemblyCoverage(
  assembly: PromptSourceAssembly,
  variables: PromptTemplateVariables = {},
): PromptSourceAssemblyCoverageReport {
  const items = assembly.sources.map((source): PromptSourceAssemblyCoverageItem => {
...
function renderPromptSourceAssemblyText(
  sources: LoadedPromptSource[],
  variables: PromptTemplateVariables = {},
): string {
  ...
  if (fullText.length <= MAX_SYSTEM_PROMPT_SIZE) return fullText

  const tailStartIndex = sources.findIndex((source) =>
    RUNTIME_TAIL_PRESERVATION_SOURCE_IDS.includes(source.sourceId),
  )
  if (tailStartIndex < 0) return fullText.slice(0, MAX_SYSTEM_PROMPT_SIZE)
  ...
  const prefixBudget =
    MAX_SYSTEM_PROMPT_SIZE - reservedTail.length - PROMPT_SOURCE_FRAGMENT_SEPARATOR.length
...
export function loadBundledPromptTemplate(input: {
  ...
  if (!content) throw new Error(`bundled prompt template not found: ${input.sourceId}`)
  return renderPromptTemplate(content, input.variables ?? {})
}
...
export function loadSystemPromptSourceAssembly(
  ...
  states: PromptSourceState[] = [],
  variables: PromptTemplateVariables = {},
  profile: SystemPromptAssemblyProfile = "full",
...
export function loadFirstRunPromptSourceAssembly(
  workDir: string,
  locale: "ko" | "en" = "en",
  states: PromptSourceState[] = [],
): PromptSourceAssembly | null {
...
export function promptSourceFileExists(
  workDir: string,
  sourceId: string,
  locale: "ko" | "en",
): boolean {
...
export function buildPromptSourceContentDiff(
...
export function createPromptSourceBackup(
  workDir: string,
  sourceId: string,
  locale: "ko" | "en",
...
export function importPromptSourcesFromFile(input: {
  ...
  const parsed = JSON.parse(
    readFileSync(input.exportPath, "utf-8"),
  ) as Partial<PromptSourceExportFile>
  if (
    parsed.kind !== "knowbee.prompt-sources.export" ||
    parsed.version !== 1 ||
  ...
  return {
    ...
    skipped,
    backups,
    registry: loadPromptSourceRegistry(input.workDir).map(
...
export function writePromptSourceWithBackup(input: {
  ...
  if (!quality.ok) throw new PromptSourceContentQualityError(quality.issues)
  ...
  const backup =
    diff.changed && input.createBackup !== false
  ...
  const source = loadPromptSourceRegistry(input.workDir).find(
    (item) => item.sourceId === input.sourceId && item.locale === input.locale,
  )
...
export class PromptSourceHarnessValidationError extends Error {
  ...
  constructor(
    ...
    decision?: PromptImprovementHarnessBlockedDecision,
  ) {
...
export function writePromptSourceWithHarness(input: {
  ...
  const harnessValidation: PromptImprovementHarnessValidationResult = {
    ok: true,
    risk: harnessDecision.risk,
    issues: [],
  }
  const harnessInput = input.harnessInput as PromptImprovementHarnessInput
      ...
      issues: [
        {
          ...
          path: "sourceId",
          message:
            "The prompt source being written must exactly match a validated targetPromptSources entry.",
        },
      ],
    })
  }
  const currentSource = loadPromptSourceRegistry(input.workDir).find(
    (source) => source.sourceId === input.sourceId && source.locale === input.locale,
  )
  ...
  const sourceWriteState: PromptImprovementSourceWriteState = result.diff.changed
    ? "written"
    : "unchanged"
  const activationState: PromptImprovementActivationState =
    sourceWriteState === "written" ? "activation_pending" : "unchanged"
...
export function rollbackPromptSourceBackup(input: {
  ...
  return {
    ...
    activationStateAfterRollback: "rolled_back",
    remainingRisk:
      "Runtime may still need reload or restart before the restored prompt source is active.",
    nextRecommendedAction:
      "Run prompt regression checks and confirm runtime activation before reporting the prompt as active.",
  }
}
...
export function dryRunPromptSourceAssembly(
  ...
  return {
    ...
    diagnostics:
      assembly?.snapshot.diagnostics ?? buildRequiredPromptSourceDiagnostics([], locale, "runtime"),
  }
}
...
export function initKnowbeeMd(dir: string): string {
