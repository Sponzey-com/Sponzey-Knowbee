...
function memoryRestorePromptContextLabel(key, variables = {}) {
...
function renderMessageContent(message) {
    ...
    return message.content.map((block) => {
        ...
        if (block.type === "tool_use")
            return `[${memoryRestorePromptContextLabel("structured_tool_use_label")}:${block.name}] ${safeJson(block.input)}`;
        if (block.type === "tool_result") {
            return `[${memoryRestorePromptContextLabel("structured_tool_result_label")}:${block.tool_use_id}] ${typeof block.content === "string" ? block.content : safeJson(block.content)}`;
        }
...
function renderCapsuleInline(header, capsule, maxSummaryChars) {
