...
const PROTECTED_COMMON_SOURCE_REFS: ReadonlySet<string> = new Set(PROTECTED_COMMON_PROMPT_SOURCES.map((source) => source.sourceRef))
...
export function authorizeImprovementMutation(input: {
  ...
  if (input.target.targetKind !== "file") {
    if (input.target.targetKind === "compiled_artifact") return { status: "blocked", reasonCode: "compiled_artifact_forbidden" }
    if (input.target.targetKind === "provider_configuration") return { status: "blocked", reasonCode: "provider_configuration_forbidden" }
    if (input.target.targetKind === "runtime_environment") return { status: "blocked", reasonCode: "runtime_environment_forbidden" }
    if (input.target.targetKind === "yeonjang_permission_policy") return { status: "blocked", reasonCode: "yeonjang_permission_policy_forbidden" }
  ...
  if (!requested || !canonical) return { status: "blocked", reasonCode: "path_receipt_invalid" }
  ...
  if (input.target.traversedSymlink) return { status: "blocked", reasonCode: "symlink_forbidden" }
  if (COMPILED_ARTIFACT.test(canonical)) return { status: "blocked", reasonCode: "compiled_artifact_forbidden" }
  ...
  if (DEPLOYMENT_SCRIPT.test(canonical)) return { status: "blocked", reasonCode: "deployment_script_forbidden" }
  if (PROVIDER_CONFIGURATION.test(canonical)) return { status: "blocked", reasonCode: "provider_configuration_forbidden" }
  if (RUNTIME_ENVIRONMENT.test(canonical)) return { status: "blocked", reasonCode: "runtime_environment_forbidden" }
  if (YEONJANG_PERMISSION_POLICY.test(canonical)) return { status: "blocked", reasonCode: "yeonjang_permission_policy_forbidden" }
  if (input.target.sourceAuthorization === "prompt_source") {
    ...
    if (PROTECTED_COMMON_SOURCE_REFS.has(canonical)) {
      ...
      if (!approval) return { status: "blocked", reasonCode: "common_policy_approval_required" }
...
export async function executeAuthorizedImprovementMutation<T>(input: {
  decision: ImprovementMutationDecision
  mutate: (target: ImprovementMutationTargetReceipt) => Promise<T>
}): Promise<{ status: "mutated"; result: T } | { status: "blocked"; reasonCode: string }> {
