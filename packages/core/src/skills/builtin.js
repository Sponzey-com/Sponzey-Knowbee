import { listAgentCapabilityBindings, upsertAgentCapabilityBinding, upsertSkillCatalogEntry, } from "../db/index.js";
export const YEONJANG_SKILL_ID = "skill:yeonjang";
export const WEB_RESEARCH_SKILL_ID = "skill:web-research";
export const WEB_RESEARCH_SKILL_TOOL_NAMES = ["web_search", "web_fetch"];
...
function backfillYeonjangCameraBindings(now) {
    for (const binding of listAgentCapabilityBindings({
        capabilityKind: "yeonjang",
        includeArchived: true,
    })) {
        if (binding.status !== "enabled" || binding.permission_profile_json)
            continue;
        ...
        upsertAgentCapabilityBinding({
            ...
            ...(parseRecord(binding.rate_limit_json) ? { rateLimit: parseRecord(binding.rate_limit_json) } : {}),
...
export function registerBuiltinSkills(options = {}) {
