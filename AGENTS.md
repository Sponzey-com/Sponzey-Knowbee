...
- 노우비도 에이전트이고, 모든 에이전트는 자신의 direct child를 가질 수 있다.
...
- 서브 에이전트 설정은 쉬운 설정과 고급 설정 양쪽에서 접근 가능해야 하며, 둘은 같은 source-of-truth에서 파생된 서로 다른 view model이어야 한다.
- 쉬운 설정은 이름/별명/역할/하는 일/준비 상태처럼 필수 항목만 다루고, 고급 설정은 모델, Skill/MCP, 메모리, 권한, 위임, 모니터링을 section 단위로 다룬다.
- 단일 노우비 모드에서 서브 에이전트가 없다는 이유만으로 오류 상태를 만들지 않는다.
- 고급 설정에서도 raw 배열, raw contract, secret, token, 내부 ID를 그대로 편집/노출하지 않는다.
