...
describe("task008 web retrieval fixture regression", () => {
  it("runs finance, weather, timeout fallback, and no-network fixtures without network", () => {
    ...
    expect(
      summary.results.find((result) => result.fixtureId === "finance-kospi-latest")
        ?.llmDiagnosisExpectation,
    ).toMatchObject({
      status: "followup",
      requiredEvidenceSourceIds: ["kospi-search-snippet"],
      changedStrategyRequired: true,
    })
    expect(
      summary.results.find(
        (result) => result.fixtureId === "finance-nasdaq-browser-timeout-fallback",
      )?.attempts,
    ).toBe(2)
    expect(
      summary.results.find((result) => result.fixtureId === "no-network-limited-completion")
        ?.llmDiagnosisExpectation,
    ).toMatchObject({
      status: "followup",
      requiredConditionVerdicts: ["existence:unsatisfied"],
    })
  ...
  it("fails an early surrender regression before the minimum source ladder is exhausted", () => {
    const fixture = loadWebRetrievalFixturesFromDir(fixtureDir).find(
      (item) => item.id === "no-network-limited-completion",
    )
