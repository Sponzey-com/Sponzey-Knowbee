...
function useTempState(): void {
  ...
  const rootDir = mkdtempSync(join(tmpdir(), "knowbee-task0020-work-record-snapshot-"))
...
describe("task0020 runtime work record snapshot ledger", () => {
  it("records valid handoff and child result snapshots during delegated execution", async () => {
    seedDispatchParent("run:parent:snapshot")
    const provider = new DualDiagnosisProvider()

    const result = await dispatchForSnapshot({
      parentRunId: "run:parent:snapshot",
      diagnosisProvider: provider,
      diagnosisRepairProvider: provider,
    })
    const snapshots = snapshotPayloads("run:parent:snapshot")
    const persistedSubSessions = listRunSubSessionsForParentRun("run:parent:snapshot")
    const exchange = getAgentDataExchange(
      "exchange:handoff:command:run:parent:snapshot:task:implement",
    )
    ...
    expect(persistedSubSessions).toHaveLength(1)
    expect(JSON.parse(persistedSubSessions[0]!.contract_json)).toMatchObject({
      delegatedExecutionSnapshotFingerprint: expect.stringMatching(/^sha256:[a-f0-9]{64}$/u),
    })
    expect(exchange).toMatchObject({
      source_owner_type: "knowbee",
      source_owner_id: "agent:knowbee",
      recipient_owner_type: "sub_agent",
      recipient_owner_id: "agent:alpha",
    })
    expect(JSON.parse(exchange!.provenance_refs_json)).toEqual(expect.arrayContaining([
      "work-handoff:handoff:command:run:parent:snapshot:task:implement",
      expect.stringMatching(/^execution-snapshot:sha256:[a-f0-9]{64}$/u),
    ]))
  ...
  it("blocks dispatch and does not record work record snapshots when runtime diagnosis is missing", async () => {
    seedDispatchParent("run:parent:missing")

    const result = await dispatchForSnapshot({ parentRunId: "run:parent:missing" })
    ...
    expect(snapshotPayloads("run:parent:missing")).toHaveLength(0)
  })
})
