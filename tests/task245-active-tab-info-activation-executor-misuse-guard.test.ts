...
describe("task245 active tab info activation executor misuse guard", () => {
  it("rejects approval evidence that tries to carry executor dry-run plan", () => {
    ...
    expect(validateReleaseApprovalEvidenceProjection({
      ...evidence,
      yeonjangBrowserActiveTabInfoActivationExecutorBoundary: executorDryRunPlan(),
    })).toEqual({
      status: "rejected",
  ...
  it("does not accept executor dry-run plan as final response or product log evidence", () => {
    ...
    expect(buildYeonjangBrowserActiveTabInfoProductLogProjection({
      evidenceRef,
      fields: ["evidenceRef", "yeonjangBrowserActiveTabInfoActivationExecutorBoundary"],
    })).toEqual({
      ok: false,
