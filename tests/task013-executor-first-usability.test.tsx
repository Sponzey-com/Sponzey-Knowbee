...
describe("task013 Executor-first usability", () => {
  it("covers the first-executor to run-history happy path with only three typed input kinds", () => {
    ...
    expect(combinedHtml).toContain("메인 에이전트가 이해한 내용")
    ...
    expect(combinedHtml).not.toContain("실행자")
  ...
  it("keeps blocked internal terms out of the Simple UI and normalizes advanced requests to the same executor-first screen", () => {
    ...
    expect(simpleHtml).toContain("서브 에이전트 구성하기")
    expect(simpleHtml).toContain("서브 에이전트 이름과 성격 정하기")
    ...
    expect(simpleHtml).not.toContain("먼저 서브 에이전트 1개를 추가하세요")
    expect(simpleHtml).not.toContain("첫 서브 에이전트 추가")
