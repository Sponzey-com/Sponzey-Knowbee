...
describe("task251 active tab info runtime change skeleton misuse guard", () => {
  it("rejects approval evidence that tries to carry runtime change skeleton", () => {
    ...
    expect(validateReleaseApprovalEvidenceProjection({
      ...evidence,
      yeonjangBrowserActiveTabInfoRuntimeChangeSkeleton: runtimeChangeSkeleton(),
    })).toEqual({
      status: "rejected",
  ...
  it("does not accept runtime change skeleton as final response or product log evidence", () => {
    ...
    expect(buildYeonjangBrowserActiveTabInfoProductLogProjection({
      evidenceRef,
      fields: ["evidenceRef", "yeonjangBrowserActiveTabInfoRuntimeChangeSkeleton"],
    })).toEqual({
      ok: false,
