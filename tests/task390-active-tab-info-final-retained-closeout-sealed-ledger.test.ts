...
describe("task390 active tab info final retained closeout sealed ledger", () => {
  it("builds a minimal redacted final retained closeout sealed ledger without release or activation readiness", () => {
    const ledger =
      buildYeonjangBrowserActiveTabInfoFinalRetainedCloseoutSealedLedger({
  ...
  it("blocks unready operator final retained completion closeout acknowledgement receipt and unsafe refs", () => {
    const ledger =
      buildYeonjangBrowserActiveTabInfoFinalRetainedCloseoutSealedLedger({
        operatorFinalRetainedCompletionCloseoutAcknowledgementReceipt: {
          ...READY_OPERATOR_FINAL_RETAINED_COMPLETION_CLOSEOUT_ACKNOWLEDGEMENT_RECEIPT,
