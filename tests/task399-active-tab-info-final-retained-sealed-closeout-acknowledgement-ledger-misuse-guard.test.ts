...
function finalRetainedSealedCloseoutAcknowledgementLedger() {
  return buildYeonjangBrowserActiveTabInfoFinalRetainedSealedCloseoutAcknowledgementLedger({
...
describe("task399 active tab info final retained sealed closeout acknowledgement ledger misuse guard", () => {
  it("rejects approval evidence that tries to carry final retained sealed closeout acknowledgement ledger state", () => {
    const manifest = buildReleaseManifest({
      rootDir: process.cwd(),
      now: new Date("2026-07-23T07:50:00.000Z"),
  ...
  it("does not accept final retained sealed closeout acknowledgement ledger as final response or product log evidence", () => {
