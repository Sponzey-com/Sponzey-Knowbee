...
describe("task402 active tab info final retained sealed closeout completion ledger", () => {
  it("builds a minimal redacted final retained sealed closeout completion ledger without release or activation readiness", () => {
    const ledger =
      buildYeonjangBrowserActiveTabInfoFinalRetainedSealedCloseoutCompletionLedger({
  ...
  it("blocks unready operator final retained sealed closeout completion acknowledgement receipt and unsafe refs", () => {
    const ledger =
      buildYeonjangBrowserActiveTabInfoFinalRetainedSealedCloseoutCompletionLedger({
        operatorFinalRetainedSealedCloseoutCompletionAcknowledgementReceipt: {
          ...READY_OPERATOR_FINAL_RETAINED_SEALED_CLOSEOUT_COMPLETION_ACKNOWLEDGEMENT_RECEIPT,
