...
function useTempState(): void {
  closeDb()
  memoryJournal?.close()
  const stateDir = mkdtempSync(join(tmpdir(), "knowbee-task005-writeback-"))
  tempDirs.push(stateDir)
  const paths = createRuntimePaths({ KNOWBEE_STATE_DIR: stateDir }, { homeDir: stateDir, exists: () => false })
  initializeTestDbRuntime(paths.stateDir)
  memoryJournal = createMemoryJournalRepository(paths)
}
