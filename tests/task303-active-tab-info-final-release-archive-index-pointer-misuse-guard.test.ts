...
function finalReleaseArchiveIndexPointer() {
  return buildYeonjangBrowserActiveTabInfoFinalReleaseArchiveIndexPointer({
...
describe("task303 active tab info final release archive index pointer misuse guard", () => {
  it("rejects approval evidence that tries to carry final release archive index pointer state", () => {
    const manifest = buildReleaseManifest({
      rootDir: process.cwd(),
      now: new Date("2026-07-22T03:19:00.000Z"),
  ...
  it("does not accept final release archive index pointer as final response or product log evidence", () => {
