...
describe("task007 topology workspace inspector and executor mapping", () => {
  it("shows task template, executor, output preset, checklist, and allowed resource choices", () => {
    ...
    expect(html).toContain("자동 처리")
    expect(html).toContain("기존 서브 에이전트 사용")
    expect(html).toContain("외부 도구 실행")
    expect(html).toContain("최종 검토")
    expect(html).toContain("업무 유형 선택")
    expect(html).toContain("결과 형식")
    expect(html).toContain("완료 기준")
    expect(html).toContain("허용 외부 도구/데이터")
  })
  ...
  it("shows tool/data controls as pickers instead of raw permission fields", () => {
    ...
    expect(html).toContain("외부 도구 선택")
    expect(html).toContain("권한 방식")
    expect(html).toContain("재시도 설정")
    expect(html).toContain("제한 시간 설정")
  ...
  it("shows decision condition and branch label presets", () => {
    ...
    expect(html).toContain("판단 조건")
    expect(html).toContain("분기 결과")
  ...
  it("shows approval approver and threshold presets", () => {
    ...
    expect(html).toContain("승인 담당자 선택")
    expect(html).toContain("승인 기준")
