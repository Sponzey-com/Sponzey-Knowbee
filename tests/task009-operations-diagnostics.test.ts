...
function useTempState(): void {
  closeDb()
  const rootDir = mkdtempSync(join(tmpdir(), "knowbee-task009-ops-"))
...
describe("task009 operations diagnostics", () => {
  ...
  it("cleans only stale active runs, records audit diagnostics, and blocks active history deletion", () => {
    ...
    const deleteCompleted = deleteRunHistory("run-done")
    expect(deleteCompleted).toEqual({ deletedRunCount: 1 })
    expect(getRootRun("run-done")).toBeUndefined()
  })
})
