...
function inspectProcessState(pid) {
  let signalable = true
  try {
    process.kill(pid, 0)
  } catch (error) {
    if (error?.code !== "EPERM") return "exited"
    signalable = false
  }
  const state = commandOutput("ps", ["-p", String(pid), "-o", "stat="])
  if (state) return state.startsWith("Z") ? "exited" : "running"
  if (
    commandOutput("lsof", ["-a", "-p", String(pid), "-d", "cwd", "-t"])
      === String(pid)
  ) {
    return "running"
  }
  if (signalable) return "running"
  return "unknown"
}
...
async function main() {
  ...
  const result = await observeGatewayStartupEvidence({
    ...
    processPort: {
      async inspect(pid) {
        const processState = inspectProcessState(pid)
